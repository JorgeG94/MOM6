...
type, public :: surface_forcing_CS ; private
  ...
  real(wp) :: latent_heat_fusion    !< latent heat of fusion [J/kg]
  real(wp) :: latent_heat_vapor     !< latent heat of vaporization [J/kg]

  real(wp) :: max_p_surf            !< maximum surface pressure that can be exerted by the
  ...
  logical :: enthalpy_cpl       !< Controls if enthalpy terms are provided by the coupler or computed
                                !! internally.
  real(wp) :: gust_const            !< constant unresolved background gustiness for ustar [R L Z T-2 ~> Pa]
  ...
  real(wp) :: cd_tides              !< drag coefficient that applies to the tides (nondimensional)
  real(wp) :: utide                 !< constant tidal velocity to use if read_tideamp
  ...
  real(wp)    :: density_sea_ice    !< Typical density of sea-ice [R ~> kg m-3]. The value is
                                !! only used to convert the ice pressure into
                                !! appropriate units for use with Kv_sea_ice.
  real(wp)    :: rigid_sea_ice_mass !< A mass per unit area of sea-ice beyond which
  ...
  real(wp)    :: Flux_const                     !< piston velocity for surface restoring [Z T-1 ~> m s-1]
  ...
  real(wp), pointer, dimension(:,:) :: basin_mask => NULL() !< mask for SSS restoring by basin
  ...
  logical                  :: mask_srestore         !< if true, apply a 2-dimensional mask to the surface
                                                    ...
                                                    !! be named 'mask'
  real(wp), pointer, dimension(:,:) :: srestore_mask => NULL() !< mask for SSS restoring
  ...
  logical                  :: mask_trestore         !< if true, apply a 2-dimensional mask to the surface
                                                    ...
                                                    !! be named 'mask'
  real(wp), pointer, dimension(:,:) :: trestore_mask => NULL() !< mask for SST restoring
...
type, public :: ice_ocean_boundary_type
  real(wp), pointer, dimension(:,:) :: lrunoff           =>NULL() !< liquid runoff [km m-2 s-1]
  real(wp), pointer, dimension(:,:) :: frunoff           =>NULL() !< ice runoff [km m-2 s-1]
  real(wp), pointer, dimension(:,:) :: lrunoff_glc       =>NULL() !< liquid glc runoff via rof [km m-2 s-1]
  real(wp), pointer, dimension(:,:) :: frunoff_glc       =>NULL() !< frozen glc runoff via rof [km m-2 s-1]
  ...
  real(wp), pointer, dimension(:,:) :: q_flux            =>NULL() !< specific humidity flux [km m-2 s-1]
  real(wp), pointer, dimension(:,:) :: salt_flux         =>NULL() !< salt flux [km m-2 s-1]
  real(wp), pointer, dimension(:,:) :: seaice_melt_heat  =>NULL() !< sea ice and snow melt heat flux [W m-2]
  real(wp), pointer, dimension(:,:) :: seaice_melt       =>NULL() !< water flux due to sea ice and snow melting [km m-2 s-1]
  ...
  real(wp), pointer, dimension(:,:) :: lprec             =>NULL() !< mass flux of liquid precip [km m-2 s-1]
  real(wp), pointer, dimension(:,:) :: fprec             =>NULL() !< mass flux of frozen precip [km m-2 s-1]
  real(wp), pointer, dimension(:,:) :: ustar_berg        =>NULL() !< frictional velocity beneath icebergs [m s-1]
  real(wp), pointer, dimension(:,:) :: area_berg         =>NULL() !< area covered by icebergs[m2 m-2]
  real(wp), pointer, dimension(:,:) :: mass_berg         =>NULL() !< mass of icebergs(kg m-2)
  real(wp), pointer, dimension(:,:) :: hrofl             =>NULL() !< heat content from liquid runoff [W m-2]
  real(wp), pointer, dimension(:,:) :: hrofi             =>NULL() !< heat content from frozen runoff [W m-2]
  real(wp), pointer, dimension(:,:) :: hrofl_glc         =>NULL() !< heat content from liquid glc runoff [W m-2]
  real(wp), pointer, dimension(:,:) :: hrofi_glc         =>NULL() !< heat content from frozen glc runoff [W m-2]
  real(wp), pointer, dimension(:,:) :: hrain             =>NULL() !< heat content from liquid precipitation [W m-2]
  real(wp), pointer, dimension(:,:) :: hsnow             =>NULL() !< heat content from frozen precipitation [W m-2]
  real(wp), pointer, dimension(:,:) :: hevap             =>NULL() !< heat content from evaporation [W m-2]
  real(wp), pointer, dimension(:,:) :: hcond             =>NULL() !< heat content from condensation [W m-2]
  real(wp), pointer, dimension(:,:) :: p                 =>NULL() !< pressure of overlying ice and atmosphere
                                                              !< on ocean surface [Pa]
  real(wp), pointer, dimension(:,:) :: ice_fraction      =>NULL() !< fractional ice area [1]
  real(wp), pointer, dimension(:,:) :: u10_sqr           =>NULL() !< wind speed squared at 10m [m2 s-2]
  real(wp), pointer, dimension(:,:) :: nhx_dep           =>NULL() !< Nitrogen deposition [kg m-2 s-1]
  real(wp), pointer, dimension(:,:) :: noy_dep           =>NULL() !< Nitrogen deposition [kg m-2 s-1]
  ...
  real(wp), pointer, dimension(:,:) :: atm_fine_dust_flux   =>NULL() !< Fine dust flux from atmosphere [kg m-2 s-1]
  real(wp), pointer, dimension(:,:) :: atm_coarse_dust_flux =>NULL() !< Coarse dust flux from atmosphere [kg m-2 s-1]
  real(wp), pointer, dimension(:,:) :: seaice_dust_flux     =>NULL() !< Dust flux from seaice [kg m-2 s-1]
  real(wp), pointer, dimension(:,:) :: atm_bc_flux          =>NULL() !< Black carbon flux from atmosphere [kg m-2 s-1]
  real(wp), pointer, dimension(:,:) :: seaice_bc_flux       =>NULL() !< Black carbon flux from seaice [kg m-2 s-1]
  real(wp), pointer, dimension(:,:) :: afracr               =>NULL() !< Fractional atmosphere coverage wrt ocean [1]
  real(wp), pointer, dimension(:,:) :: swnet_afracr         =>NULL() !< Net shortwave radiation times atmosphere fraction
                                                                 !! positive => into the ocean [W m-2]
  real(wp), pointer, dimension(:,:,:) :: swpen_ifrac_n      =>NULL() !< Net shortwave radiation penetrating into ice and
                                                                 !! ocean times ice fraction for thickness
                                                                 !! positive => into the ocean [W m-2]
  real(wp), pointer, dimension(:,:,:) :: ifrac_n            =>NULL() !< Ice fraction per category [1]
  real(wp), pointer, dimension(:,:) :: mi                =>NULL() !< mass of ice [km m-2]
  ...
  real(wp), pointer, dimension(:,:)   :: lamult          => NULL() !< Langmuir enhancement factor [1]
  real(wp), pointer, dimension(:)     :: stk_wavenumbers => NULL() !< The central wave number of Stokes bands [rad/m]
  real(wp), pointer, dimension(:,:,:) :: ustkb           => NULL() !< Stokes Drift spectrum, zonal [m s-1]
                                                               !! Horizontal  - u points
                                                               !! 3rd dimension - wavenumber
  real(wp), pointer, dimension(:,:,:) :: vstkb           => NULL() !< Stokes Drift spectrum, meridional [m s-1]
                                                               !! Horizontal  - v points
                                                               !! 3rd dimension - wavenumber
...
subroutine convert_IOB_to_fluxes(IOB, fluxes, index_bounds, Time, valid_time, G, US, CS, &
  ...
  real(wp) :: C_p                 !< heat capacity of seawater [J kg-1 degC-1]
  real(wp) :: sign_for_net_FW_bug !< Should be +1. but an old bug can be recovered by using -1.
  ...
  if (restore_salinity) then
    ...
    if (CS%mask_srestore_under_ice) then ! Do not restore under sea-ice
      do j=js,je ; do i=is,ie
        if (sfc_state%SST(i,j) <= -0.0539_wp*US%degC_to_C*US%S_to_ppt*sfc_state%SSS(i,j)) open_ocn_mask(i,j)=0.0_wp
      enddo ; enddo
    endif
    if (CS%salt_restore_as_sflux) then
      do j=js,je ; do i=is,ie
        ...
        fluxes%salt_flux(i,j) = 1.e-3_wp*US%S_to_ppt*G%mask2dT(i,j) * (CS%Rho0*CS%Flux_const)* &
  ...
  if ( associated(IOB%lamult)) then
    do j=js,je; do i=is,ie
      if (IOB%ice_fraction(i-i0,j-j0) <= 0.05_wp ) then
        fluxes%lamult(i,j) = IOB%lamult(i-i0,j-j0)
      else
        fluxes%lamult(i,j) = 1.0_wp
      endif
    enddo ; enddo
...
subroutine convert_IOB_to_forces(IOB, forces, index_bounds, Time, G, US, CS)
  ...
  real(wp) :: gustiness     !< unresolved gustiness that contributes to ustar [R Z L T-2 ~> Pa]
  real(wp) :: Irho0         !< inverse of the mean density in [Z L-1 R-1 ~> m3 kg-1]
  real(wp) :: taux2, tauy2  !< squared wind stresses [R2 Z2 L2 T-4 ~> Pa2]
  real(wp) :: tau_mag       !< magnitude of the wind stress [R Z L T-2 ~> Pa]
  real(wp) :: Pa_conversion ! A unit conversion factor from Pa to the internal wind stress units [R Z L T-2 Pa-1 ~> 1]
  ...
  if (wind_stagger == BGRID_NE) then
    ...
    taux_at_q(:,:) = 0.0_wp ; tauy_at_q(:,:) = 0.0_wp
  endif
  if (wind_stagger == AGRID) then
    ! This is necessary to fill in the halo points.
    taux_at_h(:,:) = 0.0_wp ; tauy_at_h(:,:) = 0.0_wp
  endif
...
subroutine apply_flux_adjustments(G, US, CS, Time, fluxes)
  ...
  ! Local variables
  real(wp), dimension(SZI_(G),SZJ_(G)) :: temp_at_h !< Fluxes at h points [W m-2 or kg m-2 s-1]
...
subroutine apply_force_adjustments(G, US, CS, Time, forces)
  ...
  integer :: isc, iec, jsc, jec, i, j
  real(wp) :: dLonDx, dLonDy, rDlon, cosA, sinA, zonal_tau, merid_tau
  real(wp) :: Pa_conversion ! A unit conversion factor from Pa to the internal units [R Z L T-2 Pa-1 ~> 1]
...
# include "version_variable.h"
  ...
  call get_param(param_file, mdl, "MAX_P_SURF", CS%max_p_surf, &
                 ...
                 units="Pa", default=-1.0_wp, scale=US%kg_m3_to_R*US%m_s_to_L_T**2)
...
! constant.
  ...
  call get_param(param_file, mdl, "GUST_CONST", CS%gust_const, &
               ...
               units="Pa", default=0.0_wp, scale=US%kg_m3_to_R*US%m_s_to_L_T**2*US%L_to_Z)
  if (CS%read_gust_2d) then
