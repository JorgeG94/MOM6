...
type, public :: particles_gridded
  ...
  real(wp) :: Lx !< Length of the domain in x direction
  real(wp), dimension(:,:), allocatable :: lon !< Longitude of cell corners (degree E)
  real(wp), dimension(:,:), allocatable :: lat !< Latitude of cell corners (degree N)
  real(wp), dimension(:,:), allocatable :: lonc !< Longitude of cell centers (degree E)
  real(wp), dimension(:,:), allocatable :: latc !< Latitude of cell centers (degree N)
  real(wp), dimension(:,:), allocatable :: dx !< Length of cell edge (m)
  real(wp), dimension(:,:), allocatable :: dy !< Length of cell edge (m)
  real(wp), dimension(:,:), allocatable :: area !< Area of cell (m^2)
  real(wp), dimension(:,:), allocatable :: msk !< Ocean-land mask (1=ocean)
  real(wp), dimension(:,:), allocatable :: cos !< Cosine from rotation matrix to lat-lon coords
  real(wp), dimension(:,:), allocatable :: sin !< Sine from rotation matrix to lat-lon coords
  real(wp), dimension(:,:), allocatable :: ocean_depth !< Depth of ocean (m)
  real(wp), dimension(:,:), allocatable :: uo !< Ocean zonal flow (m/s)
  real(wp), dimension(:,:), allocatable :: vo !< Ocean meridional flow (m/s)
  real(wp), dimension(:,:), allocatable :: tmp !< Temporary work space
  real(wp), dimension(:,:), allocatable :: tmpc !< Temporary work space
  real(wp), dimension(:,:), allocatable :: parity_x !< X component of vector point from i,j to i+1,j+1
  real(wp), dimension(:,:), allocatable :: parity_y !< Y component of vector point from i,j to i+1,j+1
...
type, public :: xyt
  real(wp) :: lon !< Longitude of particle (degree N or unit of grid coordinate)
  real(wp) :: lat !< Latitude of particle (degree N or unit of grid coordinate)
  real(wp) :: day      !< Day of this record (days)
  real(wp) :: lat_old  !< Previous latitude
  real(wp) :: lon_old   !< Previous longitude
  real(wp) :: uvel       !< Zonal velocity of particle (m/s)
  real(wp) :: vvel       !< Meridional velocity of particle (m/s)
  real(wp) :: uvel_old  !< Previous zonal velocity component (m/s)
  real(wp) :: vvel_old !< Previous meridional velocity component (m/s)
...
! State variables (specific to the particles, needed for restarts)
  real(wp) :: lon !< Longitude of particle (degree N or unit of grid coordinate)
  real(wp) :: lat !< Latitude of particle (degree E or unit of grid coordinate)
  real(wp) :: depth !< Depth of particle
  real(wp) :: uvel !< Zonal velocity of particle (m/s)
  real(wp) :: vvel !< Meridional velocity of particle (m/s)
  real(wp) :: lon_old !< previous lon (degrees)
  real(wp) :: lat_old !< previous lat (degrees)
  real(wp) :: uvel_old  !< previous uvel
  real(wp) :: vvel_old  !< previous vvel
  real(wp) :: start_lon !< starting longitude where particle was created
  real(wp) :: start_lat !< starting latitude where particle was created
  real(wp) :: start_day       !< origination position (degrees) and day
  integer :: start_year                         !< origination year
  real(wp) :: halo_part  !< equal to zero for particles on the computational domain, and 1 for particles on the halo
  ...
  real(wp) :: xi                               !< non-dimensional x-coordinate within current cell (0..1)
  real(wp) :: yj                                !< non-dimensional y-coordinate within current cell (0..1)
  real(wp) :: uo                                !< zonal ocean velocity
  real(wp) :: vo                                !< meridional ocean velocity
                                                !< by the particle (m/s)
...
type, public :: buffer
  ...
  real(wp), dimension(:,:), pointer :: data !< Buffer memory
end type buffer
...
type, public :: particles !; private
  ...
  real(wp) :: dt !< Time-step between particle calls
  integer :: current_year !< Current year (years)
  real(wp) :: current_yearday !< Current year-day, 1.00-365.99, (days)
