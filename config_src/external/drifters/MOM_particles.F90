...
subroutine particles_init(parts, Grid, Time, dt, u, v, h)
  ...
  real(wp), intent(in)            :: dt !< particle timestep in seconds [T ~> s]
  real(wp), dimension(:,:,:),intent(in)      :: u !< Zonal velocity field [L T-1 ~> m s-1]
  real(wp), dimension(:,:,:),intent(in)      :: v !< Meridional velocity field [L T-1 ~> m s-1]
  real(wp), dimension(:,:,:),intent(in)      :: h !< Thickness of each layer [H ~> m or kg m-2]
...
subroutine particles_run(parts, time, uo, vo, ho, tv, dt_adv, use_uh)
  ...
  real(wp), dimension(:,:,:), intent(in) :: uo !< If use_uh is false, ocean zonal velocity [L T-1 ~>m s-1].
                                           !! If use_uh is true, accumulated zonal thickness fluxes
                                           !! that are used to advect tracers [H L2 ~> m3 or kg]
  real(wp), dimension(:,:,:), intent(in) :: vo !< If use_uh is false, ocean meridional velocity [L T-1 ~>m s-1].
                                           !! If use_uh is true, accumulated meridional thickness fluxes
                                           !! that are used to advect tracers [H L2 ~> m3 or kg]
  real(wp), dimension(:,:,:), intent(in) :: ho !< Ocean layer thickness [H ~> m or kg m-2]
  type(thermo_var_ptrs),  intent(in) :: tv !< structure containing pointers to available thermodynamic fields
  real(wp), intent(in) :: dt_adv !< timestep for advecting particles [s]
...
subroutine particles_end(parts, h, temp, salt)
  ...
  type(particles), pointer :: parts !< Container for all types and memory
  real(wp), dimension(:,:,:),intent(in)      :: h !< Thickness of each layer [H ~> m or kg m-2]
  real(wp), dimension(:,:,:), optional, intent(in) :: temp !< Optional container for temperature [C ~> degC]
  real(wp), dimension(:,:,:), optional, intent(in) :: salt !< Optional container for salinity [S ~> ppt]
...
subroutine particles_to_k_space(parts, h)
  ! Arguments
  type(particles), pointer :: parts !< Container for all types and memory
  real(wp), dimension(:,:,:),intent(in)      :: h !< Thickness of layers [H ~> m or kg m-2]
