...
!  type(time_type)  :: real_to_time !< The output time as a time_type
  real(wp),                       intent(in)  :: x       !< The input time in real seconds.
  character(len=*), optional, intent(out) :: err_msg !< An optional returned error message.

  ! Local variables
  integer          :: seconds, days, ticks
  real(wp)             :: real_subsecond_remainder

  days = floor(x/86400._wp)
  seconds = floor(x - 86400._wp*days)
  real_subsecond_remainder = x - (days*86400._wp + seconds)
