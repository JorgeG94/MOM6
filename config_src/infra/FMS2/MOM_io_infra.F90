...
type :: file_type ; private
  ...
  real(wp)    :: file_time !< The time of the latest entry in the file.
end type file_type
...
type :: axistype ; private
  ...
  real(wp), allocatable, dimension(:) :: ax_data !< The values of the data on the axis.
...
subroutine close_file_type(IO_handle)
  ...
  IO_handle%num_times = 0 ; IO_handle%file_time = 0.0_wp
...
integer function write_time_if_later(IO_handle, field_time)
  type(file_type), intent(inout) :: IO_handle  !< Handle for a file that is open for writing
  real(wp),            intent(in)    :: field_time !< Model time of this field
