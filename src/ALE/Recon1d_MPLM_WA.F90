...
subroutine reconstruct(this, h, u)
  ...
  integer :: k, n
  real(wp) :: u_tmp, u_min, u_max ! Working values of cells [A]
  ...
  this%ur(1) = u(1)
  do k = 2, n-1
    u_tmp = u(k-1) + 0.5_wp * mslp(k-1) ! Right edge value of cell k-1
    u_min = min( u(k), u_tmp )
    u_max = max( u(k), u_tmp )
    u_tmp = u(k) - 0.5_wp * mslp(k) ! Left edge value of cell k
    this%ul(k) = max( min( u_tmp, u_max), u_min ) ! Bounded to handle roundoff
    u_tmp = u(k+1) - 0.5_wp * mslp(k-1) ! Left edge value of cell k+1
    u_min = min( u(k), u_tmp )
    u_max = max( u(k), u_tmp )
    u_tmp = u(k) + 0.5_wp * mslp(k) ! Right edge value of cell k
...
real(wp) elemental pure function PLM_monotonized_slope(u_l, u_c, u_r, s_l, s_c, s_r)
  ...
  ! Local variables
  real(wp) :: neighbor_edge ! Edge value of nieghbor cell [A]
  real(wp) :: this_edge ! Edge value of this cell [A]
  ...
  neighbor_edge = u_l + 0.5_wp * s_l
  this_edge = u_c - 0.5_wp * s_c
  if ( ( this_edge - neighbor_edge ) * ( u_c - this_edge ) < 0._wp ) then
    ! Using the midpoint works because the neighbor is similarly adjusted
    this_edge = 0.5_wp * ( this_edge + neighbor_edge )
    slp = min( slp, abs( this_edge - u_c ) * 2._wp )
  endif

  ! Check that right edge is between left edge of cell to the right and this cell mean
  neighbor_edge = u_r - 0.5_wp * s_r
  this_edge = u_c + 0.5_wp * s_c
  if ( ( this_edge - u_c ) * ( neighbor_edge - this_edge ) < 0._wp ) then
    ! Using the midpoint works because the neighbor is similarly adjusted
    this_edge = 0.5_wp * ( this_edge + neighbor_edge )
    slp = min( slp, abs( this_edge - u_c ) * 2._wp )
  endif
