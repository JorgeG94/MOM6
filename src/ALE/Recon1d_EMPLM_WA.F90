...
subroutine reconstruct(this, h, u)
  ...
  real(wp) :: edge_h2 ! Edge value found by linear interpolation [A]
  real(wp) :: slope_h2 ! Twice the difference between cell center and 2nd order edge value [A]
  real(wp) :: slope_e ! Twice the difference between cell center and neighbor edge value [A]
  real(wp) :: hn, hc ! Neighbor and central cell thicknesses adjusted by h_neglect [H]
  real(wp) :: u_min, u_max ! Working values for bounding edge values [A]
  ...
  slope_h2 = 2.0_wp * ( edge_h2 - u(1) )
  slope_e = 2.0_wp * ( this%ul(2) - u(1) )
  slope = sign( min( abs(slope_h2), abs(slope_e) ), u(2) - u(1) )
  edge_h2 = u(1) + 0.5_wp * slope
...
! this%ur(1) = u(1) + 0.5 * slope
  ...
  slope_h2 = 2.0_wp * ( u(n) - edge_h2 )
  slope_e = 2.0_wp * ( u(n) - this%ur(n-1) )
  slope = sign( min( abs(slope_h2), abs(slope_e) ), u(n) - u(n-1) )
  edge_h2 = u(n) - 0.5_wp * slope
