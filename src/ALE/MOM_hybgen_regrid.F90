...
type, public :: hybgen_regrid_CS ; private
  ...
  real(wp) :: qhybrlx  !< Fractional relaxation within a regridding step [nondim]

  real(wp), allocatable, dimension(:) ::  &
    dp0k, & !< minimum deep    z-layer separation [H ~> m or kg m-2]
    ds0k    !< minimum shallow z-layer separation [H ~> m or kg m-2]

  real(wp) :: coord_scale = 1.0_wp     !< A scaling factor to restores the depth coordinates to
                                !! values in m [m H-1 ~> 1 or m3 kg-1]
  real(wp) :: Rho_coord_scale = 1.0_wp !< A scaling factor to restores the denesity coordinates to
  ...
  real(wp) :: thkbot !< Thickness of a bottom boundary layer, within which hybgen does
                 !! something different. [H ~> m or kg m-2]

  !> Shallowest depth for isopycnal layers [H ~> m or kg m-2]
  real(wp) :: topiso_const
  ! real, dimension(:,:), allocatable :: topiso

  !> Nominal density of interfaces [R ~> kg m-3]
  real(wp), allocatable, dimension(:) :: target_density

  real(wp) :: dp_far_from_sfc  !< A distance that determines when an interface is suffiently far from
  ...
  real(wp) :: dp_far_from_bot  !< A distance that determines when an interface is suffiently far from
  ...
  real(wp) :: h_thin     !< A layer thickness below which a layer is considered to be too thin for
  ...
  real(wp) :: rho_eps    !< A small nonzero density that is used to prevent division by zero
                     !! in several expressions in the Hybgen regridding code [R ~> kg m-3].

  real(wp) :: onem       !< Nominally one m in thickness units [H ~> m or kg m-2], used only in
...
subroutine init_hybgen_regrid(CS, GV, US, param_file)
  ...
  real(wp)    :: hybrlx  ! The number of remappings over which to move toward the target coordinate [timesteps]
  ...
  allocate(CS%dp0k(CS%nk), source=0.0_wp) ! minimum deep z-layer separation
  allocate(CS%ds0k(CS%nk), source=0.0_wp) ! minimum shallow z-layer separation
  ...
  call get_param(param_file, mdl, "HYBGEN_MIN_THICKNESS", CS%min_thickness, &
                 ...
                 units="m", default=0.0_wp, scale=GV%m_to_H )
  ...
  call get_param(param_file, mdl, "HYBGEN_DEEP_DZ_PR0FILE", CS%dp0k, &
                 ...
                 units="m", default=0.0_wp, scale=GV%m_to_H, do_not_log=use_coord_file)
  ...
  call get_param(param_file, mdl, "HYBGEN_RELAX_PERIOD", hybrlx, &
                 ...
                 units="timesteps", default=1.0_wp)
  if (hybrlx < 1.0_wp) call MOM_error(FATAL, "init_hybgen_regrid: HYBGEN_RELAX_PERIOD must be at least 1.")
  CS%qhybrlx = 1.0_wp / hybrlx
  ...
  call get_param(param_file, mdl, "HYBGEN_FAR_FROM_SURFACE", CS%dp_far_from_sfc, &
                 ...
                 units="m", default=10.0_wp, scale=GV%m_to_H)
  ...
  call get_param(param_file, mdl, "HYBGEN_H_THIN", CS%h_thin, &
                 ...
                 "In Hycom, this is set to onemm (nominally 0.001 m).", &
                 units="m", default=0.001_wp, scale=GV%m_to_H)
  ...
  call get_param(param_file, mdl, "HYBGEN_DENSITY_EPSILON", CS%rho_eps, &
                 ...
                 units="kg m-3", default=1e-11_wp, scale=US%kg_m3_to_R)
  ...
  call get_param(param_file, mdl, "HYBGEN_REMAP_DENSITY_MATCH", CS%hybiso, &
                 ...
                 units="kg m-3", default=0.0_wp, scale=US%kg_m3_to_R)
...
subroutine get_hybgen_regrid_params(CS, nk, ref_pressure, hybiso, nsigma, dp00i, qhybrlx, &
  ...
  real(wp),    optional, intent(out) :: ref_pressure !< Reference pressure for density calculations [R L2 T-2 ~> Pa]
  real(wp),    optional, intent(out) :: hybiso  !< Hybgen uses PCM if layer is within hybiso of target density [R ~> kg m-3]
  integer, optional, intent(out) :: nsigma  !< Number of sigma levels used by HYBGEN
  real(wp),    optional, intent(out) :: dp00i   !< Deep isopycnal spacing minimum thickness [H ~> m or kg m-2]
  real(wp),    optional, intent(out) :: qhybrlx !< Fractional relaxation amount per timestep, 0 < qyhbrlx <= 1 [nondim]
  real(wp),    optional, intent(out) :: dp0k(:) !< minimum deep    z-layer separation [H ~> m or kg m-2]
  real(wp),    optional, intent(out) :: ds0k(:) !< minimum shallow z-layer separation [H ~> m or kg m-2]
  real(wp),    optional, intent(out) :: dpns    !< depth to start terrain following [H ~> m or kg m-2]
  real(wp),    optional, intent(out) :: dsns    !< depth to stop terrain following [H ~> m or kg m-2]
  real(wp),    optional, intent(out) :: min_dilate !< The minimum amount of dilation that is permitted when
                                            !! converting target coordinates from z to z* [nondim].
                                            !! This limit applies when wetting occurs.
  real(wp),    optional, intent(out) :: max_dilate !< The maximum amount of dilation that is permitted when
                                            !! converting target coordinates from z to z* [nondim].
                                            !! This limit applies when drying occurs.
  real(wp),    optional, intent(out) :: thkbot  !< Thickness of a bottom boundary layer, within which
                                            !! hybgen does something different. [H ~> m or kg m-2]
  real(wp),    optional, intent(out) :: topiso_const !< Shallowest depth for isopycnal layers [H ~> m or kg m-2]
  ! real, dimension(:,:), allocatable :: topiso
  real(wp),    optional, intent(out) :: target_density(:) !< Nominal density of interfaces [R ~> kg m-3]
...
subroutine hybgen_regrid(G, GV, US, dp, nom_depth_H, tv, CS, dzInterface, PCM_cell)
  ...
  real(wp), dimension(SZI_(G),SZJ_(G),CS%nk+1), &
  ...
  real(wp) :: temp_in(GV%ke)    ! A column of input potential temperatures [C ~> degC]
  real(wp) :: saln_in(GV%ke)    ! A column of input layer salinities [S ~> ppt]
  real(wp) :: Rcv_in(GV%ke)     ! An input column of coordinate potential density [R ~> kg m-3]
  real(wp) :: dp_in(GV%ke)      ! The input column of layer thicknesses [H ~> m or kg m-2]
  logical :: PCM_lay(GV%ke) ! If true for a layer, use PCM remapping for that layer

  ! These arrays are on the target grid.
  real(wp) :: Rcv_tgt(CS%nk)    ! Target potential density [R ~> kg m-3]
  real(wp) :: Rcv(CS%nk)        ! Initial values of coordinate potential density on the target grid [R ~> kg m-3]
  real(wp) :: h_col(CS%nk)      ! A column of layer thicknesses [H ~> m or kg m-2]
  real(wp) :: dz_int(CS%nk+1)   ! The change in interface height due to remapping [H ~> m or kg m-2]
  real(wp) :: Rcv_integral      ! Integrated coordinate potential density in a layer [R H ~> kg m-2 or kg2 m-5]

  real(wp) :: qhrlx(CS%nk+1)    ! Fractional relaxation within a timestep (between 0 and 1) [nondim]
  real(wp) :: dp0ij(CS%nk)      ! minimum layer thickness [H ~> m or kg m-2]
  real(wp) :: dp0cum(CS%nk+1)   ! minimum interface depth [H ~> m or kg m-2]

  real(wp) :: h_tot             ! Total thickness of the water column [H ~> m or kg m-2]
  real(wp) :: nominalDepth      ! Depth of ocean bottom (positive downward) [H ~> m or kg m-2]
  ...
  do j=G%jsc-1,G%jec+1 ; do i=G%isc-1,G%iec+1 ; if (G%mask2dT(i,j)>0._wp) then
    ...
    if (CS%nk >= nk_in) then
      ...
      do k=1,CS%nk
        h_col(k) = 0.0_wp ; Rcv_integral = 0.0_wp
    ...
    enddo ; endif
    do k=1,CS%nk+1 ; dzInterface(i,j,k) = 0.0_wp ; enddo
...
subroutine hybgen_column_init(nk, nsigma, dp0k, ds0k, dp00i, topiso_i_j, &
  ...
  real(wp),    intent(in)    :: dp0k(nk)     !< Layer deep z-level spacing minimum thicknesses [H ~> m or kg m-2]
  real(wp),    intent(in)    :: ds0k(nsigma) !< Layer shallow z-level spacing minimum thicknesses [H ~> m or kg m-2]
  real(wp),    intent(in)    :: dp00i        !< Deep isopycnal spacing minimum thickness [H ~> m or kg m-2]
  real(wp),    intent(in)    :: topiso_i_j   !< Shallowest depth for isopycnal layers [H ~> m or kg m-2]
  real(wp),    intent(in)    :: qhybrlx      !< Fractional relaxation amount per timestep, 0 < qyhbrlx <= 1 [nondim]
  real(wp),    intent(in)    :: h_tot        !< The sum of the initial layer thicknesses [H ~> m or kg m-2]
  real(wp),    intent(in)    :: dilate       !< A factor by which to dilate the target positions
                                         !! from z to z* [nondim]
  real(wp),    intent(in)    :: h_col(nk)    !< Initial layer thicknesses [H ~> m or kg m-2]
  real(wp),    intent(in)    :: dpns         !< Vertical sum of dp0k [H ~> m or kg m-2]
  real(wp),    intent(in)    :: dsns         !< Vertical sum of ds0k [H ~> m or kg m-2]
  integer, intent(out)   :: fixlay       !< Deepest fixed coordinate layer
  real(wp),    intent(out)   :: qhrlx(nk+1)  !< Fractional relaxation within a timestep (between 0 and 1) [nondim]
  real(wp),    intent(out)   :: dp0ij(nk)    !< minimum layer thickness [H ~> m or kg m-2]
  real(wp),    intent(out)   :: dp0cum(nk+1) !< minimum interface depth [H ~> m or kg m-2]
  ...
  ! Local variables
  real(wp) :: qdep    ! Total water column thickness as a fraction of dp0k (vs ds0k) [nondim]
  real(wp) :: q       ! A portion of the thickness that contributes to the new cell [H ~> m or kg m-2]
  real(wp) :: p_int(nk+1)  ! Interface depths [H ~> m or kg m-2]
  integer :: k, fixall
  ...
  if ((h_tot >= dilate * dpns) .or. (dpns <= dsns)) then
    qdep = 1.0_wp  ! Not terrain following - this column is too thick or terrain following is disabled.
  elseif (h_tot <= dilate * dsns) then
    qdep = 0.0_wp  ! Not terrain following - this column is too thin
  else
  ...
  if (qdep < 1.0_wp) then
    ...
    dp0cum(1) = 0.0_wp
    qhrlx( 1) = 1.0_wp
    dp0ij( 1) = dilate * (qdep*dp0k(1) + (1.0_wp-qdep)*ds0k(1))
    ...
    do k=2,nk
      qhrlx( k+1) = 1.0_wp
      dp0ij( k)   = dilate * (qdep*dp0k(k) + (1.0_wp-qdep)*ds0k(k))
    ...
    p_int( 1) = 0.0_wp
    dp0cum(1) = 0.0_wp
    qhrlx( 1) = 1.0_wp !no relaxation in top layer
    dp0ij( 1) = dilate * dp0k(1)

    dp0cum(2) = dp0cum(1) + dp0ij(1)
    qhrlx( 2) = 1.0_wp !no relaxation in top layer
    p_int( 2) = p_int(1) + h_col(1)
    do k=2,nk
  ...
  do k=2,nk
    ...
    qhrlx(k+1) = 1.0_wp  !no relaxation in fixed layers
    fixlay     = fixlay+1
  enddo !k

  fixall = fixlay
  do k=fixall+1,nk
    if (p_int(k+1) > dp0cum(k+1) + 0.1_wp*dp0ij(k)) then
    ...
    qhrlx(k) = 1.0_wp  !no relaxation in fixed layers
    fixlay   = fixlay+1
  enddo !k
...
real(wp) function cushn(delp, dp0)
  real(wp), intent(in) :: delp  ! A thickness change [H ~> m or kg m-2]
  real(wp), intent(in) :: dp0   ! A non-negative reference thickness [H ~> m or kg m-2]

  ! These are the nondimensional parameters that define the cushion function.
  real(wp), parameter :: qqmn=-4.0_wp, qqmx=2.0_wp  ! shifted range for cushn [nondim]
...
! real, parameter :: qqmn=-4.0, qqmx=6.0  ! somewhat wider range for cushn [nondim]
  ...
  real(wp), parameter :: qq_scale = (qqmx-1.0_wp) / (qqmx-qqmn)**2  ! A scaling factor based on qqmn and qqmx [nondim]
  real(wp), parameter :: I_qqmx = 1.0_wp / qqmx  ! The inverse of qqmx [nondim]
  ...
  elseif (delp < qqmn*dp0) then
    ...
    cushn = max(dp0, delp * I_qqmx) * (1.0_wp + qq_scale * ((delp / dp0) - qqmn)**2)
  endif
...
subroutine hybgen_column_regrid(CS, nk, thkbot, Rcv_tgt, &
  ...
  real(wp),    intent(in)    :: thkbot        !< thickness of bottom boundary layer [H ~> m or kg m-2]
  real(wp),    intent(in)    :: Rcv_tgt(nk)   !< Target potential density [R ~> kg m-3]
  integer, intent(in)    :: fixlay        !< deepest fixed coordinate layer
  real(wp),    intent(in)    :: qhrlx( nk+1)  !< relaxation coefficient per timestep [nondim]
  real(wp),    intent(in)    :: dp0ij( nk)    !< minimum layer thickness [H ~> m or kg m-2]
  real(wp),    intent(in)    :: dp0cum(nk+1)  !< minimum interface depth [H ~> m or kg m-2]
  real(wp),    intent(in)    :: Rcv(nk)       !< Coordinate potential density [R ~> kg m-3]
  real(wp),    intent(in)    :: h_in(nk)      !< Layer thicknesses [H ~> m or kg m-2]
  real(wp),    intent(out)   :: dp_int(nk+1)  !< The change in interface positions [H ~> m or kg m-2]
  ...
  ! Local variables
  real(wp) :: p_new  ! A new interface position [H ~> m or kg m-2]
  real(wp) :: pres_in(nk+1) ! layer interface positions [H ~> m or kg m-2]
  real(wp) :: p_int(nk+1)   ! layer interface positions [H ~> m or kg m-2]
  real(wp) :: h_col(nk)     ! Updated layer thicknesses [H ~> m or kg m-2]
  real(wp) :: q_frac ! A fraction of a layer to entrain [nondim]
  real(wp) :: h_min  ! The minimum layer thickness [H ~> m or kg m-2]
  real(wp) :: h_hat3 ! Thickness movement upward across the interface between layers k-2 and k-3 [H ~> m or kg m-2]
  real(wp) :: h_hat2 ! Thickness movement upward across the interface between layers k-1 and k-2 [H ~> m or kg m-2]
  real(wp) :: h_hat  ! Thickness movement upward across the interface between layers k and k-1 [H ~> m or kg m-2]
  real(wp) :: h_hat0 ! A first guess at thickness movement upward across the interface
                 ! between layers k and k-1 [H ~> m or kg m-2]
  real(wp) :: dh_cor ! Thickness changes [H ~> m or kg m-2]
  logical :: trap_errors
  integer :: k
...
! real, parameter :: qqmn=-4.0, qqmx=6.0  ! somewhat wider range for cushn [nondim]
  ...
  do K=1,nk+1 ; dp_int(K) = 0.0_wp ; enddo

  p_int(1) = 0.0_wp
  do k=1,nk
    h_col(k) = max(h_in(k), 0.0_wp)
    p_int(K+1) = p_int(K) + h_col(k)
  enddo
  h_min = min( CS%min_thickness, p_int(nk+1)/real(CS%nk, wp) )
    ...
    if ((Rcv(k) > Rcv_tgt(k) + CS%rho_eps)) then
      ...
      if ((Rcv(k-1) >= Rcv_tgt(k-1)) .or. &
        ...
        if ((Rcv_tgt(k) - Rcv(k-1)) <= CS%rho_eps) then
          ...
          h_hat0 = 0.0_wp  ! This line was not in the Hycom version of hybgen.F90.
        ...
        if ((h_hat >= 0.0_wp) .and. (k > fixlay+2) .and. (p_int(k-1) > dp0cum(k-1) + CS%dp_far_from_sfc)) then
      ...
      if (p_int(k+1) < p_int(nk+1)) then  ! k<nk
        if ((Rcv(k+1) <= Rcv_tgt(k+1)) .or. &
          ...
          if (p_int(min(k+3,nk+1)) < p_int(nk+1)) then
            ...
            h_hat = min(h_hat, max(0.5_wp*h_col(k+1), h_col(k+1) - dp0ij(k+1)) )
          else
  ...
  if (trap_errors) then
    ...
    do k=1,nk
      if (abs((h_col(k) - h_in(k)) + (dp_int(K) - dp_int(K+1))) > 1.0e-13_wp*max(p_int(nk+1), CS%onem)) then
      ...
      if (h_col(k) < 0.0_wp) then ! Could instead do: -1.0e-15*max(p_int(nk+1), CS%onem)) then
    ...
    enddo
    do K=1,nk+1
      if (abs(dp_int(K) - (p_int(K) - pres_in(K))) > 1.0e-13_wp*max(p_int(nk+1), CS%onem)) then
