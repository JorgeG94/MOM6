...
type, public :: unit_scale_type
  real(wp) :: m_to_Z     !< A constant that translates distances in meters to the units of depth              [Z m-1 ~> 1]
  real(wp) :: Z_to_m     !< A constant that translates distances in the units of depth to meters              [m Z-1 ~> 1]
  real(wp) :: m_to_L     !< A constant that translates lengths in meters to the units of horizontal lengths   [L m-1 ~> 1]
  real(wp) :: L_to_m     !< A constant that translates lengths in the units of horizontal lengths to meters   [m L-1 ~> 1]
  real(wp) :: s_to_T     !< A constant that translates time intervals in seconds to the units of time         [T s-1 ~> 1]
  real(wp) :: T_to_s     !< A constant that translates the units of time to seconds                           [s T-1 ~> 1]
  real(wp) :: R_to_kg_m3 !< A constant that translates the units of density to kilograms per meter cubed [kg m-3 R-1 ~> 1]
  real(wp) :: kg_m3_to_R !< A constant that translates kilograms per meter cubed to the units of density  [R m3 kg-1 ~> 1]
  real(wp) :: Q_to_J_kg  !< A constant that translates the units of enthalpy to Joules per kilogram      [J kg-1 Q-1 ~> 1]
  real(wp) :: J_kg_to_Q  !< A constant that translates Joules per kilogram to the units of enthalpy        [Q kg J-1 ~> 1]
  real(wp) :: C_to_degC  !< A constant that translates the units of temperature to degrees Celsius         [degC C-1 ~> 1]
  real(wp) :: degC_to_C  !< A constant that translates degrees Celsius to the units of temperature         [C degC-1 ~> 1]
  real(wp) :: S_to_ppt   !< A constant that translates the units of salinity to parts per thousand          [ppt S-1 ~> 1]
  real(wp) :: ppt_to_S   !< A constant that translates parts per thousand to the units of salinity          [S ppt-1 ~> 1]

  ! These are useful combinations of the fundamental scale conversion factors above.
  real(wp) :: Z_to_L          !< Convert vertical distances to lateral lengths                                [L Z-1 ~> 1]
  real(wp) :: L_to_Z          !< Convert lateral lengths to vertical distances                                [Z L-1 ~> 1]
  real(wp) :: L_T_to_m_s      !< Convert lateral velocities from L T-1 to m s-1                         [T m L-1 s-1 ~> 1]
  real(wp) :: m_s_to_L_T      !< Convert lateral velocities from m s-1 to L T-1                         [L s T-1 m-1 ~> 1]
  real(wp) :: L_T2_to_m_s2    !< Convert lateral accelerations from L T-2 to m s-2                     [L s2 T-2 m-1 ~> 1]
  real(wp) :: Z2_T_to_m2_s    !< Convert vertical diffusivities from Z2 T-1 to m2 s-1                  [T m2 Z-2 s-1 ~> 1]
  real(wp) :: m2_s_to_Z2_T    !< Convert vertical diffusivities from m2 s-1 to Z2 T-1                  [Z2 s T-1 m-2 ~> 1]
  real(wp) :: W_m2_to_QRZ_T   !< Convert heat fluxes from W m-2 to Q R Z T-1                       [Q R Z m2 T-1 W-1 ~> 1]
  real(wp) :: QRZ_T_to_W_m2   !< Convert heat fluxes from Q R Z T-1 to W m-2                    [W T Q-1 R-1 Z-1 m-2 ~> 1]
  ! Not used enough:  real :: kg_m2_to_RZ   !< Convert mass loads from kg m-2 to R Z                [R Z m2 kg-1 ~> 1]
  real(wp) :: RZ_to_kg_m2     !< Convert mass loads from R Z to kg m-2                               [kg R-1 Z-1 m-2 ~> 1]
  real(wp) :: RZL2_to_kg      !< Convert masses from R Z L2 to kg                                    [kg R-1 Z-1 L-2 ~> 1]
  real(wp) :: kg_m2s_to_RZ_T  !< Convert mass fluxes from kg m-2 s-1 to R Z T-1                   [R Z m2 s T-1 kg-1 ~> 1]
  real(wp) :: RZ_T_to_kg_m2s  !< Convert mass fluxes from R Z T-1 to kg m-2 s-1                [T kg R-1 Z-1 m-2 s-1 ~> 1]
  real(wp) :: RZ3_T3_to_W_m2  !< Convert turbulent kinetic energy fluxes from R Z3 T-3 to W m-2    [W T3 R-1 Z-3 m-2 ~> 1]
  real(wp) :: W_m2_to_RZ3_T3  !< Convert turbulent kinetic energy fluxes from W m-2 to R Z3 T-3     [R Z3 m2 T-3 W-1 ~> 1]
  real(wp) :: RL2_T2_to_Pa    !< Convert pressures from R L2 T-2 to Pa                                [Pa T2 R-1 L-2 ~> 1]
  real(wp) :: RLZ_T2_to_Pa    !< Convert wind stresses from R L Z T-2 to Pa                       [Pa T2 R-1 L-1 Z-1 ~> 1]
  real(wp) :: Pa_to_RL2_T2    !< Convert pressures from Pa to R L2 T-2                                [R L2 T-2 Pa-1 ~> 1]
  real(wp) :: Pa_to_RLZ_T2    !< Convert wind stresses from Pa to R L Z T-2                          [R L Z T-2 Pa-1 ~> 1]

  ! These are no longer used for changing scaling across restarts.
  real(wp) :: m_to_Z_restart = 1.0_wp !< A copy of the m_to_Z that is used in restart files.
  real(wp) :: m_to_L_restart = 1.0_wp !< A copy of the m_to_L that is used in restart files.
  real(wp) :: s_to_T_restart = 1.0_wp !< A copy of the s_to_T that is used in restart files.
  real(wp) :: kg_m3_to_R_restart = 1.0_wp !< A copy of the kg_m3_to_R that is used in restart files.
  real(wp) :: J_kg_to_Q_restart = 1.0_wp !< A copy of the J_kg_to_Q that is used in restart files.
...
subroutine unit_scaling_init( param_file, US )
  ...
  real(wp)    :: Z_rescale_factor, L_rescale_factor, T_rescale_factor, R_rescale_factor, Q_rescale_factor
  real(wp)    :: C_rescale_factor, S_rescale_factor
...
# include "version_variable.h"
  ...
  Z_rescale_factor = 1.0_wp
  if (Z_power /= 0) Z_rescale_factor = 2.0_wp**Z_power
  US%Z_to_m = 1.0_wp * Z_rescale_factor
  US%m_to_Z = 1.0_wp / Z_rescale_factor

  L_rescale_factor = 1.0_wp
  if (L_power /= 0) L_rescale_factor = 2.0_wp**L_power
  US%L_to_m = 1.0_wp * L_rescale_factor
  US%m_to_L = 1.0_wp / L_rescale_factor

  T_rescale_factor = 1.0_wp
  if (T_power /= 0) T_rescale_factor = 2.0_wp**T_power
  US%T_to_s = 1.0_wp * T_rescale_factor
  US%s_to_T = 1.0_wp / T_rescale_factor

  R_rescale_factor = 1.0_wp
  if (R_power /= 0) R_rescale_factor = 2.0_wp**R_power
  US%R_to_kg_m3 = 1.0_wp * R_rescale_factor
  US%kg_m3_to_R = 1.0_wp / R_rescale_factor

  Q_Rescale_factor = 1.0_wp
  if (Q_power /= 0) Q_Rescale_factor = 2.0_wp**Q_power
  US%Q_to_J_kg = 1.0_wp * Q_Rescale_factor
  US%J_kg_to_Q = 1.0_wp / Q_Rescale_factor

  C_Rescale_factor = 1.0_wp
  if (C_power /= 0) C_Rescale_factor = 2.0_wp**C_power
  US%C_to_degC = 1.0_wp * C_Rescale_factor
  US%degC_to_C = 1.0_wp / C_Rescale_factor

  S_Rescale_factor = 1.0_wp
  if (S_power /= 0) S_Rescale_factor = 2.0_wp**S_power
  US%S_to_ppt = 1.0_wp * S_Rescale_factor
  US%ppt_to_S = 1.0_wp / S_Rescale_factor
...
subroutine unit_no_scaling_init(US)
  ...
  US%Z_to_m = 1.0_wp ; US%m_to_Z = 1.0_wp
  US%L_to_m = 1.0_wp ; US%m_to_L = 1.0_wp
  US%T_to_s = 1.0_wp ; US%s_to_T = 1.0_wp
  US%R_to_kg_m3 = 1.0_wp ; US%kg_m3_to_R = 1.0_wp
  US%Q_to_J_kg = 1.0_wp ; US%J_kg_to_Q = 1.0_wp
  US%C_to_degC = 1.0_wp ; US%degC_to_C = 1.0_wp
  US%S_to_ppt = 1.0_wp ; US%ppt_to_S = 1.0_wp
...
subroutine fix_restart_unit_scaling(US, unscaled)
  ...
  US%m_to_Z_restart = 1.0_wp ! US%m_to_Z
  US%m_to_L_restart = 1.0_wp ! US%m_to_L
  US%s_to_T_restart = 1.0_wp ! US%s_to_T
  US%kg_m3_to_R_restart = 1.0_wp ! US%kg_m3_to_R
  US%J_kg_to_Q_restart = 1.0_wp ! US%J_kg_to_Q

  if (present(unscaled)) then ; if (unscaled) then
    US%m_to_Z_restart = 1.0_wp
    US%m_to_L_restart = 1.0_wp
    US%s_to_T_restart = 1.0_wp
    US%kg_m3_to_R_restart = 1.0_wp
    US%J_kg_to_Q_restart = 1.0_wp
  endif ; endif
