...
subroutine CT_rescale_data_2d(var, scale)
  ...
  real(wp),                       intent(in)    :: scale !< A scaling factor to multiply fields by in
...
subroutine extract_coupler_type_data(var_in, bc_index, array_out, scale_factor, &
  ...
  integer,                    intent(in)    :: bc_index  !< The index of the boundary condition
                                                         !! that is being copied
  real(wp), dimension(1:,1:),     intent(out)   :: array_out !< The recipient array for the field in
                                                         !! arbitrary units [A]; the size of this array
                                                         !! must match the size of the data being copied
                                                         !! unless idim and jdim are supplied.
  real(wp),             optional, intent(in)    :: scale_factor !< A scaling factor for the data that is being
  ...
  ! Local variables
  real(wp), allocatable :: array_unrot(:,:)  ! Array on the unrotated grid in arbitrary units [A]
...
subroutine set_coupler_type_data(array_in, bc_index, var, solubility, scale_factor, &
                                 halo_size, idim, jdim, field_index, turns)
  real(wp), dimension(1:,1:),     intent(in)   :: array_in   !< The source array for the field in
                                                         !! arbitrary units [A]; the size of this array
                                                         !! must match the size of the data being copied
                                                         !! unless idim and jdim are supplied.
  integer,                    intent(in)    :: bc_index  !< The index of the boundary condition
  ...
  ! Local variables
  real(wp), allocatable :: array_unrot(:,:)  ! Array on the unrotated grid in the same arbitrary units
                                         ! as array_in [A]
