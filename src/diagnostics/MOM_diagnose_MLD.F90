...
subroutine diagnoseMLDbyDensityDifference(id_MLD, h, tv, densityDiff, G, GV, US, diagPtr, &
  ...
  real(wp),                    intent(in) :: densityDiff !< Density difference to determine MLD [R ~> kg m-3]
  type(diag_ctrl),         pointer    :: diagPtr     !< Diagnostics structure
  real(wp),                    intent(in) :: ref_h_mld   !< Depth of the calculated "surface" densisty [Z ~> m]
  ...
  real(wp),          optional, intent(in) :: dz_subML    !< The distance over which to calculate N2subML
                                                     !! or 50 m if missing [Z ~> m]
  real(wp), dimension(SZI_(G),SZJ_(G)), &
              optional, intent(out)   :: MLD_out     !< Send MLD to other routines [Z ~> m]

  ! Local variables
  real(wp), dimension(SZI_(G)) :: deltaRhoAtKm1, deltaRhoAtK ! Density differences [R ~> kg m-3].
  real(wp), dimension(SZI_(G)) :: pRef_MLD, pRef_N2 ! Reference pressures [R L2 T-2 ~> Pa].
  real(wp), dimension(SZI_(G)) :: hRef_MLD          ! Reference depth  [Z ~> m].
  real(wp), dimension(SZI_(G)) :: H_subML, dH_N2    ! Summed thicknesses used in N2 calculation [H ~> m or kg m-2]
  real(wp), dimension(SZI_(G)) :: dZ_N2             ! Summed vertical distance used in N2 calculation [Z ~> m]
  real(wp), dimension(SZI_(G)) :: T_subML, T_deeper ! Temperatures used in the N2 calculation [C ~> degC].
  real(wp), dimension(SZI_(G)) :: S_subML, S_deeper ! Salinities used in the N2 calculation [S ~> ppt].
  real(wp), dimension(SZI_(G)) :: rho_subML, rho_deeper ! Densities used in the N2 calculation [R ~> kg m-3].
  real(wp), dimension(SZI_(G),SZK_(GV)) :: dZ_2d   ! Layer thicknesses in depth units [Z ~> m]
  real(wp), dimension(SZI_(G)) :: dZ, dZm1         ! Layer thicknesses associated with interfaces [Z ~> m]
  real(wp), dimension(SZI_(G)) :: rhoSurf          ! Density used in finding the mixed layer depth [R ~> kg m-3].
  real(wp), dimension(SZI_(G), SZJ_(G)) :: z_ref_diag ! The actual depth of the reference density [Z ~> m].
  real(wp), dimension(SZI_(G), SZJ_(G)) :: MLD     ! Diagnosed mixed layer depth [Z ~> m].
  real(wp), dimension(SZI_(G), SZJ_(G)) :: subMLN2 ! Diagnosed stratification below ML [T-2 ~> s-2].
  real(wp), dimension(SZI_(G), SZJ_(G)) :: MLD2    ! Diagnosed MLD^2 [Z2 ~> m2].
  logical, dimension(SZI_(G)) :: N2_region_set ! If true, all necessary values for calculating N2
                                               ! have been stored already.
  real(wp) :: gE_Rho0          ! The gravitational acceleration, sometimes divided by the Boussinesq
                           ! reference density [H T-2 R-1 ~> m4 s-2 kg-1 or m s-2].
  real(wp) :: dZ_sub_ML        ! Depth below ML over which to diagnose stratification [Z ~> m]
  real(wp) :: aFac             ! A nondimensional factor [nondim]
  real(wp) :: ddRho            ! A density difference [R ~> kg m-3]
  real(wp) :: dddpth           ! A depth difference [Z ~> m]
  real(wp) :: rhoSurf_k, rhoSurf_km1  ! Desisty in the layers below and above the target reference depth [R ~> kg m-3].
  real(wp), dimension(SZI_(G), SZJ_(G)) :: rhoSurf_2d ! The density that is considered the "surface" when calculating
  ...
  do j=js,je
    ...
    if (pRef_MLD(is) /= 0.0_wp) then
      rhoSurf(:) = 0.0_wp
      do i=is,ie
        dZ(i) = 0.5_wp * dZ_2d(i,1) ! Depth of center of surface layer
      ...
      do k=2,nz
        do i=is,ie
          ...
          dZ(i) = dZ(i) + 0.5_wp * ( dZ_2d(i,k) + dZ_2d(i,k-1) ) ! Depth of center of layer K
          dddpth = dZ(i) - dZm1(i)
          if ((rhoSurf(i) == 0._wp) .and. &
              (dZm1(i) < hRef_MLD(i)) .and. (dZ(i) >= hRef_MLD(i))) then
            aFac = ( hRef_MLD(i) - dZm1(i) ) / dddpth
            z_ref_diag(i,j) = (dZ(i) * aFac + dZm1(i) * (1._wp - aFac))
            ...
            rhoSurf(i) = (rhoSurf_k * aFac + rhoSurf_km1 * (1._wp - aFac))
            H_subML(i) = h(i,j,k)
          elseif ((rhoSurf(i) == 0._wp) .and. (k >= nz)) then
            call calculate_density(tv%T(i,j,1), tv%S(i,j,1), pRef_MLD(i), rhoSurf_k, tv%eqn_of_state)
            rhoSurf(i) = rhoSurf_k
          endif
        enddo
      enddo
      do i=is,ie
        dZ(i) = 0.5_wp * dZ_2d(i,1) ! reset dZ to surface depth
        rhoSurf_2d(i,j) = rhoSurf(i)
        deltaRhoAtK(i) = 0._wp
        MLD(i,j) = 0._wp
        if (id_N2>0) then
          subMLN2(i,j) = 0.0_wp
          dH_N2(i) = 0.0_wp ; dZ_N2(i) = 0.0_wp
          T_subML(i) = 0.0_wp  ; S_subML(i) = 0.0_wp ; T_deeper(i) = 0.0_wp ; S_deeper(i) = 0.0_wp
          N2_region_set(i) = (G%mask2dT(i,j)<0.5_wp) ! Only need to work on ocean points.
        endif
      enddo
    elseif (pRef_MLD(is) == 0.0_wp) then
      rhoSurf(:) = 0.0_wp
      do i=is,ie ; dZ(i) = 0.5_wp * dZ_2d(i,1) ; enddo ! Depth of center of surface layer
      ...
      do i=is,ie
        ...
        if (id_N2>0) then
          subMLN2(i,j) = 0.0_wp
          H_subML(i) = h(i,j,1) ; dH_N2(i) = 0.0_wp ; dZ_N2(i) = 0.0_wp
          T_subML(i) = 0.0_wp  ; S_subML(i) = 0.0_wp ; T_deeper(i) = 0.0_wp ; S_deeper(i) = 0.0_wp
          N2_region_set(i) = (G%mask2dT(i,j)<0.5_wp) ! Only need to work on ocean points.
        endif
      enddo
    ...
    do k=2,nz
      ...
      if (id_N2>0) then
         do i=is,ie
          if (MLD(i,j) == 0.0_wp) then  ! Still in the mixed layer.
          ...
          elseif (.not.N2_region_set(i)) then ! This block is below the mixed layer, but N2 has not been found yet.
            if (dZ_N2(i) == 0.0_wp) then ! Record the temperature, salinity, pressure, immediately below the ML
              T_subML(i) = tv%T(i,j,k) ; S_subML(i) = tv%S(i,j,k)
              H_subML(i) = H_subML(i) + 0.5_wp * h(i,j,k) ! Start midway through this layer.
              dH_N2(i) = 0.5_wp * h(i,j,k)
              dZ_N2(i) = 0.5_wp * dz_2d(i,k)
            elseif (dZ_N2(i) + dZ_2d(i,k) < dZ_sub_ML) then
              ...
              dH_N2(i) = dH_N2(i) + 0.5_wp * h(i,j,k)
              dZ_N2(i) = dZ_N2(i) + 0.5_wp * dz_2d(i,k)
      ...
      do i = is, ie
        ...
        if ((MLD(i,j) == 0._wp) .and. (ddRho > 0._wp) .and. &
            (deltaRhoAtKm1(i) < densityDiff) .and. (deltaRhoAtK(i) >= densityDiff)) then
          aFac = ( densityDiff - deltaRhoAtKm1(i) ) / ddRho
          MLD(i,j) = (dZ(i) * aFac + dZm1(i) * (1._wp - aFac))
        endif
        if (id_SQ > 0) MLD2(i,j) = MLD(i,j)**2
      enddo ! i-loop
    enddo ! k-loop
    do i=is,ie
      if ((MLD(i,j) == 0._wp) .and. (deltaRhoAtK(i) < densityDiff)) MLD(i,j) = dZ(i) ! Mixing goes to the bottom
    enddo

    if (id_N2>0) then  ! Now actually calculate stratification, N2, below the mixed layer.
      do i=is,ie ; pRef_N2(i) = (GV%g_Earth * GV%H_to_RZ) * (H_subML(i) + 0.5_wp*dH_N2(i)) ; enddo
      ...
      do i=is,ie ; if ((G%mask2dT(i,j) > 0.0_wp) .and. N2_region_set(i)) then
  ...
  if ((id_ref_z > 0) .and. (pRef_MLD(is)/=0._wp)) call post_data(id_ref_z, z_ref_diag , diagPtr)
...
subroutine diagnoseMLDbyEnergy(id_MLD, h, tv, G, GV, US, Mixing_Energy, k_bounds, diagPtr, OM4_iteration, MLD_out)
  ...
  type(verticalGrid_type), intent(in) :: GV          !< ocean vertical grid structure
  type(unit_scale_type),   intent(in) :: US          !< A dimensional unit scaling type
  real(wp), dimension(3),      intent(in) :: Mixing_Energy !< Energy values for up to 3 MLDs [R Z3 T-2 ~> J m-2]
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV)), &
                           intent(in) :: h           !< Layer thickness [H ~> m or kg m-2]
  type(thermo_var_ptrs),   intent(in) :: tv          !< Structure containing pointers to any
                                                     !! available thermodynamic fields.
  ...
  ! Local variables
  real(wp), dimension(SZI_(G),SZJ_(G),3) :: MLD  ! Diagnosed mixed layer depth [Z ~> m].
  real(wp), dimension(SZK_(GV)+1) :: Z_int    ! Depths of the interfaces from the surface [Z ~> m]
  real(wp), dimension(SZI_(G),SZK_(GV)) :: dZ ! Layer thicknesses in depth units [Z ~> m]
  real(wp), dimension(SZI_(G),SZK_(GV)) :: Rho_c ! Columns of layer densities [R ~> kg m-3]
  real(wp), dimension(SZI_(G)) :: pRef_MLD   ! The reference pressure for the mixed layer
                                          ! depth calculation [R L2 T-2 ~> Pa]
  real(wp), dimension(3) :: PE_threshold      ! The energy threshold divided by g [R Z2 ~> kg m-1]

  real(wp) :: PE_Threshold_fraction   ! The fractional tolerance of the specified energy
                                  ! for the energy used to mix to the diagnosed depth [nondim]
  real(wp) :: H_ML                    ! The accumulated depth of the mixed layer [Z ~> m]
  real(wp) :: PE                      ! The cumulative potential energy of the unmixed water column to a depth
                                  ! of H_ML, divided by the gravitational acceleration [R Z2 ~> kg m-1]
  real(wp) :: PE_Mixed                ! The potential energy of the completely mixed water column to a depth
                                  ! of H_ML, divided by the gravitational acceleration [R Z2 ~> kg m-1]
  real(wp) :: RhoDZ_ML                ! The depth integrated density of the mixed layer [R Z ~> kg m-2]
  real(wp) :: H_ML_TST                ! A new test value for the depth of the mixed layer [Z ~> m]
  real(wp) :: PE_Mixed_TST            ! The potential energy of the completely mixed water column to a depth
                                  ! of H_ML_TST, divided by the gravitational acceleration [R Z2 ~> kg m-1]
  real(wp) :: RhoDZ_ML_TST            ! A test value of the new depth integrated density of the mixed layer [R Z ~> kg m-2]
  real(wp) :: Rho_ML                  ! The average density of the mixed layer [R ~> kg m-3]

  ! These are all temporary variables used to shorten the expressions in the iterations.
  real(wp) :: R1, R2, Ca, Ca2 ! Some densities [R ~> kg m-3]
  real(wp) :: D1, D2, X, X2   ! Some thicknesses [Z ~> m]
  real(wp) :: Cb, Cb2    ! A depth integrated density [R Z ~> kg m-2]
  real(wp) :: C, D       ! A depth squared [Z2 ~> m2]
  real(wp) :: Cc, Cc2    ! A density times a depth squared [R Z2 ~> kg m-1]
  real(wp) :: Cd         ! A density times a depth cubed [R Z3 ~> kg]
  real(wp) :: Gx         ! A triple integral in depth of density [R Z3 ~> kg]
  real(wp) :: Gpx        ! The derivative of Gx with x  [R Z2 ~> kg m-1]
  real(wp) :: Hx         ! The vertical integral depth [Z ~> m]
  real(wp) :: iHx        ! The inverse of Hx [Z-1 ~> m-1]
  real(wp) :: Hpx        ! The derivative of Hx with x, since H(x) = constant + x, its derivative is 1. [nondim]
  real(wp) :: Ix         ! A double integral in depth of density [R Z2 ~> kg m-1]
  real(wp) :: Ipx        ! The derivative of Ix with x [R Z ~> kg m-2]
  real(wp) :: Fgx        ! The mixing energy difference from the target [R Z2 ~> kg m-1]
  real(wp) :: Fpx        ! The derivative of Fgx with x  [R Z ~> kg m-2]
  real(wp) :: Zr         ! An upper (lower) bound for the PE integration in surface (bottom) mixed layer mode [Z ~> m]
  integer :: k_Zr    ! Sets the index of Zr
  real(wp) :: pe_dir     ! A factor that is used to generalize the iteration for upper and lower mixed layers
  ...
  pRef_MLD(:) = 0.0_wp
  mld(:,:,:) = 0.0_wp
  PE_Threshold_fraction = 1.e-4_wp !Fixed threshold of 0.01%, could be runtime.
  ...
  if (k_bounds(1)<k_bounds(2)) then
    ...
    pe_dir = -1._wp ! A "down" factor so calculations forward and backward use the same algorithm
    k_Zr = k_bounds(1) !top of cell indicated by k_bounds(1)
  else
    k_int = -1     ! k_interval is backward in k-space
    pe_dir = 1._wp  ! An "up" factor so calculations forward and backward use the same algorithm
      ...
      do iM=1,3
        ...
        do k=k_bounds(1),k_bounds(2),k_int
          ...
          if (use_OM4_iteration) then
            PE = PE + 0.5_wp * Rho_c(i,k) * (Z_int(K)**2 - Z_int(K+1)**2)
          else
            PE = PE + 0.5_wp * (Rho_c(i,k) * dZ(i,k)) * (Z_int(K) + Z_int(K+1))
          endif
          ...
          PE_Mixed_TST = (0.5_wp * (Rho_ML*pe_dir)) * (H_ML_TST * (H_ML_TST + 2.0_wp*pe_dir*Zr))
          ...
          if (PE_Mixed_TST - PE <= PE_threshold(iM)) then
            ...
            if (k_int>0 .and. use_OM4_iteration) then
              Ca  = -(R2)
              Cb  = -(R1 * D1 + R2 * (2._wp * D1))
              D   = D1**2
              Cc  = -(R1 * D1 * (2._wp * D1) + (R2 * D))
              Cd  = -R1 * (D1 * D)
              Ca2 = R2
              Cb2 = R2 * (2._wp * D1)
              C   = D2**2 + D1**2 + 2._wp * (D1 * D2)
              D   = D1**2
              Cc2 = R2 * (D - C)
           else
            ...
            do while(IT<10)!We can iterate up to 10 times
              ...
              Gx = 0.5_wp * (Ca * (X*X*X) + Cb * X**2 + Cc * X + Cd)
              Gpx = 0.5_wp * (3._wp * (Ca * X**2) + 2._wp * (Cb * X) + Cc)
              ! H, its inverse, and its derivative
              Hx = D1 + X
              iHx = 1._wp / Hx
              !Hpx = 1. ! The derivative is always 1 so it was moved outside the loop
              ! I and its derivative
              Ix = 0.5_wp * (Ca2 * X**2 + Cb2 * X + Cc2)
              Ipx = 0.5_wp * (2._wp * Ca2 * X + Cb2)
