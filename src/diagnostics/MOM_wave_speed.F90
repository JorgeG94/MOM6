...
type, public :: wave_speed_CS ; private
  ...
  logical :: better_cg1_est = .false.  !< If true, use an improved estimate of the first mode
                                       !! internal wave speed.
  real(wp) :: mono_N2_column_fraction = 0._wp !< The lower fraction of water column over which N2 is limited as
  ...
  real(wp) :: mono_N2_depth = -1._wp          !< The depth below which N2 is limited as monotonic for the purposes of
                                       !! calculating the equivalent barotropic wave speed [H ~> m or kg m-2].
                                       !! If this parameter is negative, this limiting does not occur.
                                       !! This parameter controls the default behavior of wave_speed() which
                                       !! can be overridden by optional arguments.
  real(wp) :: min_speed2 = 0._wp              !< The minimum mode 1 internal wave speed squared [L2 T-2 ~> m2 s-2]
  real(wp) :: wave_speed_tol = 0.001_wp       !< The fractional tolerance with which to solve for the wave
                                       !! speeds [nondim]
  real(wp) :: c1_thresh = -1.0_wp             !< A minimal value of the first mode internal wave speed
...
subroutine wave_speed(h, tv, G, GV, US, cg1, CS, halo_size, use_ebt_mode, mono_N2_column_fraction, &
  ...
  real(wp), dimension(SZI_(G),SZJ_(G)), intent(out) :: cg1 !< First mode internal wave speed [L T-1 ~> m s-1]
  ...
  real(wp),                   optional, intent(in)  :: mono_N2_column_fraction !< The lower fraction
                                          !! of water column over which N2 is limited as monotonic
                                          !! for the purposes of calculating vertical modal structure [nondim].
  real(wp),                   optional, intent(in)  :: mono_N2_depth !< A depth below which N2 is limited as
  ...
  real(wp), dimension(SZK_(GV),SZI_(G)) :: &
  ...
  real(wp) :: I_Htot  ! The inverse of the total filtered thicknesses [H-1 ~> m-1 or m2 kg-1]
  real(wp) :: det, ddet ! Determinant of the eigen system and its derivative with lam.  Because the
                  ...
                  ! of [T2 L-2 ~> s2 m-2]
  real(wp) :: lam     ! The eigenvalue [T2 L-2 ~> s2 m-2]
  real(wp) :: dlam    ! The change in estimates of the eigenvalue [T2 L-2 ~> s2 m-2]
  real(wp) :: lam0    ! The first guess of the eigenvalue [T2 L-2 ~> s2 m-2]
  ...
  real(wp) :: speed2_tot ! overestimate of the mode-1 speed squared [L2 T-2 ~> m2 s-2]
  real(wp) :: cg1_min2 ! A floor in the squared first mode speed below which 0 is returned [L2 T-2 ~> m2 s-2]
  real(wp) :: cg1_est  ! An initial estimate of the squared first mode speed [L2 T-2 ~> m2 s-2]
  real(wp) :: I_Hnew   ! The inverse of a new layer thickness [H-1 ~> m-1 or m2 kg-1]
  real(wp) :: drxh_sum ! The sum of density differences across interfaces times thicknesses [R H ~> kg m-2 or kg2 m-5]
  real(wp) :: dSpVxh_sum ! The sum of specific volume differences across interfaces times
                   ! thicknesses [H R-1 ~> m4 kg-1 or m], negative for stable stratification.
  real(wp) :: g_Rho0   ! G_Earth/Rho0 [L2 T-2 H-1 R-1 ~> m4 s-2 kg-1 or m7 s-2 kg-2].
  real(wp) :: c2_scale ! A scaling factor for wave speeds to help control the growth of the determinant and
  ...
  real(wp) :: tol_Hfrac ! Layers that together are smaller than this fraction of
                    ! the total water column can be merged for efficiency [nondim].
  real(wp) :: min_h_frac ! tol_Hfrac divided by the total number of layers [nondim].
  real(wp) :: tol_solve ! The fractional tolerance with which to solve for the wave speeds [nondim]
  real(wp) :: tol_merge ! The fractional change in estimated wave speed that is allowed
                    ! when deciding to merge layers in the calculation [nondim]
  real(wp) :: rescale   ! A rescaling factor to control the magnitude of the determinant [nondim]
  real(wp) :: I_rescale ! The reciprocal of the rescaling factor to control the magnitude of the determinant [nondim]
  integer :: kf(SZI_(G)) ! The number of active layers after filtering.
  integer, parameter :: max_itt = 10
  real(wp) :: lam_it(max_itt)  ! The guess at the eignevalue with each iteration [T2 L-2 ~> s2 m-2]
  real(wp) :: det_it(max_itt), ddet_it(max_itt)  ! The determinant of the matrix and its derivative with lam
  ...
  real(wp) :: hw      ! The mean of the adjacent layer thicknesses [H ~> m or kg m-2]
  real(wp) :: sum_hc  ! The sum of the layer thicknesses [H ~> m or kg m-2]
  real(wp) :: gp      ! A limited local copy of gprime [L2 H-1 T-2 ~> m s-2 or m4 s-2 kg-1]
  real(wp) :: N2min   ! A minimum buoyancy frequency, including a slope rescaling factor [L2 H-2 T-2 ~> s-2 or m6 kg-2 s-2]
  ...
  real(wp) :: l_mono_N2_column_fraction ! A local value of mono_N2_column_fraction [nondim]
  real(wp) :: l_mono_N2_depth ! A local value of mono_N2_column_depth [H ~> m or kg m-2]
  real(wp) :: mode_struct(SZK_(GV)) ! The mode structure [nondim], but it is also temporarily
                         ! in units of [L2 T-2 ~> m2 s-2] after it is modified inside of tdma6.
  real(wp) :: ms_min, ms_max ! The minimum and maximum mode structure values returned from tdma6 [L2 T-2 ~> m2 s-2]
  real(wp) :: ms_sq          ! The sum of the square of the values returned from tdma6 [L4 T-4 ~> m4 s-4]
  ...
  if (better_est) then
    ...
    tol_Hfrac  = 0.1_wp*tol_solve ; tol_merge = tol_solve / real(nz, wp)
  else
    tol_solve = 0.001_wp ; tol_Hfrac  = 0.0001_wp ; tol_merge = 0.001_wp
  endif
  ...
  rescale = 1024.0_wp**4 ; I_rescale = 1.0_wp/rescale
  c2_scale = US%m_s_to_L_T**2 / 4096.0_wp**2 ! Other powers of 2 give identical results.

  min_h_frac = tol_Hfrac / real(nz, wp)
  ...
  do j=js,je
    ...
    do i=is,ie ; htot(i) = 0.0_wp ; enddo
    do k=1,nz ; do i=is,ie ; htot(i) = htot(i) + h(i,j,k) ; enddo ; enddo

    do i=is,ie
      hmin(i) = htot(i)*min_h_frac ; kf(i) = 1 ; H_here(i) = 0.0_wp
      HxT_here(i) = 0.0_wp ; HxS_here(i) = 0.0_wp ; HxR_here(i) = 0.0_wp
    enddo
    if (use_EOS) then
      ...
      enddo ; enddo
      do i=is,ie ; if (H_here(i) > 0.0_wp) then
        Hf(kf(i),i) = H_here(i)
        Tf(kf(i),i) = HxT_here(i) / H_here(i)
        Sf(kf(i),i) = HxS_here(i) / H_here(i)
      endif ; enddo
    ...
    do i=is,ie ; if (G%mask2dT(i,j) > 0.0_wp) then
      if (use_EOS) then
        pres(1) = 0.0_wp ; H_top(1) = 0.0_wp
        do K=2,kf(i)
          pres(K) = pres(K-1) + H_to_pres*Hf(k-1,i)
          T_int(K) = 0.5_wp*(Tf(k,i)+Tf(k-1,i))
          S_int(K) = 0.5_wp*(Sf(k,i)+Sf(k-1,i))
        ...
        drxh_sum = 0.0_wp ; dSpVxh_sum = 0.0_wp
        if (better_est) then
          ...
          if (H_top(kf(i)) > 0.0_wp) then
            I_Htot = 1.0_wp / (H_top(kf(i)) + Hf(kf(i),i))  ! = 1.0 / (H_top(K) + H_bot(K)) for all K.
            H_bot(kf(i)+1) = 0.0_wp
            if (nonBous) then
              do K=kf(i),2,-1
                H_bot(K) = H_bot(K+1) + Hf(k,i)
                dSpVxh_sum = dSpVxh_sum + ((H_top(K) * H_bot(K)) * I_Htot) * &
                    min(0.0_wp, dSpV_dT(K)*(Tf(k,i)-Tf(k-1,i)) + dSpV_dS(K)*(Sf(k,i)-Sf(k-1,i)))
              enddo
            else
              do K=kf(i),2,-1
                H_bot(K) = H_bot(K+1) + Hf(k,i)
                drxh_sum = drxh_sum + ((H_top(K) * H_bot(K)) * I_Htot) * &
                    max(0.0_wp, drho_dT(K)*(Tf(k,i)-Tf(k-1,i)) + drho_dS(K)*(Sf(k,i)-Sf(k-1,i)))
              enddo
            endif
          endif
        else
          ...
          if (nonBous) then
            do K=2,kf(i)
              dSpVxh_sum = dSpVxh_sum + 0.5_wp*(Hf(k-1,i)+Hf(k,i)) * &
                  min(0.0_wp, dSpV_dT(K)*(Tf(k,i)-Tf(k-1,i)) + dSpV_dS(K)*(Sf(k,i)-Sf(k-1,i)))
            enddo
          else
            do K=2,kf(i)
              drxh_sum = drxh_sum + 0.5_wp*(Hf(k-1,i)+Hf(k,i)) * &
                  max(0.0_wp, drho_dT(K)*(Tf(k,i)-Tf(k-1,i)) + drho_dS(K)*(Sf(k,i)-Sf(k-1,i)))
            enddo
          endif
        endif
        ...
        if (better_est) then
          ...
          if (H_top(kf(i)) > 0.0_wp) then
            ...
            if (nonBous) then
              do K=kf(i),2,-1
                H_bot(K) = H_bot(K+1) + Hf(k,i)
                dSpVxh_sum = dSpVxh_sum + ((H_top(K) * H_bot(K)) * I_Htot) * &
                    min(0.0_wp, (Rf(k-1,i)-Rf(k,i)) / (Rf(k,i)*Rf(k-1,i)))
              enddo
            else
              do K=kf(i),2,-1
                H_bot(K) = H_bot(K+1) + Hf(k,i)
                drxh_sum = drxh_sum + ((H_top(K) * H_bot(K)) * I_Htot) * max(0.0_wp,Rf(k,i)-Rf(k-1,i))
              enddo
            endif
          endif
        else
          if (nonBous) then
            ...
            do K=2,kf(i)
              drxh_sum = drxh_sum + 0.5_wp*(Hf(k-1,i)+Hf(k,i)) * max(0.0_wp,Rf(k,i)-Rf(k-1,i))
            enddo
          endif
        endif
      endif ! use_EOS
      ...
      if (cg1_est <= cg1_min2) then
        cg1(i,j) = 0.0_wp
        if (present(modal_structure)) modal_structure(i,j,:) = 0._wp
      else
          ...
          do k=2,kf(i)
            if (better_est .and. nonBous) then
              merge = ((dSpV_dT(K)*(Tc(kc)-Tf(k,i)) + dSpV_dS(K)*(Sc(kc)-Sf(k,i))) * &
                       ((Hc(kc) * Hf(k,i))*I_Htot) < abs(2.0_wp * tol_merge * dSpVxh_sum))
            elseif (better_est) then
              merge = ((drho_dT(K)*(Tf(k,i)-Tc(kc)) + drho_dS(K)*(Sf(k,i)-Sc(kc))) * &
                       ((Hc(kc) * Hf(k,i))*I_Htot) < 2.0_wp * tol_merge*drxh_sum)
            elseif (nonBous) then
              merge = ((dSpV_dT(K)*(Tc(kc)-Tf(k,i)) + dSpV_dS(K)*(Sc(kc)-Sf(k,i))) * &
                       (Hc(kc) + Hf(k,i)) < abs(2.0_wp * tol_merge * dSpVxh_sum))
            else
              merge = ((drho_dT(K)*(Tf(k,i)-Tc(kc)) + drho_dS(K)*(Sf(k,i)-Sc(kc))) * &
                       (Hc(kc) + Hf(k,i)) < 2.0_wp * tol_merge*drxh_sum)
            endif
            if (merge) then
              ! Merge this layer with the one above and backtrack.
              I_Hnew = 1.0_wp / (Hc(kc) + Hf(k,i))
          ...
          do k=2,kf(i)
            if (nonBous .and. better_est) then
              merge = ((Rf(k,i) - Rc(kc)) *  ((Hc(kc) * Hf(k,i))*I_Htot) < &
                       (Rc(kc)*Rf(k,i)) * abs(2.0_wp * tol_merge * dSpVxh_sum))
            elseif (nonBous) then
              merge = ((Rf(k,i) - Rc(kc)) * (Hc(kc) + Hf(k,i)) < &
                       (Rc(kc)*Rf(k,i)) * abs(2.0_wp * tol_merge * dSpVxh_sum))
            elseif (better_est) then
              merge = ((Rf(k,i) - Rc(kc)) * ((Hc(kc) * Hf(k,i))*I_Htot) < 2.0_wp*tol_merge*drxh_sum)
            else
              merge = ((Rf(k,i) - Rc(kc)) * (Hc(kc) + Hf(k,i)) < 2.0_wp*tol_merge*drxh_sum)
            endif
            if (merge) then
              ! Merge this layer with the one above and backtrack.
              ...
              do k2=kc,2,-1
                if (nonBous .and. better_est) then
                  merge = ((Rc(k2) - Rc(k2-1)) *  ((Hc(kc) * Hf(k,i))*I_Htot) < &
                          (Rc(k2-1)*Rc(k2)) * abs(2.0_wp * tol_merge * dSpVxh_sum))
                elseif (nonBous) then
        ...
        if (kc >= 2) then
          speed2_tot = 0.0_wp
          if (better_est) then
            H_top(1) = 0.0_wp ; H_bot(kc+1) = 0.0_wp
            do K=2,kc+1 ; H_top(K) = H_top(K-1) + Hc(k-1) ; enddo
            do K=kc,2,-1 ; H_bot(K) = H_bot(K+1) + Hc(k) ; enddo
            I_Htot = 0.0_wp ; if (H_top(kc+1) > 0.0_wp) I_Htot = 1.0_wp / H_top(kc+1)
          endif

          if (l_use_ebt_mode) then
            Igu(1) = 0._wp ! Neumann condition for pressure modes
            sum_hc = Hc(1)
            N2min = gprime(2)/Hc(1)
            ...
            do k=2,kc
              hw = 0.5_wp*(Hc(k-1)+Hc(k))
              gp = gprime(K)

              if (l_mono_N2_column_fraction>0._wp .or. l_mono_N2_depth>=0._wp) then
                ! Determine whether N2 estimates should not be allowed to increase with depth.
                if (l_mono_N2_column_fraction>0._wp) then
                  if (GV%Boussinesq .or. GV%semi_Boussinesq) then
                    below_mono_N2_frac = &
                        (max(G%meanSL(i,j) + G%bathyT(i,j), 0.0_wp) - GV%H_to_Z * sum_hc < &
                         l_mono_N2_column_fraction * max(G%meanSL(i,j) + G%bathyT(i,j), 0.0_wp))
                  else
                    below_mono_N2_frac = (htot(i) - sum_hc < l_mono_N2_column_fraction*htot(i))
                  endif
                endif
                if (l_mono_N2_depth >= 0._wp) below_mono_N2_depth = (sum_hc > l_mono_N2_depth)
              ...
              Igu(k) = 1.0_wp/(gp*Hc(k))
              Igl(k-1) = 1.0_wp/(gp*Hc(k-1))
              sum_hc = sum_hc + Hc(k)

              if (better_est) then
                ! Estimate that the ebt_mode is sqrt(2) times the speed of the flat bottom modes.
                speed2_tot = speed2_tot + 2.0_wp * gprime(K)*((H_top(K) * H_bot(K)) * I_Htot)
              else ! The ebt_mode wave should be faster than the flat-bottom mode, so 0.707 should be > 1?
                speed2_tot = speed2_tot + gprime(K)*(Hc(k-1)+Hc(k))*0.707_wp
              endif
            enddo
           !Igl(kc) = 0. ! Neumann condition for pressure modes
            Igl(kc) = 2._wp*Igu(kc) ! Dirichlet condition for pressure modes
          else ! .not. l_use_ebt_mode
            do K=2,kc
              Igl(K) = 1.0_wp/(gprime(K)*Hc(k)) ; Igu(K) = 1.0_wp/(gprime(K)*Hc(k-1))
              if (better_est) then
          ...
          if (calc_modal_structure) then
            mode_struct(:) = 0._wp
            mode_struct(1:kc) = 1._wp ! Uniform flow, first guess
          endif

          ! Under estimate the first eigenvalue (overestimate the speed) to start with.
          if (calc_modal_structure) then
            lam0 = 0.5_wp / speed2_tot ; lam = lam0
          else
            lam0 = 1.0_wp / speed2_tot ; lam = lam0
          endif
          ...
          do itt=1,max_itt
            ...
            if ((ddet >= 0.0_wp) .or. (-det > -0.5_wp*lam*ddet)) then
            ...
            if (calc_modal_structure) then
              ...
              enddo
              if (ms_min<0._wp .and. ms_max>0._wp) then ! Any zero crossings => lam is too high
                lam = 0.5_wp * ( lam - dlam )
                dlam = -lam
          ...
          cg1(i,j) = 0.0_wp
          if (lam > 0.0_wp) cg1(i,j) = 1.0_wp / sqrt(lam)

          if (present(modal_structure)) then
            if (mode_struct(1)/=0._wp) then ! Normalize
              mode_struct(1:kc) = mode_struct(1:kc) / mode_struct(1)
            else
              mode_struct(1:kc)=0._wp
            endif
      ...
      cg1(i,j) = 0.0_wp ! This is a land point.
      if (present(modal_structure)) modal_structure(i,j,:) = 0._wp
    endif ; enddo ! i-loop
  enddo ! j-loop
...
subroutine tdma6(n, a, c, lam, y)
  ...
  real(wp), dimension(:), intent(in)    :: a !< Lower diagonal   [T2 L-2 ~> s2 m-2]
  real(wp), dimension(:), intent(in)    :: c !< Upper diagonal   [T2 L-2 ~> s2 m-2]
  real(wp),               intent(in)    :: lam !< Scalar subtracted from leading diagonal [T2 L-2 ~> s2 m-2]
  real(wp), dimension(:), intent(inout) :: y !< RHS on entry [A ~> a], result on exit [A L2 T-2 ~> a m2 s-2]

  ! Local variables
  real(wp) :: lambda     ! A temporary variable in [T2 L-2 ~> s2 m-2]
  real(wp) :: beta(n)    ! A temporary variable in [T2 L-2 ~> s2 m-2]
  real(wp) :: I_beta(n)  ! A temporary variable in [L2 T-2 ~> m2 s-2]
  real(wp) :: yy(n)      ! A temporary variable with the same units as y on entry [A ~> a]
  integer :: k, m

  lambda = lam
  beta(1) = (a(1)+c(1)) - lambda
  if (beta(1)==0._wp) then ! lam was chosen too perfectly
    ! Change lambda and redo this first row
    lambda = (1._wp + 1.e-5_wp) * lambda
    beta(1) = (a(1)+c(1)) - lambda
  endif
  I_beta(1) = 1._wp / beta(1)
  yy(1) = y(1)
  do k = 2, n
    beta(k) = ( (a(k)+c(k)) - lambda ) - a(k) * c(k-1) * I_beta(k-1)
    ! Perhaps the following 0 needs to become a tolerance to handle underflow?
    if (beta(k)==0._wp) then ! lam was chosen too perfectly
      ! Change lambda and redo everything up to row k
      lambda = (1._wp + 1.e-5_wp) * lambda
      I_beta(1) = 1._wp / ( (a(1)+c(1)) - lambda )
      do m = 2, k
        I_beta(m) = 1._wp / ( ( (a(m)+c(m)) - lambda ) - a(m) * c(m-1) * I_beta(m-1) )
        yy(m) = y(m) + a(m) * yy(m-1) * I_beta(m-1)
      enddo
    else
      I_beta(k) = 1._wp / beta(k)
    endif
...
subroutine wave_speeds(h, tv, G, GV, US, nmodes, cn, CS, w_struct, u_struct, u_struct_max, u_struct_bot, Nb, int_w2, &
  ...
  type(wave_speed_CS),                             intent(in)  :: CS !< Wave speed control struct
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV)+1,nmodes),intent(out) :: w_struct !< Wave vertical velocity profile [nondim]
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV),nmodes),intent(out) :: u_struct !< Wave horizontal velocity profile
                                                                     !! [Z-1 ~> m-1]
  real(wp), dimension(SZI_(G),SZJ_(G),nmodes),         intent(out) :: cn !< Waves speeds [L T-1 ~> m s-1]
  real(wp), dimension(SZI_(G),SZJ_(G),nmodes),         intent(out) :: u_struct_max !< Maximum of wave horizontal velocity
                                                                     !! profile [Z-1 ~> m-1]
  real(wp), dimension(SZI_(G),SZJ_(G),nmodes),         intent(out) :: u_struct_bot !< Bottom value of wave horizontal
                                                                     !! velocity profile [Z-1 ~> m-1]
  real(wp), dimension(SZI_(G),SZJ_(G)),                intent(out) :: Nb !< Bottom value of buoyancy freqency
                                                                     !! [T-1 ~> s-1]
  real(wp), dimension(SZI_(G),SZJ_(G),nmodes),         intent(out) :: int_w2 !< depth-integrated vertical velocity
                                                                     !! profile squared [H ~> m or kg m-2]
  real(wp), dimension(SZI_(G),SZJ_(G),nmodes),         intent(out) :: int_U2 !< depth-integrated horizontal velocity
                                                                     !! profile squared [H Z-2 ~> m-1 or kg m-4]
  real(wp), dimension(SZI_(G),SZJ_(G),nmodes),         intent(out) :: int_N2w2 !< depth-integrated buoyancy frequency
  ...
  real(wp), dimension(SZK_(GV)) :: &
    ...
    Rc            ! A column of layer densities after convective instabilities are removed [R ~> kg m-3]
  real(wp) :: I_Htot  ! The inverse of the total filtered thicknesses [H-1 ~> m-1 or m2 kg-1]
  real(wp) :: c2_scale ! A scaling factor for wave speeds to help control the growth of the determinant and its
  ...
  real(wp) :: det, ddet       ! Determinant of the eigen system and its derivative with lam.  Because the
                          ...
                          ! the units of of det are hard to interpret, but det/ddet is always in units
                          ! of [T2 L-2 ~> s2 m-2]
  real(wp) :: lam_1           ! approximate mode-1 eigenvalue [T2 L-2 ~> s2 m-2]
  real(wp) :: lam_n           ! approximate mode-n eigenvalue [T2 L-2 ~> s2 m-2]
  real(wp) :: dlam            ! The change in estimates of the eigenvalue [T2 L-2 ~> s2 m-2]
  real(wp) :: lamMin          ! minimum lam value for root searching range [T2 L-2 ~> s2 m-2]
  real(wp) :: lamMax          ! maximum lam value for root searching range [T2 L-2 ~> s2 m-2]
  real(wp) :: lamInc          ! width of moving window for root searching [T2 L-2 ~> s2 m-2]
  real(wp) :: det_l, ddet_l   ! determinant of the eigensystem and its derivative with lam at the lower
  ...
  real(wp) :: det_r, ddet_r   ! determinant and its derivative with lam at the lower end of the
                          ! bracket in arbitrarily rescaled units, but such that the units of
                          ! det_r/ddet_r are [T2 L-2 ~> s2 m-2]
  real(wp) :: det_sub, ddet_sub ! determinant and its derivative with lam at a subinterval endpoint that
  ...
  real(wp) :: xl, xr          ! lam guesses at left and right of window [T2 L-2 ~> s2 m-2]
  real(wp) :: xl_sub          ! lam guess at left of subinterval window [T2 L-2 ~> s2 m-2]
  real(wp), dimension(nmodes) :: &
  ...
  real(wp), dimension(SZI_(G)) :: &
    ...
    HxR_here       ! A layer integrated density [R H ~> kg m-2 or kg2 m-5]
  real(wp) :: speed2_tot ! overestimate of the mode-1 speed squared [L2 T-2 ~> m2 s-2]
  real(wp) :: speed2_min ! minimum mode speed (squared) to consider in root searching [L2 T-2 ~> m2 s-2]
  real(wp) :: cg1_min2   ! A floor in the squared first mode speed below which 0 is returned [L2 T-2 ~> m2 s-2]
  real(wp) :: cg1_est    ! An initial estimate of the squared first mode speed [L2 T-2 ~> m2 s-2]
  real(wp), parameter :: reduct_factor = 0.5_wp  ! A factor used in setting speed2_min [nondim]
  real(wp) :: I_Hnew     ! The inverse of a new layer thickness [H-1 ~> m-1 or m2 kg-1]
  real(wp) :: drxh_sum   ! The sum of density differences across interfaces times thicknesses [R H ~> kg m-2 or kg2 m-5]
  real(wp) :: dSpVxh_sum ! The sum of specific volume differences across interfaces times
                     ! thicknesses [H R-1 ~> m4 kg-1 or m], negative for stable stratification.
  ...
  real(wp), dimension(SZK_(GV)+1) ::  modal_structure !< Normalized model structure [nondim]
  real(wp), dimension(SZK_(GV)) ::  modal_structure_fder !< Normalized model structure [Z-1 ~> m-1]
  real(wp) :: mode_struct(SZK_(GV)+1) ! The mode structure [nondim], but it is also temporarily
                         ! in units of [L2 T-2 ~> m2 s-2] after it is modified inside of tdma6.
  real(wp) :: mode_struct_fder(SZK_(GV)) ! The mode structure 1st derivative [Z-1 ~> m-1], but it is also temporarily
                         ! in units of [L2 Z-1 T-2 ~> m s-2] after it is modified inside of tdma6.
  real(wp) :: mode_struct_sq(SZK_(GV)+1) ! The square of mode structure [nondim]
  real(wp) :: mode_struct_fder_sq(SZK_(GV)) ! The square of mode structure 1st derivative [Z-2 ~> m-2]

  real(wp) :: w2avg          ! A total for renormalization [H L4 T-4 ~> m5 s-4 or kg m2 s-4]
  real(wp), parameter :: a_int = 0.5_wp ! Integral total for normalization [nondim]
  real(wp) :: renorm         ! Normalization factor [T2 L-2 ~> s2 m-2]
  ...
  do j=js,je
    ...
    do i=is,ie
      hmin(i) = htot(i)*min_h_frac ; kf(i) = 1 ; H_here(i) = 0.0_wp ; dz_here(i) = 0.0_wp
      HxT_here(i) = 0.0_wp ; HxS_here(i) = 0.0_wp ; HxR_here(i) = 0.0_wp
    enddo
    if (use_EOS) then
      do k=1,nz ; do i=is,ie
        ...
        if (cg1_est > cg1_min2) then
          ...
          if (kc >= 2) then
            ...
            do K=2,kc
              Igl(K) = 1.0_wp / (gprime(K)*Hc(k)) ; Igu(K) = 1.0_wp / (gprime(K)*Hc(k-1))
              if (nonBous) then
                N2(K) = 2.0_wp*US%L_to_Z**2*gprime(K) * (Hc(k) + Hc(k-1)) / &  ! Units are [T-2 ~> s-2]
                       (dzc(k) + dzc(k-1))**2
              else
                N2(K) = 2.0_wp*US%L_to_Z**2*GV%Z_to_H*gprime(K) / (dzc(k) + dzc(k-1))  ! Units are [T-2 ~> s-2]
              endif
              if (better_est) then
                speed2_tot = speed2_tot + gprime(K)*((H_top(K) * H_bot(K)) * I_Htot)
              else
            ...
            ! Under estimate the first eigenvalue (overestimate the speed) to start with.
            lam_1 = 1.0_wp / speed2_tot

            ! init and first guess for mode structure
            mode_struct(:) = 0._wp
            mode_struct_fder(:) = 0._wp
            mode_struct(2:kc) = 1._wp ! Uniform flow, first guess
            modal_structure(:) = 0._wp
            modal_structure_fder(:) = 0._wp
            ...
            do itt=1,max_itt
              ...
              if ((ddet >= 0.0_wp) .or. (-det > -0.5_wp*lam_1*ddet)) then
              ...
              ! apply BC
              mode_struct(1) = 0._wp
              mode_struct(kc+1) = 0._wp

              ! renormalization of the integral of the profile
              w2avg = 0.0_wp
              do k=1,kc
                w2avg = w2avg + 0.5_wp*(mode_struct(K)**2+mode_struct(K+1)**2)*Hc(k) ! [H L4 T-4 ~> m5 s-4 or kg m2 s-4]
              enddo
            ...
            if (lam_1 > 0.0_wp) cn(i,j,1) = 1.0_wp / sqrt(lam_1)

            ! sign of wave structure is irrelevant, flip to positive if needed
            if (mode_struct(2)<0._wp) then
              mode_struct(2:kc) = -1._wp * mode_struct(2:kc)
            endif
            ...
            do K=1,kc
              int_w2(i,j,1) = int_w2(i,j,1) + 0.5_wp*(mode_struct_sq(K)+mode_struct_sq(K+1)) * Hc(k)
              int_N2w2(i,j,1) = int_N2w2(i,j,1) + 0.5_wp*(mode_struct_sq(K)*N2(K) + &
            ...
            if ((nmodes > 1) .and. (kc >= nmodes+1) .and. (cn(i,j,1) > CS%c1_thresh)) then
              ...
              lamMin = lam_1*(1.0_wp + tol_solve)
              ! set max value based on a low guess at wavespeed for highest mode
              speed2_min = (reduct_factor*cn(i,j,1)/real(nmodes, wp))**2
              lamMax = 1.0_wp / speed2_min
              ...
              do iint=1,numint
                ...
                if (det_l*det_r < 0.0_wp) then  ! if function changes sign
                  if (det_l*ddet_l < 0.0_wp) then ! if function at left is headed to zero
                    ...
                    do sub_it=1,sub_it_max
                      ...
                      do sub=1,nsub-1,2 ! only check odds; sub = 1; 1,3; 1,3,5,7;...
                        ...
                        call tridiag_det(Igu, Igl, 2, kc, xl_sub, det_sub, ddet_sub, &
                                         row_scale=c2_scale)
                        if (det_sub*det_r < 0.0_wp) then  ! if function changes sign
                          if (det_sub*ddet_sub < 0.0_wp) then ! if function at left is headed to zero
                            sub_rootfound = .true.
                            nrootsfound = nrootsfound + 1
              ...
              do m=1,nrootsfound ! loop over the root-containing widows (excluding 1st mode)
                ...
                do itt=1,max_itt
                  ...
                  w2avg = 0.0_wp
                  do k=1,kc
                    w2avg = w2avg + 0.5_wp*(mode_struct(K)**2+mode_struct(K+1)**2)*Hc(k)
                  enddo
                ...
                if (lam_n > 0.0_wp) cn(i,j,m+1) = 1.0_wp / sqrt(lam_n)
                ...
                do K=1,kc
                  int_w2(i,j,m) = int_w2(i,j,m) + 0.5_wp*(mode_struct_sq(K)+mode_struct_sq(K+1)) * Hc(k)
                  int_N2w2(i,j,m) = int_N2w2(i,j,m) + 0.5_wp*(mode_struct_sq(K)*N2(K) + &
                                                           mode_struct_sq(K+1)*N2(K+1)) * Hc(k)
                enddo
...
subroutine tridiag_det(a, c, ks, ke, lam, det, ddet, row_scale)
  real(wp), dimension(:), intent(in) :: a     !< Lower diagonal of matrix (first entry unused) [T2 L-2 ~> s2 m-2]
  real(wp), dimension(:), intent(in) :: c     !< Upper diagonal of matrix (last entry unused) [T2 L-2 ~> s2 m-2]
  integer,            intent(in) :: ks    !< Starting index to use in determinant
  integer,            intent(in) :: ke    !< Ending index to use in determinant
  real(wp),               intent(in) :: lam   !< Value subtracted from b [T2 L-2 ~> s2 m-2]
  real(wp),               intent(out):: det   !< Determinant of the matrix in dynamically rescaled units that
  ...
  real(wp),               intent(out):: ddet  !< Derivative of determinant with lam in units that are dynamically
                                          !! rescaled along with those of det, such that the units of
                                          !! det/ddet are always in [T2 L-2 ~> s2 m-2]
  real(wp),               intent(in) :: row_scale !< A scaling factor of the rows of the matrix to
                                          !! limit the growth of the determinant [L2 s2 T-2 m-2 ~> 1]
  ! Local variables
  real(wp) :: detKm1, detKm2   ! Cumulative value of the determinant for the previous two layers in units
                           ! that vary with the number of layers that have been worked on [various]
  real(wp) :: ddetKm1, ddetKm2 ! Derivative of the cumulative determinant with lam for the previous two
                           ! layers [various], but the units of detKm1/ddetKm1 are [T2 L-2 ~> s2 m-2]
  real(wp), parameter :: rescale = 1024.0_wp**4 ! max value of determinant allowed before rescaling [nondim]
  real(wp) :: I_rescale ! inverse of rescale [nondim]
  integer :: k      ! row (layer interface) index

  I_rescale = 1.0_wp / rescale

  detKm1 = 1.0_wp ; ddetKm1 = 0.0_wp
  det = (a(ks)+c(ks)) - lam ; ddet = -1.0_wp
  do k=ks+1,ke
...
subroutine wave_speed_init(CS, GV, use_ebt_mode, mono_N2_column_fraction, mono_N2_depth, remap_answers_2018, &
  ...
  logical, optional, intent(in) :: use_ebt_mode  !< If true, use the equivalent
                                     !! barotropic mode instead of the first baroclinic mode.
  real(wp),    optional, intent(in) :: mono_N2_column_fraction !< The lower fraction of water column over
                                     !! which N2 is limited as monotonic for the purposes of
                                     !! calculating the vertical modal structure [nondim].
  real(wp),    optional, intent(in) :: mono_N2_depth !< The depth below which N2 is limited
  ...
  real(wp),    optional, intent(in) :: min_speed !< If present, set a floor in the first mode speed
                                     !! below which 0 is returned [L T-1 ~> m s-1].
  real(wp),    optional, intent(in) :: wave_speed_tol !< The fractional tolerance for finding the
                                     !! wave speeds [nondim]
  real(wp),    optional, intent(in) :: c1_thresh !< A minimal value of the first mode internal wave speed
