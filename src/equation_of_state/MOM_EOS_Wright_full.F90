...
real(wp), parameter :: a0 = 7.133718e-4_wp  ! A parameter in the Wright alpha_0 fit [m3 kg-1]
real(wp), parameter :: a1 = 2.724670e-7_wp  ! A parameter in the Wright alpha_0 fit [m3 kg-1 degC-1]
real(wp), parameter :: a2 = -1.646582e-7_wp ! A parameter in the Wright alpha_0 fit [m3 kg-1 PSU-1]
real(wp), parameter :: b0 = 5.613770e8_wp   ! A parameter in the Wright p_0 fit [Pa]
real(wp), parameter :: b1 = 3.600337e6_wp   ! A parameter in the Wright p_0 fit [Pa degC-1]
real(wp), parameter :: b2 = -3.727194e4_wp  ! A parameter in the Wright p_0 fit [Pa degC-2]
real(wp), parameter :: b3 = 1.660557e2_wp   ! A parameter in the Wright p_0 fit [Pa degC-3]
real(wp), parameter :: b4 = 6.844158e5_wp   ! A parameter in the Wright p_0 fit [Pa PSU-1]
real(wp), parameter :: b5 = -8.389457e3_wp  ! A parameter in the Wright p_0 fit [Pa degC-1 PSU-1]
real(wp), parameter :: c0 = 1.609893e5_wp   ! A parameter in the Wright lambda fit [m2 s-2]
real(wp), parameter :: c1 = 8.427815e2_wp   ! A parameter in the Wright lambda fit [m2 s-2 degC-1]
real(wp), parameter :: c2 = -6.931554_wp    ! A parameter in the Wright lambda fit [m2 s-2 degC-2]
real(wp), parameter :: c3 = 3.869318e-2_wp  ! A parameter in the Wright lambda fit [m2 s-2 degC-3]
real(wp), parameter :: c4 = -1.664201e2_wp  ! A parameter in the Wright lambda fit [m2 s-2 PSU-1]
real(wp), parameter :: c5 = -2.765195_wp    ! A parameter in the Wright lambda fit [m2 s-2 degC-1 PSU-1]
!>@}
...
real(wp) elemental function density_elem_Wright_full(this, T, S, pressure)
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function density_anomaly_elem_Wright_full(this, T, S, pressure, rho_ref)
  class(Wright_full_EOS), intent(in) :: this !< This EOS
  real(wp), intent(in) :: T        !< Potential temperature relative to the surface [degC]
  real(wp), intent(in) :: S        !< Salinity [PSU]
  real(wp), intent(in) :: pressure !< Pressure [Pa]
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function spec_vol_elem_Wright_full(this, T, S, pressure)
  class(Wright_full_EOS), intent(in) :: this !< This EOS
  real(wp),           intent(in) :: T        !< Potential temperature relative to the surface [degC]
  real(wp),           intent(in) :: S        !< Salinity [PSU]
  real(wp),           intent(in) :: pressure !< Pressure [Pa]
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function spec_vol_anomaly_elem_Wright_full(this, T, S, pressure, spv_ref)
  class(Wright_full_EOS), intent(in) :: this !< This EOS
  real(wp),           intent(in) :: T        !< Potential temperature relative to the surface [degC]
  real(wp),           intent(in) :: S        !< Salinity [PSU]
  real(wp),           intent(in) :: pressure !< Pressure [Pa]
...
subroutine EoS_fit_range_Wright_full(this, T_min, T_max, S_min, S_max, p_min, p_max)
  ...
  if (present(S_max)) S_max = 40.0_wp
