...
real(wp), parameter :: a0 = 7.057924e-4_wp  ! A parameter in the Wright alpha_0 fit [m3 kg-1]
real(wp), parameter :: a1 = 3.480336e-7_wp  ! A parameter in the Wright alpha_0 fit [m3 kg-1 degC-1]
real(wp), parameter :: a2 = -1.112733e-7_wp ! A parameter in the Wright alpha_0 fit [m3 kg-1 PSU-1]
real(wp), parameter :: b0 = 5.790749e8_wp   ! A parameter in the Wright p_0 fit [Pa]
real(wp), parameter :: b1 = 3.516535e6_wp   ! A parameter in the Wright p_0 fit [Pa degC-1]
real(wp), parameter :: b2 = -4.002714e4_wp  ! A parameter in the Wright p_0 fit [Pa degC-2]
real(wp), parameter :: b3 = 2.084372e2_wp   ! A parameter in the Wright p_0 fit [Pa degC-3]
real(wp), parameter :: b4 = 5.944068e5_wp   ! A parameter in the Wright p_0 fit [Pa PSU-1]
real(wp), parameter :: b5 = -9.643486e3_wp  ! A parameter in the Wright p_0 fit [Pa degC-1 PSU-1]
real(wp), parameter :: c0 = 1.704853e5_wp   ! A parameter in the Wright lambda fit [m2 s-2]
real(wp), parameter :: c1 = 7.904722e2_wp   ! A parameter in the Wright lambda fit [m2 s-2 degC-1]
real(wp), parameter :: c2 = -7.984422_wp    ! A parameter in the Wright lambda fit [m2 s-2 degC-2]
real(wp), parameter :: c3 = 5.140652e-2_wp  ! A parameter in the Wright lambda fit [m2 s-2 degC-3]
real(wp), parameter :: c4 = -2.302158e2_wp  ! A parameter in the Wright lambda fit [m2 s-2 PSU-1]
real(wp), parameter :: c5 = -3.079464_wp    ! A parameter in the Wright lambda fit [m2 s-2 degC-1 PSU-1]
!>@}
...
real(wp) elemental function density_elem_Wright_red(this, T, S, pressure)
  ...
  ! Local variables
  real(wp) :: al0     ! The specific volume at 0 lambda in the Wright EOS [m3 kg-1]
  real(wp) :: p0      ! The pressure offset in the Wright EOS [Pa]
  real(wp) :: lambda  ! The sound speed squared at 0 alpha in the Wright EOS [m2 s-2]
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function density_anomaly_elem_Wright_red(this, T, S, pressure, rho_ref)
  class(Wright_red_EOS), intent(in) :: this !< This EOS
  real(wp), intent(in) :: T        !< potential temperature relative to the surface [degC].
  real(wp), intent(in) :: S        !< salinity [PSU].
  real(wp), intent(in) :: pressure !< pressure [Pa].
  ...
  ! Local variables
  real(wp) :: al0     ! The specific volume at 0 lambda in the Wright EOS [m3 kg-1]
  real(wp) :: al_TS   ! The contributions of temperature and salinity to al0 [m3 kg-1]
  real(wp) :: p_TSp   ! A combination of the pressure and the temperature and salinity contributions to p0 [Pa]
  real(wp) :: lam_TS  ! The contributions of temperature and salinity to lambda [m2 s-2]
  real(wp) :: pa_000  ! A corrected offset to the pressure, including contributions from rho_ref [Pa]

  pa_000 = b0*(1.0_wp - a0*rho_ref) - rho_ref*c0
  al_TS = a1*T + a2*S
  al0 = a0 + al_TS
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function spec_vol_elem_Wright_red(this, T, S, pressure)
  class(Wright_red_EOS), intent(in) :: this !< This EOS
  real(wp),           intent(in) :: T        !< potential temperature relative to the surface [degC]
  real(wp),           intent(in) :: S        !< salinity [PSU]
  real(wp),           intent(in) :: pressure !< pressure [Pa]
  ...
  real(wp) :: al0     ! The specific volume at 0 lambda in the Wright EOS [m3 kg-1]
  real(wp) :: p0      ! The pressure offset in the Wright EOS [Pa]
  real(wp) :: lambda  ! The sound speed squared at 0 alpha in the Wright EOS [m2 s-2], perhaps with
                  ! an offset to account for spv_ref
  real(wp) :: al_TS   ! The contributions of temperature and salinity to al0 [m3 kg-1]
  real(wp) :: p_TSp   ! A combination of the pressure and the temperature and salinity contributions to p0 [Pa]
  real(wp) :: lam_000 ! A corrected offset to lambda, including contributions from spv_ref [m2 s-2]
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function spec_vol_anomaly_elem_Wright_red(this, T, S, pressure, spv_ref)
  class(Wright_red_EOS), intent(in) :: this !< This EOS
  real(wp),           intent(in) :: T        !< potential temperature relative to the surface [degC]
  real(wp),           intent(in) :: S        !< salinity [PSU]
  real(wp),           intent(in) :: pressure !< pressure [Pa]
...
elemental subroutine calculate_density_derivs_elem_Wright_red(this, T, S, pressure, drho_dT, drho_dS)
  ...
  real(wp) :: p0      ! The pressure offset in the Wright EOS [Pa]
  real(wp) :: lambda  ! The sound speed squared at 0 alpha in the Wright EOS [m2 s-2]
  real(wp) :: I_denom2 ! The inverse of the square of the denominator of density in the Wright EOS [s4 m-4]

  al0 = a0 + (a1*T + a2*S)
  p0 = b0 + ( b4*S + T * (b1 + (T*(b2 + b3*T) + b5*S)) )
  lambda = c0 + ( c4*S + T * (c1 + (T*(c2 + c3*T) + c5*S)) )

  I_denom2 = 1.0_wp / (lambda + al0*(pressure + p0))**2
  drho_dT = I_denom2 * (lambda * (b1 + (T*(2.0_wp*b2 + 3.0_wp*b3*T) + b5*S)) - &
     (pressure+p0) * ( (pressure+p0)*a1 + (c1 + (T*(c2*2.0_wp + c3*3.0_wp*T) + c5*S)) ))
...
elemental subroutine calculate_density_second_derivs_elem_Wright_red(this, T, S, pressure, &
  ...
  real(wp) :: al0     ! The specific volume at 0 lambda in the Wright EOS [m3 kg-1]
  real(wp) :: lambda  ! The sound speed squared at 0 alpha in the Wright EOS [m2 s-2]
  real(wp) :: p_p0    ! A local work variable combining the pressure and pressure
                  ! offset (p0 elsewhere) in the Wright EOS [Pa]
  real(wp) :: dp0_dT  ! The partial derivative of p0 with temperature [Pa degC-1]
  real(wp) :: dp0_dS  ! The partial derivative of p0 with salinity [Pa PSU-1]
  real(wp) :: dlam_dT ! The partial derivative of lambda with temperature [m2 s-2 degC-1]
  real(wp) :: dlam_dS ! The partial derivative of lambda with salinity [m2 s-2 degC-1]
  real(wp) :: dRdT_num  ! The numerator in the expression for drho_dT [Pa m2 s-2 degC-1] = [kg m s-4 degC-1]
  real(wp) :: dRdS_num  ! The numerator in the expression for drho_ds [Pa m2 s-2 PSU-1] = [kg m s-4 PSU-1]
  real(wp) :: ddenom_dT ! The derivative of the denominator of density in the Wright EOS with temperature [m2 s-2 deg-1]
  real(wp) :: ddenom_dS ! The derivative of the denominator of density in the Wright EOS with salinity [m2 s-2 PSU-1]
  real(wp) :: I_denom   ! The inverse of the denominator of density in the Wright EOS [s2 m-2]
  real(wp) :: I_denom2  ! The inverse of the square of the denominator of density in the Wright EOS [s4 m-4]
  real(wp) :: I_denom3  ! The inverse of the cube of the denominator of density in the Wright EOS [s6 m-6]

  al0 = a0 + (a1*T + a2*S)
  p_p0 = pressure + ( b0 + (b4*S + T*(b1 + (b5*S + T*(b2 + b3*T)))) ) ! P + p0
  lambda = c0 + ( c4*S + T * (c1 + (T*(c2 + c3*T) + c5*S)) )
  dp0_dT = b1 + (b5*S + T*(2._wp*b2 + 3._wp*b3*T))
  dp0_dS = b4 + b5*T
  dlam_dT = c1 + (c5*S + T*(2._wp*c2 + 3._wp*c3*T))
  dlam_dS = c4 + c5*T
  I_denom = 1.0_wp / (lambda + al0*p_p0)
  ...
  drho_ds_ds = -2._wp*(p_p0*(a2*dp0_dS)) * I_denom2 - 2._wp*(dRdS_num*ddenom_dS) * I_denom3
  drho_ds_dt = ((b5*lambda - p_p0*(c5 + 2._wp*a2*dp0_dT)) + (dp0_dS*dlam_dT - dp0_dT*dlam_dS))*I_denom2 - &
                  2._wp*(ddenom_dT*dRdS_num) * I_denom3
  drho_dt_dt = 2._wp*((b2 + 3._wp*b3*T)*lambda - p_p0*((c2 + 3._wp*c3*T) + a1*dp0_dT))*I_denom2 - &
                  2._wp*(dRdT_num * ddenom_dT) * I_denom3
  ...
  drho_ds_dp = (-dlam_dS - 2._wp*a2*p_p0) * I_denom2 - (2._wp*al0*dRdS_num) * I_denom3
  drho_dt_dp = (-dlam_dT - 2._wp*a1*p_p0) * I_denom2 - (2._wp*al0*dRdT_num) * I_denom3
...
elemental subroutine calculate_specvol_derivs_elem_Wright_red(this, T, S, pressure, dSV_dT, dSV_dS)
  ...
  real(wp) :: p0      ! The pressure offset in the Wright EOS [Pa]
  real(wp) :: lambda  ! The sound speed squared at 0 alpha in the Wright EOS [m2 s-2]
  real(wp) :: I_denom ! The inverse of the denominator of specific volume in the Wright EOS [Pa-1]
  ...
  I_denom = 1.0_wp / (pressure + p0)
  dSV_dT = a1 + I_denom * ((c1 + (T*(2.0_wp*c2 + 3.0_wp*c3*T) + c5*S)) - &
                              (I_denom * lambda) * (b1 + (T*(2.0_wp*b2 + 3.0_wp*b3*T) + b5*S)))
...
elemental subroutine calculate_compress_elem_Wright_red(this, T, S, pressure, rho, drho_dp)
  ...
  I_denom = 1.0_wp / (lambda + al0*(pressure + p0))
...
subroutine avg_spec_vol_Wright_red(T, S, p_t, dp, SpV_avg, start, npts)
  real(wp), dimension(:), intent(in)    :: T         !< Potential temperature relative to the surface
                                                 !! [degC].
  real(wp), dimension(:), intent(in)    :: S         !< Salinity [PSU].
  real(wp), dimension(:), intent(in)    :: p_t       !< Pressure at the top of the layer [Pa]
  real(wp), dimension(:), intent(in)    :: dp        !< Pressure change in the layer [Pa]
  ...
  ! Local variables
  real(wp) :: al0        ! A term in the Wright EOS [m3 kg-1]
  real(wp) :: p0         ! A term in the Wright EOS [Pa]
  real(wp) :: lambda     ! A term in the Wright EOS [m2 s-2]
  real(wp) :: eps2       ! The square of a nondimensional ratio [nondim]
  real(wp) :: I_pterm    ! The inverse of p0 plus p_ave [Pa-1].
  real(wp), parameter :: C1_3 = 1.0_wp/3.0_wp, C1_7 = 1.0_wp/7.0_wp, C1_9 = 1.0_wp/9.0_wp ! Rational constants [nondim]
  integer :: j
  ...
  do j=start,start+npts-1
    ...
    I_pterm = 1.0_wp / (p0 + (p_t(j) + 0.5_wp*dp(j)))
    eps2 = (0.5_wp * dp(j) * I_pterm)**2
    SpV_avg(j) = al0 + (lambda * I_pterm) * &
                         (1.0_wp + eps2*(C1_3 + eps2*(0.2_wp + eps2*(C1_7 + eps2*C1_9))))
  enddo
...
subroutine EoS_fit_range_Wright_red(this, T_min, T_max, S_min, S_max, p_min, p_max)
  ...
  if (present(T_min)) T_min = -2.0_wp
  if (present(T_max)) T_max = 30.0_wp
  if (present(S_min)) S_min = 28.0_wp
  if (present(S_max)) S_max = 38.0_wp
  if (present(p_min)) p_min = 0.0_wp
  if (present(p_max)) p_max = 5.0e7_wp
...
subroutine int_density_dz_wright_red(T, S, z_t, z_b, rho_ref, rho_0, G_e, HI, &
  ...
  real(wp),                 intent(in)  :: rho_0    !< Density [R ~> kg m-3], that is used
  ...
  real(wp),       optional, intent(in)  :: rho_scale !< A multiplicative factor by which to scale density
  ...
  real(wp),       optional, intent(in)  :: temp_scale  !< A multiplicative factor by which to scale
                            !! temperature into degC [degC C-1 ~> 1]
  real(wp),       optional, intent(in)  :: saln_scale !< A multiplicative factor to convert pressure
                            !! into PSU [PSU S-1 ~> 1].
  real(wp), dimension(HI%isd:HI%ied,HI%jsd:HI%jed), &
              optional, intent(in)  :: Z_0p      !< The height at which the pressure is 0 [Z ~> m]

  ! Local variables
  real(wp), dimension(HI%isd:HI%ied,HI%jsd:HI%jed) :: al0_2d ! A term in the Wright EOS [m3 kg-1]
  real(wp), dimension(HI%isd:HI%ied,HI%jsd:HI%jed) :: p0_2d  ! A term in the Wright EOS [Pa]
  real(wp), dimension(HI%isd:HI%ied,HI%jsd:HI%jed) :: lambda_2d ! A term in the Wright EOS [m2 s-2]
  real(wp), dimension(HI%isd:HI%ied,HI%jsd:HI%jed) :: z0pres ! The height at which the pressure is zero [Z ~> m]
  real(wp) :: al0        ! A term in the Wright EOS [m3 kg-1]
  real(wp) :: p0         ! A term in the Wright EOS [Pa]
  real(wp) :: lambda     ! A term in the Wright EOS [m2 s-2]
  real(wp) :: rho_anom   ! The density anomaly from rho_ref [kg m-3].
  real(wp) :: eps, eps2  ! A nondimensional ratio and its square [nondim]
  real(wp) :: rem        ! [kg m-1 s-2]
  real(wp) :: GxRho      ! The gravitational acceleration times density and unit conversion factors [Pa Z-1 ~> kg m-2 s-2]
  real(wp) :: g_Earth    ! The gravitational acceleration [m2 Z-1 s-2 ~> m s-2]
  real(wp) :: I_Rho      ! The inverse of the Boussinesq density [m3 kg-1]
  real(wp) :: rho_ref_mks ! The reference density in MKS units [kg m-3]
  real(wp) :: p_ave      ! The layer averaged pressure [Pa]
  real(wp) :: I_al0      ! The inverse of al0 [kg m-3]
  real(wp) :: I_Lzz      ! The inverse of the denominator [Pa-1]
  ...
  real(wp) :: Pa_to_RL2_T2 ! A conversion factor of pressures from Pa to the output units indicated by
                       ! pres_scale [R L2 T-2 Pa-1 ~> 1].
  real(wp) :: a1s        ! Partly rescaled version of a1 [m3 kg-1 C-1 ~> m3 kg-1 degC-1]
  real(wp) :: a2s        ! Partly rescaled version of a2 [m3 kg-1 S-1 ~> m3 kg-1 PSU-1]
  real(wp) :: b1s        ! Partly rescaled version of b1 [Pa C-1 ~> Pa degC-1]
  real(wp) :: b2s        ! Partly rescaled version of b2 [Pa C-2 ~> Pa degC-2]
  real(wp) :: b3s        ! Partly rescaled version of b3 [Pa C-3 ~> Pa degC-3]
  real(wp) :: b4s        ! Partly rescaled version of b4 [Pa S-1 ~> Pa PSU-1]
  real(wp) :: b5s        ! Partly rescaled version of b5 [Pa C-1 S-1 ~> Pa degC-1 PSU-1]
  real(wp) :: c1s        ! Partly rescaled version of c1 [m2 s-2 C-1 ~> m2 s-2 degC-1]
  real(wp) :: c2s        ! Partly rescaled version of c2 [m2 s-2 C-2 ~> m2 s-2 degC-2]
  real(wp) :: c3s        ! Partly rescaled version of c3 [m2 s-2 C-3 ~> m2 s-2 degC-3]
  real(wp) :: c4s        ! Partly rescaled version of c4 [m2 s-2 S-1 ~> m2 s-2 PSU-1]
  real(wp) :: c5s        ! Partly rescaled version of c5 [m2 s-2 C-1 S-1 ~> m2 s-2 degC-1 PSU-1]
  logical :: do_massWeight ! Indicates whether to do mass weighting.
  logical :: top_massWeight ! Indicates whether to do mass weighting the sea surface
  real(wp), parameter :: C1_3 = 1.0_wp/3.0_wp, C1_7 = 1.0_wp/7.0_wp    ! Rational constants [nondim]
  real(wp), parameter :: C1_9 = 1.0_wp/9.0_wp, C1_90 = 1.0_wp/90.0_wp  ! Rational constants [nondim]
  ...
  if (present(pres_scale)) then
    ...
    Pa_to_RL2_T2 = 1.0_wp / pres_scale
  else
  ...
  if (present(rho_scale)) then
    ...
    rho_ref_mks = rho_ref ; I_Rho = 1.0_wp / rho_0
  endif
  ...
  if (present(temp_scale)) then ; if (temp_scale /= 1.0_wp) then
    a1s = a1s * temp_scale
  ...
  if (present(saln_scale)) then ; if (saln_scale /= 1.0_wp) then
    a2s = a2s * saln_scale
  ...
  do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
    ...
    dz = z_t(i,j) - z_b(i,j)
    p_ave = -GxRho*(0.5_wp*(z_t(i,j)+z_b(i,j)) - z0pres(i,j))

    I_al0 = 1.0_wp / al0
    I_Lzz = 1.0_wp / ((p0 + p_ave) + lambda * I_al0)
    eps = 0.5_wp*(GxRho*dz)*I_Lzz ; eps2 = eps*eps

!     rho(j) = (pressure(j) + p0) / (lambda + al0*(pressure(j) + p0))

    rho_anom = (p0 + p_ave)*(I_Lzz*I_al0) - rho_ref_mks
    rem = (I_Rho * (lambda * I_al0**2)) * (eps2 * (C1_3 + eps2*(0.2_wp + eps2*(C1_7 + C1_9*eps2))))
    dpa(i,j) = Pa_to_RL2_T2 * ((g_Earth*rho_anom)*dz - 2.0_wp*eps*rem)
    if (present(intz_dpa)) &
      intz_dpa(i,j) = Pa_to_RL2_T2 * (0.5_wp*(g_Earth*rho_anom)*dz**2 - dz*((1.0_wp+eps)*rem))
  enddo ; enddo
  ...
  if (present(intx_dpa)) then ; do j=js,je ; do I=Isq,Ieq
    ...
    do m=2,4
      ...
      p_ave = -GxRho*((wt_L * (0.5_wp*(z_t(i,j)+z_b(i,j)) - z0pres(i,j))) + &
                      (wt_R * (0.5_wp*(z_t(i+1,j)+z_b(i+1,j)) - z0pres(i+1,j))))

      I_al0 = 1.0_wp / al0
      I_Lzz = 1.0_wp / ((p0 + p_ave) + lambda * I_al0)
      eps = 0.5_wp*(GxRho*dz)*I_Lzz ; eps2 = eps*eps

      intz(m) = Pa_to_RL2_T2 * ( (g_Earth*dz) * ((p0 + p_ave)*(I_Lzz*I_al0) - rho_ref_mks) - 2.0_wp*eps * &
                  (I_Rho * (lambda * I_al0**2)) * (eps2 * (C1_3 + eps2*(0.2_wp + eps2*(C1_7 + C1_9*eps2)))) )
    enddo
  ...
  if (present(inty_dpa)) then ; do J=Jsq,Jeq ; do i=is,ie
    ...
    do m=2,4
      ...
      p_ave = -GxRho*((wt_L*(0.5_wp*(z_t(i,j)+z_b(i,j))-z0pres(i,j))) + &
                      (wt_R*(0.5_wp*(z_t(i,j+1)+z_b(i,j+1))-z0pres(i,j+1))))
...
subroutine int_spec_vol_dp_wright_red(T, S, p_t, p_b, spv_ref, HI, dza, &
  ...
  real(wp),                 intent(in)  :: spv_ref   !< A mean specific volume that is subtracted out
  ...
  real(wp),       optional, intent(in)  :: dP_neglect !< A miniscule pressure change with
                                                 !! the same units as p_t [R L2 T-2 ~> Pa]
  integer,    optional, intent(in)  :: MassWghtInterp !< A flag indicating whether and how to use
                                                 !! mass weighting to interpolate T/S in integrals
  real(wp),       optional, intent(in)  :: SV_scale  !< A multiplicative factor by which to scale specific
  ...
  ! Local variables
  real(wp), dimension(HI%isd:HI%ied,HI%jsd:HI%jed) :: al0_2d ! A term in the Wright EOS [R-1 ~> m3 kg-1]
  real(wp), dimension(HI%isd:HI%ied,HI%jsd:HI%jed) :: p0_2d  ! A term in the Wright EOS [R L2 T-2 ~> Pa]
  real(wp), dimension(HI%isd:HI%ied,HI%jsd:HI%jed) :: lambda_2d ! A term in the Wright EOS [L2 T-2 ~> m2 s-2]
  real(wp) :: al0        ! A term in the Wright EOS [R-1 ~> m3 kg-1]
  real(wp) :: p0         ! A term in the Wright EOS [R L2 T-2 ~> Pa]
  real(wp) :: lambda     ! A term in the Wright EOS [L2 T-2 ~> m2 s-2]
  real(wp) :: al0_scale  ! Scaling factor to convert al0 from MKS units [kg m-3 R-1 ~> 1]
  real(wp) :: p0_scale   ! Scaling factor to convert p0 from MKS units [R L2 T-2 Pa-1 ~> 1]
  real(wp) :: lam_scale  ! Scaling factor to convert lambda from MKS units [L2 s2 T-2 m-2 ~> 1]
  real(wp) :: p_ave      ! The layer average pressure [R L2 T-2 ~> Pa]
  real(wp) :: rem        ! [L2 T-2 ~> m2 s-2]
  real(wp) :: eps, eps2  ! A nondimensional ratio and its square [nondim]
  real(wp) :: alpha_anom ! The depth averaged specific volume anomaly [R-1 ~> m3 kg-1].
  ...
  real(wp) :: I_pterm    ! The inverse of p0 plus p_ave [T2 R-1 L-2 ~> Pa-1].
  real(wp) :: hWt_LL, hWt_LR ! hWt_LA is the weighted influence of A on the left column [nondim].
  real(wp) :: hWt_RL, hWt_RR ! hWt_RA is the weighted influence of A on the right column [nondim].
  real(wp) :: wt_L, wt_R ! The linear weights of the left and right columns [nondim].
  real(wp) :: wtT_L, wtT_R ! The weights for tracers from the left and right columns [nondim].
  ...
  al0_scale = 1.0_wp ; if (present(SV_scale)) al0_scale = SV_scale
  p0_scale = 1.0_wp
  if (present(pres_scale)) then ; if (pres_scale /= 1.0_wp) then
    p0_scale = 1.0_wp / pres_scale
  endif ; endif
  ...
  do j=jsh,jeh ; do i=ish,ieh
    ...
    al0 = al0_2d(i,j) ; p0 = p0_2d(i,j) ; lambda = lambda_2d(i,j)
    dp = p_b(i,j) - p_t(i,j)
    p_ave = 0.5_wp*(p_t(i,j)+p_b(i,j))
    I_pterm = 1.0_wp / (p0 + p_ave)

    eps = 0.5_wp * dp * I_pterm ; eps2 = eps*eps
    alpha_anom = (al0 - spv_ref) + lambda * I_pterm
    rem = (lambda * eps2) * (C1_3 + eps2*(0.2_wp + eps2*(C1_7 + C1_9*eps2)))
    dza(i,j) = alpha_anom*dp + 2.0_wp*eps*rem
    if (present(intp_dza)) &
      intp_dza(i,j) = 0.5_wp*alpha_anom*dp**2 - dp*((1.0_wp-eps)*rem)
  enddo ; enddo
  ...
  if (present(intx_dza)) then ; do j=HI%jsc,HI%jec ; do I=Isq,Ieq
    ...
    do m=2,4
      ...
      p_ave = 0.5_wp*((wt_L*(p_t(i,j)+p_b(i,j))) + (wt_R*(p_t(i+1,j)+p_b(i+1,j))))
      I_pterm = 1.0_wp / (p0 + p_ave)

      eps = 0.5_wp * dp * I_pterm ; eps2 = eps*eps
      intp(m) = ((al0 - spv_ref) + lambda * I_pterm)*dp + 2.0_wp*eps* &
               lambda * eps2 * (C1_3 + eps2*(0.2_wp + eps2*(C1_7 + C1_9*eps2)))
    enddo
    ! Use Boole's rule to integrate the values.
  ...
  if (present(inty_dza)) then ; do J=Jsq,Jeq ; do i=HI%isc,HI%iec
    ...
    do m=2,4
      ...
      p_ave = 0.5_wp*((wt_L*(p_t(i,j)+p_b(i,j))) + (wt_R*(p_t(i,j+1)+p_b(i,j+1))))
      I_pterm = 1.0_wp / (p0 + p_ave)
