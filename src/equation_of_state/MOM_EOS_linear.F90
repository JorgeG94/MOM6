...
type, extends (EOS_base) :: linear_EOS
  ...
  real(wp) :: Rho_T0_S0 !< The density at T=0, S=0 and p=0 [kg m-3].
  real(wp) :: dRho_dT   !< The derivative of density with temperature [kg m-3 degC-1].
  real(wp) :: dRho_dS   !< The derivative of density with salinity [kg m-3 ppt-1].
  real(wp) :: dRho_dp   !< The derivative of density with pressure [s2 m-2].
...
real(wp) elemental function density_elem_linear(this, T, S, pressure)
...
!! This is an elemental function that can be applied to any combination of
!! scalar and array inputs.
real(wp) elemental function density_anomaly_elem_linear(this, T, S, pressure, rho_ref)
  class(linear_EOS), intent(in) :: this     !< This EOS
  real(wp), intent(in) :: T        !< Potential temperature relative to the surface [degC]
  real(wp), intent(in) :: S        !< Salinity [ppt]
  real(wp), intent(in) :: pressure !< Pressure [Pa]
...
!! This is an elemental function that can be applied to any combination of
!! scalar and array inputs.
real(wp) elemental function spec_vol_elem_linear(this, T, S, pressure)
  class(linear_EOS), intent(in) :: this     !< This EOS
  real(wp),              intent(in) :: T        !< Potential temperature relative to the surface [degC].
  real(wp),              intent(in) :: S        !< Salinity [ppt].
  real(wp),              intent(in) :: pressure !< Pressure [Pa].
  ...
  spec_vol_elem_linear = &
      1.0_wp / ( this%Rho_T0_S0 + ((this%dRho_dT*T + this%dRho_dS*S) + this%dRho_dp*pressure) )
...
!! This is an elemental function that can be applied to any combination of
!! scalar and array inputs.
real(wp) elemental function spec_vol_anomaly_elem_linear(this, T, S, pressure, spv_ref)
  class(linear_EOS), intent(in) :: this     !< This EOS
  real(wp),              intent(in) :: T        !< Potential temperature relative to the surface [degC].
  real(wp),              intent(in) :: S        !< Salinity [ppt].
  real(wp),              intent(in) :: pressure !< Pressure [Pa].
...
elemental subroutine calculate_density_second_derivs_elem_linear(this, T, S, pressure, &
  ...
  real(wp), intent(in)    :: S           !< Salinity [ppt].
  real(wp), intent(in)    :: pressure    !< pressure [Pa].
  real(wp), intent(inout) :: drho_dS_dS  !< The second derivative of density with
                                     !! salinity [kg m-3 ppt-2].
  real(wp), intent(inout) :: drho_dS_dT  !< The second derivative of density with
                                     !! temperature and salinity [kg m-3 ppt-1 degC-1].
  real(wp), intent(inout) :: drho_dT_dT  !< The second derivative of density with
                                     !! temperature [kg m-3 degC-2].
  real(wp), intent(inout) :: drho_dS_dP  !< The second derivative of density with
                                     !! salinity and pressure [kg m-3 ppt-1 Pa-1].
  real(wp), intent(inout) :: drho_dT_dP  !< The second derivative of density with
...
elemental subroutine calculate_specvol_derivs_elem_linear(this, T, S, pressure, dSV_dT, dSV_dS)
  ...
  ! Local variables
  real(wp) :: I_rho2  ! The inverse of density squared [m6 kg-2]

  ! Sv = 1.0 / (Rho_T0_S0 + dRho_dT*T + dRho_dS*S)
  I_rho2 = 1.0_wp / (this%Rho_T0_S0 + ((this%dRho_dT*T + this%dRho_dS*S) + this%dRho_dp*pressure))**2
...
subroutine avg_spec_vol_linear(T, S, p_t, dp, SpV_avg, start, npts, Rho_T0_S0, dRho_dT, dRho_dS, dRho_dp)
  ...
  real(wp),               intent(in)    :: Rho_T0_S0 !< The density at T=0, S=0 [kg m-3]
  real(wp),               intent(in)    :: dRho_dT   !< The derivative of density with temperature
                                                 !! [kg m-3 degC-1]
  real(wp),               intent(in)    :: dRho_dS   !< The derivative of density with salinity
                                                 !! [kg m-3 ppt-1]
  real(wp),               intent(in)    :: dRho_dp   !< The derivative of density with pressure
                                                 !! [s2 m-2]
  ! Local variables
  real(wp) :: eps2        ! The square of a nondimensional ratio [nondim]
  real(wp) :: alpha_p_ave ! The specific volume at pressure mid-point [R-1 ~> m3 kg-1]
  real(wp), parameter :: C1_3 = 1.0_wp/3.0_wp, C1_7 = 1.0_wp/7.0_wp, C1_9 = 1.0_wp/9.0_wp ! Rational constants [nondim]
  integer :: j

  do j=start,start+npts-1
    alpha_p_ave = &
      1.0_wp / (Rho_T0_S0 + ((dRho_dT*T(j) + dRho_dS*S(j)) + dRho_dp*(p_t(j) + 0.5_wp * dp(j))))
    eps2 = (0.5_wp * (dRho_dp * dp(j)) * alpha_p_ave)**2
    SpV_avg(j) = alpha_p_ave * (1.0_wp + eps2 * (C1_3 + eps2 * (0.2_wp + eps2 * (C1_7 + C1_9 * eps2))))
  enddo
...
subroutine EoS_fit_range_linear(this, T_min, T_max, S_min, S_max, p_min, p_max)
  ...
  if (present(T_min)) T_min = -273.0_wp
  if (present(T_max)) T_max = 100.0_wp
  if (present(S_min)) S_min = 0.0_wp
  if (present(S_max)) S_max = 1000.0_wp
  if (present(p_min)) p_min = 0.0_wp
  if (present(p_max)) p_max = 1.0e9_wp
...
subroutine set_params_linear(this, Rho_T0_S0, dRho_dT, dRho_dS, dRho_dp)
  ...
  real(wp), optional,    intent(in)    :: Rho_T0_S0 !< The density at T=0, S=0 [kg m-3]
  real(wp), optional,    intent(in)    :: dRho_dT   !< The derivative of density with temperature,
                                                !! [kg m-3 degC-1]
  real(wp), optional,    intent(in)    :: dRho_dS   !< The derivative of density with salinity,
                                                !! in [kg m-3 ppt-1]
  real(wp), optional,    intent(in)    :: dRho_dp   !< The derivative of density with pressure,
                                                !! in [s2 m-2]
...
subroutine int_density_dz_linear(T, S, z_t, z_b, rho_ref, rho_0, G_e, HI, &
  ...
  real(wp),                 intent(in)  :: rho_ref   !< A mean density [R ~> kg m-3], that
                                                 !! is subtracted out to reduce the magnitude of
                                                 !! each of the integrals.
  real(wp),                 intent(in)  :: rho_0     !< A density [R ~> kg m-3], used to calculate
                                                 !! the pressure (as p~=-z*rho_0*G_e) used in
                                                 !! the equation of state.
  real(wp),                 intent(in)  :: G_e       !< The Earth's gravitational acceleration
                                                 !! [L2 Z-1 T-2 ~> m s-2]
  real(wp),                 intent(in)  :: Rho_T0_S0 !< The density at T=0, S=0 [R ~> kg m-3]
  real(wp),                 intent(in)  :: dRho_dT   !< The derivative of density with temperature,
                                                 !! [R C-1 ~> kg m-3 degC-1]
  real(wp),                 intent(in)  :: dRho_dS   !< The derivative of density with salinity,
  ...
  real(wp) :: rho_anom      ! The density anomaly from rho_ref [R ~> kg m-3].
  real(wp) :: raL, raR      ! rho_anom to the left and right [R ~> kg m-3].
  real(wp) :: dz, dzL, dzR  ! Layer thicknesses [Z ~> m].
  real(wp) :: GxRho      ! The gravitational acceleration times mean ocean density [R L2 Z-1 T-2 ~> Pa m-1]
  real(wp) :: p_ave      ! The layer averaged pressure [R L2 T-2 ~> Pa]
  ...
  real(wp) :: intz(5)    ! The integrals of density with height at the
  ...
  real(wp), parameter :: C1_6 = 1.0_wp/6.0_wp, C1_90 = 1.0_wp/90.0_wp  ! Rational constants [nondim].
  ...
  do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
    ...
    if (present(intz_dpa)) &
      intz_dpa(i,j) = 0.5_wp * G_e * (rho_anom - C1_6 * dRho_dp * (GxRho * dz)) * dz**2
  enddo ; enddo
  ...
  if (present(intx_dpa)) then ; do j=js,je ; do I=Isq,Ieq
    ...
    if (hWght <= 0.0_wp) then
      ...
      p_ave = -GxRho * (0.5_wp * (z_t(i+1,j) + z_b(i+1,j)) - z0pres(i+1,j))
      raR = (Rho_T0_S0 - rho_ref) + ((dRho_dT*T(i+1,j) + dRho_dS*S(i+1,j)) + dRho_dp*p_ave)

      intx_dpa(i,j) = G_e*C1_6 * ((dzL*(2.0_wp*raL + raR)) + (dzR*(2.0_wp*raR + raL)))
    else
  ...
  if (present(inty_dpa)) then ; do J=Jsq,Jeq ; do i=is,ie
    ...
    if (hWght <= 0.0_wp) then
      ...
      p_ave = -GxRho * (0.5_wp * (z_t(i,j+1) + z_b(i,j+1)) - z0pres(i,j+1))
      raR = (Rho_T0_S0 - rho_ref) + ((dRho_dT*T(i,j+1) + dRho_dS*S(i,j+1)) + dRho_dp*p_ave)

      inty_dpa(i,j) = G_e*C1_6 * ((dzL*(2.0_wp*raL + raR)) + (dzR*(2.0_wp*raR + raL)))
    else
      hL = (z_t(i,j) - z_b(i,j)) + dz_neglect
...
subroutine int_spec_vol_dp_linear(T, S, p_t, p_b, alpha_ref, HI, Rho_T0_S0, &
  ...
  integer,    optional, intent(in)  :: MassWghtInterp !< A flag indicating whether and how to use
                                                 !! mass weighting to interpolate T/S in integrals
  ! Local variables
  real(wp) :: dRho          ! The density anomaly due to T, S and p [R ~> kg m-3]
  real(wp) :: lambda        ! The sound speed squared [L2 T-2 ~> m2 s-2]
  real(wp) :: eps, eps2     ! A nondimensional ratio and its square [nondim]
  real(wp) :: rem           ! [L2 T-2 ~> m2 s-2]
  real(wp) :: p_ave         ! The layer averaged pressure [R L2 T-2 ~> Pa]
  real(wp) :: alpha_p_ave   ! The specific volume at p_ave [R-1 ~> m3 kg-1]
  real(wp) :: alpha_anom    ! The specific volume anomaly from 1/rho_ref [R-1 ~> m3 kg-1]
  real(wp) :: aaL, aaR      ! The specific volume anomaly to the left and right [R-1 ~> m3 kg-1]
  real(wp) :: dp, dpL, dpR  ! Layer pressure thicknesses [R L2 T-2 ~> Pa]
  ...
  lambda = 0.0_wp ; if (dRho_dp/=0.0_wp) lambda = 1.0_wp / dRho_dp
  do j=jsh,jeh ; do i=ish,ieh
    dp = p_b(i,j) - p_t(i,j)
    p_ave = 0.5_wp * (p_t(i,j) + p_b(i,j))

    drho = (dRho_dT * T(i,j) + dRho_dS * S(i,j)) + dRho_dp * p_ave
    alpha_p_ave = 1.0_wp / (Rho_T0_S0 + drho)
    ...
    ! point precision.
    eps = 0.5_wp * (dRho_dp * dp) * alpha_p_ave ; eps2 = eps * eps
    ! alpha_anom = 1.0/(Rho_T0_S0 + dRho) - alpha_ref
    alpha_anom = ((1.0_wp - Rho_T0_S0 * alpha_ref) - drho * alpha_ref) / (Rho_T0_S0 + drho)
  ...
  if (present(intx_dza)) then ; do j=HI%jsc,HI%jec ; do I=Isq,Ieq
    ...
    if (hWght <= 0.0_wp) then
      ...
      p_ave = 0.5_wp * (p_b(i,j) + p_t(i,j))
      drho = (dRho_dT*T(i,j) + dRho_dS*S(i,j)) + dRho_dp * p_ave
      aaL = ((1.0_wp - Rho_T0_S0*alpha_ref) - drho*alpha_ref) / (Rho_T0_S0 + drho)

      p_ave = 0.5_wp * (p_b(i+1,j) + p_t(i+1,j))
      drho = (dRho_dT*T(i+1,j) + dRho_dS*S(i+1,j)) + dRho_dp * p_ave
      aaR = ((1.0_wp - Rho_T0_S0*alpha_ref) - drho*alpha_ref) / (Rho_T0_S0 + drho)

      intx_dza(i,j) = C1_6 * (2.0_wp*((dpL*aaL) + (dpR*aaR)) + ((dpL*aaR) + (dpR*aaL)))
    else
  ...
  if (present(inty_dza)) then ; do J=Jsq,Jeq ; do i=HI%isc,HI%iec
    ...
    if (hWght <= 0.0_wp) then
      ...
      p_ave = 0.5_wp * (p_b(i,j) + p_t(i,j)) + dRho_dp * p_ave
      drho = (dRho_dT*T(i,j) + dRho_dS*S(i,j)) + dRho_dp * p_ave
      aaL = ((1.0_wp - Rho_T0_S0*alpha_ref) - drho*alpha_ref) / (Rho_T0_S0 + drho)

      p_ave = 0.5_wp * (p_b(i,j+1) + p_t(i,j+1)) + dRho_dp * p_ave
      drho = (dRho_dT*T(i,j+1) + dRho_dS*S(i,j+1)) + dRho_dp * p_ave
      aaR = ((1.0_wp - Rho_T0_S0*alpha_ref) - drho*alpha_ref) / (Rho_T0_S0 + drho)

      inty_dza(i,j) = C1_6 * (2.0_wp*((dpL*aaL) + (dpR*aaR)) + ((dpL*aaR) + (dpR*aaL)))
    else
      hL = (p_b(i,j) - p_t(i,j)) + dP_neglect
