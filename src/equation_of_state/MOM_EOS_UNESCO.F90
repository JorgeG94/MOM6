...
real(wp), parameter :: R00 = 999.842594_wp   ! A coefficient in the fit for rho0 [kg m-3]
real(wp), parameter :: R01 = 6.793952e-2_wp  ! A coefficient in the fit for rho0 [kg m-3 degC-1]
real(wp), parameter :: R02 = -9.095290e-3_wp ! A coefficient in the fit for rho0 [kg m-3 degC-2]
real(wp), parameter :: R03 = 1.001685e-4_wp  ! A coefficient in the fit for rho0 [kg m-3 degC-3]
real(wp), parameter :: R04 = -1.120083e-6_wp ! A coefficient in the fit for rho0 [kg m-3 degC-4]
real(wp), parameter :: R05 = 6.536332e-9_wp  ! A coefficient in the fit for rho0 [kg m-3 degC-5]
real(wp), parameter :: R10 = 0.824493_wp     ! A coefficient in the fit for rho0 [kg m-3 PSU-1]
real(wp), parameter :: R11 = -4.0899e-3_wp   ! A coefficient in the fit for rho0 [kg m-3 degC-1 PSU-1]
real(wp), parameter :: R12 = 7.6438e-5_wp    ! A coefficient in the fit for rho0 [kg m-3 degC-2 PSU-1]
real(wp), parameter :: R13 = -8.2467e-7_wp   ! A coefficient in the fit for rho0 [kg m-3 degC-3 PSU-1]
real(wp), parameter :: R14 = 5.3875e-9_wp    ! A coefficient in the fit for rho0 [kg m-3 degC-4 PSU-1]
real(wp), parameter :: R60 = -5.72466e-3_wp  ! A coefficient in the fit for rho0 [kg m-3 PSU-1.5]
real(wp), parameter :: R61 = 1.0227e-4_wp    ! A coefficient in the fit for rho0 [kg m-3 degC-1 PSU-1.5]
real(wp), parameter :: R62 = -1.6546e-6_wp   ! A coefficient in the fit for rho0 [kg m-3 degC-2 PSU-1.5]
real(wp), parameter :: R20 = 4.8314e-4_wp    ! A coefficient in the fit for rho0 [kg m-3 PSU-2]
...
real(wp), parameter :: S000 = 1.965933e4_wp   ! A coefficient in the secant bulk modulus fit [bar]
real(wp), parameter :: S010 = 1.444304e2_wp   ! A coefficient in the secant bulk modulus fit [bar degC-1]
real(wp), parameter :: S020 = -1.706103_wp    ! A coefficient in the secant bulk modulus fit [bar degC-2]
real(wp), parameter :: S030 = 9.648704e-3_wp  ! A coefficient in the secant bulk modulus fit [bar degC-3]
real(wp), parameter :: S040 = -4.190253e-5_wp ! A coefficient in the secant bulk modulus fit [bar degC-4]
real(wp), parameter :: S100 = 52.84855_wp     ! A coefficient in the secant bulk modulus fit [bar PSU-1]
real(wp), parameter :: S110 = -3.101089e-1_wp ! A coefficient in the secant bulk modulus fit [bar degC-1 PSU-1]
real(wp), parameter :: S120 = 6.283263e-3_wp  ! A coefficient in the secant bulk modulus fit [bar degC-2 PSU-1]
real(wp), parameter :: S130 = -5.084188e-5_wp ! A coefficient in the secant bulk modulus fit [bar degC-3 PSU-1]
real(wp), parameter :: S600 = 3.886640e-1_wp  ! A coefficient in the secant bulk modulus fit [bar PSU-1.5]
real(wp), parameter :: S610 = 9.085835e-3_wp  ! A coefficient in the secant bulk modulus fit [bar degC-1 PSU-1.5]
real(wp), parameter :: S620 = -4.619924e-4_wp ! A coefficient in the secant bulk modulus fit [bar degC-2 PSU-1.5]

real(wp), parameter :: S001 = 3.186519_wp     ! A coefficient in the secant bulk modulus fit [nondim]
real(wp), parameter :: S011 = 2.212276e-2_wp  ! A coefficient in the secant bulk modulus fit [degC-1]
real(wp), parameter :: S021 = -2.984642e-4_wp ! A coefficient in the secant bulk modulus fit [degC-2]
real(wp), parameter :: S031 = 1.956415e-6_wp  ! A coefficient in the secant bulk modulus fit [degC-3]
real(wp), parameter :: S101 = 6.704388e-3_wp  ! A coefficient in the secant bulk modulus fit [PSU-1]
real(wp), parameter :: S111 = -1.847318e-4_wp ! A coefficient in the secant bulk modulus fit [degC-1 PSU-1]
real(wp), parameter :: S121 = 2.059331e-7_wp  ! A coefficient in the secant bulk modulus fit [degC-2 PSU-1]
real(wp), parameter :: S601 = 1.480266e-4_wp  ! A coefficient in the secant bulk modulus fit [PSU-1.5]

real(wp), parameter :: S002 = 2.102898e-4_wp  ! A coefficient in the secant bulk modulus fit [bar-1]
real(wp), parameter :: S012 = -1.202016e-5_wp ! A coefficient in the secant bulk modulus fit [bar-1 degC-1]
real(wp), parameter :: S022 = 1.394680e-7_wp  ! A coefficient in the secant bulk modulus fit [bar-1 degC-2]
real(wp), parameter :: S102 = -2.040237e-6_wp ! A coefficient in the secant bulk modulus fit [bar-1 PSU-1]
real(wp), parameter :: S112 = 6.128773e-8_wp  ! A coefficient in the secant bulk modulus fit [bar-1 degC-1 PSU-1]
real(wp), parameter :: S122 = 6.207323e-10_wp ! A coefficient in the secant bulk modulus fit [bar-1 degC-2 PSU-1]
!>@}
...
real(wp) elemental function density_elem_UNESCO(this, T, S, pressure)
  ...
  ! Local variables
  real(wp) :: t1   ! A copy of the temperature at a point [degC]
  real(wp) :: s1   ! A copy of the salinity at a point [PSU]
  real(wp) :: p1   ! Pressure converted to bars [bar]
  real(wp) :: s12  ! The square root of salinity [PSU1/2]
  real(wp) :: rho0 ! Density at 1 bar pressure [kg m-3]
  real(wp) :: sig0 ! The anomaly of rho0 from R00 [kg m-3]
  real(wp) :: ks   ! The secant bulk modulus [bar]

  p1 = pressure*1.0e-5_wp ; t1 = T
  s1 = max(S, 0.0_wp) ; s12 = sqrt(s1)
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function density_anomaly_elem_UNESCO(this, T, S, pressure, rho_ref)
  class(UNESCO_EOS), intent(in) :: this     !< This EOS
  real(wp),              intent(in) :: T        !< Potential temperature relative to the surface [degC]
  real(wp),              intent(in) :: S        !< Salinity [PSU]
  real(wp),              intent(in) :: pressure !< Pressure [Pa]
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function spec_vol_elem_UNESCO(this, T, S, pressure)
  class(UNESCO_EOS), intent(in) :: this    !< This EOS
  real(wp),           intent(in) :: T        !< Potential temperature relative to the surface [degC]
  real(wp),           intent(in) :: S        !< Salinity [PSU]
  real(wp),           intent(in) :: pressure !< Pressure [Pa]
  ...
  real(wp) :: s1   ! A copy of the salinity at a point [PSU]
  real(wp) :: p1   ! Pressure converted to bars [bar]
  real(wp) :: s12  ! The square root of salinity [PSU1/2]l553
  real(wp) :: rho0 ! Density at 1 bar pressure [kg m-3]
  real(wp) :: ks   ! The secant bulk modulus [bar]
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function spec_vol_anomaly_elem_UNESCO(this, T, S, pressure, spv_ref)
  class(UNESCO_EOS), intent(in) :: this    !< This EOS
  real(wp),           intent(in) :: T        !< Potential temperature relative to the surface [degC]
  real(wp),           intent(in) :: S        !< Salinity [PSU]
  real(wp),           intent(in) :: pressure !< Pressure [Pa]
  ...
  spec_vol_anomaly_elem_UNESCO = (ks*(1.0_wp - (rho0*spv_ref)) - p1) / (rho0*ks)
...
elemental subroutine calculate_density_derivs_elem_UNESCO(this, T, S, pressure, drho_dT, drho_dS)
  ...
  real(wp) :: rho0     ! Density at 1 bar pressure [kg m-3]
  real(wp) :: ks       ! The secant bulk modulus [bar]
  real(wp) :: drho0_dT ! Derivative of rho0 with T [kg m-3 degC-1]
  real(wp) :: drho0_dS ! Derivative of rho0 with S [kg m-3 PSU-1]
  real(wp) :: dks_dT   ! Derivative of ks with T [bar degC-1]
  real(wp) :: dks_dS   ! Derivative of ks with S [bar psu-1]
  real(wp) :: I_denom  ! 1.0 / (ks - p1) [bar-1]
  ...
  rho0 = R00 + ( t1*(R01 + t1*(R02 + t1*(R03 + t1*(R04 + t1*R05)))) + &
                 s1*((R10 + t1*(R11 + t1*(R12 + t1*(R13 + t1*R14)))) + &
                     (s12*(R60 + t1*(R61 + t1*R62)) + s1*R20)) )
  drho0_dT = R01 + ( t1*(2.0_wp*R02 + t1*(3.0_wp*R03 + t1*(4.0_wp*R04 + t1*(5.0_wp*R05)))) + &
                     s1*(R11 + (t1*(2.0_wp*R12 + t1*(3.0_wp*R13 + t1*(4.0_wp*R14))) + &
                                s12*(R61 + t1*(2.0_wp*R62)) )) )
  drho0_dS = R10 + ( t1*(R11 + t1*(R12 + t1*(R13 + t1*R14))) + &
                     (1.5_wp*(s12*(R60 + t1*(R61 + t1*R62))) + s1*(2.0_wp*R20)) )
  ...
  ks = ( S000 + (t1*(S010 + t1*(S020 + t1*(S030 + t1*S040))) + &
       ...
       p1*( (S001 + ( t1*(S011 + t1*(S021 + t1*S031)) + &
                      s1*((S101 + t1*(S111 + t1*S121)) + s12*S601) )) + &
            p1*(S002 + ( t1*(S012 + t1*S022) + s1*(S102 + t1*(S112 + t1*S122)) )) )
  dks_dT = ( S010 + (t1*(2.0_wp*S020 + t1*(3.0_wp*S030 + t1*(4.0_wp*S040))) + &
                     s1*((S110 + t1*(2.0_wp*S120 + t1*(3.0_wp*S130))) + s12*(S610 + t1*(2.0_wp*S620)))) ) + &
           p1*(((S011 + t1*(2.0_wp*S021 + t1*(3.0_wp*S031))) + s1*(S111 + t1*(2.0_wp*S121)) ) + &
               p1*(S012 + t1*(2.0_wp*S022) + s1*(S112 + t1*(2.0_wp*S122))) )
  dks_dS = ( S100 + (t1*(S110 + t1*(S120 + t1*S130)) + 1.5_wp*(s12*(S600 + t1*(S610 + t1*S620)))) ) + &
           p1*((S101 + t1*(S111 + t1*S121) + s12*(1.5_wp*S601)) + &
               p1*(S102 + t1*(S112 + t1*S122)) )

  I_denom = 1.0_wp / (ks - p1)
...
elemental subroutine calculate_density_second_derivs_elem_UNESCO(this, T, S, pressure, &
  ...
  real(wp) :: p1          ! Pressure converted to bars [bar]
  real(wp) :: s12         ! The square root of salinity [PSU1/2]
  real(wp) :: I_s12       ! The inverse of the square root of salinity [PSU-1/2]
  real(wp) :: rho0        ! Density at 1 bar pressure [kg m-3]
  real(wp) :: drho0_dT    ! Derivative of rho0 with T [kg m-3 degC-1]
  real(wp) :: drho0_dS    ! Derivative of rho0 with S [kg m-3 PSU-1]
  real(wp) :: d2rho0_dS2  ! Second derivative of rho0 with salinity [kg m-3 PSU-1]
  real(wp) :: d2rho0_dSdT ! Second derivative of rho0 with temperature and salinity [kg m-3 degC-1 PSU-1]
  real(wp) :: d2rho0_dT2  ! Second derivative of rho0 with temperature [kg m-3 degC-2]
  real(wp) :: ks          ! The secant bulk modulus [bar]
  real(wp) :: ks_0        ! The secant bulk modulus at zero pressure [bar]
  real(wp) :: ks_1        ! The linear pressure dependence of the secant bulk modulus at zero pressure [nondim]
  real(wp) :: ks_2        ! The quadratic pressure dependence of the secant bulk modulus at zero pressure [bar-1]
  real(wp) :: dks_dp      ! The derivative of the secant bulk modulus with pressure [nondim]
  real(wp) :: dks_dT      ! Derivative of the secant bulk modulus with temperature [bar degC-1]
  real(wp) :: dks_dS      ! Derivative of the secant bulk modulus with salinity [bar psu-1]
  real(wp) :: d2ks_dT2    ! Second derivative of the secant bulk modulus with temperature [bar degC-2]
  real(wp) :: d2ks_dSdT   ! Second derivative of the secant bulk modulus with salinity and temperature [bar psu-1 degC-1]
  real(wp) :: d2ks_dS2    ! Second derivative of the secant bulk modulus with salinity [bar psu-2]
  real(wp) :: d2ks_dSdp   ! Second derivative of the secant bulk modulus with salinity and pressure [psu-1]
  real(wp) :: d2ks_dTdp   ! Second derivative of the secant bulk modulus with temperature and pressure [degC-1]
  real(wp) :: I_denom     ! The inverse of the denominator of the expression for density [bar-1]
  ...
  I_s12 = 1.0_wp / (max(s12, 1.0e-4_wp))
  ...
  drho0_dS = R10 + ( t1*(R11 + t1*(R12 + t1*(R13 + t1*R14))) + &
                     (1.5_wp*(s12*(R60 + t1*(R61 + t1*R62))) + s1*(2.0_wp*R20)) )
  d2rho0_dS2 = 0.75_wp*(R60 + t1*(R61 + t1*R62))*I_s12 + 2.0_wp*R20
  d2rho0_dSdT = R11 + ( t1*(2.0_wp*R12 + t1*(3.0_wp*R13 + t1*(4.0_wp*R14))) + s12*(1.5_wp*R61 + t1*(3.0_wp*R62)) )
  d2rho0_dT2 = 2.0_wp*R02 + ( t1*(6.0_wp*R03 + t1*(12.0_wp*R04 + t1*(20.0_wp*R05))) + &
                           s1*((2.0_wp*R12 + t1*(6.0_wp*R13 + t1*(12.0_wp*R14))) + s12*(2.0_wp*R62)) )
  ...
  dks_dS = (S100 + ( t1*(S110 + t1*(S120 + t1*S130)) + 1.5_wp*(s12*(S600 + t1*(S610 + t1*S620))) )) + &
           p1*((S101 + t1*(S111 + t1*S121) + s12*(1.5_wp*S601)) + &
               p1*(S102 + t1*(S112 + t1*S122)))
  d2ks_dS2 = 0.75_wp*((S600 + t1*(S610 + t1*S620)) + p1*S601)*I_s12
  d2ks_dSdT = (S110 + ( t1*(2.0_wp*S120 + t1*(3.0_wp*S130)) + s12*(1.5_wp*S610 + t1*(3.0_wp*S620)) )) + &
              p1*((S111 + t1*(2.0_wp*S121)) +  p1*(S112 + t1*(2.0_wp*S122)))
  d2ks_dT2 = 2.0_wp*(S020 + ( t1*(3.0_wp*S030 + t1*(6.0_wp*S040)) + s1*((S120 + t1*(3.0_wp*S130)) + s12*S620) )) + &
             2.0_wp*p1*((S021 + (t1*(3.0_wp*S031) + s1*S121)) + p1*(S022 + s1*S122))

  d2ks_dSdp = (S101 + (t1*(S111 + t1*S121) + s12*(1.5_wp*S601))) + &
              2.0_wp*p1*(S102 + t1*(S112 + t1*S122))
  d2ks_dTdp = (S011 + (t1*(2.0_wp*S021 + t1*(3.0_wp*S031)) + s1*(S111 + t1*(2.0_wp*S121)))) + &
              2.0_wp*p1*(S012 + t1*(2.0_wp*S022) + s1*(S112 + t1*(2.0_wp*S122)))
  I_denom = 1.0_wp / (ks - p1)
  ...
  drho_dS_dS = I_denom * ( ks*d2rho0_dS2 - (p1*I_denom) * &
                    (2.0_wp*drho0_dS*dks_dS + rho0*(d2ks_dS2 - 2.0_wp*dks_dS**2*I_denom)) )
  drho_dS_dT = I_denom * (ks * d2rho0_dSdT - (p1*I_denom) * &
                      ((drho0_dT*dks_dS + drho0_dS*dks_dT) + &
                       rho0*(d2ks_dSdT - 2.0_wp*(dks_dS*dks_dT)*I_denom)) )
  drho_dT_dT = I_denom * ( ks*d2rho0_dT2 - (p1*I_denom) * &
                    (2.0_wp*drho0_dT*dks_dT + rho0*(d2ks_dT2 - 2.0_wp*dks_dT**2*I_denom)) )

  ! The factor of 1.0e-5 is because pressure here is in bars, not Pa.
  drho_dS_dp = (1.0e-5_wp * I_denom**2) * ( (ks*drho0_dS - rho0*dks_dS) - &
                    p1*( (dks_dp*drho0_dS + rho0*d2ks_dSdp) - &
                         2.0_wp*(rho0*dks_dS) * ((dks_dp - 1.0_wp)*I_denom) ) )
  drho_dT_dp = (1.0e-5_wp * I_denom**2) * ( (ks*drho0_dT - rho0*dks_dT) - &
                    p1*( (dks_dp*drho0_dT + rho0*d2ks_dTdp) - &
                         2.0_wp*(rho0*dks_dT) * ((dks_dp - 1.0_wp)*I_denom) ) )
...
elemental subroutine calculate_specvol_derivs_elem_UNESCO(this, T, S, pressure, dSV_dT, dSV_dS)
  ...
  real(wp) :: dks_dT   ! Derivative of ks with T [bar degC-1]
  real(wp) :: dks_dS   ! Derivative of ks with S [bar psu-1]
  real(wp) :: I_denom2 ! 1.0 / (rho0*ks)**2 [m6 kg-2 bar-2]
...
elemental subroutine calculate_compress_elem_UNESCO(this, T, S, pressure, rho, drho_dp)
  ...
  rho = rho0*ks * I_denom
  ! The factor of 1.0e-5 is because pressure here is in bars, not Pa.
  drho_dp = 1.0e-5_wp * ((rho0 * (ks - p1*dks_dp)) * I_denom**2)
