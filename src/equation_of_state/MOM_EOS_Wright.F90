...
type, extends (EOS_base) :: buggy_Wright_EOS
  ...
  real(wp) :: three = 3.0_wp !< A constant that can be adjusted to recreate some bugs [nondim]
...
real(wp) elemental function density_elem_buggy_Wright(this, T, S, pressure)
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function density_anomaly_elem_buggy_Wright(this, T, S, pressure, rho_ref)
  class(buggy_Wright_EOS), intent(in) :: this !< This EOS
  real(wp), intent(in) :: T        !< potential temperature relative to the surface [degC].
  real(wp), intent(in) :: S        !< salinity [PSU].
  real(wp), intent(in) :: pressure !< pressure [Pa].
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function spec_vol_elem_buggy_Wright(this, T, S, pressure)
  class(buggy_Wright_EOS), intent(in) :: this !< This EOS
  real(wp),           intent(in) :: T        !< potential temperature relative to the surface [degC].
  real(wp),           intent(in) :: S        !< salinity [PSU].
  real(wp),           intent(in) :: pressure !< pressure [Pa].
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function spec_vol_anomaly_elem_buggy_Wright(this, T, S, pressure, spv_ref)
  class(buggy_Wright_EOS), intent(in) :: this !< This EOS
  real(wp),           intent(in) :: T        !< potential temperature relative to the surface [degC].
  real(wp),           intent(in) :: S        !< salinity [PSU].
  real(wp),           intent(in) :: pressure !< pressure [Pa].
...
elemental subroutine calculate_density_derivs_elem_buggy_Wright(this, T, S, pressure, drho_dT, drho_dS)
  ...
  I_denom2 = 1.0_wp / (lambda + al0*(pressure + p0))
  I_denom2 = I_denom2 *I_denom2
  drho_dT = I_denom2 * &
    (lambda* (b1 + T*(2.0_wp*b2 + 3.0_wp*b3*T) + b5*S) - &
     (pressure+p0) * ( (pressure+p0)*a1 + &
      (c1 + T*(c2*2.0_wp + c3*3.0_wp*T) + c5*S) ))
...
elemental subroutine calculate_density_second_derivs_elem_buggy_Wright(this, T, S, pressure, &
  ...
  ! Local variables
  real(wp) :: z0, z1 ! Local work variables [Pa]
  real(wp) :: z2, z4 ! Local work variables [m2 s-2]
  real(wp) :: z3, z5 ! Local work variables [Pa degC-1]
  real(wp) :: z6, z8 ! Local work variables [m2 s-2 degC-1]
  real(wp) :: z7     ! A local work variable [m2 s-2 PSU-1]
  real(wp) :: z9     ! A local work variable [m3 kg-1]
  real(wp) :: z10    ! A local work variable [Pa PSU-1]
  real(wp) :: z11    ! A local work variable [Pa m2 s-2 PSU-1] = [kg m s-4 PSU-1]
  real(wp) :: z2_2   ! A local work variable [m4 s-4]
  real(wp) :: z2_3   ! A local work variable [m6 s-6]
  real(wp) :: six    ! A constant that can be adjusted from 6. to 4. to recreate a bug [nondim]
  ...
  six = 2.0_wp*this%three  ! When recreating a bug from the original version of this routine, six = 4.

  z0 = T*(b1 + b5*S + T*(b2 + b3*T))
  z1 = (b0 + pressure + b4*S + z0)
  z3 = (b1 + b5*S + T*(2._wp*b2 + this%three*b3*T))  ! When recreating a bug here this%three = 2.
  z4 = (c0 + c4*S + T*(c1 + c5*S + T*(c2 + c3*T)))
  z5 = (b1 + b5*S + T*(b2 + b3*T) + T*(b2 + 2._wp*b3*T))
  z6 = c1 + c5*S + T*(c2 + c3*T) + T*(c2 + 2._wp*c3*T)
  z7 = (c4 + c5*T + a2*z1)
  z8 = (c1 + c5*S + T*(2._wp*c2 + 3._wp*c3*T) + a1*z1)
  z9 = (a0 + a2*S + a1*T)
  z10 = (b4 + b5*T)
  z11 = (z10*z4 - z1*z7)
  ...
  drho_ds_ds = (z10*(c4 + c5*T) - a2*z10*z1 - z10*z7)/z2_2 - (2._wp*(c4 + c5*T + z9*z10 + a2*z1)*z11)/z2_3
  drho_ds_dt = (z10*z6 - z1*(c5 + a2*z5) + b5*z4 - z5*z7)/z2_2 - (2._wp*(z6 + z9*z5 + a1*z1)*z11)/z2_3
  drho_dt_dt = (z3*z6 - z1*(2._wp*c2 + 6._wp*c3*T + a1*z5) + (2._wp*b2 + six*b3*T)*z4 - z5*z8)/z2_2 - &
                  (2._wp*(z6 + z9*z5 + a1*z1)*(z3*z4 - z1*z8))/z2_3
  drho_ds_dp = (-c4 - c5*T - 2._wp*a2*z1)/z2_2 - (2._wp*z9*z11)/z2_3
  drho_dt_dp = (-c1 - c5*S - T*(2._wp*c2 + 3._wp*c3*T) - 2._wp*a1*z1)/z2_2 - (2._wp*z9*(z3*z4 - z1*z8))/z2_3
...
! al0 = (a0 + a1*T) + a2*S
  ...
  dSV_dT = (a1 + I_denom * (c1 + T*((2.0_wp*c2 + 3.0_wp*c3*T)) + c5*S)) - &
              (I_denom**2 * lambda) *  (b1 + T*((2.0_wp*b2 + 3.0_wp*b3*T)) + b5*S)
...
subroutine int_density_dz_wright(T, S, z_t, z_b, rho_ref, rho_0, G_e, HI, &
  ...
  do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
    ...
    I_al0 = 1.0_wp / al0
    I_Lzz = 1.0_wp / (p0 + (lambda * I_al0) + p_ave)
...
subroutine int_spec_vol_dp_wright(T, S, p_t, p_b, spv_ref, HI, dza, &
  ...
  do j=jsh,jeh ; do i=ish,ieh
    ...
    eps = 0.5_wp * dp / (p0 + p_ave) ; eps2 = eps*eps
  ...
  if (present(intx_dza)) then ; do j=HI%jsc,HI%jec ; do I=Isq,Ieq
    ...
    do m=2,4
      ...
      eps = 0.5_wp * dp / (p0 + p_ave) ; eps2 = eps*eps
      intp(m) = (al0 + lambda / (p0 + p_ave) - spv_ref)*dp + 2.0_wp*eps* &
...
subroutine set_params_buggy_Wright(this, use_Wright_2nd_deriv_bug)
  ...
  if (present(use_Wright_2nd_deriv_bug)) then
    if (use_Wright_2nd_deriv_bug) then ; this%three = 2.0_wp
    else  ; this%three = 3.0_wp ; endif
  endif
