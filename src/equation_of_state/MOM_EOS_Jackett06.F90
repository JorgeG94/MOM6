...
real(wp), parameter :: &
  RN000 =  9.9984085444849347e+02_wp, & ! Density numerator constant coefficient [kg m-3]
  RN001 =  1.1798263740430364e-06_wp, & ! Density numerator P       coefficient [kg m-3 Pa-1]
  RN002 = -2.5862187075154352e-16_wp, & ! Density numerator P^2     coefficient [kg m-3 Pa-2]
  RN010 =  7.3471625860981584e+00_wp, & ! Density numerator T       coefficient [kg m-3 degC-1]
  RN020 = -5.3211231792841769e-02_wp, & ! Density numerator T^2     coefficient [kg m-3 degC-2]
  RN021 =  9.8920219266399117e-12_wp, & ! Density numerator T^2 P   coefficient [kg m-3 degC-2 Pa-1]
  RN022 = -3.2921414007960662e-20_wp, & ! Density numerator T^2 P^2 coefficient [kg m-3 degC-2 Pa-2]
  RN030 =  3.6492439109814549e-04_wp, & ! Density numerator T^3     coefficient [kg m-3 degC-3]
  RN100 =  2.5880571023991390e+00_wp, & ! Density numerator S       coefficient [kg m-3 PSU-1]
  RN101 =  4.6996642771754730e-10_wp, & ! Density numerator S P     coefficient [kg m-3 PSU-1 Pa-1]
  RN110 = -6.7168282786692355e-03_wp, & ! Density numerator S T     coefficient [kg m-3 degC-1 PSU-1]
  RN200 =  1.9203202055760151e-03_wp, & ! Density numerator S^2      coefficient [kg m-3]

  RD001 =  6.7103246285651894e-10_wp, & ! Density denominator P       coefficient [Pa-1]
  RD010 =  7.2815210113327091e-03_wp, & ! Density denominator T       coefficient [degC-1]
  RD013 = -9.1534417604289062e-30_wp, & ! Density denominator T P^3   coefficient [degC-1 Pa-3]
  RD020 = -4.4787265461983921e-05_wp, & ! Density denominator T^2     coefficient [degC-2]
  RD030 =  3.3851002965802430e-07_wp, & ! Density denominator T^3     coefficient [degC-3]
  RD032 = -2.4461698007024582e-25_wp, & ! Density denominator T^3 P^2 coefficient [degC-3  Pa-2]
  RD040 =  1.3651202389758572e-10_wp, & ! Density denominator T^4     coefficient [degC-4]
  RD100 =  1.7632126669040377e-03_wp, & ! Density denominator S       coefficient [PSU-1]
  RD110 = -8.8066583251206474e-06_wp, & ! Density denominator S T     coefficient [degC-1 PSU-1]
  RD130 = -1.8832689434804897e-10_wp, & ! Density denominator S T^3   coefficient [degC-3 PSU-1]
  RD600 =  5.7463776745432097e-06_wp, & ! Density denominator S^1.5   coefficient [PSU-1.5]
  RD620 =  1.4716275472242334e-09_wp    ! Density denominator S^1.5 T^2 coefficient [degC-2 PSU-1.5]
!>@}
...
real(wp) elemental function density_elem_Jackett06(this, T, S, pressure)
  ...
  ! Local variables
  real(wp) :: num_STP ! State dependent part of the numerator of the rational expresion
                  ! for density [kg m-3]
  real(wp) :: den     ! Denominator of the rational expresion for density [nondim]
  real(wp) :: den_STP ! State dependent part of the denominator of the rational expresion
                  ! for density [nondim]
  real(wp) :: I_den   ! The inverse of the denominator of the rational expresion for density [nondim]
  real(wp) :: T2      ! Temperature squared [degC2]
  real(wp) :: S1_2    ! Limited square root of salinity [PSU1/2]

  S1_2 = sqrt(max(0.0_wp,s))
  T2 = T*T

  num_STP = (T*(RN010 + T*(RN020 + T*RN030)) + &
             S*(RN100 + (T*RN110 + S*RN200)) ) + &
            pressure*(RN001 + ((T2*RN021 + S*RN101) + pressure*(RN002 + T2*RN022)))
  den = 1.0_wp + ((T*(RD010 + T*(RD020 + T*(RD030 + T* RD040))) + &
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function density_anomaly_elem_Jackett06(this, T, S, pressure, rho_ref)
  class(Jackett06_EOS), intent(in) :: this !< This EOS
  real(wp), intent(in) :: T        !< Potential temperature relative to the surface [degC].
  real(wp), intent(in) :: S        !< Salinity [PSU].
  real(wp), intent(in) :: pressure !< Pressure [Pa].
  ...
  real(wp) :: T2      ! Temperature squared [degC2]
  real(wp) :: S1_2    ! Limited square root of salinity [PSU1/2]
  real(wp) :: rho0    ! The surface density of fresh water at 0 degC, perhaps less the refernce density [kg m-3]
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function spec_vol_elem_Jackett06(this, T, S, pressure)
  class(Jackett06_EOS), intent(in) :: this !< This EOS
  real(wp),           intent(in) :: T        !< potential temperature relative to the surface [degC].
  real(wp),           intent(in) :: S        !< salinity [PSU].
  real(wp),           intent(in) :: pressure !< pressure [Pa].
  ...
  real(wp) :: I_num   ! The inverse of the numerator of the rational expresion for density [nondim]
  real(wp) :: T2      ! Temperature squared [degC2]
  real(wp) :: S1_2    ! Limited square root of salinity [PSU1/2]
  ...
  den_STP = (T*(RD010 + T*(RD020 + T*(RD030 + T* RD040))) + &
             S*(RD100 + (T*(RD110 + T2*RD130) + S1_2*(RD600 + T2*RD620))) ) + &
            pressure*(RD001 + pressure*T*(T2*RD032 + pressure*RD013))
  I_num = 1.0_wp / (RN000 + num_STP)

  spec_vol_elem_Jackett06 = (1.0_wp + den_STP) * I_num
...
!!
!! This is an elemental function that can be applied to any combination of scalar and array inputs.
real(wp) elemental function spec_vol_anomaly_elem_Jackett06(this, T, S, pressure, spv_ref)
  class(Jackett06_EOS), intent(in) :: this !< This EOS
  real(wp),           intent(in) :: T        !< potential temperature relative to the surface [degC].
  real(wp),           intent(in) :: S        !< salinity [PSU].
  real(wp),           intent(in) :: pressure !< pressure [Pa].
  ...
  spec_vol_anomaly_elem_Jackett06 = ((1.0_wp - spv_ref*RN000) + (den_STP - spv_ref*num_STP)) * I_num
...
elemental subroutine calculate_density_derivs_elem_Jackett06(this, T, S, pressure, drho_dT, drho_dS)
  ...
  real(wp),                 intent(out) :: drho_dS  !< The partial derivative of density with salinity,
                                                !! in [kg m-3 PSU-1]
  ! Local variables
  real(wp) :: num     ! Numerator of the rational expresion for density [kg m-3]
  real(wp) :: den     ! Denominator of the rational expresion for density [nondim]
  real(wp) :: I_denom2 ! The inverse of the square of the denominator of the rational expression
                  ! for density [nondim]
  real(wp) :: dnum_dT ! The derivative of num with potential temperature [kg m-3 degC-1]
  real(wp) :: dnum_dS ! The derivative of num with salinity [kg m-3 PSU-1]
  real(wp) :: dden_dT ! The derivative of den with potential temperature [degC-1]
  real(wp) :: dden_dS ! The derivative of den with salinity PSU-1]
  real(wp) :: T2      ! Temperature squared [degC2]
  real(wp) :: S1_2    ! Limited square root of salinity [PSU1/2]
  ...
  dnum_dT = ((RN010 + T*(2._wp*RN020 + T*(3._wp*RN030))) + S*RN110) + &
            pressure*T*(2._wp*RN021 + pressure*(2._wp*RN022))
  dnum_dS = (RN100 + (T*RN110 + S*(2._wp*RN200))) + pressure*RN101
  dden_dT = ((RD010 + T*((2._wp*RD020) + T*((3._wp*RD030) + T*(4._wp*RD040)))) + &
             S*((RD110 + T2*(3._wp*RD130)) + S1_2*T*(2._wp*RD620)) ) + &
            pressure**2*(T2*3._wp*RD032 + pressure*RD013)
  dden_dS = RD100 + (T*(RD110 + T2*RD130) + S1_2*(1.5_wp*RD600 + T2*(1.5_wp*RD620)))
  I_denom2 = 1.0_wp / den**2
...
elemental subroutine calculate_density_second_derivs_elem_Jackett06(this, T, S, pressure, &
  ...
  real(wp) :: num         ! Numerator of the rational expresion for density [kg m-3]
  real(wp) :: den         ! Denominator of the rational expresion for density [nondim]
  real(wp) :: I_num2      ! The inverse of the square of the numerator of the rational expression
                      ! for density [nondim]
  real(wp) :: dnum_dT     ! The derivative of num with potential temperature [kg m-3 degC-1]
  real(wp) :: dnum_dS     ! The derivative of num with salinity [kg m-3 PSU-1]
  real(wp) :: dden_dT     ! The derivative of den with potential temperature [degC-1]
  real(wp) :: dden_dS     ! The derivative of den with salinity PSU-1]
  real(wp) :: dnum_dp     ! The derivative of num with pressure [kg m-3 dbar-1]
  real(wp) :: dden_dp     ! The derivative of det with pressure [dbar-1]
  real(wp) :: d2num_dT2   ! The second derivative of num with potential temperature [kg m-3 degC-2]
  real(wp) :: d2num_dT_dS ! The second derivative of num with potential temperature and
                      ! salinity [kg m-3 degC-1 PSU-1]
  real(wp) :: d2num_dS2   ! The second derivative of num with salinity [kg m-3 PSU-2]
  real(wp) :: d2num_dT_dp ! The second derivative of num with potential temperature and
                      ! pressure [kg m-3 degC-1 dbar-1]
  real(wp) :: d2num_dS_dp ! The second derivative of num with salinity and
                      ! pressure [kg m-3 PSU-1 dbar-1]
  real(wp) :: d2den_dT2   ! The second derivative of den with potential temperature [degC-2]
  real(wp) :: d2den_dT_dS ! The second derivative of den with potential temperature and salinity [degC-1 PSU-1]
  real(wp) :: d2den_dS2   ! The second derivative of den with salinity [PSU-2]
  real(wp) :: d2den_dT_dp ! The second derivative of den with potential temperature and pressure [degC-1 dbar-1]
  real(wp) :: d2den_dS_dp ! The second derivative of den with salinity and pressure [PSU-1 dbar-1]
  real(wp) :: T2          ! Temperature squared [degC2]
  real(wp) :: S1_2        ! Limited square root of salinity [PSU1/2]
  real(wp) :: I_s12       ! The inverse of the square root of salinity [PSU-1/2]
  real(wp) :: I_denom2    ! The inverse of the square of the denominator of the rational expression
                      ! for density [nondim]
  real(wp) :: I_denom3    ! The inverse of the cube of the denominator of the rational expression
                      ! for density [nondim]
  ...
  dnum_dT = ((RN010 + T*(2._wp*RN020 + T*(3._wp*RN030))) + S*RN110) + &
            pressure*T*(2._wp*RN021 + pressure*(2._wp*RN022))
  dnum_dS = (RN100 + (T*RN110 + S*(2._wp*RN200))) + pressure*RN101
  dnum_dp = RN001 + ((T2*RN021 + S*RN101) + pressure*(2._wp*RN002 + T2*(2._wp*RN022)))
  d2num_dT2 = 2._wp*RN020 + T*(6._wp*RN030) + pressure*(2._wp*RN021 + pressure*(2._wp*RN022))
  d2num_dT_dS = RN110
  d2num_dS2 = 2._wp*RN200
  d2num_dT_dp = T*(2._wp*RN021 + pressure*(4._wp*RN022))
  d2num_dS_dp = RN101
  ...
  dden_dT = ((RD010 + T*((2._wp*RD020) + T*((3._wp*RD030) + T*(4._wp*RD040)))) + &
            ...
            pressure**2*(T2*3._wp*RD032 + pressure*RD013)
  dden_dS = RD100 + (T*(RD110 + T2*RD130) + S1_2*(1.5_wp*RD600 + T2*(1.5_wp*RD620)))
  dden_dp = RD001 + pressure*T*(T2*(2._wp*RD032) + pressure*(3._wp*RD013))

  d2den_dT2 = (((2._wp*RD020) + T*((6._wp*RD030) + T*(12._wp*RD040))) + &
               S*(T*(6._wp*RD130) + S1_2*(2._wp*RD620)) ) + pressure**2*(T*(6._wp*RD032))
  d2den_dT_dS = (RD110 + T2*3._wp*RD130) + (T*S1_2)*(3.0_wp*RD620)
  d2den_dT_dp = pressure*(T2*(6._wp*RD032) + pressure*(3._wp*RD013))
  d2den_dS_dp = 0.0_wp
  ...
  I_S12 = 1.0_wp / (max(S1_2, 1.0e-4_wp))
  d2den_dS2 = (0.75_wp*RD600 + T2*(0.75_wp*RD620)) * I_S12

  I_denom3 = 1.0_wp / den**3
  ...
  drho_dS_dS = (den*(den*d2num_dS2 - 2._wp*dnum_dS*dden_dS) + num*(2._wp*dden_dS**2 - den*d2den_dS2)) * I_denom3
  drho_dS_dt = (den*(den*d2num_dT_dS - (dnum_dT*dden_dS + dnum_dS*dden_dT)) + &
                   num*(2._wp*dden_dT*dden_dS - den*d2den_dT_dS)) * I_denom3
  drho_dT_dT = (den*(den*d2num_dT2 - 2._wp*dnum_dT*dden_dT) + num*(2._wp*dden_dT**2 - den*d2den_dT2)) * I_denom3

  drho_dS_dp = (den*(den*d2num_dS_dp - (dnum_dp*dden_dS + dnum_dS*dden_dp)) + &
                   num*(2._wp*dden_dS*dden_dp - den*d2den_dS_dp)) * I_denom3
  drho_dT_dp = (den*(den*d2num_dT_dp - (dnum_dp*dden_dT + dnum_dT*dden_dp)) + &
                   num*(2._wp*dden_dT*dden_dp - den*d2den_dT_dp)) * I_denom3
...
elemental subroutine calculate_specvol_derivs_elem_Jackett06(this, T, S, pressure, dSV_dT, dSV_dS)
  ...
  ! Local variables
  real(wp) :: num     ! Numerator of the rational expresion for density (not specific volume) [kg m-3]
  real(wp) :: den     ! Denominator of the rational expresion for density (not specific volume) [nondim]
  real(wp) :: I_num2  ! The inverse of the square of the numerator of the rational expression
                  ! for density [nondim]
  real(wp) :: dnum_dT ! The derivative of num with potential temperature [kg m-3 degC-1]
  real(wp) :: dnum_dS ! The derivative of num with salinity [kg m-3 PSU-1]
...
elemental subroutine calculate_compress_elem_Jackett06(this, T, S, pressure, rho, drho_dp)
  ...
  real(wp) :: num     ! Numerator of the rational expresion for density [kg m-3]
  real(wp) :: den     ! Denominator of the rational expresion for density [nondim]
  real(wp) :: I_den   ! The inverse of the denominator of the rational expression for density [nondim]
  real(wp) :: dnum_dp ! The derivative of num with pressure [kg m-3 dbar-1]
  real(wp) :: dden_dp ! The derivative of den with pressure [dbar-1]
  real(wp) :: T2      ! Temperature squared [degC2]
  real(wp) :: S1_2    ! Limited square root of salinity [PSU1/2]
  integer :: j
