...
! The following parameters are used with the linear equation of state only.
  real(wp) :: Rho_T0_S0 !< The density at T=0, S=0 [kg m-3]
  real(wp) :: dRho_dT   !< The partial derivative of density with temperature [kg m-3 degC-1]
  real(wp) :: dRho_dS   !< The partial derivative of density with salinity [kg m-3 ppt-1]
  real(wp) :: dRho_dp   !< The partial derivative of density with pressure [s2 m-2]
! The following parameters are use with the linear expression for the freezing
! point only.
  real(wp) :: TFr_S0_P0 !< The freezing potential temperature at S=0, P=0 [degC]
  real(wp) :: dTFr_dS   !< The derivative of freezing point with salinity [degC ppt-1]
  real(wp) :: dTFr_dp   !< The derivative of freezing point with pressure [degC Pa-1]
...
! change of units of arguments to functions)
  real(wp) :: m_to_Z = 1._wp      !< A constant that translates distances in meters to the units of depth [Z m-1 ~> 1]
  real(wp) :: kg_m3_to_R = 1._wp  !< A constant that translates kilograms per meter cubed to the
                           !! units of density [R m3 kg-1 ~> 1]
  real(wp) :: R_to_kg_m3 = 1._wp  !< A constant that translates the units of density to
                           !! kilograms per meter cubed [kg m-3 R-1 ~> 1]
  real(wp) :: RL2_T2_to_Pa = 1._wp!< Convert pressures from R L2 T-2 to Pa [Pa T2 R-1 L-2 ~> 1]
  real(wp) :: L_T_to_m_s = 1._wp  !< Convert lateral velocities from L T-1 to m s-1 [m T s-1 L-1 ~> 1]
  real(wp) :: degC_to_C = 1._wp   !< A constant that translates degrees Celsius to the units of temperature [C degC-1 ~> 1]
  real(wp) :: C_to_degC = 1._wp   !< A constant that translates the units of temperature to degrees Celsius [degC C-1 ~> 1]
  real(wp) :: ppt_to_S = 1._wp    !< A constant that translates parts per thousand to the units of salinity [S ppt-1 ~> 1]
  real(wp) :: S_to_ppt = 1._wp    !< A constant that translates the units of salinity to parts per thousand [ppt S-1 ~> 1]
...
real(wp) elemental function calculate_density_elem(EOS, T, S, pressure, rho_ref, scale)
  type(EOS_type), intent(in)  :: EOS      !< Equation of state structure
  real(wp),           intent(in)  :: T        !< Potential temperature referenced to the surface [C ~> degC]
  real(wp),           intent(in)  :: S        !< Salinity [S ~> ppt]
  real(wp),           intent(in)  :: pressure !< Pressure [R L2 T-2 ~> Pa]
  real(wp), optional, intent(in)  :: rho_ref  !< A reference density [R ~> kg m-3]
  real(wp), optional, intent(in)  :: scale    !< A multiplicative factor by which to scale output density in
                                          !! combination with scaling stored in EOS [various]
  real(wp) :: Ta      ! An array of temperatures [degC]
  real(wp) :: Sa      ! An array of salinities [ppt]
  real(wp) :: pres    ! An mks version of the pressure to use [Pa]
  real(wp) :: rho_mks ! An mks version of the density to be returned [kg m-3]
  real(wp) :: rho_scale  ! A factor to convert density from kg m-3 to the desired units [R m3 kg-1 ~> 1]
...
subroutine calculate_density_scalar(T, S, pressure, rho, EOS, rho_ref, scale)
  ...
  real(wp),           intent(in)  :: S        !< Salinity [S ~> ppt]
  real(wp),           intent(in)  :: pressure !< Pressure [R L2 T-2 ~> Pa]
  real(wp),           intent(out) :: rho      !< Density (in-situ if pressure is local) [R ~> kg m-3]
  type(EOS_type), intent(in)  :: EOS      !< Equation of state structure
  real(wp), optional, intent(in)  :: rho_ref  !< A reference density [R ~> kg m-3]
  real(wp), optional, intent(in)  :: scale    !< A multiplicative factor by which to scale output density in
                                          !! combination with scaling stored in EOS [various]
...
subroutine calculate_stanley_density_scalar(T, S, pressure, Tvar, TScov, Svar, rho, EOS, rho_ref, scale)
  real(wp),           intent(in)  :: T        !< Potential temperature referenced to the surface [C ~> degC]
  real(wp),           intent(in)  :: S        !< Salinity [S ~> ppt]
  real(wp),           intent(in)  :: Tvar     !< Variance of potential temperature referenced to the surface [C2 ~> degC2]
  real(wp),           intent(in)  :: TScov    !< Covariance of potential temperature and salinity [C S ~> degC ppt]
  real(wp),           intent(in)  :: Svar     !< Variance of salinity [S2 ~> ppt2]
  real(wp),           intent(in)  :: pressure !< Pressure [R L2 T-2 ~> Pa]
  real(wp),           intent(out) :: rho      !< Density (in-situ if pressure is local) [R ~> kg m-3]
  type(EOS_type), intent(in)  :: EOS      !< Equation of state structure
  real(wp), optional, intent(in)  :: rho_ref  !< A reference density [R ~> kg m-3].
  real(wp), optional, intent(in)  :: scale    !< A multiplicative factor by which to scale output density in
                                          !! combination with scaling stored in EOS [various]
  ! Local variables
  real(wp) :: d2RdTT   ! Second derivative of density with temperature [R C-2 ~> kg m-3 degC-2]
  real(wp) :: d2RdST   ! Second derivative of density with temperature and salinity [R S-1 C-1 ~> kg m-3 degC-1 ppt-1]
  real(wp) :: d2RdSS   ! Second derivative of density with salinity [R S-2 ~> kg m-3 ppt-2]
  real(wp) :: d2RdSp   ! Second derivative of density with salinity and pressure [T2 S-1 L-2 ~> kg m-3 ppt-1 Pa-1]
  real(wp) :: d2RdTp   ! Second derivative of density with temperature and pressure [T2 C-1 L-2 ~> kg m-3 degC-1 Pa-1]
  ...
  rho = rho + ( 0.5_wp * d2RdTT * Tvar + ( d2RdST * TScov + 0.5_wp * d2RdSS * Svar ) )
...
subroutine calculate_density_1d(T, S, pressure, rho, EOS, dom, rho_ref, scale)
  real(wp), dimension(:),    intent(in)    :: T        !< Potential temperature referenced to the surface [C ~> degC]
  real(wp), dimension(:),    intent(in)    :: S        !< Salinity [S ~> ppt]
  real(wp), dimension(:),    intent(in)    :: pressure !< Pressure [R L2 T-2 ~> Pa]
  real(wp), dimension(:),    intent(inout) :: rho      !< Density (in-situ if pressure is local) [R ~> kg m-3]
  type(EOS_type),        intent(in)    :: EOS      !< Equation of state structure
  integer, dimension(2), optional, intent(in) :: dom   !< The domain of indices to work on, taking
                                                       !! into account that arrays start at 1.
  real(wp),                  optional, intent(in) :: rho_ref !< A reference density [R ~> kg m-3]
  real(wp),                  optional, intent(in) :: scale !< A multiplicative factor by which to scale density
                                                   !! in combination with scaling stored in EOS [various]
  ! Local variables
  real(wp) :: rho_scale ! A factor to convert density from kg m-3 to the desired units [R m3 kg-1 ~> 1]
  real(wp), dimension(size(rho)) :: pres  ! Pressure converted to [Pa]
  real(wp), dimension(size(rho)) :: Ta    ! Temperature converted to [degC]
  real(wp), dimension(size(rho)) :: Sa    ! Salinity converted to [ppt]
  ...
  if ((EOS%RL2_T2_to_Pa == 1.0_wp) .and. (EOS%R_to_kg_m3 == 1.0_wp) .and. &
      (EOS%C_to_degC == 1.0_wp) .and. (EOS%S_to_ppt == 1.0_wp)) then
  ...
  rho_scale = EOS%kg_m3_to_R
  if (present(scale)) rho_scale = rho_scale * scale
  if (rho_scale /= 1.0_wp) then ; do i=is,ie
...
subroutine calculate_stanley_density_1d(T, S, pressure, Tvar, TScov, Svar, rho, EOS, dom, rho_ref, scale)
  ...
  real(wp), dimension(:),    intent(in)    :: S        !< Salinity [S ~> ppt]
  real(wp), dimension(:),    intent(in)    :: pressure !< Pressure [R L2 T-2 ~> Pa]
  real(wp), dimension(:),    intent(in)    :: Tvar     !< Variance of potential temperature [C2 ~> degC2]
  real(wp), dimension(:),    intent(in)    :: TScov    !< Covariance of potential temperature and salinity [C S ~> degC ppt]
  real(wp), dimension(:),    intent(in)    :: Svar     !< Variance of salinity [S2 ~> ppt2]
  real(wp), dimension(:),    intent(inout) :: rho      !< Density (in-situ if pressure is local) [R ~> kg m-3]
  type(EOS_type),        intent(in)    :: EOS      !< Equation of state structure
  integer, dimension(2), optional, intent(in) :: dom   !< The domain of indices to work on, taking
                                                       !! into account that arrays start at 1.
  ...
  ! Equation 25 of Stanley et al., 2020.
  do i=is,ie
    rho(i) = rho(i) + ( 0.5_wp * d2RdTT(i) * Tvar(i) + ( d2RdST(i) * TScov(i) + 0.5_wp * d2RdSS(i) * Svar(i) ) )
  enddo

  if (present(scale)) then ; if (scale /= 1.0_wp) then ; do i=is,ie
    rho(i) = scale * rho(i)
  enddo ; endif ; endif
...
subroutine calculate_spec_vol_array(T, S, pressure, specvol, start, npts, EOS, spv_ref, scale)
  ...
  real(wp), dimension(:), intent(inout) :: specvol  !< in situ specific volume [kg m-3]
  integer,            intent(in)    :: start    !< the starting point in the arrays.
  integer,            intent(in)    :: npts     !< the number of values to calculate.
  type(EOS_type),     intent(in)    :: EOS      !< Equation of state structure
  real(wp),     optional, intent(in)    :: spv_ref  !< A reference specific volume [m3 kg-1]
  real(wp),     optional, intent(in)    :: scale    !< A multiplicative factor by which to scale specific
  ...
  if (present(scale)) then ; if (scale /= 1.0_wp) then ; do j=start,start+npts-1
...
subroutine calc_spec_vol_scalar(T, S, pressure, specvol, EOS, spv_ref, scale)
  ...
  real(wp),           intent(in)  :: S        !< Salinity [S ~> ppt]
  real(wp),           intent(in)  :: pressure !< Pressure [R L2 T-2 ~> Pa]
  real(wp),           intent(out) :: specvol  !< In situ or potential specific volume [R-1 ~> m3 kg-1]
                                          !! or other units determined by the scale argument
  type(EOS_type), intent(in)  :: EOS      !< Equation of state structure
  real(wp), optional, intent(in)  :: spv_ref  !< A reference specific volume [R-1 ~> m3 kg-1]
  real(wp), optional, intent(in)  :: scale    !< A multiplicative factor by which to scale specific
                                          !! volume in combination with scaling stored in EOS [various]

  real(wp), dimension(1) :: Ta   ! Rescaled single element array version of temperature [degC]
  real(wp), dimension(1) :: Sa   ! Rescaled single element array version of salinity [ppt]
  real(wp), dimension(1) :: pres ! Rescaled single element array version of pressure [Pa]
  real(wp), dimension(1) :: spv  ! Rescaled single element array version of specific volume [m3 kg-1]
  real(wp) :: spv_scale ! A factor to convert specific volume from m3 kg-1 to the desired units [kg R-1 m-3 ~> 1]
...
subroutine calc_spec_vol_1d(T, S, pressure, specvol, EOS, dom, spv_ref, scale)
  ...
  real(wp), dimension(:),    intent(in)    :: S        !< Salinity [S ~> ppt]
  real(wp), dimension(:),    intent(in)    :: pressure !< Pressure [R L2 T-2 ~> Pa]
  real(wp), dimension(:),    intent(inout) :: specvol  !< In situ specific volume [R-1 ~> m3 kg-1]
  type(EOS_type),        intent(in)    :: EOS      !< Equation of state structure
  integer, dimension(2), optional, intent(in) :: dom   !< The domain of indices to work on, taking
                                                       !! into account that arrays start at 1.
  real(wp),                  optional, intent(in) :: spv_ref !< A reference specific volume [R-1 ~> m3 kg-1]
  real(wp),                  optional, intent(in) :: scale !< A multiplicative factor by which to scale
                                                       !! output specific volume in combination with
                                                       !! scaling stored in EOS [various]
  ! Local variables
  real(wp), dimension(size(T)) :: pres  ! Pressure converted to [Pa]
  real(wp), dimension(size(T)) :: Ta    ! Temperature converted to [degC]
  real(wp), dimension(size(T)) :: Sa    ! Salinity converted to [ppt]
  real(wp) :: spv_scale ! A factor to convert specific volume from m3 kg-1 to the desired units [kg m-3 R-1 ~> 1]
  integer :: i, is, ie, npts
  ...
  if ((EOS%RL2_T2_to_Pa == 1.0_wp) .and. (EOS%kg_m3_to_R == 1.0_wp) .and. &
      (EOS%C_to_degC == 1.0_wp) .and. (EOS%S_to_ppt == 1.0_wp)) then
  ...
  spv_scale = EOS%R_to_kg_m3
  if (present(scale)) spv_scale = spv_scale * scale
  if (spv_scale /= 1.0_wp) then ; do i=is,ie
...
subroutine calculate_TFreeze_scalar(S, pressure, T_fr, EOS, pres_scale, scale_from_EOS)
  real(wp),           intent(in)  :: S    !< Salinity, [ppt] or [S ~> ppt] depending on scale_from_EOS
  real(wp),           intent(in)  :: pressure !< Pressure, in [Pa] or [R L2 T-2 ~> Pa] depending on
                                      !! pres_scale or scale_from_EOS
  real(wp),           intent(out) :: T_fr !< Freezing point potential temperature referenced to the
                                      !! surface [degC] or [C ~> degC] depending on scale_from_EOS
  type(EOS_type), intent(in)  :: EOS  !< Equation of state structure
  real(wp), optional, intent(in)  :: pres_scale  !< A multiplicative factor to convert pressure
  ...
  ! Local variables
  real(wp) :: p_scale ! A factor to convert pressure to units of Pa [Pa T2 R-1 L-2 ~> 1]
  real(wp) :: S_scale ! A factor to convert salinity to units of ppt [ppt S-1 ~> 1]
  real(wp) :: iS_scale! A factor to convert salinity to units of S [S ppt-1 ~> 1]
  real(wp) :: absS    ! A salinity converted to absolute salinity, only used in specific scenarios [ppt]
  real(wp) :: TFreeze_S ! The salinity for the freezing equation in model units [S ~> PSU or ppt]

  p_scale = 1.0_wp ; S_scale = 1.0_wp ; iS_scale = 1.0_wp
...
subroutine calculate_TFreeze_array(S, pressure, T_fr, start, npts, EOS, pres_scale)
  real(wp), dimension(:), intent(in)    :: S        !< Salinity [ppt]
  real(wp), dimension(:), intent(in)    :: pressure !< Pressure, in [Pa] or [R L2 T-2 ~> Pa] depending on pres_scale
  real(wp), dimension(:), intent(inout) :: T_fr     !< Freezing point, either potential temperature referenced to the
  ...
  ! Local variables
  real(wp), dimension(size(pressure)) :: pres  ! Pressure converted to [Pa]
  real(wp) :: p_scale  ! A factor to convert pressure to units of Pa [Pa T2 R-1 L-2 ~> 1]
  real(wp), dimension(size(S)) :: absS ! A salinity converted to absolute salinity, only used in specific scenarios [ppt]
  real(wp), dimension(size(S)) :: TFreeze_S ! The salinity for the freezing equation in model units [S ~> PSU or ppt]
  integer :: j

  p_scale = 1.0_wp ; if (present(pres_scale)) p_scale = pres_scale
...
subroutine calculate_TFreeze_1d(S, pressure, T_fr, EOS, dom)
  ...
  ! Local variables
  real(wp), dimension(size(T_fr)) :: pres  ! Pressure converted to [Pa]
  real(wp), dimension(size(T_fr)) :: Sa    ! Salinity converted to [ppt]
  real(wp), dimension(size(T_fr)) :: absS  ! Salinity converted to absoluate salinity [ppt]
  real(wp), dimension(size(T_fr)) :: TFreeze_S ! The salinity for the freezing equation in model units [S ~> PSU or ppt]
  integer :: i, is, ie, npts
  ...
  if ((EOS%RL2_T2_to_Pa == 1.0_wp) .and. (EOS%S_to_ppt == 1.0_wp)) then
    select case (EOS%form_of_TFreeze)
      case (TFREEZE_LINEAR)
...
subroutine calculate_density_derivs_array(T, S, pressure, drho_dT, drho_dS, start, npts, EOS, scale)
  real(wp), dimension(:), intent(in)    :: T        !< Potential temperature referenced to the surface [degC]
  real(wp), dimension(:), intent(in)    :: S        !< Salinity [ppt]
  real(wp), dimension(:), intent(in)    :: pressure !< Pressure [Pa]
  real(wp), dimension(:), intent(inout) :: drho_dT  !< The partial derivative of density with potential
                                                !! temperature [kg m-3 degC-1] or other units determined
                                                !! by the optional scale argument
  real(wp), dimension(:), intent(inout) :: drho_dS  !< The partial derivative of density with salinity,
...
subroutine calculate_density_derivs_1d(T, S, pressure, drho_dT, drho_dS, EOS, dom, scale)
  ...
  real(wp),                  optional, intent(in) :: scale !< A multiplicative factor by which to scale density
                                                       !! in combination with scaling stored in EOS [various]
  ! Local variables
  real(wp), dimension(size(drho_dT)) :: pres  ! Pressure converted to [Pa]
  real(wp), dimension(size(drho_dT)) :: Ta    ! Temperature converted to [degC]
  real(wp), dimension(size(drho_dT)) :: Sa    ! Salinity converted to [ppt]
  real(wp) :: rho_scale ! A factor to convert density from kg m-3 to the desired units [R m3 kg-1 ~> 1]
  real(wp) :: dRdT_scale ! A factor to convert drho_dT to the desired units [R degC m3 C-1 kg-1 ~> 1]
  real(wp) :: dRdS_scale ! A factor to convert drho_dS to the desired units [R ppt m3 S-1 kg-1 ~> 1]
  integer :: i, is, ie, npts
  ...
  if ((EOS%RL2_T2_to_Pa == 1.0_wp) .and. (EOS%C_to_degC == 1.0_wp) .and. (EOS%S_to_ppt == 1.0_wp)) then
  ...
  if ((dRdT_scale /= 1.0_wp) .or. (dRdS_scale /= 1.0_wp)) then ; do i=is,ie
...
subroutine calculate_density_derivs_scalar(T, S, pressure, drho_dT, drho_dS, EOS, scale)
  ...
  real(wp) :: dRdT_scale ! A factor to convert drho_dT to the desired units [R degC m3 C-1 kg-1 ~> 1]
  real(wp) :: dRdS_scale ! A factor to convert drho_dS to the desired units [R ppt m3 S-1 kg-1 ~> 1]
  real(wp) :: pres(1)  ! Pressure converted to [Pa]
  real(wp) :: Ta(1)    ! Temperature converted to [degC]
  real(wp) :: Sa(1)    ! Salinity converted to [ppt]
  ...
  dRdT_scale = rho_scale * EOS%C_to_degC
  dRdS_scale = rho_scale * EOS%S_to_ppt
  if ((dRdT_scale /= 1.0_wp) .or. (dRdS_scale /= 1.0_wp)) then
...
subroutine calculate_density_second_derivs_1d(T, S, pressure, drho_dS_dS, drho_dS_dT, drho_dT_dT, &
  ...
  real(wp), dimension(:), intent(in)  :: S !< Salinity [S ~> ppt]
  real(wp), dimension(:), intent(in)  :: pressure   !< Pressure [R L2 T-2 ~> Pa]
  real(wp), dimension(:), intent(inout) :: drho_dS_dS !< Partial derivative of beta with respect to S
                                                  !! [R S-2 ~> kg m-3 ppt-2]
  real(wp), dimension(:), intent(inout) :: drho_dS_dT !< Partial derivative of beta with respect to T
                                                  !! [R S-1 C-1 ~> kg m-3 ppt-1 degC-1]
  real(wp), dimension(:), intent(inout) :: drho_dT_dT !< Partial derivative of alpha with respect to T
                                                  !! [R C-2 ~> kg m-3 degC-2]
  real(wp), dimension(:), intent(inout) :: drho_dS_dP !< Partial derivative of beta with respect to pressure
                                                  !! [T2 S-1 L-2 ~> kg m-3 ppt-1 Pa-1]
  real(wp), dimension(:), intent(inout) :: drho_dT_dP !< Partial derivative of alpha with respect to pressure
...
subroutine calculate_density_second_derivs_scalar(T, S, pressure, drho_dS_dS, drho_dS_dT, drho_dT_dT, &
  ...
  real(wp), intent(in)  :: S !< Salinity [S ~> ppt]
  real(wp), intent(in)  :: pressure   !< Pressure [R L2 T-2 ~> Pa]
  real(wp), intent(out) :: drho_dS_dS !< Partial derivative of beta with respect to S
                                  !! [R S-2 ~> kg m-3 ppt-2]
  real(wp), intent(out) :: drho_dS_dT !< Partial derivative of beta with respect to T
                                  !! [R S-1 C-1 ~> kg m-3 ppt-1 degC-1]
  real(wp), intent(out) :: drho_dT_dT !< Partial derivative of alpha with respect to T
                                  !! [R C-2 ~> kg m-3 degC-2]
  real(wp), intent(out) :: drho_dS_dP !< Partial derivative of beta with respect to pressure
                                  !! [T2 S-1 L-2 ~> kg m-3 ppt-1 Pa-1]
  real(wp), intent(out) :: drho_dT_dP !< Partial derivative of alpha with respect to pressure
                                  !! [T2 C-1 L-2 ~> kg m-3 degC-1 Pa-1]
  type(EOS_type), intent(in) :: EOS !< Equation of state structure
  real(wp), optional, intent(in) :: scale !< A multiplicative factor by which to scale density
                                  !! in combination with scaling stored in EOS [various]
  ! Local variables
  real(wp) :: rho_scale ! A factor to convert density from kg m-3 to the desired units [R m3 kg-1 ~> 1]
  real(wp) :: pres  ! Pressure converted to [Pa]
  real(wp) :: Ta    ! Temperature converted to [degC]
  real(wp) :: Sa    ! Salinity converted to [ppt]
...
subroutine calculate_spec_vol_derivs_array(T, S, pressure, dSV_dT, dSV_dS, start, npts, EOS)
  ...
  real(wp), dimension(:), intent(in)  :: S !< Salinity [ppt]
  real(wp), dimension(:), intent(in)  :: pressure !< Pressure [Pa]
  real(wp), dimension(:), intent(inout) :: dSV_dT !< The partial derivative of specific volume with potential
                                              !! temperature [m3 kg-1 degC-1]
  real(wp), dimension(:), intent(inout) :: dSV_dS !< The partial derivative of specific volume with salinity
                                              !! [m3 kg-1 ppt-1]
  integer,            intent(in)  :: start  !< Starting index within the array
  integer,            intent(in)  :: npts   !< The number of values to calculate
  type(EOS_type),     intent(in)  :: EOS    !< Equation of state structure
...
subroutine calc_spec_vol_derivs_1d(T, S, pressure, dSV_dT, dSV_dS, EOS, dom, scale)
  ...
  real(wp), dimension(size(T)) :: Sa    ! Salinity converted to [ppt]
  real(wp) :: spv_scale ! A factor to convert specific volume from m3 kg-1 to the desired units [kg R-1 m-3 ~> 1]
  real(wp) :: dSVdT_scale ! A factor to convert dSV_dT to the desired units [kg degC R-1 C-1 m-3 ~> 1]
  real(wp) :: dSVdS_scale ! A factor to convert dSV_dS to the desired units [kg ppt R-1 S-1 m-3 ~> 1]
  integer :: i, is, ie, npts
  ...
  if ((dSVdT_scale /= 1.0_wp) .or. (dSVdS_scale /= 1.0_wp)) then ; do i=is,ie
...
subroutine calculate_compress_1d(T, S, pressure, rho, drho_dp, EOS, dom)
  ...
  real(wp), dimension(:), intent(in)    :: S        !< Salinity [S ~> ppt]
  real(wp), dimension(:), intent(in)    :: pressure !< Pressure [R L2 T-2 ~> Pa]
  real(wp), dimension(:), intent(inout) :: rho      !< In situ density [R ~> kg m-3]
  real(wp), dimension(:), intent(inout) :: drho_dp  !< The partial derivative of density with pressure
  ...
  if (EOS%kg_m3_to_R /= 1.0_wp) then ; do i=is,ie
    rho(i) = EOS%kg_m3_to_R * rho(i)
  enddo ; endif
  if (EOS%L_T_to_m_s /= 1.0_wp) then ; do i=is,ie
...
subroutine calculate_compress_scalar(T, S, pressure, rho, drho_dp, EOS)
  ...
  real(wp), intent(in)        :: S        !< Salinity [S ~> ppt]
  real(wp), intent(in)        :: pressure !< Pressure [R L2 T-2 ~> Pa]
  real(wp), intent(out)       :: rho      !< In situ density [R ~> kg m-3]
  real(wp), intent(out)       :: drho_dp  !< The partial derivative of density with pressure (also the
  ...
  real(wp), dimension(1) :: pa    ! Pressure in a size-1 1d array [R L2 T-2 ~> Pa]
  real(wp), dimension(1) :: Ta    ! Temperature in a size-1 1d array [C ~> degC]
  real(wp), dimension(1) :: Sa    ! Salinity in a size-1 1d array [S ~> ppt]
  real(wp), dimension(1) :: rhoa  ! In situ density in a size-1 1d array [R ~> kg m-3]
  real(wp), dimension(1) :: drho_dpa ! The partial derivative of density with pressure (also the
...
subroutine average_specific_vol(T, S, p_t, dp, SpV_avg, EOS, dom, scale)
  real(wp), dimension(:),    intent(in)    :: T   !< Potential temperature referenced to the surface [C ~> degC]
  real(wp), dimension(:),    intent(in)    :: S   !< Salinity [S ~> ppt]
  real(wp), dimension(:),    intent(in)    :: p_t !< Pressure at the top of the layer [R L2 T-2 ~> Pa]
  real(wp), dimension(:),    intent(in)    :: dp  !< Pressure change in the layer [R L2 T-2 ~> Pa]
  real(wp), dimension(:),    intent(inout) :: SpV_avg !< The vertical average specific volume
  ...
  ! Local variables
  real(wp), dimension(size(T)) :: pres  ! Layer-top pressure converted to [Pa]
  real(wp), dimension(size(T)) :: dpres ! Pressure change converted to [Pa]
  real(wp), dimension(size(T)) :: Ta    ! Temperature converted to [degC]
  real(wp), dimension(size(T)) :: Sa    ! Salinity converted to [ppt]
  real(wp) :: T5(5)      ! Temperatures at five quadrature points [C ~> degC]
  real(wp) :: S5(5)      ! Salinities at five quadrature points [S ~> ppt]
  real(wp) :: p5(5)      ! Pressures at five quadrature points [R L2 T-2 ~> Pa]
  real(wp) :: a5(5)      ! Specific volumes at five quadrature points [R-1 ~> m3 kg-1]
  ...
  if (EOS%EOS_quadrature) then
    do i=is,ie
      do n=1,5
        T5(n) = T(i) ; S5(n) = S(i)
        p5(n) = p_t(i) + 0.25_wp*real(5-n, wp)*dp(i)
      enddo
      call calculate_spec_vol(T5, S5, p5, a5, EOS)

      ! Use Boole's rule to estimate the average specific volume.
      SpV_avg(i) = C1_90*(7.0_wp*(a5(1)+a5(5)) + 32.0_wp*(a5(2)+a5(4)) + 12.0_wp*a5(3))
    enddo
  elseif ((EOS%RL2_T2_to_Pa == 1.0_wp) .and. (EOS%C_to_degC == 1.0_wp) .and. (EOS%S_to_ppt == 1.0_wp)) then
  ...
  spv_scale = EOS%R_to_kg_m3
  if (EOS%EOS_quadrature) spv_scale = 1.0_wp
  if (present(scale)) spv_scale = spv_scale * scale
  if (spv_scale /= 1.0_wp) then ; do i=is,ie
...
subroutine EoS_fit_range(EOS, T_min, T_max, S_min, S_max, p_min, p_max)
  type(EOS_type), intent(in) :: EOS   !< Equation of state structure
  real(wp), optional, intent(out) :: T_min !< The minimum temperature over which this EoS is fitted [degC]
  real(wp), optional, intent(out) :: T_max !< The maximum temperature over which this EoS is fitted [degC]
  real(wp), optional, intent(out) :: S_min !< The minimum salinity over which this EoS is fitted [ppt]
  real(wp), optional, intent(out) :: S_max !< The maximum salinity over which this EoS is fitted [ppt]
...
subroutine analytic_int_specific_vol_dp(T, S, p_t, p_b, alpha_ref, HI, EOS, &
  ...
  real(wp), dimension(HI%IsdB:HI%IedB,HI%jsd:HI%jed), &
  ...
  real(wp), dimension(HI%isd:HI%ied,HI%JsdB:HI%JedB), &
  ...
  real(wp),       optional, intent(in)  :: dP_tiny !< A miniscule pressure change with
  ...
  real(wp) :: dRdT_scale ! A factor to convert drho_dT to the desired units [R degC m3 C-1 kg-1 ~> 1]
  real(wp) :: dRdS_scale ! A factor to convert drho_dS to the desired units [R ppt m3 S-1 kg-1 ~> 1]
  real(wp) :: dRdp_scale ! A factor to convert drho_dp to the desired units [T-2 L2 s2 m-2 ~> 1]
...
subroutine analytic_int_density_dz(T, S, z_t, z_b, rho_ref, rho_0, G_e, HI, EOS, dpa, &
  ...
  real(wp),       optional, intent(in)  :: dz_neglect !< A miniscule thickness change [Z ~> m]
  integer,    optional, intent(in)  :: MassWghtInterp !< A flag indicating whether and how to use
                                          !! mass weighting to interpolate T/S in integrals
  real(wp), dimension(HI%isd:HI%ied,HI%jsd:HI%jed), &
  ...
  ! Local variables
  real(wp) :: rho_scale  ! A multiplicative factor by which to scale density from kg m-3 to the
                     ! desired units [R m3 kg-1 ~> 1]
  real(wp) :: dRdT_scale ! A factor to convert drho_dT to the desired units [R degC m3 C-1 kg-1 ~> 1]
  real(wp) :: dRdS_scale ! A factor to convert drho_dS to the desired units [R ppt m3 S-1 kg-1 ~> 1]
  real(wp) :: dRdp_scale ! A factor to convert drho_dp to the desired units [T-2 L2 s2 m-2 ~> 1]
  real(wp) :: pres_scale ! A multiplicative factor to convert pressure into Pa [Pa T2 R-1 L-2 ~> 1]
  ...
  select case (EOS%form_of_EOS)
    case (EOS_LINEAR)
      ...
      dRdS_scale = EOS%kg_m3_to_R * EOS%S_to_ppt
      dRdp_scale = EOS%kg_m3_to_R * EOS%RL2_T2_to_Pa
      if ((rho_scale /= 1.0_wp) .or. (dRdT_scale /= 1.0_wp) .or. (dRdS_scale /= 1.0_wp) .or. (dRdp_scale /= 1.0_wp)) then
    ...
    case (EOS_WRIGHT)
      ...
      if ((rho_scale /= 1.0_wp) .or. (pres_scale /= 1.0_wp) .or. (EOS%C_to_degC /= 1.0_wp) .or. (EOS%S_to_ppt /= 1.0_wp)) then
...
# include "version_variable.h"
  ...
  real(wp) :: Rho_Tref_Sref ! Density at Tref degC and Sref ppt [kg m-3]
  real(wp) :: Tref          ! Reference temperature [degC]
  real(wp) :: Sref          ! Reference salinity [psu]
  real(wp) :: pref          ! Reference pressure [Pa]
  real(wp) :: rho0          ! Density at T=0, S=0 and p=0 [kg m-3]
  ...
  if (EOS%form_of_EOS == EOS_LINEAR) then
    ...
    EOS%Compressible = (EOS%dRho_dp/=0.0_wp)
  ...
  ! Unit conversions
  EOS%m_to_Z = 1._wp ; if (present(US)) EOS%m_to_Z = US%m_to_Z
  EOS%kg_m3_to_R = 1._wp ; if (present(US)) EOS%kg_m3_to_R = US%kg_m3_to_R
  EOS%R_to_kg_m3 = 1._wp ; if (present(US)) EOS%R_to_kg_m3 = US%R_to_kg_m3
  EOS%RL2_T2_to_Pa = 1._wp ; if (present(US)) EOS%RL2_T2_to_Pa = US%RL2_T2_to_Pa
  EOS%L_T_to_m_s = 1._wp ; if (present(US)) EOS%L_T_to_m_s = US%L_T_to_m_s
  EOS%degC_to_C = 1._wp ; if (present(US)) EOS%degC_to_C = US%degC_to_C
  EOS%C_to_degC = 1._wp ; if (present(US)) EOS%C_to_degC = US%C_to_degC
  EOS%ppt_to_S = 1._wp ; if (present(US)) EOS%ppt_to_S = US%ppt_to_S
  EOS%S_to_ppt = 1._wp ; if (present(US)) EOS%S_to_ppt = US%S_to_ppt
...
subroutine EOS_manual_init(EOS, form_of_EOS, form_of_TFreeze, EOS_quadrature, Compressible, &
  ...
  real(wp)   , optional, intent(in) :: Rho_T0_S0 !< Density at T=0 degC and S=0 ppt [kg m-3]
  real(wp)   , optional, intent(in) :: drho_dT   !< Partial derivative of density with temperature
                                             !! in [kg m-3 degC-1]
  real(wp)   , optional, intent(in) :: dRho_dS   !< Partial derivative of density with salinity
                                             !! in [kg m-3 ppt-1]
  real(wp)   , optional, intent(in) :: dRho_dp   !< Partial derivative of density with pressure
                                             !! in [s2 m-2]
  real(wp)   , optional, intent(in) :: TFr_S0_P0 !< The freezing potential temperature at S=0, P=0 [degC]
  real(wp)   , optional, intent(in) :: dTFr_dS   !< The derivative of freezing point with salinity
                                             !! in [degC ppt-1]
  real(wp)   , optional, intent(in) :: dTFr_dp   !< The derivative of freezing point with pressure
                                             !! in [degC Pa-1]
...
subroutine convert_temp_salt_for_TEOS10(T, S, HI, kd, mask_z, EOS)
  ...
  type(hor_index_type),  intent(in)    :: HI       !< The horizontal index structure
  real(wp), dimension(HI%isd:HI%ied,HI%jsd:HI%jed,kd), &
  ...
  real(wp), parameter :: Sref_Sprac = (35.16504_wp/35.0_wp) ! The TEOS 10 conversion factor to go from
...
subroutine cons_temp_to_pot_temp(T, S, poTemp, EOS, dom, scale)
  real(wp), dimension(:), intent(in)    :: T        !< Conservative temperature [C ~> degC]
  real(wp), dimension(:), intent(in)    :: S        !< Absolute salinity [S ~> ppt]
  real(wp), dimension(:), intent(inout) :: poTemp   !< The potential temperature with a reference pressure
                                                !! of 0 Pa, [C ~> degC]
  type(EOS_type),     intent(in)    :: EOS      !< Equation of state structure
  integer, dimension(2), optional, intent(in) :: dom  !< The domain of indices to work on, taking
                                                !! into account that arrays start at 1.
  real(wp),     optional, intent(in)    :: scale    !< A multiplicative factor by which to scale the output
  ...
  ! Local variables
  real(wp), dimension(size(T)) :: Ta    ! Temperature converted to [degC]
  real(wp), dimension(size(S)) :: Sa    ! Salinity converted to [ppt]
  real(wp) :: T_scale ! A factor to convert potential temperature from degC to the desired units [C degC-1 ~> 1]
  integer :: i, is, ie
  ...
  if ((EOS%C_to_degC == 1.0_wp) .and. (EOS%S_to_ppt == 1.0_wp)) then
  ...
  if (T_scale /= 1.0_wp) then ; do i=is,ie
...
subroutine pot_temp_to_cons_temp(T, S, consTemp, EOS, dom, scale)
  real(wp), dimension(:), intent(in)    :: T        !< Potential temperature [C ~> degC]
  real(wp), dimension(:), intent(in)    :: S        !< Absolute salinity [S ~> ppt]
  real(wp), dimension(:), intent(inout) :: consTemp !< The conservative temperature [C ~> degC]
  type(EOS_type),     intent(in)    :: EOS      !< Equation of state structure
  integer, dimension(2), optional, intent(in) :: dom  !< The domain of indices to work on, taking
                                                !! into account that arrays start at 1.
  real(wp),     optional, intent(in)    :: scale    !< A multiplicative factor by which to scale the output
  ...
  ! Local variables
  real(wp), dimension(size(T)) :: Tp    ! Potential temperature converted to [degC]
  real(wp), dimension(size(S)) :: Sa    ! Absolute salinity converted to [ppt]
  real(wp) :: T_scale ! A factor to convert potential temperature from degC to the desired units [C degC-1 ~> 1]
  integer :: i, is, ie
...
subroutine abs_saln_to_prac_saln(S, prSaln, EOS, dom, scale)
  real(wp), dimension(:), intent(in)    :: S        !< Absolute salinity [S ~> ppt]
  real(wp), dimension(:), intent(inout) :: prSaln   !< Practical salinity [S ~> PSU]
  type(EOS_type),     intent(in)    :: EOS      !< Equation of state structure
  integer, dimension(2), optional, intent(in) :: dom  !< The domain of indices to work on, taking
                                                !! into account that arrays start at 1.
  real(wp),     optional, intent(in)    :: scale    !< A multiplicative factor by which to scale the output
  ...
  ! Local variables
  real(wp) :: S_scale ! A factor to convert practical salinity from ppt to the desired units [S PSU-1 ~> 1]
...
subroutine prac_saln_to_abs_saln(S, absSaln, EOS, dom, scale)
  real(wp), dimension(:), intent(in)    :: S        !< Practical salinity [S ~> PSU]
  real(wp), dimension(:), intent(inout) :: absSaln  !< Absolute salinity [S ~> ppt]
  type(EOS_type),     intent(in)    :: EOS      !< Equation of state structure
  integer, dimension(2), optional, intent(in) :: dom  !< The domain of indices to work on, taking
                                                !! into account that arrays start at 1.
  real(wp),     optional, intent(in)    :: scale    !< A multiplicative factor by which to scale the output
  ...
  ! Local variables
  real(wp) :: S_scale ! A factor to convert absolute salinity from ppt to the desired units [S ppt-1 ~> 1]
  real(wp), parameter :: Sref_Sprac = (35.16504_wp/35.0_wp) ! The TEOS 10 conversion factor to go from
                                    ! practical salinity to reference salinity [PSU ppt-1]
  integer :: i, is, ie
...
logical function EOS_unit_tests(verbose)
  ...
  fail = test_TS_conversion_consistency(T_cons=9.989811727177308_wp, S_abs=35.16504_wp, &
                                        T_pot=10.0_wp, S_prac=35.0_wp, EOS=EOS_tmp, verbose=verbose)
  ...
  fail = test_EOS_consistency(25.0_wp, 35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "UNESCO", &
                              rho_check=1027.54345796120_wp*EOS_tmp%kg_m3_to_R)
  ...
  fail = test_EOS_consistency(25.0_wp, 35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "WRIGHT_FULL", &
                              rho_check=1027.55177447616_wp*EOS_tmp%kg_m3_to_R, avg_Sv_check=.true.)
  ...
  fail = test_EOS_consistency(25.0_wp, 35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "WRIGHT_REDUCED", &
                              rho_check=1027.54303596346_wp*EOS_tmp%kg_m3_to_R, avg_Sv_check=.true.)
  ...
  fail = test_EOS_consistency(25.0_wp, 35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "WRIGHT", &
                              rho_check=1027.54303596346_wp*EOS_tmp%kg_m3_to_R, avg_Sv_check=.true.)
  ...
  fail = test_EOS_consistency(25.0_wp, 35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "ROQUET_RHO", &
                              rho_check=1027.42385663668_wp*EOS_tmp%kg_m3_to_R)
  ...
  fail = test_EOS_consistency(25.0_wp, 35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "ROQUET_SPV", &
                              rho_check=1027.42387475199_wp*EOS_tmp%kg_m3_to_R)
  ...
  fail = test_EOS_consistency(25.0_wp, 35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "JACKETT06", &
                              rho_check=1027.539690758425_wp*EOS_tmp%kg_m3_to_R)
  ...
  call EOS_manual_init(EOS_tmp, form_of_EOS=EOS_TEOS10)
  fail = test_EOS_consistency(25.0_wp, 35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "TEOS10", skip_2nd=.true., &
                              rho_check=1027.42355961492_wp*EOS_tmp%kg_m3_to_R)
  if (verbose .and. fail) call MOM_error(WARNING, "TEOS10 EOS has failed some self-consistency tests.")
  EOS_unit_tests = EOS_unit_tests .or. fail

  call EOS_manual_init(EOS_tmp, form_of_EOS=EOS_ROQUET_RHO)
  fail = test_EOS_consistency(10.0_wp, 30.0_wp, 1.0e7_wp, EOS_tmp, verbose, "ROQUET_RHO", &
                              rho_check=1027.45140117152_wp*EOS_tmp%kg_m3_to_R)
  ! The corresponding check value published by Roquet et al. (2015) is 1027.45140 [kg m-3].
  if (verbose .and. fail) call MOM_error(WARNING, "Roquet_rho EOS has failed some self-consistency tests.")
  EOS_unit_tests = EOS_unit_tests .or. fail

  call EOS_manual_init(EOS_tmp, form_of_EOS=EOS_ROQUET_SPV)
  fail = test_EOS_consistency(10.0_wp, 30.0_wp, 1.0e7_wp, EOS_tmp, verbose, "ROQUET_SPV", &
                              spv_check=9.73282046614623e-04_wp*EOS_tmp%R_to_kg_m3)
  ...
  call EOS_manual_init(EOS_tmp, form_of_EOS=EOS_LINEAR, Rho_T0_S0=1000.0_wp, drho_dT=-0.2_wp, dRho_dS=0.8_wp, dRho_dp=5.0e-7_wp)
  fail = test_EOS_consistency(25.0_wp, 35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "LINEAR", &
                              rho_check=1028.0_wp*EOS_tmp%kg_m3_to_R, avg_Sv_check=.true.)
  ...
  call EOS_manual_init(EOS_tmp, form_of_TFreeze=TFREEZE_LINEAR, TFr_S0_P0=0.0_wp, dTFr_dS=-0.054_wp, &
                       dTFr_dP=-7.6e-8_wp)
  fail = test_TFr_consistency(35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "LINEAR", TFr_check=-2.65_wp*EOS_tmp%degC_to_C)
  ...
  fail = test_TFr_consistency(35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "MILLERO_78", &
                              TFr_check=-2.69730134114106_wp*EOS_tmp%degC_to_C)
  ...
  fail = test_TFr_consistency(35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "TEOS10", &
                              TFr_check=-2.69099996992861_wp*EOS_tmp%degC_to_C)
  ...
  fail = test_TFr_consistency(35.0_wp, 1.0e7_wp, EOS_tmp, verbose, "TEOS_POLY", &
                              TFr_check=-2.691165259327735_wp*EOS_tmp%degC_to_C)
...
logical function test_TS_conversion_consistency(T_cons, S_abs, T_pot, S_prac, EOS, verbose) &
                                      result(inconsistent)
  real(wp),              intent(in) :: T_cons    !< Conservative temperature [degC]
  real(wp),              intent(in) :: S_abs     !< Absolute salinity [g kg-1]
  real(wp),              intent(in) :: T_pot     !< Potential temperature [degC]
  real(wp),              intent(in) :: S_prac    !< Practical salinity [PSU]
  type(EOS_type),    intent(in) :: EOS      !< Equation of state structure
  logical,           intent(in) :: verbose  !< If true, write results to stdout

  ! Local variables
  real(wp) :: Sabs(1)  ! Absolute or reference salinity [g kg-1]
  real(wp) :: Sprac(1) ! Practical salinity [PSU]
  real(wp) :: Stest(1) ! A converted salinity [ppt]
  real(wp) :: Tcons(1) ! Conservative temperature [degC]
  real(wp) :: Tpot(1)  ! Potential temperature [degC]
  real(wp) :: Ttest(1) ! A converted temperature [degC]
  real(wp) :: Stol     ! Roundoff error on a typical value of salinities [ppt]
  real(wp) :: Ttol     ! Roundoff error on a typical value of temperatures [degC]
  ...
  Ttol = 2.0_wp * 400.0_wp*epsilon(Ttol)
  Stol = 35.0_wp * 400.0_wp*epsilon(Stol)
...
logical function test_TFr_consistency(S_test, p_test, EOS, verbose, EOS_name, TFr_check) &
                                      result(inconsistent)
  real(wp),              intent(in) :: S_test   !< Salinity or absolute salinity [S ~> ppt]
  real(wp),              intent(in) :: p_test   !< Pressure [R L2 T-2 ~> Pa]
  type(EOS_type),    intent(in) :: EOS      !< Equation of state structure
  logical,           intent(in) :: verbose  !< If true, write results to stdout
  character(len=*),  intent(in) :: EOS_name !< A name used in error messages to describe the EoS
  real(wp),    optional, intent(in) :: TFr_check  !< A check value for the Freezing point [C ~> degC]

  ! Local variables
  real(wp), dimension(-3:3,-3:3) :: S ! Salinities at the test value and perturbed points [S ~> ppt]
  real(wp), dimension(-3:3,-3:3) :: P ! Pressures at the test value and perturbed points [R L2 T-2 ~> Pa]
  real(wp), dimension(-3:3,-3:3,2) :: TFr ! Freezing point at the test value and perturbed points [C ~> degC]
  real(wp) :: dS         ! Magnitude of salinity perturbations [S ~> ppt]
  real(wp) :: dp         ! Magnitude of pressure perturbations [R L2 T-2 ~> Pa]
  ! real :: tol        ! The nondimensional tolerance from roundoff [nondim]
  real(wp) :: TFr_tol    ! Roundoff error on a typical value of TFreeze [C ~> degC]
  logical :: test_OK ! True if a particular test is consistent.
  logical :: OK      ! True if all checks so far are consistent.
  integer :: i, j, n
  ...
  dS = 0.5_wp*EOS%ppt_to_S      ! Salinity perturbations [S ~> ppt]
  dp = 10.0e4_wp / EOS%RL2_T2_to_Pa ! Pressure perturbations [R L2 T-2 ~> Pa]

  ! TEOS 10 requires a tolerance that is ~20 times larger than other freezing point
  ! expressions because it lacks parentheses.
  TFr_tol = 2.0_wp*EOS%degC_to_C * 400.0_wp*epsilon(TFr_tol)
  ...
  do n=1,2
    ...
    do j=-3,3 ; do i=-3,3
      S(i,j) = max(S_test + n*dS*i, 0.0_wp)
      p(i,j) = max(p_test + n*dp*j, 0.0_wp)
    enddo ; enddo
    do j=-3,3
...
subroutine write_check_msg(var_name, val, val_chk, val_tol, test_OK)
  ...
  real(wp),             intent(in) :: val      !< The value being checked [various]
  real(wp),             intent(in) :: val_chk  !< The value being checked [various]
  real(wp),             intent(in) :: val_tol  !< The value being checked [various]
...
logical function test_EOS_consistency(T_test, S_test, p_test, EOS, verbose, &
  ...
  real(wp),             intent(in) :: T_test   !< Potential temperature or conservative temperature [C ~> degC]
  real(wp),             intent(in) :: S_test   !< Salinity or absolute salinity [S ~> ppt]
  real(wp),             intent(in) :: p_test   !< Pressure [R L2 T-2 ~> Pa]
  type(EOS_type),   intent(in) :: EOS      !< Equation of state structure
  logical,          intent(in) :: verbose  !< If true, write results to stdout
  character(len=*), intent(in) :: EOS_name !< A name used in error messages to describe the EoS
  real(wp),   optional, intent(in) :: rho_check  !< A check value for the density [R ~> kg m-3]
  real(wp),   optional, intent(in) :: spv_check  !< A check value for the specific volume [R-1 ~> m3 kg-1]
  ...
  ! Local variables
  real(wp), dimension(-3:3,-3:3,-3:3) :: T ! Temperatures at the test value and perturbed points [C ~> degC]
  real(wp), dimension(-3:3,-3:3,-3:3) :: S ! Salinities at the test value and perturbed points [S ~> ppt]
  real(wp), dimension(-3:3,-3:3,-3:3) :: P ! Pressures at the test value and perturbed points [R L2 T-2 ~> Pa]
  real(wp), dimension(-3:3,-3:3,-3:3,2) :: rho ! Densities relative to rho_ref at the test value and
                                       ! perturbed points [R ~> kg m-3]
  real(wp), dimension(-3:3,-3:3,-3:3,2) :: spv ! Specific volumes relative to spv_ref at the test value and
                                       ! perturbed points [R-1 ~> m3 kg-1]
  real(wp) :: dT        ! Magnitude of temperature perturbations [C ~> degC]
  real(wp) :: dS        ! Magnitude of salinity perturbations [S ~> ppt]
  real(wp) :: dp        ! Magnitude of pressure perturbations [R L2 T-2 ~> Pa]
  real(wp) :: rho_ref   ! A reference density that is extracted for greater accuracy [R ~> kg m-3]
  real(wp) :: spv_ref   ! A reference specific volume that is extracted for greater accuracy [R-1 ~> m3 kg-1]
  real(wp) :: rho_nooff ! Density with no reference offset [R ~> kg m-3]
  real(wp) :: spv_nooff ! Specific volume with no reference offset [R-1 ~> m3 kg-1]
  real(wp) :: drho_dT   ! The partial derivative of density with potential
                    ! temperature [R C-1 ~> kg m-3 degC-1]
  real(wp) :: drho_dS   ! The partial derivative of density with salinity
                    ! in [R S-1 ~> kg m-3 ppt-1]
  real(wp) :: drho_dp   ! The partial derivative of density with pressure (also the
                    ! inverse of the square of sound speed) [T2 L-2 ~> s2 m-2]
  real(wp) :: dSV_dT(1) ! The partial derivative of specific volume with potential
                    ! temperature [R-1 C-1 ~> m3 kg-1 degC-1]
  real(wp) :: dSV_dS(1) ! The partial derivative of specific volume with salinity
                    ! [R-1 S-1 ~> m3 kg-1 ppt-1]
  real(wp) :: SpV_avg_a(1) ! The pressure-averaged specific volume determined analytically [R-1 ~> m3 kg-1]
  real(wp) :: SpV_avg_q(1) ! The pressure-averaged specific volume determined via quadrature [R-1 ~> m3 kg-1]
  real(wp) :: drho_dS_dS ! Second derivative of density with respect to S  [R S-2 ~> kg m-3 ppt-2]
  real(wp) :: drho_dS_dT ! Second derivative of density with respect to T and S [R S-1 C-1 ~> kg m-3 ppt-1 degC-1]
  real(wp) :: drho_dT_dT ! Second derivative of density with respect to T [R C-2 ~> kg m-3 degC-2]
  real(wp) :: drho_dS_dP ! Second derivative of density with respect to salinity and pressure
                     ! [T2 S-1 L-2 ~> kg m-3 ppt-1 Pa-1]
  real(wp) :: drho_dT_dP ! Second derivative of density with respect to temperature and pressure
                     ! [T2 C-1 L-2 ~> kg m-3 degC-1 Pa-1]

  real(wp) :: drho_dT_fd(2) ! Two 6th order finite difference estimates of the partial derivative of density
                        ! with potential temperature [R C-1 ~> kg m-3 degC-1]
  real(wp) :: drho_dS_fd(2) ! Two 6th order finite difference estimates of the partial derivative of density
                        ! with salinity [R S-1 ~> kg m-3 ppt-1]
  real(wp) :: drho_dp_fd(2) ! Two 6th order finite difference estimates of the partial derivative of density
                        ! with pressure (also the inverse of the square of sound speed) [T2 L-2 ~> s2 m-2]
  real(wp) :: dSV_dT_fd(2)  ! Two 6th order finite difference estimates of the partial derivative of
                        ! specific volume with potential temperature [R-1 C-1 ~> m3 kg-1 degC-1]
  real(wp) :: dSV_dS_fd(2)  ! Two 6th order finite difference estimates of the partial derivative of
                        ! specific volume with salinity [R-1 S-1 ~> m3 kg-1 ppt-1]
  real(wp) :: drho_dS_dS_fd(2)  ! Two 6th order finite difference estimates of the second derivative of
                            ! density with respect to salinity [R S-2 ~> kg m-3 ppt-2]
  real(wp) :: drho_dS_dT_fd(2)  ! Two 6th order finite difference estimates of the second derivative of density
                            ! with respect to temperature and salinity [R S-1 C-1 ~> kg m-3 ppt-1 degC-1]
  real(wp) :: drho_dT_dT_fd(2)  ! Two 6th order finite difference estimates of the second derivative of
                            ! density with respect to temperature [R C-2 ~> kg m-3 degC-2]
  real(wp) :: drho_dS_dP_fd(2)  ! Two 6th order finite difference estimates of the second derivative of density
                            ! with respect to salinity and pressure [T2 S-1 L-2 ~> kg m-3 ppt-1 Pa-1]
  real(wp) :: drho_dT_dP_fd(2)  ! Two 6th order finite difference estimates of the second derivative of density
                            ! with respect to temperature and pressure [T2 C-1 L-2 ~> kg m-3 degC-1 Pa-1]
  real(wp) :: rho_tmp    ! A temporary copy of the situ density [R ~> kg m-3]
  real(wp) :: tol        ! The nondimensional tolerance from roundoff [nondim]
  real(wp) :: r_tol      ! Roundoff error on a typical value of density anomaly [R ~> kg m-3]
  real(wp) :: sv_tol     ! Roundoff error on a typical value of specific volume anomaly [R-1 ~> m3 kg-1]
  real(wp) :: tol_here   ! The tolerance for each check, in various units [various]
  real(wp) :: T_min, T_max ! The minimum and maximum temperature over which this EoS is fitted [degC]
  real(wp) :: S_min, S_max ! The minimum and maximum temperature over which this EoS is fitted [ppt]
  real(wp) :: p_min, p_max ! The minimum and maximum temperature over which this EoS is fitted [Pa]
  real(wp) :: count_fac  ! A factor in the roundoff estimates based on the factors in the numerator and
                     ! denominator in the finite difference derivative expression [nondim]
  real(wp) :: count_fac2 ! A factor in the roundoff estimates based on the factors in the numerator and
  ...
  dT = 0.1_wp*EOS%degC_to_C     ! Temperature perturbations [C ~> degC]
  dS = 0.5_wp*EOS%ppt_to_S      ! Salinity perturbations [S ~> ppt]
  dp = 10.0e4_wp / EOS%RL2_T2_to_Pa ! Pressure perturbations [R L2 T-2 ~> Pa]

  r_tol = 50.0_wp*EOS%kg_m3_to_R * 10._wp*epsilon(r_tol)
  sv_tol = 5.0e-5_wp*EOS%R_to_kg_m3 * 10._wp*epsilon(sv_tol)
  rho_ref = 1000.0_wp*EOS%kg_m3_to_R
  spv_ref = 1.0_wp / rho_ref
  ...
  if (present(spv_check)) then
    ...
    test_OK = (abs((rho_ref+rho(0,0,0,1)) * (spv_ref + spv(0,0,0,1)) - 1.0_wp) < tol)
    OK = OK .and. test_OK
    if (verbose) then
      write(mesg, '(ES16.8," and ",ES16.8,", ratio - 1 = ",ES16.8)') &
          rho_ref+rho(0,0,0,1), 1.0_wp/(spv_ref + spv(0,0,0,1)), &
          (rho_ref+rho(0,0,0,1)) * (spv_ref + spv(0,0,0,1)) - 1.0_wp
      if (test_OK) then
  ...
  if (order == 6) then
    count_fac = 110.0_wp/60.0_wp ; count_fac2 = 1088.0_wp/180.0_wp
  elseif (order == 4) then ! Use values appropriate for 4th order schemes.
    count_fac = 18.0_wp/12.0_wp ; count_fac2 = 64.0_wp/12.0_wp
  else ! Use values appropriate for 2nd order schemes.
    count_fac = 2.0_wp/2.0_wp ; count_fac2 = 4.0_wp
  endif
  ...
  if (test_2nd) then
    ...
    tol_here = 10.0_wp*(tol*abs(drho_dS_dS) + count_fac2*r_tol/dS**2)
  ...
  if (test_avg_Sv) then
    tol_here = 0.5_wp*tol*(abs(SpV_avg_a(1)) + abs(SpV_avg_q(1)))
    ...
    if (verbose) then
      write(mesg, '(ES24.16," and ",ES24.16," differ by ",ES16.8," (",ES10.2,"), tol=",ES16.8)') &
        ...
        2.0_wp*(SpV_avg_a(1) - SpV_avg_q(1)) / (abs(SpV_avg_a(1)) + abs(SpV_avg_q(1)) + tiny(SpV_avg_a(1))), &
        tol_here
      if (verbose .and. .not.test_OK) then
  ...
  real(wp) function first_deriv(R, dx, order)
    real(wp),    intent(in) :: R(-3:3) !< The field whose derivative is being taken, in arbitrary units [A]
    real(wp),    intent(in) :: dx      !< The spacing in parameter space, in different arbitrary units [B]
  ...
  real(wp) function derivs_2d(R, dxdy, order)
    real(wp),    intent(in) :: R(-3:3,-3:3) !< The field whose derivative is being taken in arbitrary units [A]
    real(wp),    intent(in) :: dxdy   !< The spacing in two directions in parameter space in different arbitrary units [B C]
    integer, intent(in) :: order  !< The order of accuracy of the centered finite difference estimates (2, 4 or 6)

    real(wp) :: dRdx(-3:3)  ! The first derivative in one direction times the grid spacing in that direction [A]
    integer :: i

    do i=-3,3
      dRdx(i) = first_deriv(R(:,i), 1.0_wp, order)
    enddo
  ...
  logical function check_FD(val, val_fd, tol, verbose, field_name, order)
    real(wp), intent(in) :: val       !< The derivative being checked, in arbitrary units [arbitrary]
    real(wp), intent(in) :: val_fd(2) !< Two finite difference estimates of val taken with a spacing
                                  !! in parameter space and twice this spacing, in the same
                                  !! arbitrary units as val [arbitrary]
    real(wp), intent(in) :: tol       !< An estimated fractional tolerance due to roundoff [arbitrary]
    logical, intent(in) :: verbose !< If true, write results to stdout
    ...
    check_FD = ( abs(val_fd(1) - val) < (1.2_wp*abs(val_fd(2) - val)/2**order + abs(tol)) )

    write(mesg, '(ES24.16," and ",ES24.16," differ by ",ES16.8," (",ES10.2,"), tol=",ES16.8)') &
          val, val_fd(1), val - val_fd(1), &
          2.0_wp*(val - val_fd(1)) / (abs(val) + abs(val_fd(1)) + tiny(val)), &
          (1.2_wp*abs(val_fd(2) - val)/2**order + abs(tol))
