...
subroutine soliton_initialize_thickness(h, depth_tot, G, GV, US, param_file, just_read)
  ...
  ! Local variables
  real(wp)    :: max_depth  ! Maximum depth of the model bathymetry [Z ~> m]
  real(wp)    :: cg_max     ! The external wave speed based on max_depth [L T-1 ~> m s-1]
  real(wp)    :: beta       ! The meridional gradient of the Coriolis parameter [T-1 L-1 ~> s-1 m-1]
  real(wp)    :: L_eq       ! The equatorial deformation radius used in nondimensionalizing this problem [L ~> m]
  real(wp)    :: scale_pos  ! A conversion factor to nondimensionalize the axis units, usually [m-1]
  real(wp)    :: x0    ! Initial x-position of the soliton in the same units as geoLonT, often [m].
  real(wp)    :: y0    ! Initial y-position of the soliton in the same units as geoLatT, often [m].
  real(wp)    :: x, y  ! Nondimensionalized positions [nondim]
  real(wp)    :: I_nz  ! The inverse of the number of layers [nondim]
  real(wp)    :: val1  ! A nondimensionlized zonal decay scale [nondim]
  real(wp)    :: val2  ! An overall surface height anomaly amplitude [L T-1 ~> m s-1]
  real(wp)    :: val3  ! A decay factor [nondim]
  real(wp)    :: val4  ! The local velocity amplitude [L T-1 ~> m s-1]
...
# include "version_variable.h"
  ...
  call get_param(param_file, mdl, "MAXIMUM_DEPTH", max_depth, &
                 units="m", default=-1.e9_wp, scale=US%m_to_Z, do_not_log=.true.)
  call get_param(param_file, mdl, "BETA", beta, &
                 "The northward gradient of the Coriolis parameter with the betaplane option.", &
                 units="m-1 s-1", default=0.0_wp, scale=US%T_to_s*US%L_to_m, do_not_log=.true.)
  ...
  if (abs(beta) <= 0.0_wp) call MOM_error(FATAL, &
      "soliton_initialization, soliton_initialize_thickness: "//&
      "This module requires a non-zero value of BETA.")
  if (G%grid_unit_to_L <= 0._wp) call MOM_error(FATAL, "soliton_initialization.F90: "//&
  ...
  I_nz = 1.0_wp / real(nz, wp)

  x0 = 2.0_wp*G%len_lon/3.0_wp
  y0 = 0.0_wp
  val1 = 0.395_wp
  val2 = max_depth * 0.771_wp*(val1*val1)
  ...
  do j = G%jsc,G%jec ; do i = G%isc,G%iec
    do k = 1, nz
      ...
      val3 = exp(-val1*x)
      val4 = val2 * ( 2.0_wp*val3 / (1.0_wp + (val3*val3)) )**2
      h(i,j,k) = (0.25_wp*val4*(6.0_wp*y*y + 3.0_wp) * exp(-0.5_wp*y*y) + depth_tot(i,j)) * I_nz
    enddo
  enddo ; enddo
...
subroutine soliton_initialize_velocity(u, v, G, GV, US, param_file, just_read)
  ...
  real(wp)    :: x, y  ! Nondimensionalized positions [nondim]
  real(wp)    :: val1  ! A nondimensionlized zonal decay scale [nondim]
  real(wp)    :: val2  ! An overall velocity amplitude [L T-1 ~> m s-1]
  real(wp)    :: val3  ! A decay factor [nondim]
  real(wp)    :: val4  ! The local velocity amplitude [L T-1 ~> m s-1]
  integer :: i, j, k, is, ie, js, je, nz
  ...
  y0 = 0.0_wp
  val1 = 0.395_wp
  val2 = cg_max * 0.771_wp*(val1*val1)
  ...
  do j = G%jsc,G%jec ; do I = G%isc-1,G%iec+1
    do k = 1, nz
      x = (0.5_wp*(G%geoLonT(i+1,j)+G%geoLonT(i,j))-x0) * scale_pos
      y = (0.5_wp*(G%geoLatT(i+1,j)+G%geoLatT(i,j))-y0) * scale_pos
      val3 = exp(-val1*x)
      val4 = val2*((2.0_wp*val3/(1.0_wp+(val3*val3)))**2)
      u(I,j,k) = 0.25_wp*val4*(6.0_wp*y*y-9.0_wp) * exp(-0.5_wp*y*y)
    enddo
  enddo ; enddo
  do j = G%jsc-1,G%jec+1 ; do I = G%isc,G%iec
    do k = 1, nz
      x = 0.5_wp*(G%geoLonT(i,j+1)+G%geoLonT(i,j))-x0
      y = 0.5_wp*(G%geoLatT(i,j+1)+G%geoLatT(i,j))-y0
      val3 = exp(-val1*x)
      val4 = val2*((2.0_wp*val3/(1.0_wp+(val3*val3)))**2)
      v(i,J,k) = 2.0_wp*val4*y*(-2.0_wp*val1*tanh(val1*x)) * exp(-0.5_wp*y*y)
    enddo
  enddo ; enddo
