...
subroutine basin_builder_topography(D, G, param_file, max_depth)
  ...
  real(wp), dimension(20) :: pars ! Parameters for each function [various]
...
real(wp) function angled_coast(lon, lat, lon_eq, lat_mer, dr, sh)
  real(wp), intent(in) :: lon     !< Longitude [degrees_E]
  real(wp), intent(in) :: lat     !< Latitude [degrees_N]
  real(wp), intent(in) :: lon_eq  !< Longitude intersection with Equator [degrees_E]
  real(wp), intent(in) :: lat_mer !< Latitude intersection with Prime Meridian [degrees_N]
  real(wp), intent(in) :: dr      !< "Radius" of coast profile [degrees]
  real(wp), intent(in) :: sh      !< depth of shelf as fraction of full depth [nondim]
  real(wp) :: r ! A relative position [degrees]
  real(wp) :: I_dr ! The inverse of a distance [degrees-1]

  I_dr = 1/sqrt( lat_mer*lat_mer + lon_eq*lon_eq )
  r = I_dr * ( lat_mer*lon + lon_eq*lat - lon_eq*lat_mer)
  angled_coast = cstprof(r, 0._wp, dr, 0.125_wp, 0.125_wp, 0.5_wp, sh)
end function angled_coast

!> A "coast profile" applied in an N-S line from lonC,lat0 to lonC,lat1 [nondim].
real(wp) function NS_coast(lon, lat, lonC, lat0, lat1, dlon, sh)
  real(wp), intent(in) :: lon     !< Longitude [degrees_E]
  real(wp), intent(in) :: lat     !< Latitude [degrees_N]
  real(wp), intent(in) :: lonC    !< Longitude of coast [degrees_E]
...
real(wp) function EW_coast(lon, lat, latC, lon0, lon1, dlat, sh)
  real(wp), intent(in) :: lon     !< Longitude [degrees_E]
  real(wp), intent(in) :: lat     !< Latitude [degrees_N]
  real(wp), intent(in) :: latC    !< Latitude of coast [degrees_N]
...
real(wp) function NS_conic_ridge(lon, lat, lonC, lat0, lat1, dlon, rh)
  real(wp), intent(in) :: lon     !< Longitude [degrees_E]
  real(wp), intent(in) :: lat     !< Latitude [degrees_N]
  real(wp), intent(in) :: lonC    !< Longitude of ridge center [degrees_E]
  ...
  r = dist_line_fixed_x( lon, lat, lonC, lat0, lat1 )
  NS_conic_ridge = 1._wp - rh * cone(r, 0._wp, dlon)
end function NS_conic_ridge

!> A NS ridge with an scurve profile [nondim]
real(wp) function NS_scurve_ridge(lon, lat, lonC, lat0, lat1, dlon, rh)
  real(wp), intent(in) :: lon     !< Longitude [degrees_E]
  real(wp), intent(in) :: lat     !< Latitude [degrees_N]
  real(wp), intent(in) :: lonC    !< Longitude of ridge center [degrees_E]
  real(wp), intent(in) :: lat0    !< Latitude of ridge end [degrees_N]
  ...
  r = dist_line_fixed_x( lon, lat, lonC, lat0, lat1 )
  NS_scurve_ridge = 1._wp - rh * (1._wp - scurve(r, 0._wp, dlon) )
end function NS_scurve_ridge

!> A circular ridge with cutoff conic profile [nondim]
real(wp) function circ_conic_ridge(lon, lat, lon0, lat0, ring_radius, ring_thickness, ridge_height)
  real(wp), intent(in) :: lon            !< Longitude [degrees_E]
  real(wp), intent(in) :: lat            !< Latitude [degrees_N]
  ...
  circ_conic_ridge = 1._wp - frac_ht ! nondim depths (1-frac_ridge_height) .. 1
end function circ_conic_ridge

!> A circular ridge with cutoff scurve profile [nondim]
real(wp) function circ_scurve_ridge(lon, lat, lon0, lat0, ring_radius, ring_thickness, ridge_height)
  real(wp), intent(in) :: lon            !< Longitude [degrees_E]
  real(wp), intent(in) :: lat            !< Latitude [degrees_N]
  real(wp), intent(in) :: lon0           !< Longitude of center of ring [degrees_E]
  real(wp), intent(in) :: lat0           !< Latitude of center of ring [degrees_N]
  ...
  real(wp), intent(in) :: ridge_height   !< Ridge height as fraction of full depth [nondim]
  real(wp) :: r  ! A relative position [degrees]
  real(wp) :: s  ! A function of the normalized position [nondim]
  real(wp) :: frac_ht ! The fractional height of the topography [nondim]

  r = sqrt( ((lon - lon0)**2) + ((lat - lat0)**2) ) ! Pseudo-distance from a point
  r = abs( r - ring_radius) ! Pseudo-distance from a circle
  s = 1._wp - scurve(r, 0._wp, ring_thickness) ! 0 .. 1
  frac_ht = s * ridge_height ! 0 .. frac_ridge_height
  circ_scurve_ridge = 1._wp - frac_ht ! nondim depths (1-frac_ridge_height) .. 1
