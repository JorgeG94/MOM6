...
subroutine DOME_initialize_topography(D, G, param_file, max_depth, US)
  ...
  ! Local variables
  real(wp) :: min_depth  ! The minimum ocean depth [Z ~> m]
  real(wp) :: shelf_depth ! The ocean depth on the shelf in the DOME configuration [Z ~> m]
  real(wp) :: slope      ! The bottom slope in the DOME configuration [Z L-1 ~> nondim]
  real(wp) :: shelf_edge_lat ! The latitude of the edge of the topographic shelf in the same units as geolat, often [km]
  real(wp) :: inflow_lon ! The edge longitude of the DOME inflow in the same units as geolon, often [km]
  real(wp) :: inflow_width ! The longitudinal width of the DOME inflow channel in the same units as geolat, often [km]
...
# include "version_variable.h"
  ...
  if (G%grid_unit_to_L <= 0._wp) call MOM_error(FATAL, "DOME_initialization: "//&
  ...
  call get_param(param_file, mdl, "MINIMUM_DEPTH", min_depth, &
                 "The minimum depth of the ocean.", default=0.0_wp, units="m", scale=US%m_to_Z)
  call get_param(param_file, mdl, "DOME_TOPOG_SLOPE", slope, &
                 "The slope of the bottom topography in the DOME configuration.", &
                 default=0.01_wp, units="nondim", scale=US%L_to_Z)
...
subroutine DOME_initialize_thickness(h, depth_tot, G, GV, param_file, just_read)
  ...
  e0(1)=0.0_wp
  do k=2,nz
    e0(K) = -G%max_depth * (real(k-1, wp)-0.5_wp)/real(nz-1, wp)
  enddo
...
subroutine DOME_initialize_sponges(G, GV, US, tv, depth_tot, PF, CSp)
  ...
  real(wp) :: e_tgt(SZK_(GV)+1) ! Target interface heights [Z ~> m].
  real(wp) :: min_depth    ! The minimum depth at which to apply damping [Z ~> m]
  real(wp) :: damp_W, damp_E ! Damping rates in the western and eastern sponges [T-1 ~> s-1]
  real(wp) :: peak_damping ! The maximum sponge damping rates as the edges [T-1 ~> s-1]
  real(wp) :: edge_dist    ! The distance to an edge [L ~> m]
  real(wp) :: sponge_width ! The width of the sponges [L ~> m]
  ...
  call get_param(PF, mdl, "DOME_SPONGE_DAMP_RATE", peak_damping, &
                 ...
                 default=10.0_wp, units="day-1", scale=1.0_wp/(86400.0_wp*US%s_to_T))
  call get_param(PF, mdl, "DOME_SPONGE_WIDTH", sponge_width, &
                 "The width of the the DOME sponges.", &
                 default=200.0_wp, units="km", scale=1.0e3_wp*US%m_to_L)
  ...
  do j=js,je ; do i=is,ie ; if (depth_tot(i,j) > min_depth) then
    ...
    if (edge_dist < 0.5_wp*sponge_width) then
      damp_W = peak_damping
    elseif (edge_dist < sponge_width) then
      damp_W = peak_damping * (sponge_width - edge_dist) / (0.5_wp*sponge_width)
    else
      damp_W = 0.0_wp
    endif
    ...
    if (edge_dist < 0.5_wp*sponge_width) then
      damp_E = peak_damping
    elseif (edge_dist < sponge_width) then
      damp_E = peak_damping * (sponge_width - edge_dist) / (0.5_wp*sponge_width)
    else
      damp_E = 0.0_wp
    endif
  ...
  e_tgt(1) = 0.0_wp
  do K=2,nz ; e_tgt(K) = -(real(K-1, wp)-0.5_wp)*G%max_depth / real(nz-1, wp) ; enddo
...
subroutine DOME_set_OBC_data(OBC, tv, G, GV, US, PF, tr_Reg)
  ...
  ! Local variables
  real(wp) :: T0(SZK_(GV))       ! A profile of target temperatures [C ~> degC]
  real(wp) :: S0(SZK_(GV))       ! A profile of target salinities [S ~> ppt]
  ...
  real(wp) :: S_ref             ! A default value for salinities [S ~> ppt]
  real(wp) :: T_light           ! A first guess at the temperature of the lightest layer [C ~> degC]
  ! The following variables are used to set up the transport in the DOME example.
  real(wp) :: tr_0              ! The total integrated inflow transport [H L2 T-1 ~> m3 s-1 or kg s-1]
  real(wp) :: tr_k              ! The integrated inflow transport of a layer [H L2 T-1 ~> m3 s-1 or kg s-1]
  real(wp) :: v_k               ! The velocity of a layer at the edge [L T-1 ~> m s-1]
  real(wp) :: yt, yb            ! The log of these variables gives the fractional velocities at the
                            ! top and bottom of a layer [nondim]
  real(wp) :: rst, rsb          ! The relative position of the top and bottom of a layer [nondim],
                            ! with a range from 0 for the densest water to -1 for the lightest
  real(wp) :: rc                ! The relative position of the center of a layer [nondim]
  real(wp) :: lon_im1           ! An extrapolated value for the longitude of the western edge of a
                            ! v-velocity face, in the same units as G%geoLon [km]
  real(wp) :: D_edge            ! The thickness [Z ~> m] of the dense fluid at the
  ...
  real(wp) :: f_inflow          ! The value of the Coriolis parameter used to determine DOME inflow
                            ! properties [T-1 ~> s-1]
  real(wp) :: g_prime_tot       ! The reduced gravity across all layers [L2 Z-1 T-2 ~> m s-2]
  real(wp) :: Def_Rad           ! The deformation radius, based on fluid of thickness D_edge [L ~> m]
  real(wp) :: inflow_lon        ! The edge longitude of the DOME inflow in the same units as geolon, often [km]
  real(wp) :: I_Def_Rad         ! The inverse of the deformation radius in the same units as G%geoLon [km-1]
  real(wp) :: Ri_trans          ! The shear Richardson number in the transition
  ...
  if (associated(tv%S) .or. associated(tv%T)) then
    ...
    call get_param(PF, mdl, "DOME_T_LIGHT", T_light, &
                 ...
                 units="degC", default=25.0_wp, scale=US%degC_to_C)
  endif
  ...
  if (GV%Boussinesq) then
    ...
    tr_0 = (-D_edge*sqrt(D_edge*g_prime_tot)*0.5_wp*Def_Rad) * GV%Z_to_H
  else
    g_prime_tot = (GV%g_Earth / (Rlay_Ref + 0.5_wp*Rlay_range)) * Rlay_range
    Def_Rad = sqrt(D_edge*g_prime_tot) / abs(f_inflow)
    tr_0 = (-D_edge*sqrt(D_edge*g_prime_tot)*0.5_wp*Def_Rad) * (Rlay_Ref + 0.5_wp*Rlay_range) * GV%RZ_to_H
  endif

  I_Def_Rad = 1.0_wp / ((1.0e-3_wp*US%L_to_m*km_to_grid_unit) * Def_Rad)
  ...
  do k=1,nz
    rst = -1.0_wp
    if (k>1) rst = -1.0_wp + (real(k-1, wp)-0.5_wp)/real(nz-1, wp)

    rsb = 0.0_wp
    if (k<nz) rsb = -1.0_wp + (real(k-1, wp)+0.5_wp)/real(nz-1, wp)
    rc = -1.0_wp + real(k-1, wp)/real(nz-1, wp)

    ! These come from assuming geostrophy and a constant Ri profile.
    yt = (2.0_wp*Ri_trans*rst + Ri_trans + 2.0_wp)/(2.0_wp - Ri_trans)
    yb = (2.0_wp*Ri_trans*rsb + Ri_trans + 2.0_wp)/(2.0_wp - Ri_trans)
    tr_k = tr_0 * (2.0_wp/(Ri_trans*(2.0_wp-Ri_trans))) * &
           ((log(yt)+1.0_wp)/yt - (log(yb)+1.0_wp)/yb)
    v_k = -sqrt(D_edge*g_prime_tot)*log((2.0_wp + Ri_trans*(1.0_wp + 2.0_wp*rc)) / &
                                        (2.0_wp - Ri_trans))
    if (k == nz)  tr_k = tr_k + tr_0 * (2.0_wp/(Ri_trans*(2.0_wp+Ri_trans))) * &
                                       log((2.0_wp+Ri_trans)/(2.0_wp-Ri_trans))
    ! New way
    ...
    do J=JsdB,JedB ; do i=isd,ied
      ...
      lon_im1 = 2.0_wp*G%geoLonCv(i,J) - G%geoLonBu(I,J)
      segment%normal_trans(i,J,k) = tr_k * (exp(-2.0_wp*(lon_im1 - inflow_lon) * I_Def_Rad) - &
                                  exp(-2.0_wp*(G%geoLonBu(I,J) - inflow_lon) * I_Def_Rad))
      segment%normal_vel(i,J,k) = v_k * exp(-2.0_wp*(G%geoLonCv(i,J) - inflow_lon) * I_Def_Rad)
    enddo ; enddo
  enddo
  ...
  do k=1,nz ; do j=segment%HI%jsd,segment%HI%jed ; do i=segment%HI%isd,segment%HI%ied
    if (k < nz/2) then ; segment%field(ntherm+1)%buffer_src(i,j,k) = 0.0_wp
    else ; segment%field(ntherm+1)%buffer_src(i,j,k) = 1.0_wp ; endif
  enddo ; enddo ; enddo
  name = 'tr_D1'
  call tracer_name_lookup(tr_Reg, ntr_id, tr_ptr, name)
  ...
  do m=2,tr_Reg%ntr
    ...
    call tracer_name_lookup(tr_Reg, ntr_id, tr_ptr, name)
    call register_segment_tracer(tr_ptr, ntr_id, PF, GV, OBC%segment(1), OBC_scalar=0.0_wp)
  enddo
