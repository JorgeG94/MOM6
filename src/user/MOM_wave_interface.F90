...
type, public :: wave_parameters_CS ; private
  ...
  real(wp) :: Stokes_min_thick_avg !< A layer thickness below which the cell-center Stokes drift is
  ...
  real(wp) :: land_speed    !< A large Stokes velocity that can be used to indicate land values in
  ...
  real(wp) :: LA_FracHBL         !< Fraction of OSBL for averaging Langmuir number [nondim]
  real(wp) :: LA_HBL_min         !< Minimum boundary layer depth for averaging Langmuir number [Z ~> m]
  ...
  real(wp) :: g_Earth      !< The gravitational acceleration, equivalent to GV%g_Earth but with
                       !! different dimensional rescaling appropriate for deep-water gravity
                       !! waves [Z T-2 ~> m s-2]
  real(wp) :: I_g_Earth    !< The inverse of the gravitational acceleration, with dimensional rescaling
  ...
  real(wp) :: La_min       !< An arbitrary lower-bound on the Langmuir number [nondim].
  ...
  real(wp) :: La_Stk_backgnd !< A small background Stokes velocity used in the denominator of
  ...
  real(wp) :: VonKar = -1.0_wp !< The von Karman coefficient as used in the MOM_wave_interface module [nondim]
  real(wp) :: rho_air  !< A typical density of air at sea level, as used in wave calculations [R ~> kg m-3]
  real(wp) :: nu_air   !< The viscosity of air, as used in wave calculations [Z2 T-1 ~> m2 s-1]
  real(wp) :: rho_ocn  !< A typical surface density of seawater, as used in wave calculations in
                   !! comparison with the density of air [R ~> kg m-3].  The default is RHO_0.
  real(wp) :: SWH_from_u10sq !< A factor for converting the square of the 10 m wind speed to the
                   !! significant wave height [Z T2 L-2 ~> s2 m-1]
  real(wp) :: Charnock_min !< The minimum value of the Charnock coefficient, which relates the square of
                   !! the air friction velocity divided by the gravitational acceleration to the
                   !! wave roughness length [nondim]
  real(wp) :: Charnock_slope_U10 !< The partial derivative of the Charnock coefficient with the 10 m wind
  ...
  real(wp) :: Charnock_intercept !< The intercept of the fit for the Charnock coefficient in the limit of
  ...
  real(wp)    :: TP_STKX0     !< Test profile x-stokes drift amplitude [L T-1 ~> m s-1]
  real(wp)    :: TP_STKY0     !< Test profile y-stokes drift amplitude [L T-1 ~> m s-1]
  real(wp)    :: TP_WVL       !< Test profile wavelength [Z ~> m]
  ...
  real(wp)    :: WaveAge      !< The fixed wave age used with the DHH85 spectrum [nondim]
  real(wp)    :: WaveWind     !< Wind speed for the DHH85 spectrum [L T-1 ~> m s-1]
  real(wp)    :: omega_min    !< Minimum wave frequency with the DHH85 spectrum [T-1 ~> s-1]
  real(wp)    :: omega_max    !< Maximum wave frequency with the DHH85 spectrum [T-1 ~> s-1]
...
# include "version_variable.h"
  ...
  CS%I_g_Earth = 1.0_wp / CS%g_Earth
  ...
  call get_param(param_file, mdl, "LA_DEPTH_MIN", CS%LA_HBL_min, &
                 ...
                 "number calculation.", units="m", default=0.1_wp, scale=US%m_to_Z)
  ...
  case (SURFBANDS_STRING)! Surface Stokes Drift Bands
    ...
    call get_param(param_file, mdl, "SURFBAND_MIN_THICK_AVG", CS%Stokes_min_thick_avg, &
                 ...
                 units="m", default=0.1_wp, scale=US%m_to_Z, do_not_log=(CS%answer_date>=20230101))
    ...
    case (DATAOVR_STRING)! Using Data Override
      ...
      call get_param(param_file, mdl, "SURFBAND_OVERRIDE_LAND_SPEED", CS%land_speed, &
                 ...
                 units="m s-1", default=10.0_wp, scale=US%m_s_to_L_T)
    case (COUPLER_STRING)! Reserved for coupling
      ...
      allocate( CS%WaveNum_Cen(CS%NumBands), source=0.0_wp )
      allocate( CS%STKx0(G%isdB:G%iedB,G%jsd:G%jed,CS%NumBands), source=0.0_wp )
      allocate( CS%STKy0(G%isd:G%ied,G%jsdB:G%jedB,CS%NumBands), source=0.0_wp )
      allocate( CS%UStk_Hb(G%isc:G%iec,G%jsc:G%jec,CS%NumBands), source=0.0_wp )
      allocate( CS%VStk_Hb(G%isc:G%iec,G%jsc:G%jec,CS%NumBands), source=0.0_wp )
      allocate( CS%Omega_w2x(G%isc:G%iec,G%jsc:G%jec)          , source=0.0_wp )
      CS%PartitionMode = 0
      call get_param(param_file, mdl, "SURFBAND_WAVENUMBERS", CS%WaveNum_Cen, &
           "Central wavenumbers for surface Stokes drift bands.", &
           units='rad/m', default=0.12566_wp, scale=US%Z_to_m)
    case (INPUT_STRING)! A method to input the Stokes band (globally uniform)
      ...
      allocate( CS%WaveNum_Cen(1:CS%NumBands), source=0.0_wp )
      allocate( CS%PrescribedSurfStkX(1:CS%NumBands), source=0.0_wp )
      allocate( CS%PrescribedSurfStkY(1:CS%NumBands), source=0.0_wp )
      allocate( CS%STKx0(G%isdB:G%iedB,G%jsd:G%jed,1:CS%NumBands), source=0.0_wp )
      allocate( CS%STKy0(G%isd:G%ied,G%jsdB:G%jedB,1:CS%NumBands), source=0.0_wp )
      ...
      call get_param(param_file, mdl, "SURFBAND_STOKES_X", CS%PrescribedSurfStkX, &
           ...
           units='m/s', default=0.15_wp, scale=US%m_s_to_L_T)
  ...
  case (DHH85_STRING) !Donelan et al., 1985 spectrum
    ...
    call get_param(param_file, mdl, "DHH85_MIN_WAVE_FREQ", CS%omega_min, &
                 ...
                 units='s-1', default=0.1_wp, scale=US%T_to_s)
    call get_param(param_file, mdl, "DHH85_MAX_WAVE_FREQ", CS%omega_max, &
                 "Maximum wave frequency for the DHH85 spectrum.", &
                 units='s-1', default=10.0_wp, scale=US%T_to_s) ! The default is about a 30 cm cutoff wavelength.
  ...
  call get_param(param_file, mdl, "LANGMUIR_STOKES_BACKGROUND", CS%La_Stk_backgnd, &
         ...
         units="m s-1", default=1.0e-10_wp, scale=US%m_s_to_L_T, do_not_log=(CS%WaveMethod==LF17))

  ! Allocate and initialize
  ! a. Stokes driftProfiles
  allocate(CS%Us_x(G%isdB:G%IedB,G%jsd:G%jed,G%ke), source=0.0_wp)
  allocate(CS%Us_y(G%isd:G%Ied,G%jsdB:G%jedB,G%ke), source=0.0_wp)
  if (CS%Stokes_DDT) then
    ...
    allocate(CS%ddt_Us_x(G%isdB:G%IedB,G%jsd:G%jed,G%ke), source=0.0_wp)
    allocate(CS%ddt_Us_y(G%isd:G%Ied,G%jsdB:G%jedB,G%ke), source=0.0_wp)
    allocate(CS%Us_x_from_ddt(G%isdB:G%IedB,G%jsd:G%jed,G%ke), source=0.0_wp)
    allocate(CS%Us_y_from_ddt(G%isd:G%Ied,G%jsdB:G%jedB,G%ke), source=0.0_wp)
  endif
  ! b. Surface Values
  allocate(CS%US0_x(G%isdB:G%iedB,G%jsd:G%jed), source=0.0_wp)
  allocate(CS%US0_y(G%isd:G%ied,G%jsdB:G%jedB), source=0.0_wp)
  ! c. Langmuir number
  allocate(CS%La_turb(G%isc:G%iec,G%jsc:G%jec), source=0.0_wp)
  ! d. Viscosity for Stokes drift
  if (CS%StokesMixing) then
    allocate(CS%KvS(G%isd:G%Ied,G%jsd:G%jed,GV%ke), source=0.0_wp)
  endif
...
subroutine set_LF17_wave_params(param_file, mdl, GV, US, CS)
  ...
  call get_param(param_file, mdl, "VISCOSITY_AIR", CS%nu_air, &
                 ...
                 units="m2 s-1", default=1.0e-6_wp, scale=US%m2_s_to_Z2_T)
  call get_param(param_file, mdl, "VON_KARMAN_WAVES", CS%vonKar, &
                 ...
                 units="nondim", default=0.40_wp)  ! The default elsewhere in MOM6 is usually 0.41.
  call get_param(param_file, mdl, "RHO_AIR", CS%rho_air, &
                 "A typical density of air at sea level, as used in wave calculations", &
                 units="kg m-3", default=1.225_wp, scale=US%kg_m3_to_R)
  ...
  call get_param(param_file, mdl, "WAVE_HEIGHT_SCALE_FACTOR", CS%SWH_from_u10sq, &
                 ...
                 units="s2 m-1", default=0.0246_wp, scale=US%m_to_Z*US%L_T_to_m_s**2)
  call get_param(param_file, mdl, "CHARNOCK_MIN", CS%Charnock_min, &
                 ...
                 "the air friction velocity divided by the gravitational acceleration to the "//&
                 "wave roughness length.", units="nondim", default=0.028_wp)
  call get_param(param_file, mdl, "CHARNOCK_SLOPE_U10", CS%Charnock_slope_U10, &
                 ...
                 units="s m-1", default=0.0017_wp, scale=US%L_T_to_m_s)
...
subroutine query_wave_properties(CS, NumBands, WaveNumbers, US)
  ...
  real(wp), dimension(:),    optional, intent(out) :: Wavenumbers !< If present this returns the characteristic
  ...
  elseif (CS%WaveMethod == SURFBANDS) then
    ...
    elseif (CS%DataSource == COUPLER) then
      ...
      do b=1,CS%NumBands
        ...
        do j=G%jsc,G%jec
          do I=G%iscB,G%iecB
            CS%STKx0(I,j,b) = 0.5_wp*(forces%UStkb(i,j,b)+forces%UStkb(i+1,j,b))
          enddo
        enddo
        do J=G%jscB,G%jecB
          do i=G%isc,G%iec
            CS%STKY0(i,J,b) = 0.5_wp*(forces%VStkb(i,j,b)+forces%VStkb(i,j+1,b))
          enddo
        enddo
...
subroutine Update_Stokes_Drift(G, GV, US, CS, dz, ustar, dt, dynamics_step)
  ...
  real(wp), intent(in)                        :: dt    !< Time-step for computing Stokes-tendency [T ~> s]
  logical, intent(in)                     :: dynamics_step !< True if this call is on a dynamics step

  ! Local Variables
  real(wp)    :: Top, MidPoint, Bottom ! Positions within the layer [Z ~> m]
  real(wp)    :: level_thick ! The thickness of each layer [Z ~> m]
  real(wp)    :: DecayScale ! A vertical decay scale in the test profile [Z-1 ~> m-1]
  real(wp)    :: CMN_FAC  ! A nondimensional factor [nondim]
  real(wp)    :: WN       ! Model wavenumber [Z-1 ~> m-1]
  real(wp)    :: UStokes  ! A Stokes drift velocity [L T-1 ~> m s-1]
  real(wp)    :: PI       ! 3.1415926535... [nondim]
  real(wp)    :: La       ! The local Langmuir number [nondim]
  integer :: i, j, k, b
  real(wp)    :: I_dt     ! The inverse of the time step [T-1 ~> s-1]
  ...
  if (CS%WaveMethod==TESTPROF) then
    ...
    DecayScale = 4._wp*PI / CS%TP_WVL !4pi
    do j=G%jsc,G%jec
      do I=G%iscB,G%iecB
        Bottom = 0.0_wp
        MidPoint = 0.0_wp
        do k = 1,GV%ke
          Top = Bottom
          MidPoint = Bottom - 0.25_wp*(dz(i,j,k)+dz(i+1,j,k))
          Bottom = Bottom - 0.5_wp*(dz(i,j,k)+dz(i+1,j,k))
    ...
    do J=G%jscB,G%jecB
      do i=G%isc,G%iec
        ...
        do k = 1,GV%ke
          Top = Bottom
          MidPoint = Bottom - 0.25_wp*(dz(i,j,k)+dz(i,j+1,k))
          Bottom = Bottom - 0.5_wp*(dz(i,j,k)+dz(i,j+1,k))
  ...
  elseif (CS%WaveMethod==SURFBANDS) then
    ...
    do j=G%jsc,G%jec
      do I=G%iscB,G%iecB
        ...
        do k = 1,GV%ke
          Top = Bottom
          level_thick = 0.5_wp*(dz(i,j,k)+dz(i+1,j,k))
          MidPoint = Top - 0.5_wp*level_thick
          ...
          if (CS%answer_date >= 20230101) then
            ...
            do b = 1,CS%NumBands
              if (CS%PartitionMode == 0) then
                ...
                CMN_FAC = exp(2._wp*CS%WaveNum_Cen(b)*Top) * one_minus_exp_x(2._wp*CS%WaveNum_Cen(b)*level_thick)
              else
                ! Use an analytic expression for the average of an exponential over a layer
                WN = CS%Freq_Cen(b)**2 * CS%I_g_Earth
                CMN_FAC = exp(2._wp*WN*Top) * one_minus_exp_x(2._wp*WN*level_thick)
              endif
          ...
          elseif (level_thick > CS%Stokes_min_thick_avg) then
            ...
            do b = 1,CS%NumBands
              if (CS%PartitionMode == 0) then
                ...
                CMN_FAC = (exp(Top*2._wp*CS%WaveNum_Cen(b))-exp(Bottom*2._wp*CS%WaveNum_Cen(b))) &
                          / ((Top-Bottom)*(2._wp*CS%WaveNum_Cen(b)))
              else
                ! Use a numerical integration and then divide by layer thickness
                WN = CS%Freq_Cen(b)**2 / CS%g_Earth !bgr bug-fix missing g
                CMN_FAC = (exp(2._wp*WN*Top)-exp(2._wp*WN*Bottom)) / (2._wp*WN*(Top-Bottom))
              endif
              CS%US_x(I,j,k) = CS%US_x(I,j,k) + CS%STKx0(I,j,b)*CMN_FAC
            enddo
            ...
            do b = 1,CS%NumBands
              if (CS%PartitionMode == 0) then
                CMN_FAC = exp(MidPoint * 2._wp * CS%WaveNum_Cen(b))
              else
                CMN_FAC = exp(MidPoint * 2._wp * CS%Freq_Cen(b)**2 / CS%g_Earth)
              endif
              CS%US_x(I,j,k) = CS%US_x(I,j,k) + CS%STKx0(I,j,b)*CMN_FAC
            enddo
          endif
    ...
    do J=G%jscB,G%jecB
      do i=G%isc,G%iec
        ...
        do k = 1,GV%ke
          Top = Bottom
          level_thick = 0.5_wp*(dz(i,j,k)+dz(i,j+1,k))
          MidPoint = Top - 0.5_wp*level_thick
          Bottom = Top - level_thick
    ...
    if (.not.(CS%StaticWaves .and. CS%DHH85_is_set)) then
      do j=G%jsc,G%jec
        do I=G%iscB,G%iecB
          ...
          do k = 1,GV%ke
            Top = Bottom
            MidPoint = Top - 0.25_wp*(dz(i,j,k)+dz(i+1,j,k))
            Bottom = Top - 0.5_wp*(dz(i,j,k)+dz(i+1,j,k))
...
real(wp) function one_minus_exp_x(x)
  real(wp), intent(in) :: x !< The argument of the function ((1 - exp(-x))/x) [nondim]
  real(wp), parameter :: C1_6 = 1.0_wp/6.0_wp  ! A rational fraction [nondim]
  if (abs(x) <= 2.0e-5_wp) then
    ! The Taylor series expression for exp(-x) gives a more accurate expression for 64-bit reals.
    one_minus_exp_x = 1.0_wp - x * (0.5_wp - C1_6*x)
  else
    one_minus_exp_x = (1.0_wp - exp(-x)) / x
  endif
end function one_minus_exp_x

!> Return the value of (1 - exp(-x)) [nondim], using an accurate expression for small values of x.
real(wp) function one_minus_exp(x)
  real(wp), intent(in) :: x !< The argument of the function ((1 - exp(-x))/x) [nondim]
  real(wp), parameter :: C1_6 = 1.0_wp/6.0_wp  ! A rational fraction [nondim]
  if (abs(x) <= 2.0e-5_wp) then
    ! The Taylor series expression for exp(-x) gives a more accurate expression for 64-bit reals.
    one_minus_exp = x * (1.0_wp - x * (0.5_wp - C1_6*x))
  else
    one_minus_exp = 1.0_wp - exp(-x)
  endif
...
subroutine Surface_Bands_by_data_override(Time, G, GV, US, CS)
  ...
  ! Local variables
  real(wp)    :: temp_x(SZI_(G),SZJ_(G)) ! Pseudo-zonal Stokes drift of band at h-points [L T-1 ~> m s-1]
  real(wp)    :: temp_y(SZI_(G),SZJ_(G)) ! Pseudo-meridional Stokes drift of band at h-points [L T-1 ~> m s-1]
  ...
  real(wp) :: PI       ! 3.1415926535... [nondim]
  real(wp) :: avgx, avgy  ! The global averages of temp_x and temp_y [L T-1 ~> m s-1]
  ...
  if (.not.CS%DataOver_initialized) then
    ...
    if (wavenumber_exists) then
      ...
      allocate( CS%Freq_Cen(CS%NUMBANDS), source=0.0_wp )

      ! Reading frequencies
      PI = 4.0_wp*atan(1.0_wp)
      call read_variable(CS%SurfBandFileName, dim_name(1), CS%Freq_Cen, scale=2._wp*PI*US%T_to_s)
  ...
  do b = 1,CS%NumBands
    ...
    if (CS%Homogenize_Surfbands) then
      ...
      do j = G%jsd,G%jed ; do i = G%Isd,G%Ied ; if (G%mask2dT(i,j) > 0.0_wp) then
          temp_y(i,j) = avgy
          temp_x(i,j) = avgx
      endif ; enddo ; enddo
    endif
    ...
    do j = G%jsc,G%jec
      do I = G%IscB,G%IecB
        CS%STKx0(I,j,b) = 0.5_wp * (temp_x(i,j) + temp_x(i+1,j))
      enddo
    enddo
    do J = G%JscB,G%JecB
      do i = G%isc,G%iec
        CS%STKy0(i,J,b) = 0.5_wp * (temp_y(i,j) + temp_y(i,j+1))
      enddo
    enddo
  enddo !Closes b-loop
...
subroutine get_Langmuir_Number( LA, G, GV, US, HBL, ustar, i, j, dz, Waves, &
  ...
  real(wp),                      intent(out) :: LA    !< Langmuir number [nondim]
  type(unit_scale_type),     intent(in)  :: US    !< A dimensional unit scaling type
  real(wp),                      intent(in)  :: HBL   !< (Positive) thickness of boundary layer [Z ~> m]
  real(wp),                      intent(in)  :: ustar !< Friction velocity [Z T-1 ~> m s-1]
  integer,                   intent(in)  :: i     !< Meridional index of h-point
  integer,                   intent(in)  :: j     !< Zonal index of h-point
  real(wp), dimension(SZK_(GV)), intent(in)  :: dz    !< Grid layer thickness [Z ~> m]
...
!Local Variables
  real(wp) :: Top, Bottom, MidPoint  ! Positions within each layer [Z ~> m]
  real(wp) :: Dpt_LASL         ! Averaging depth for Stokes drift [Z ~> m]
  real(wp) :: ShearDirection   ! Shear angular direction from atan2 [radians]
  real(wp) :: WaveDirection    ! Wave angular direction from atan2 [radians]
  real(wp) :: LA_STKx, LA_STKy, LA_STK ! Stokes velocities in [L T-1 ~> m s-1]
  logical :: ContinueLoop, USE_MA
  real(wp), dimension(SZK_(GV)) :: US_H, VS_H ! Profiles of Stokes velocities [L T-1 ~> m s-1]
  real(wp), allocatable :: StkBand_X(:), StkBand_Y(:) ! Stokes drifts by band [L T-1 ~> m s-1]
  integer :: k, BB

  ! Compute averaging depth for Stokes drift (negative)
  Dpt_LASL = -1.0_wp*max(Waves%LA_FracHBL*HBL, Waves%LA_HBL_min)
  ...
  if (Waves%WaveMethod==TESTPROF) then
    do k = 1,GV%ke
      US_H(k) = 0.5_wp*(Waves%US_X(I,j,k)+Waves%US_X(I-1,j,k))
      VS_H(k) = 0.5_wp*(Waves%US_Y(i,J,k)+Waves%US_Y(i,J-1,k))
    enddo
  ...
  elseif (Waves%WaveMethod==SURFBANDS) then
    ...
    do bb = 1,Waves%NumBands
      StkBand_X(bb) = 0.5_wp*(Waves%STKx0(I,j,bb)+Waves%STKx0(I-1,j,bb))
      StkBand_Y(bb) = 0.5_wp*(Waves%STKy0(i,J,bb)+Waves%STKy0(i,J-1,bb))
    enddo
  ...
  if (Use_MA) then
    ...
    LA = LA / sqrt(max(1.e-8_wp, cos( WaveDirection - ShearDirection)))
  endif
...
subroutine get_StokesSL_LiFoxKemper(ustar, hbl, GV, US, CS, UStokes_SL, LA)
  real(wp), intent(in)  :: ustar !< water-side surface friction velocity [Z T-1 ~> m s-1].
  real(wp), intent(in)  :: hbl   !< boundary layer depth [Z ~> m].
  type(verticalGrid_type), intent(in) :: GV !< Ocean vertical grid structure
  type(unit_scale_type),   intent(in) :: US !< A dimensional unit scaling type
  type(wave_parameters_CS), pointer   :: CS  !< Wave parameter Control structure
  real(wp), intent(out) :: UStokes_SL !< Surface layer averaged Stokes drift [L T-1 ~> m s-1]
  real(wp), intent(out) :: LA    !< Langmuir number [nondim]
  ! Local variables
  ! parameters
  real(wp), parameter :: u19p5_to_u10 = 1.075_wp ! ratio of U19.5 to U10 (Holthuijsen, 2007) [nondim]
  real(wp), parameter :: fm_into_fp = 1.296_wp ! ratio of mean frequency to peak frequency for
                   ! Pierson-Moskowitz spectrum (Webb, 2011) [nondim]
  real(wp), parameter :: us_to_u10 = 0.0162_wp ! ratio of surface Stokes drift to U10 [nondim]
  real(wp), parameter :: r_loss = 0.667_wp ! loss ratio of Stokes transport [nondim]
  real(wp) :: UStokes  ! The surface Stokes drift [L T-1 ~> m s-1]
  real(wp) :: hm0      ! The significant wave height [Z ~> m]
  real(wp) :: fm       ! The mean wave frequency [T-1 ~> s-1]
  real(wp) :: fp       ! The peak wave frequency [T-1 ~> s-1]
  real(wp) :: kphil    ! A peak wavenumber in the Phillips spectrum [Z-1 ~> m-1]
  real(wp) :: kstar    ! A rescaled wavenumber? [Z-1 ~> m-1]
  real(wp) :: vstokes  ! The total Stokes transport [Z L T-1 ~> m2 s-1]
  real(wp) :: z0       ! The boundary layer depth [Z ~> m]
  real(wp) :: z0i      ! The inverse of the boundary layer depth [Z-1 ~> m-1]
  real(wp) :: r1, r2, r3, r4  ! Nondimensional ratios [nondim]
  real(wp) :: r5       ! A single expression that combines r2 and r4 [nondim]
  real(wp) :: root_2kz ! The square root of twice the peak wavenumber times the
  ...
  if (ustar > 0.0_wp) then
    ...
    fp = 0.877_wp * (US%L_to_Z*GV%g_Earth) / (2.0_wp * PI * u19p5_to_u10 * u10)
    !
    ! mean frequency
    fm = fm_into_fp * fp
    !
    ...
    if (CS%answer_date < 20230102) then
      ...
      r1 = ( 0.151_wp / kphil * z0i - 0.84_wp ) * &
           ( 1.0_wp - exp(-2.0_wp * kphil * z0) )
      r2 = -( 0.84_wp + 0.0591_wp / kphil * z0i ) * &
           sqrt( 2.0_wp * PI * kphil * z0 ) * &
           erfc( sqrt( 2.0_wp * kphil * z0 ) )
      r3 = ( 0.0632_wp / kstar * z0i + 0.125_wp ) * &
           (1.0_wp - exp(-2.0_wp * kstar * z0) )
      r4 = ( 0.125_wp + 0.0946_wp / kstar * z0i ) * &
           sqrt( 2.0_wp * PI * kstar * z0) * &
           erfc( sqrt( 2.0_wp * kstar * z0 ) )
      UStokes_sl = UStokes * (0.715_wp + r1 + r2 + r3 + r4)
    else
      ! The following is equivalent to the code above, but avoids singularities
      r1 = ( 0.302_wp - 1.68_wp*(kphil*z0) ) * one_minus_exp_x(2.0_wp * (kphil * z0))
      r3 = ( 0.1264_wp + 0.64_wp*(kphil*z0) ) * one_minus_exp_x(5.12_wp * (kphil * z0))

      root_2kz = sqrt(2.0_wp * kphil * z0)
      ...
      if (root_2kz > 1e-3_wp) then
        r5 = sqrt(PI) * (root_2kz * (-0.84_wp * erfc(root_2kz) + 0.2_wp * erfc(1.6_wp*root_2kz)) + &
                         0.1182_wp * (erfc(1.6_wp*root_2kz) - erfc(root_2kz)) / root_2kz)
      else
        ...
        r5 = -0.64_wp*sqrt(PI)*root_2kz + (-0.14184_wp + 1.0839648_wp * root_2kz**2)
      endif
      UStokes_sl = UStokes * (0.715_wp + ((r1 + r3) + r5))
    endif

    if (UStokes_sl /= 0.0_wp) LA = sqrt(US%Z_to_L*ustar / UStokes_sl)
  endif
...
subroutine Get_SL_Average_Prof( GV, AvgDepth, dz, Profile, Average )
  ...
  real(wp), intent(in)  :: AvgDepth !< Depth to average over (negative) [Z ~> m]
  real(wp), dimension(SZK_(GV)), &
  ...
  real(wp), intent(out) :: Average  !< Output quantity averaged over depth AvgDepth [A]
                                !! (used here for Stokes drift)
  !Local variables
  real(wp) :: Top, Bottom ! Depths, negative downward [Z ~> m]
  real(wp) :: Sum  ! The depth weighted vertical sum of a quantity [A Z ~> A m]
  integer :: k
...
subroutine Get_SL_Average_Band( GV, AvgDepth, NB, WaveNumbers, SurfStokes, Average )
  ...
  real(wp), intent(in)    :: AvgDepth    !< Depth to average over [Z ~> m].
  integer, intent(in) :: NB          !< Number of bands used
  real(wp), dimension(NB), &
  ...
  real(wp), intent(out)   :: Average     !< Output average Stokes drift over depth AvgDepth [L T-1 ~> m s-1]
  ...
  do bb = 1, NB
    ...
    Average = Average + SurfStokes(BB) * &
              (1._wp-EXP(-abs(AvgDepth * 2.0_wp * WaveNumbers(BB)))) / &
              abs(AvgDepth * 2.0_wp * WaveNumbers(BB))
...
subroutine DHH85_mid(GV, US, CS, zpt, UStokes)
  ...
  type(unit_scale_type),   intent(in)  :: US  !< A dimensional unit scaling type
  type(wave_parameters_CS), pointer    :: CS  !< Wave parameter Control structure
  real(wp), intent(in)  :: zpt   !< Depth to get Stokes drift [Z ~> m].
  real(wp), intent(out) :: UStokes !< Stokes drift [L T-1 ~> m s-1]
  !
  real(wp) :: ann, Bnn, Snn, Cnn, Dnn ! Nondimensional factors [nondim]
  real(wp) :: omega_peak ! The peak wave frequency [T-1 ~> s-1]
  real(wp) :: omega      ! The average frequency in the band [T-1 ~> s-1]
  real(wp) :: domega     ! The width in frequency of the band [T-1 ~> s-1]
  real(wp) :: u10        ! The wind speed for this spectrum [Z T-1 ~> m s-1]
  real(wp) :: wavespec   ! The wave spectrum [L Z T ~> m2 s]
  real(wp) :: Stokes     ! The Stokes displacement per cycle [L ~> m]
  real(wp) :: PI         ! 3.1415926535... [nondim]
  ...
  !/
  NOmega = 1000
  domega = (CS%omega_max - CS%omega_min) / real(NOmega, wp)
  ...
  if (CS%WaveAgePeakFreq) then
    ...
    PI = 4.0_wp*atan(1.0_wp)
    omega_peak = 2._wp * PI * 0.13_wp * CS%g_Earth / u10
  endif
  !/
  Ann = 0.006_wp * CS%WaveAge**(-0.55_wp)
  Bnn = 1.0_wp
  Snn = 0.08_wp * (1.0_wp + 4.0_wp * CS%WaveAge**3)
  Cnn = 1.7_wp
  if (CS%WaveAge < 1._wp) then
    Cnn = Cnn - 6.0_wp*log10(CS%WaveAge)
  endif
  !/
  UStokes = 0.0_wp
  omega = CS%omega_min + 0.5_wp*domega
  do oi = 1,nomega-1
    Dnn = exp ( -0.5_wp * (omega-omega_peak)**2 / (Snn**2 * omega_peak**2) )
    ...
    Stokes = 2.0_wp * wavespec * omega**3 * &
         exp( 2.0_wp * omega**2 * zpt / CS%g_Earth) / CS%g_Earth
...
subroutine StokesMixing(G, GV, dt, h, dz, u, v, Waves )
  ...
  type(verticalGrid_type), &
       intent(in)    :: GV    !< Ocean vertical grid
  real(wp), intent(in)   :: dt    !< Time step of MOM6 [T ~> s] for explicit solver
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV)), &
  ...
  ! Local variables
  real(wp) :: dTauUp, dTauDn ! Vertical momentum fluxes [H L T-2 ~> m2 s-2 or Pa]
  real(wp) :: h_lay  ! The layer thickness at a velocity point [H ~> m or kg m-2]
  real(wp) :: dz_lay ! The distance between interfaces at a velocity point [Z ~> m]
  integer :: i, j, k
...
! This is not ready for use...
  ...
  do k = 1, GV%ke
    do j = G%jsc, G%jec
      do I = G%iscB, G%iecB
        h_lay = 0.5_wp*(h(i,j,k)+h(i+1,j,k))
        dz_lay = 0.5_wp*(dz(i,j,k)+dz(i+1,j,k))
        dTauUp = 0.0_wp
        if (k > 1) &
          dTauUp = (0.5_wp*(waves%Kvs(i,j,k)+waves%Kvs(i+1,j,k))) * &
               (waves%us_x(i,j,k-1)-waves%us_x(i,j,k)) / &
               (0.5_wp*(dz_lay + 0.5_wp*(dz(i,j,k-1)+dz(i+1,j,k-1)) ))
        dTauDn = 0.0_wp
        if (k < GV%ke-1) &
          dTauDn = (0.5_wp*(waves%Kvs(i,j,k+1)+waves%Kvs(i+1,j,k+1))) * &
  ...
  do k = 1, GV%ke
    do J = G%jscB, G%jecB
      do i = G%isc, G%iec
        h_lay = 0.5_wp*(h(i,j,k)+h(i,j+1,k))
        dz_lay = 0.5_wp*(dz(i,j,k)+dz(i,j+1,k))
        dTauUp = 0._wp
        if (k > 1) &
          dTauUp = (0.5_wp*(waves%Kvs(i,j,k)+waves%Kvs(i,j+1,k))) * &
               (waves%us_y(i,j,k-1)-waves%us_y(i,j,k)) / &
               (0.5_wp*(dz_lay + 0.5_wp*(dz(i,j,k-1)+dz(i,j+1,k-1)) ))
        dTauDn = 0.0_wp
        if (k < GV%ke-1) &
          dTauDn = (0.5_wp*(waves%Kvs(i,j,k+1)+waves%Kvs(i,j+1,k+1))) * &
...
subroutine CoriolisStokes(G, GV, dt, h, u, v, Waves)
  ...
  type(verticalGrid_type), &
       intent(in)   :: GV     !< Ocean vertical grid
  real(wp), intent(in)  :: dt     !< Time step of MOM6 [T ~> s]
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV)), &
       intent(in)    :: h     !< Layer thicknesses [H ~> m or kg m-2]
  real(wp), dimension(SZIB_(G),SZJ_(G),SZK_(GV)), &
       intent(inout) :: u     !< Velocity i-component [L T-1 ~> m s-1]
  ...
  ! Local variables
  real(wp) :: DVel ! A rescaled velocity change [L T-2 ~> m s-2]
  integer :: i, j, k

  do k = 1, GV%ke
    do j = G%jsc, G%jec
      do I = G%iscB, G%iecB
        DVel = 0.25_wp*((Waves%us_y(i,J-1,k)+Waves%us_y(i+1,J-1,k)) * G%CoriolisBu(I,J-1)) + &
               0.25_wp*((Waves%us_y(i,J,k)+Waves%us_y(i+1,J,k)) * G%CoriolisBu(I,J))
  ...
  do k = 1, GV%ke
    do J = G%jscB, G%jecB
      do i = G%isc, G%iec
        DVel = 0.25_wp*((Waves%us_x(I-1,j,k)+Waves%us_x(I-1,j+1,k)) * G%CoriolisBu(I-1,j)) + &
               0.25_wp*((Waves%us_x(I,j,k)+Waves%us_x(I,j+1,k)) * G%CoriolisBu(I,J))
...
subroutine Stokes_PGF(G, GV, US, dz, u, v, PFu_Stokes, PFv_Stokes, CS )
  ...
  ! Local variables
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(G)) :: P_deltaStokes_L ! The Stokes induced pressure anomaly,
                                                              ! layer averaged [L2 T-2 ~> m2 s-2]
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(G)+1) :: P_deltaStokes_i ! The Stokes induced pressure anomaly
                                                              ! at interfaces [L2 T-2 ~> m2 s-2]
  real(wp) :: P_Stokes_l, P_Stokes_r ! Stokes-induced pressure anomaly over layer (left/right of point) [L2 T-2 ~> m2 s-2]
  real(wp) :: P_Stokes_l0, P_Stokes_r0 ! Stokes-induced pressure anomaly at interface
                                   ! (left/right of point) [L2 T-2 ~> m2 s-2]
  real(wp) :: dP_Stokes_l_dz, dP_Stokes_r_dz ! Contribution of layer to integrated Stokes pressure anomaly for summation
                                         ! (left/right of point) [Z L2 T-2 ~> m3 s-2]
  real(wp) :: dP_lay_Stokes_l, dP_lay_Stokes_r ! Contribution of layer to integrated Stokes pressure anomaly for summation
                                         ! (left/right of point) [L2 T-2 ~> m2 s-2]
  real(wp) :: dP_Stokes_l, dP_Stokes_r ! Net increment of Stokes pressure anomaly across layer for summation
                                   ! (left/right of point) [L2 T-2 ~> m2 s-2]
  real(wp) :: uE_l, uE_r, vE_l, vE_r ! Eulerian velocity components (left/right of point) [L T-1 ~> m s-1]
  real(wp) :: uS0_l, uS0_r, vS0_l, vS0_r ! Surface Stokes velocity components (left/right of point) [L T-1 ~> m s-1]
  real(wp) :: zi_l(SZK_(G)+1), zi_r(SZK_(G)+1)   ! The height of the edges of the cells (left/right of point) [Z ~> m].
  real(wp) :: idz_l(SZK_(G)), idz_r(SZK_(G)) ! The inverse thickness of the cells (left/right of point) [Z-1 ~> m-1]
  real(wp) :: h_l, h_r   ! The thickness of the cell (left/right of point) [Z ~> m].
  real(wp) :: exp_top    ! The decay of the surface stokes drift to the interface atop a layer [nondim]
  real(wp) :: dexp2kzL, dexp4kzL, dexp2kzR, dexp4kzR ! Analytical evaluation of multi-exponential decay
                                              ! contribution to Stokes pressure anomalies [nondim].
  real(wp) :: TwoK, FourK   ! Wavenumbers multiplied by a factor [Z-1 ~> m-1]
  real(wp) :: iTwoK, iFourK ! Inverses of wavenumbers [Z ~> m]
  ...
  PFu_Stokes(:,:,:) = 0.0_wp
  PFv_Stokes(:,:,:) = 0.0_wp
  if (CS%id_P_deltaStokes_i > 0) P_deltaStokes_i(:,:,:) = 0.0_wp
  if (CS%id_P_deltaStokes_L > 0) P_deltaStokes_L(:,:,:) = 0.0_wp
    ...
    if (G%mask2dCu(I,j)>0.5_wp) then
      ...
      do k = 1, G%ke
        ...
        if (.not.CS%robust_Stokes_PGF) then
          ...
          Idz_l(k) = 1._wp/max(0.1_wp*US%m_to_Z, h_l)
          Idz_r(k) = 1._wp/max(0.1_wp*US%m_to_Z, h_r)
        endif
      enddo
      do k = 1,G%ke
        ...
        uE_l = 0.5_wp*((u(I-1,j,k)-CS%Us_x(I-1,j,k))*G%mask2dCu(I-1,j) + &
                    (u(I,j,k)-CS%Us_x(I-1,j,k))*G%mask2dCu(I,j))
        uE_r = 0.5_wp*((u(I,j,k)-CS%Us_x(I,j,k))*G%mask2dCu(I,j) + &
                    (u(I+1,j,k)-CS%Us_x(I+1,j,k))*G%mask2dCu(I+1,j))
        vE_l = 0.5_wp*((v(i,J-1,k)-CS%Us_y(i,J-1,k))*G%mask2dCv(i,J-1) + &
                    (v(i,J,k)-CS%Us_y(i,J,k))*G%mask2dCv(i,J))
        vE_r = 0.5_wp*((v(i+1,J-1,k)-CS%Us_y(i+1,J-1,k))*G%mask2dCv(i+1,J-1) + &
                    (v(i+1,J,k)-CS%Us_y(i+1,J,k))*G%mask2dCv(i+1,J))

        dP_Stokes_l_dz = 0.0_wp
        dP_Stokes_r_dz = 0.0_wp
        dP_Stokes_l = 0.0_wp
        dP_Stokes_r = 0.0_wp
        ...
        do l = 1, CS%numbands
          ...
          uS0_l = 0.5_wp*(CS%Stkx0(I-1,j,l)*G%mask2dCu(I-1,j) + &
                       CS%Stkx0(I,j,l)*G%mask2dCu(I,j))
          uS0_r = 0.5_wp*(CS%Stkx0(I,j,l)*G%mask2dCu(I,j) + &
                       CS%Stkx0(I+1,j,l)*G%mask2dCu(I+1,j))
          vS0_l = 0.5_wp*(CS%Stky0(i,J-1,l)*G%mask2dCv(i,J-1) + &
                       CS%Stky0(i,J,l)*G%mask2dCv(i,J))
          vS0_r = 0.5_wp*(CS%Stky0(i+1,J-1,l)*G%mask2dCv(i+1,J-1) + &
                       CS%Stky0(i+1,J,l)*G%mask2dCv(i+1,J))

          ! Wavenumber terms that are useful to simplify the pressure calculations
          TwoK = 2._wp*CS%WaveNum_Cen(l)
          FourK = 2._wp*TwoK
          if (.not.CS%robust_Stokes_PGF) then
            iTwoK = 1._wp / TwoK
            iFourK = 1._wp / FourK
          ...
          if (G%mask2dT(i,j)>0.5_wp) then
            if (.not.CS%robust_Stokes_PGF) then
              ...
              dP_Stokes_l_dz = dP_Stokes_l_dz + &
                               ((uE_l*uS0_l+vE_l*vS0_l)*iTwoK*dexp2kzL + 0.5_wp*(uS0_l*uS0_l+vS0_l*vS0_l)*iFourK*dexp4kzL)
              dP_Stokes_l = dP_Stokes_l + (uE_l*uS0_l+vE_l*vS0_l)*dexp2kzL + 0.5_wp*(uS0_l*uS0_l+vS0_l*vS0_l)*dexp4kzL
              ...
              dP_lay_Stokes_l = dP_lay_Stokes_l + &
                  ((((uE_l*uS0_l)+(vE_l*vS0_l)) * exp_top) * one_minus_exp_x(TwoK*dz(i,j,k)) + &
                   (0.5_wp*((uS0_l**2)+(vS0_l**2)) * exp_top**2) * one_minus_exp_x(FourK*dz(i,j,k)) )
              dP_Stokes_l = dP_Stokes_l + &
                  ((((uE_l*uS0_l)+(vE_l*vS0_l)) * exp_top) * one_minus_exp(TwoK*dz(i,j,k)) + &
                   (0.5_wp*((uS0_l**2)+(vS0_l**2)) * exp_top**2) * one_minus_exp(FourK*dz(i,j,k)) )
            endif
          endif
          if (G%mask2dT(i+1,j)>0.5_wp) then
            if (.not.CS%robust_Stokes_PGF) then
              dexp2kzR = exp(TwoK*zi_r(k))-exp(TwoK*zi_r(k+1))
              dexp4kzR = exp(FourK*zi_r(k))-exp(FourK*zi_r(k+1))
              dP_Stokes_r_dz = dP_Stokes_r_dz + &
                               ((uE_r*uS0_r+vE_r*vS0_r)*iTwoK*dexp2kzR + 0.5_wp*(uS0_l*uS0_l+vS0_l*vS0_l)*iFourK*dexp4kzR)
              dP_Stokes_r = dP_Stokes_r + (uE_r*uS0_r+vE_r*vS0_r)*dexp2kzR + 0.5_wp*(uS0_l*uS0_l+vS0_l*vS0_l)*dexp4kzR
            else  ! These expressions are equivalent to those above for thick layers, but more accurate for thin layers.
              exp_top = exp(TwoK*zi_r(k))
              dP_lay_Stokes_r = dP_lay_Stokes_r + &
                  ((((uE_r*uS0_r)+(vE_r*vS0_r)) * exp_top) * one_minus_exp_x(TwoK*dz(i+1,j,k)) + &
                   (0.5_wp*((uS0_r**2)+(vS0_r**2)) * exp_top**2) * one_minus_exp_x(FourK*dz(i+1,j,k)) )
              dP_Stokes_r = dP_Stokes_r + &
                  ((((uE_r*uS0_r)+(vE_r*vS0_r)) * exp_top) * one_minus_exp(TwoK*dz(i+1,j,k)) + &
                   (0.5_wp*((uS0_r**2)+(vS0_r**2)) * exp_top**2) * one_minus_exp(FourK*dz(i+1,j,k)) )
            endif
          endif
        enddo
        ...
        if (G%mask2dT(i,j)>0.5_wp .and. CS%id_P_deltaStokes_L > 0) P_deltaStokes_L(i,j,k) = P_Stokes_l
        if (G%mask2dT(i,j)>0.5_wp .and. CS%id_P_deltaStokes_i > 0) P_deltaStokes_i(i,j,k+1) = P_Stokes_l0
  ...
  do J = G%jscB, G%jecB ; do i = G%isc, G%iec
    if (G%mask2dCv(i,J)>0.5_wp) then
      ...
      do k = 1,G%ke
        ...
        uE_r = 0.5_wp*((u(I-1,j+1,k)-CS%Us_x(I-1,j+1,k))*G%mask2dCu(I-1,j+1) + &
                    (u(I,j+1,k)-CS%Us_x(I,j+1,k))*G%mask2dCu(I,j+1))
        vE_l = 0.5_wp*((v(i,J-1,k)-CS%Us_y(i,J-1,k))*G%mask2dCv(i,J-1) + &
                    (v(i,J,k)-CS%Us_y(i,J,k))*G%mask2dCv(i,J))
        vE_r = 0.5_wp*((v(i,J,k)-CS%Us_y(i,J,k))*G%mask2dCv(i,J) + &
        ...
        do l = 1, CS%numbands
          ...
          uS0_l = 0.5_wp*(CS%Stkx0(I-1,j,l)*G%mask2dCu(I-1,j) + &
                       CS%Stkx0(I,j,l)*G%mask2dCu(I,j))
          uS0_r = 0.5_wp*(CS%Stkx0(I-1,j+1,l)*G%mask2dCu(I-1,j+1) + &
                       CS%Stkx0(I,j+1,l)*G%mask2dCu(I,j+1))
          vS0_l = 0.5_wp*(CS%Stky0(i,J-1,l)*G%mask2dCv(i,J-1) + &
                       CS%Stky0(i,J,l)*G%mask2dCv(i,J))
          vS0_r = 0.5_wp*(CS%Stky0(i,J,l)*G%mask2dCv(i,J) + &
          ...
          if (G%mask2dT(i,j+1)>0.5_wp) then
            if (.not.CS%robust_Stokes_PGF) then
              dexp2kzR = exp(TwoK*zi_r(k))-exp(TwoK*zi_r(k+1))
              dexp4kzR = exp(FourK*zi_r(k))-exp(FourK*zi_r(k+1))
              dP_Stokes_r_dz = dP_Stokes_r_dz + &
              ...
              dP_lay_Stokes_r = dP_lay_Stokes_r + &
                  ((((uE_r*uS0_r)+(vE_r*vS0_r)) * exp_top) * one_minus_exp_x(TwoK*dz(i,j+1,k)) + &
                   (0.5_wp*((uS0_r**2)+(vS0_r**2)) * exp_top**2) * one_minus_exp_x(FourK*dz(i,j+1,k)) )
              dP_Stokes_r = dP_Stokes_r + &
                  ((((uE_r*uS0_r)+(vE_r*vS0_r)) * exp_top) * one_minus_exp(TwoK*dz(i,j+1,k)) + &
                   (0.5_wp*((uS0_r**2)+(vS0_r**2)) * exp_top**2) * one_minus_exp(FourK*dz(i,j+1,k)) )
            endif
          endif
        enddo
...
subroutine ust_2_u10_coare3p5(USTair, U10, GV, US, CS)
  real(wp), intent(in)                    :: USTair !< Wind friction velocity [Z T-1 ~> m s-1]
  real(wp), intent(out)                   :: U10    !< 10-m neutral wind speed [L T-1 ~> m s-1]
  type(verticalGrid_type), intent(in) :: GV     !< vertical grid type
  type(unit_scale_type),   intent(in) :: US     !< A dimensional unit scaling type
  type(wave_parameters_CS), pointer   :: CS     !< Wave parameter Control structure

  ! Local variables
  real(wp) :: z0sm, z0, z0rough  ! Roughness lengths [Z ~> m]
  real(wp) :: ten_m_scale ! The 10 m reference height, in rescaled units [Z ~> m]
  real(wp) :: I_ten_m_scale ! The inverse of the 10 m reference height, in rescaled units [Z-1 ~> m-1]
  real(wp) :: u10a  ! The previous guess for u10 [L T-1 ~> m s-1]
  real(wp) :: alpha ! The Charnock coeffient relating the wind friction velocity squared to the
                ! roughness length [nondim]
  real(wp) :: Cd    ! The drag coefficient [nondim]
  real(wp) :: I_sqrtCd  ! The inverse of the square root of the drag coefficient [nondim]
  real(wp) :: I_vonKar  ! The inverse of the von Karman coefficient [nondim]
  integer :: CT
  ...
  if (CS%vonKar < 0.0_wp) call MOM_error(FATAL, &
    "ust_2_u10_coare3p5 called with a negative value of Waves%vonKar")

  z0sm = 0.11_wp * CS%nu_air / USTair ! Compute z0smooth from ustar guess
  u10a = 1000.0_wp*US%m_s_to_L_T ! An insanely large upper bound for u10.

  if (CS%answer_date < 20230103) then
    u10 = US%Z_to_L*USTair / sqrt(0.001_wp)  ! Guess for u10
    ten_m_scale = 10.0_wp*US%m_to_Z
    CT=0
    do while (abs(u10a/u10 - 1._wp) > 0.001_wp)
      CT=CT+1
      u10a = u10
      ...
      if (CT>20) then
        u10 = US%Z_to_L*USTair/sqrt(0.0015_wp) ! I don't expect to get here, but just
    ...
    u10 = US%Z_to_L*USTair * sqrt(1000.0_wp) ! First guess for u10.
    ! In the line above 1000 is the inverse of a plausible first guess of the drag coefficient.
    I_vonKar = 1.0_wp / CS%vonKar
    I_ten_m_scale = 0.1_wp*US%Z_to_m

    do CT=1,20
      if (abs(u10a - u10) <= 0.001_wp*u10) exit   ! Check for convergence.
      u10a = u10
      alpha = min(CS%Charnock_min, CS%Charnock_slope_U10 * u10 + CS%Charnock_intercept)
    ...
    if (abs(u10a - u10) > 0.001_wp*u10) u10 = US%Z_to_L*USTair * 25.82_wp
  endif
...
subroutine waves_register_restarts(CS, HI, GV, US, param_file, restart_CSp)
  ...
  if (time_tendency_term) then
    ...
    allocate(CS%Us_x_prev(HI%isdB:HI%IedB,HI%jsd:HI%jed,GV%ke), source=0.0_wp)
    allocate(CS%Us_y_prev(HI%isd:HI%Ied,HI%jsdB:HI%jedB,GV%ke), source=0.0_wp)
