...
type, public :: marine_ice_CS ; private
  real(wp) :: kv_iceberg          !< The viscosity of the icebergs [L4 Z-2 T-1 ~> m2 s-1] (for ice rigidity)
  real(wp) :: berg_area_threshold !< Fraction of grid cell which iceberg must occupy
  ...
  real(wp) :: density_iceberg     !< A typical density of icebergs [R ~> kg m-3] (for ice rigidity)
...
subroutine iceberg_forces(G, forces, use_ice_shelf, sfc_state, time_step, CS)
  ...
  real(wp),                  intent(in)    :: time_step  !< The coupling time step [T ~> s].
  type(marine_ice_CS),   pointer       :: CS      !< Pointer to the control structure for MOM_marine_ice

  real(wp) :: kv_rho_ice ! The viscosity of ice divided by its density [L4 Z-2 T-1 R-1 ~> m5 kg-1 s-1].
  ...
  if (.not. use_ice_shelf) then
    forces%frac_shelf_u(:,:) = 0.0_wp ; forces%frac_shelf_v(:,:) = 0.0_wp
  endif
  if (.not. forces%accumulate_rigidity) then
    forces%rigidity_ice_u(:,:) = 0.0_wp ; forces%rigidity_ice_v(:,:) = 0.0_wp
  endif
  ...
  do j=js,je ; do I=is-1,ie
    if ((G%areaT(i,j) + G%areaT(i+1,j) > 0.0_wp)) & ! .and. (G%dxdy_u(I,j) > 0.0)) &
  ...
  do J=js-1,je ; do i=is,ie
    if ((G%areaT(i,j) + G%areaT(i,j+1) > 0.0_wp)) & ! .and. (G%dxdy_v(i,J) > 0.0)) &
...
subroutine iceberg_fluxes(G, US, fluxes, use_ice_shelf, sfc_state, time_step, CS)
  ...
  real(wp) :: fraz      ! refreezing rate [R Z T-1 ~> kg m-2 s-1]
  ...
  if (.not.(associated(fluxes%area_berg) .and. associated(fluxes%ustar_berg) .and. &
            associated(fluxes%mass_berg) ) ) return
  if (.not. use_ice_shelf) then
    fluxes%frac_shelf_h(:,:) = 0._wp
    fluxes%ustar_shelf(:,:) = 0._wp
  endif
  do j=jsd,jed ; do i=isd,ied ; if (G%areaT(i,j) > 0.0_wp) then
  ...
  if (CS%berg_area_threshold >= 0._wp) then
    I_dt_LHF = 1.0_wp / (time_step * CS%latent_heat_fusion)
    do j=jsd,jed ; do i=isd,ied
      if (fluxes%frac_shelf_h(i,j) > CS%berg_area_threshold) then
        ! Only applying for ice shelf covering most of cell.

        if (associated(fluxes%sw)) fluxes%sw(i,j) = 0.0_wp
        if (associated(fluxes%lw)) fluxes%lw(i,j) = 0.0_wp
        if (associated(fluxes%latent)) fluxes%latent(i,j) = 0.0_wp
        if (associated(fluxes%evap)) fluxes%evap(i,j) = 0.0_wp
...
# include "version_variable.h"
  ...
  call get_param(param_file, mdl, "KV_ICEBERG",  CS%kv_iceberg, &
                 ...
                 units="m2 s-1", default=1.0e10_wp, scale=G%US%Z_to_L**2*G%US%m_to_L**2*G%US%T_to_s)
  call get_param(param_file, mdl, "DENSITY_ICEBERGS",  CS%density_iceberg, &
                 "A typical density of icebergs.", units="kg m-3", default=917.0_wp, scale=G%US%kg_m3_to_R)
