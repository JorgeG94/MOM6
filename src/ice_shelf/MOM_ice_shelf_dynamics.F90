...
type, public :: ice_shelf_dyn_CS ; private
  real(wp), pointer, dimension(:,:) :: u_shelf => NULL() !< the zonal velocity of the ice shelf/sheet
                                       !! on q-points (B grid) [L T-1 ~> m s-1]
  real(wp), pointer, dimension(:,:) :: v_shelf => NULL() !< the meridional velocity of the ice shelf/sheet
                                       !! on q-points (B grid) [L T-1 ~> m s-1]
  real(wp), pointer, dimension(:,:) :: taudx_shelf => NULL() !< the zonal driving stress of the ice shelf/sheet
                                       !! on q-points (C grid) [R L2 T-2 ~> Pa]
  real(wp), pointer, dimension(:,:) :: taudy_shelf => NULL() !< the meridional driving stress of the ice shelf/sheet
                                       !! on q-points (C grid) [R L2 T-2 ~> Pa]
  real(wp), pointer, dimension(:,:) :: sx_shelf => NULL() !< the zonal surface slope of the ice shelf/sheet
                                       !! on q-points (B grid) [nondim]
  real(wp), pointer, dimension(:,:) :: sy_shelf => NULL() !< the meridional surface slope of the ice shelf/sheet
                                       !! on q-points (B grid) [nondim]
  real(wp), pointer, dimension(:,:) :: u_face_mask => NULL() !< mask for velocity boundary conditions on the C-grid
  ...
  real(wp), pointer, dimension(:,:) :: v_face_mask => NULL()  !< A mask for velocity boundary conditions on the C-grid
                                       !! v-face, with valued defined similarly to u_face_mask, but 5 is Dirichlet for v
                                       !! and 6 is Dirichlet for u
  real(wp), pointer, dimension(:,:) :: u_face_mask_bdry => NULL() !< A duplicate copy of u_face_mask?
  real(wp), pointer, dimension(:,:) :: v_face_mask_bdry => NULL() !< A duplicate copy of v_face_mask?
  real(wp), pointer, dimension(:,:) :: u_flux_bdry_val => NULL() !< The ice volume flux per unit face length into the cell
                                       !! through open boundary u-faces (where u_face_mask=4) [Z L T-1 ~> m2 s-1]
  real(wp), pointer, dimension(:,:) :: v_flux_bdry_val => NULL() !< The ice volume flux per unit face length into the cell
  ...
  real(wp), pointer, dimension(:,:) :: umask => NULL()      !< u-mask on the actual degrees of freedom (B grid)
                                       !! 1=normal node, 3=inhomogeneous boundary node,
                                       !!  0 - no flow node (will also get ice-free nodes)
  real(wp), pointer, dimension(:,:) :: vmask => NULL()      !< v-mask on the actual degrees of freedom (B grid)
                                       !! 1=normal node, 3=inhomogeneous boundary node,
                                       !!  0 - no flow node (will also get ice-free nodes)
  real(wp), pointer, dimension(:,:) :: calve_mask => NULL() !< a mask to prevent the ice shelf front from
                                          !! advancing past its initial position (but it may retreat)
  real(wp), pointer, dimension(:,:) :: t_shelf => NULL() !< Vertically integrated temperature in the ice shelf/stream,
                                                     !! on corner-points (B grid) [C ~> degC]
  real(wp), pointer, dimension(:,:) :: tmask => NULL()   !< A mask on tracer points that is 1 where there is ice.
  real(wp), pointer, dimension(:,:,:) :: ice_visc => NULL() !< Area and depth-integrated Glen's law ice viscosity
                                                        !!  (Pa m3 s) in [R L4 Z T-1 ~> kg m2 s-1].
                                                        !!  at either 1 (cell-centered) or 4 quadrature points per cell
  real(wp), pointer, dimension(:,:) :: AGlen_visc => NULL() !< Ice-stiffness parameter in Glen's law ice viscosity,
                                                      !! often in [Pa-3 s-1] if n_Glen is 3.
  real(wp), pointer, dimension(:,:) :: u_bdry_val => NULL() !< The zonal ice velocity at inflowing boundaries
                                       !! [L yr-1 ~> m yr-1]
  real(wp), pointer, dimension(:,:) :: v_bdry_val => NULL() !< The meridional ice velocity at inflowing boundaries
                                       !! [L yr-1 ~> m yr-1]
  real(wp), pointer, dimension(:,:) :: h_bdry_val => NULL() !< The ice thickness at inflowing boundaries [Z ~> m].
  real(wp), pointer, dimension(:,:) :: t_bdry_val => NULL() !< The ice temperature at inflowing boundaries [C ~> degC].

  real(wp), pointer, dimension(:,:) :: bed_elev => NULL()  !< The bed elevation used for ice dynamics [Z ~> m],
  ...
  real(wp), pointer, dimension(:,:) :: basal_traction => NULL() !< The area-integrated taub_beta field
  ...
  real(wp), pointer, dimension(:,:) :: C_basal_friction => NULL()!< Coefficient in sliding law tau_b = C u^(n_basal_fric),
                               !! units of [R L Z T-2 (s m-1)^(n_basal_fric) ~> Pa (s m-1)^(n_basal_fric)]
  real(wp), pointer, dimension(:,:) :: OD_rt => NULL()         !< A running total for calculating OD_av [Z ~> m].
  real(wp), pointer, dimension(:,:) :: ground_frac_rt => NULL() !< A running total for calculating ground_frac.
  real(wp), pointer, dimension(:,:) :: OD_av => NULL()         !< The time average open ocean depth [Z ~> m].
  real(wp), pointer, dimension(:,:) :: ground_frac => NULL()   !< Fraction of the time a cell is "exposed", i.e. the column
                               !! thickness is below a threshold and interacting with the rock [nondim].  When this
                               !! is 1, the ice-shelf is grounded
  real(wp), pointer, dimension(:,:) :: float_cond => NULL()   !< If GL_regularize=true, indicates cells containing
                                                !! the grounding line (float_cond=1) or not (float_cond=0)
  real(wp), pointer, dimension(:,:,:,:) :: Phi => NULL() !< The gradients of bilinear basis elements at Gaussian
                                                !! 4 quadrature points surrounding the cell vertices [L-1 ~> m-1].
  real(wp), pointer, dimension(:,:,:) :: PhiC => NULL()  !< The gradients of bilinear basis elements at 1 cell-centered
                                                !! quadrature point per cell [L-1 ~> m-1].
  real(wp), pointer, dimension(:,:,:,:,:,:) :: Phisub => NULL() !< Quadrature structure weights at subgridscale
  ...
  real(wp) :: velocity_update_time_step !< The time interval over which to update the ice shelf velocity
  ...
  real(wp) :: elapsed_velocity_time  !< The elapsed time since the ice velocities were last updated [T ~> s].
  ...
  real(wp)    :: first_dir_restart_IS = -1.0_wp !< A real copy of CS%first_direction_IS for use in restart files
  ...
  real(wp)    :: CFL_factor     !< A factor used to limit subcycled advective timestep in uncoupled runs
                            !! i.e. dt <= CFL_factor * min(dx / u) [nondim]

  real(wp) :: min_h_shelf !< The minimum ice thickness used during ice dynamics [Z ~> m].
  real(wp) :: min_basal_traction !< The minimum basal traction for grounded ice (Pa m-1 s) [R Z T-1 ~> kg m-2 s-1]
  real(wp) :: max_surface_slope !< The maximum allowed ice-sheet surface slope (to ignore, set to zero) [nondim]
  real(wp) :: min_ice_visc !< The minimum allowed Glen's law ice viscosity (Pa s), in [R L2 T-1 ~> kg m-1 s-1].

  real(wp) :: n_glen            !< Nonlinearity exponent in Glen's Law [nondim]
  real(wp) :: eps_glen_min      !< Min. strain rate to avoid infinite Glen's law viscosity, [T-1 ~> s-1].
  real(wp) :: n_basal_fric      !< Exponent in sliding law tau_b = C u^(m_slide) [nondim]
  logical :: CoulombFriction !< Use Coulomb friction law (Schoof 2005, Gagliardini et al 2007)
  real(wp) :: CF_MinN           !< Minimum Coulomb friction effective pressure [R Z L T-2 ~> Pa]
  real(wp) :: CF_PostPeak       !< Coulomb friction post peak exponent [nondim]
  real(wp) :: CF_Max            !< Coulomb friction maximum coefficient [nondim]
  real(wp) :: density_ocean_avg !< A typical ocean density [R ~> kg m-3].  This does not affect ocean
  ...
  real(wp) :: thresh_float_col_depth !< The water column depth over which the shelf if considered to be floating
  ...
  real(wp) :: T_shelf_missing   !< An ice shelf temperature to use where there is no ice shelf [C ~> degC]
  real(wp) :: cg_tolerance !< The tolerance in the CG solver, relative to initial residual, that
                       !! determines when to stop the conjugate gradient iterations [nondim].
  real(wp) :: nonlinear_tolerance !< The fractional nonlinear tolerance, relative to the initial error,
  ...
  real(wp)    :: timeunit           !< The length of the units for the time axis and certain input parameters
...
function slope_limiter(num, denom)
  real(wp), intent(in)    :: num   !< The numerator of the ratio used in the Van Leer slope limiter
  real(wp), intent(in)    :: denom !< The denominator of the ratio used in the Van Leer slope limiter
  real(wp) :: slope_limiter ! The slope limiter value, between 0 and 2 [nondim].
  real(wp) :: r  ! The ratio of num/denom [nondim]
...
function quad_area (X, Y)
  real(wp), dimension(4), intent(in) :: X !< The x-positions of the vertices of the quadrilateral [L ~> m].
  real(wp), dimension(4), intent(in) :: Y !< The y-positions of the vertices of the quadrilateral [L ~> m].
  real(wp) :: quad_area ! Computed area [L2 ~> m2]
  real(wp) :: p2, q2, a2, c2, b2, d2
...
! X and Y must be passed in the form
  ...
  quad_area = .25_wp * sqrt(4*P2*Q2-(B2+D2-A2-C2)**2)
...
subroutine register_ice_shelf_dyn_restarts(G, US, param_file, CS, restart_CS)
  ...
  if (active_shelf_dynamics) then
    ...
    allocate(CS%u_shelf(IsdB:IedB,JsdB:JedB), source=0.0_wp)
    allocate(CS%v_shelf(IsdB:IedB,JsdB:JedB), source=0.0_wp)
    allocate(CS%t_shelf(isd:ied,jsd:jed), source=T_shelf_missing) ! [C ~> degC]
    allocate(CS%ice_visc(isd:ied,jsd:jed,CS%visc_qps), source=0.0_wp)
    allocate(CS%AGlen_visc(isd:ied,jsd:jed), source=2.261e-25_wp) ! [Pa-3 s-1]
    allocate(CS%basal_traction(isd:ied,jsd:jed), source=0.0_wp)   ! [R Z L2 T-1 ~> kg s-1]
    allocate(CS%C_basal_friction(isd:ied,jsd:jed), source=5.0e10_wp*US%Pa_to_RLZ_T2)
             ! Units of [R L Z T-2 (s m-1)^n_sliding ~> Pa (s m-1)^n_sliding]
    allocate(CS%OD_av(isd:ied,jsd:jed), source=0.0_wp)
    allocate(CS%ground_frac(isd:ied,jsd:jed), source=0.0_wp)
    allocate(CS%taudx_shelf(IsdB:IedB,JsdB:JedB), source=0.0_wp)
    allocate(CS%taudy_shelf(IsdB:IedB,JsdB:JedB), source=0.0_wp)
    allocate(CS%sx_shelf(isd:ied,jsd:jed), source=0.0_wp)
    allocate(CS%sy_shelf(isd:ied,jsd:jed), source=0.0_wp)
    allocate(CS%bed_elev(isd:ied,jsd:jed), source=0.0_wp)
    allocate(CS%u_bdry_val(IsdB:IedB,JsdB:JedB), source=0.0_wp)
    allocate(CS%v_bdry_val(IsdB:IedB,JsdB:JedB), source=0.0_wp)
    allocate(CS%u_face_mask_bdry(IsdB:IedB,JsdB:JedB), source=-2.0_wp)
    allocate(CS%v_face_mask_bdry(IsdB:iedB,JsdB:JedB), source=-2.0_wp)
    allocate(CS%h_bdry_val(isd:ied,jsd:jed), source=0.0_wp)
...
subroutine initialize_ice_shelf_dyn(param_file, Time, ISS, CS, G, US, diag, new_sim, Cp_ice, &
  ...
  real(wp),                    intent(in)    :: Cp_ice !< Heat capacity of ice [Q C-1 ~> J kg-1 degC-1]
  ...
  ! Local variables
  real(wp)    :: T_shelf_bdry ! A default ice shelf temperature to use for ice flowing
...
# include "version_variable.h"
  ...
  if (shelf_mass_is_dynamic) then
    ...
    call get_param(param_file, mdl, "ICE_SHELF_CFL_FACTOR", CS%CFL_factor, &
                 ...
                 "This is only used with an ice-only model.", units="nondim", default=0.25_wp)
  endif
  call get_param(param_file, mdl, "RHO_0", CS%density_ocean_avg, &
                 "avg ocean density used in floatation cond", &
                 units="kg m-3", default=1035._wp, scale=US%kg_m3_to_R)
  if (active_shelf_dynamics) then
    ...
    call get_param(param_file, mdl, "MIN_H_SHELF", CS%min_h_shelf, &
                 "min. ice thickness used during ice dynamics", &
                  units="m", default=0._wp,scale=US%m_to_Z)
    call get_param(param_file, mdl, "MIN_BASAL_TRACTION", CS%min_basal_traction, &
                 ...
                 units="Pa m-1 yr", default=0._wp, scale=365.0_wp*86400.0_wp*US%Pa_to_RLZ_T2*US%L_T_to_m_s)
    ...
    call get_param(param_file, mdl, "CONJUGATE_GRADIENT_TOLERANCE", CS%cg_tolerance, &
                "tolerance in CG solver, relative to initial residual", units="nondim", default=1.e-6_wp)
    call get_param(param_file, mdl, "ICE_NONLINEAR_TOLERANCE", CS%nonlinear_tolerance, &
                "nonlin tolerance in iterative velocity solve", units="nondim", default=1.e-6_wp)
    ...
    call get_param(param_file, mdl, "INFLOW_SHELF_TEMPERATURE", T_shelf_bdry, &
                 ...
                 "open boundaries.", units="degC", default=-15.0_wp, scale=US%degC_to_C)
  endif
  ...
  if (CS%Timeunit < 0.0_wp) CS%Timeunit = 86400.0_wp
  ...
  if ((time_type_to_real(CS%energysavedays_geometric) > 0._wp) .and. &
  ...
  if (active_shelf_dynamics) then
    ...
    allocate( CS%u_face_mask(Isdq:Iedq,Jsdq:Jedq), source=0.0_wp)
    allocate( CS%v_face_mask(Isdq:Iedq,Jsdq:Jedq), source=0.0_wp)
    allocate( CS%u_flux_bdry_val(Isdq:Iedq,jsd:jed), source=0.0_wp)
    allocate( CS%v_flux_bdry_val(isd:ied,Jsdq:Jedq), source=0.0_wp)
    allocate( CS%umask(Isdq:Iedq,Jsdq:Jedq), source=-1.0_wp)
    allocate( CS%vmask(Isdq:Iedq,Jsdq:Jedq), source=-1.0_wp)
    allocate( CS%tmask(Isdq:Iedq,Jsdq:Jedq), source=-1.0_wp)
    allocate( CS%float_cond(isd:ied,jsd:jed))

    CS%OD_rt_counter = 0
    allocate( CS%OD_rt(isd:ied,jsd:jed), source=0.0_wp)
    allocate( CS%ground_frac_rt(isd:ied,jsd:jed), source=0.0_wp)

    if (CS%calve_to_mask) then
      allocate( CS%calve_mask(isd:ied,jsd:jed), source=0.0_wp)
    endif

    allocate(CS%Phi(1:8,1:4,isd:ied,jsd:jed), source=0.0_wp)
    do j=G%jsd,G%jed ; do i=G%isd,G%ied
      call bilinear_shape_fn_grid(G, i, j, CS%Phi(:,:,i,j))
    enddo; enddo

    if (CS%GL_regularize) then
      allocate(CS%Phisub(2,2,CS%n_sub_regularize,CS%n_sub_regularize,2,2), source=0.0_wp)
    ...
    if ((trim(CS%ice_viscosity_compute) == "MODEL") .and. CS%visc_qps==1) then
      ...
      allocate(CS%PhiC(1:8,G%isc:G%iec,G%jsc:G%jec), source=0.0_wp)
      do j=G%jsc,G%jec ; do i=G%isc,G%iec
        call bilinear_shape_fn_grid_1qp(G, i, j, CS%PhiC(:,i,j))
      enddo; enddo
    endif

    CS%elapsed_velocity_time = 0.0_wp
  ...
  if (active_shelf_dynamics) then
    if (CS%first_dir_restart_IS > -1.0_wp) then
      CS%first_direction_IS = modulo(NINT(CS%first_dir_restart_IS), 2)
    else
      CS%first_dir_restart_IS = real(modulo(CS%first_direction_IS, 2), wp)
    endif
    ...
    if (CS%calve_to_mask) then
      ...
      do j=G%jsc,G%jec ; do i=G%isc,G%iec
        if (CS%calve_mask(i,j) > 0.0_wp) CS%calve_mask(i,j) = 1.0_wp
      enddo ; enddo
  ...
  ! Register diagnostics.
    ...
    CS%id_taub = register_diag_field('ice_shelf_model','taub_beta',CS%diag%axesT1, Time, &
       'taub', units='MPa yr m-1', conversion=1e-6_wp*US%RLZ_T2_to_Pa/(365.0_wp*86400.0_wp*US%L_T_to_m_s))
    ...
    CS%id_duHdx = register_diag_field('ice_shelf_model','duHdx',CS%diag%axesT1, Time, &
       'x-component of ice-sheet flux divergence', 'm yr-1', conversion=365.0_wp*86400.0_wp*US%Z_to_m*US%s_to_T)
    CS%id_dvHdy = register_diag_field('ice_shelf_model','dvHdy',CS%diag%axesT1, Time, &
       'y-component of ice-sheet flux divergence', 'm yr-1', conversion=365.0_wp*86400.0_wp*US%Z_to_m*US%s_to_T)
    CS%id_fluxdiv = register_diag_field('ice_shelf_model','fluxdiv',CS%diag%axesT1, Time, &
       'ice-sheet flux divergence', 'm yr-1', conversion=365.0_wp*86400.0_wp*US%Z_to_m*US%s_to_T)
    CS%id_strainrate_xx = register_diag_field('ice_shelf_model','strainrate_xx',CS%diag%axesT1, Time, &
       'x-component of ice-shelf strain-rate', 'yr-1', conversion=365.0_wp*86400.0_wp*US%s_to_T)
    CS%id_strainrate_yy = register_diag_field('ice_shelf_model','strainrate_yy',CS%diag%axesT1, Time, &
       'y-component of ice-shelf strain-rate', 'yr-1', conversion=365.0_wp*86400.0_wp*US%s_to_T)
    CS%id_strainrate_xy = register_diag_field('ice_shelf_model','strainrate_xy',CS%diag%axesT1, Time, &
       'xy-component of ice-shelf strain-rate', 'yr-1', conversion=365.0_wp*86400.0_wp*US%s_to_T)
    CS%id_pstrainrate_1 = register_diag_field('ice_shelf_model','pstrainrate_1',CS%diag%axesT1, Time, &
       'max principal horizontal ice-shelf strain-rate', 'yr-1', conversion=365.0_wp*86400.0_wp*US%s_to_T)
    CS%id_pstrainrate_2 = register_diag_field('ice_shelf_model','pstrainrate_2',CS%diag%axesT1, Time, &
       'min principal horizontal ice-shelf strain-rate', 'yr-1', conversion=365.0_wp*86400.0_wp*US%s_to_T)
    CS%id_devstress_xx = register_diag_field('ice_shelf_model','devstress_xx',CS%diag%axesT1, Time, &
       'x-component of ice-shelf deviatoric stress', 'kPa', conversion=1.e-3_wp*US%RLZ_T2_to_Pa)
    CS%id_devstress_yy = register_diag_field('ice_shelf_model','devstress_yy',CS%diag%axesT1, Time, &
       'y-component of ice-shelf deviatoric stress', 'kPa', conversion=1.e-3_wp*US%RLZ_T2_to_Pa)
    CS%id_devstress_xy = register_diag_field('ice_shelf_model','devstress_xy',CS%diag%axesT1, Time, &
       'xy-component of ice-shelf deviatoric stress', 'kPa', conversion=1.e-3_wp*US%RLZ_T2_to_Pa)
    CS%id_pdevstress_1 = register_diag_field('ice_shelf_model','pdevstress_1',CS%diag%axesT1, Time, &
       'max principal horizontal ice-shelf deviatoric stress', 'kPa', conversion=1.e-3_wp*US%RLZ_T2_to_Pa)
    CS%id_pdevstress_2 = register_diag_field('ice_shelf_model','pdevstress_2',CS%diag%axesT1, Time, &
       'min principal ice-shelf deviatoric stress', 'kPa', conversion=1.e-3_wp*US%RLZ_T2_to_Pa)
...
subroutine initialize_diagnostic_fields(CS, ISS, G, US, Time)
  ...
  real(wp)            :: rhoi_rhow
  real(wp)            :: OD  ! Depth of open water below the ice shelf [Z ~> m]
...
!
  ...
  do j=jsd,jed
    ...
    ! ice thickness does not take up whole ocean column -> floating
        CS%OD_av(i,j) = OD
        CS%ground_frac(i,j) = 0._wp
      else
        CS%OD_av(i,j) = 0._wp
        CS%ground_frac(i,j) = 1._wp
      endif
    enddo
  enddo
...
function ice_time_step_CFL(CS, ISS, G)
  ...
  type(ice_shelf_state),  intent(inout) :: ISS !< A structure with elements that describe
                                               !! the ice-shelf state
  type(ocean_grid_type),  intent(inout) :: G   !< The grid structure used by the ice shelf.
  real(wp) :: ice_time_step_CFL !< The maximum permitted timestep based on the ice velocities [T ~> s].

  real(wp) :: dt_local, min_dt ! These should be the minimum stable timesteps at a CFL of 1 [T ~> s]
  real(wp) :: min_vel          ! A minimal velocity for estimating a timestep [L T-1 ~> m s-1]
  integer :: i, j

  min_dt = 5.0e17_wp*G%US%s_to_T ! The starting maximum is roughly the lifetime of the universe.
  min_vel = (1.0e-12_wp/(365.0_wp*86400.0_wp)) * G%US%m_s_to_L_T
  do j=G%jsc,G%jec ; do i=G%isc,G%iec ; if (ISS%hmask(i,j) == 1.0_wp .or. ISS%hmask(i,j)==3) then
    dt_local = 2.0_wp*G%areaT(i,j) / &
...
subroutine update_ice_shelf(CS, ISS, G, US, time_step, Time, calve_ice_shelf_bergs, &
  ...
  type(ocean_grid_type),  intent(inout) :: G  !< The grid structure used by the ice shelf.
  type(unit_scale_type),  intent(in)    :: US !< A structure containing unit conversion factors
  real(wp),                   intent(in)    :: time_step !< time step [T ~> s]
  type(time_type),        intent(in)    :: Time !< The current model time
...
!
  if (CS%advect_shelf) then
    ...
    if (CS%alternate_first_direction_IS) then
      ...
      CS%first_dir_restart_IS = real(CS%first_direction_IS, wp)
    endif
  endif
...
subroutine volume_above_floatation(CS, G, ISS, vaf, hemisphere)
  ...
  type(ice_shelf_state),  intent(in) :: ISS !< A structure with elements that describe
                                            !! the ice-shelf state
  real(wp), intent(out) :: vaf !< area integrated volume above floatation [Z L2 ~> m3]
  ...
  real(wp), dimension(SZI_(G),SZJ_(G))  :: vaf_cell !< cell-wise volume above floatation [Z L2 ~> m3]
  integer, dimension(SZI_(G),SZJ_(G))  :: mask ! a mask for active cells depending on hemisphere indicated
  integer :: is,ie,js,je,i,j
  real(wp) :: rhoi_rhow, rhow_rhoi
  ...
  do j = js,je; do i = is,ie
    if (mask(i,j)>0) then
      if (CS%bed_elev(i,j) <= 0) then
        ...
        vaf_cell(i,j) = max(ISS%h_shelf(i,j) - rhow_rhoi * CS%bed_elev(i,j), 0.0_wp) * ISS%area_shelf_h(i,j)
      endif
    endif
  enddo; enddo
...
subroutine masked_var_grounded(G,CS,var,varout)
  type(ocean_grid_type), intent(in) :: G !< The grid structure used by the ice shelf.
  type(ice_shelf_dyn_CS), intent(in) :: CS !< The ice shelf dynamics control structure
  real(wp), dimension(SZI_(G),SZJ_(G)), intent(in)  :: var !< variable in
  real(wp), dimension(SZI_(G),SZJ_(G)), intent(out)  :: varout !<variable out
  integer :: i,j
  do j = G%jsc,G%jec; do i = G%isc,G%iec
...
subroutine IS_dynamics_post_data(time_step, Time, CS, ISS, G)
  real(wp) :: time_step !< Length of time for post data averaging [T ~> s].
  type(time_type),        intent(in)    :: Time !< The current model time
  type(ice_shelf_dyn_CS), intent(inout) :: CS !< The ice shelf dynamics control structure
  type(ice_shelf_state),  intent(inout) :: ISS !< A structure with elements that describe
                                               !! the ice-shelf state
  type(ocean_grid_type),  intent(in) :: G  !< The grid structure used by the ice shelf.
  real(wp), dimension(SZDIB_(G),SZDJB_(G))  :: taud_x, taud_y, taud  ! area-averaged driving stress [R L2 T-2 ~> Pa]
  real(wp), dimension(SZDI_(G),SZDJ_(G))  :: ice_visc ! area-averaged vertically integrated ice viscosity
                                                  !! [R L2 Z T-1 ~> Pa s m]
  real(wp), dimension(SZDI_(G),SZDJ_(G))  :: basal_tr ! area-averaged taub_beta field related to basal traction,
                                                  !! [R L T-1 ~> Pa s m-1]
  real(wp), dimension(SZDIB_(G),SZDJB_(G)) :: surf_slope ! the surface slope of the ice shelf/sheet [nondim]
  real(wp), dimension(SZDIB_(G),SZDJB_(G)) :: ice_speed ! ice sheet flow speed [L T-1 ~> m s-1]
...
subroutine ice_visc_diag(CS,G,ice_visc)
  type(ice_shelf_dyn_CS), intent(in) :: CS !< The ice shelf dynamics control structure
  type(ocean_grid_type),  intent(in) :: G  !< The grid structure used by the ice shelf.
  real(wp), dimension(SZDI_(G),SZDJ_(G)), intent(out)  :: ice_visc !< area-averaged vertically integrated ice viscosity
                                                               !! [R L2 Z T-1 ~> Pa s m]
  integer :: i,j
  ...
  if (CS%visc_qps==4) then
    do j=G%jsc,G%jec ; do i=G%isc,G%iec
      ice_visc(i,j) = (0.25_wp * G%IareaT(i,j)) * &
...
subroutine write_ice_shelf_energy(CS, G, US, mass, area, day, time_step)
  ...
  real(wp), dimension(SZDI_(G),SZDJ_(G)) :: tmp1 ! A temporary array used in reproducing sums [various]
  real(wp) :: KE_tot    ! The total kinetic energy [R Z L4 T-2 ~> J]
  real(wp) :: mass_tot  ! The total mass [R Z L2 ~> kg]
  ...
  real(wp)    :: reday  ! Time in units given by CS%Timeunit, but often [days]
  ...
  tmp1(:,:) = 0.0_wp
  do j=js,je ; do i=is,ie
    tmp1(i,j) = 0.03125_wp * (mass(i,j) * area(i,j)) * &
  ...
  if (is_root_pe()) then  ! Only the root PE actually writes anything.
    if (day > CS%Start_time) then
      ...
      if (abs(CS%timeunit - 86400.0_wp) < 1.0_wp) then
        ...
        if ((CS%timeunit >= 0.99_wp) .and. (CS%timeunit < 1.01_wp)) then
          time_units = "           [seconds]     "
        elseif ((CS%timeunit >= 3599.0_wp) .and. (CS%timeunit < 3601.0_wp)) then
          time_units = "            [hours]      "
        elseif ((CS%timeunit >= 86399.0_wp) .and. (CS%timeunit < 86401.0_wp)) then
          time_units = "             [days]      "
        elseif ((CS%timeunit >= 3.0e7_wp) .and. (CS%timeunit < 3.2e7_wp)) then
          time_units = "            [years]      "
        else
    ...
    if (abs(CS%timeunit - 86400.0_wp) < 1.0_wp) then
      ...
      reday = REAL(num_days, wp)*(86400.0_wp/CS%timeunit) + REAL(start_of_day, wp)/abs(CS%timeunit)
    endif

    if (reday < 1.0e8_wp) then ;      write(day_str, '(F12.3)') reday
    elseif (reday < 1.0e11_wp) then ; write(day_str, '(F15.3)') reday
...
!    into partial cells
  ...
  real(wp), dimension(SZDI_(G),SZDJ_(G))   :: h_after_flux1, h_after_flux2 ! Ice thicknesses [Z ~> m].
  real(wp), dimension(SZDIB_(G),SZDJ_(G))  :: uh_ice  ! The accumulated zonal ice volume flux [Z L2 ~> m3]
  real(wp), dimension(SZDI_(G),SZDJB_(G))  :: vh_ice  ! The accumulated meridional ice volume flux [Z L2 ~> m3]
  ...
  h_after_flux1(:,:) = 0.0_wp
  h_after_flux2(:,:) = 0.0_wp
  ! call MOM_mesg("MOM_ice_shelf.F90: ice_shelf_advect called")

  do j=jsd,jed ; do i=isd,ied ; if (CS%h_bdry_val(i,j) /= 0.0_wp) then
  ...
  elseif (calve_ice_shelf_bergs) then
    ...
    do j=jsc,jec; do i=isc,iec
      if (ISS%hmask(i,j)==2) then
        ...
        ISS%h_shelf(i,j) = 0.0_wp; ISS%area_shelf_h(i,j) = 0.0_wp; ISS%hmask(i,j) = 0.0_wp
      endif
    enddo; enddo
  endif
...
subroutine ice_shelf_solve_outer(CS, ISS, G, US, u_shlf, v_shlf, taudx, taudy, iters, Time)
  ...
  type(ocean_grid_type),  intent(inout) :: G  !< The grid structure used by the ice shelf.
  type(unit_scale_type),  intent(in)    :: US !< A structure containing unit conversion factors
  real(wp), dimension(SZDIB_(G),SZDJB_(G)), &
  ...
  real(wp), dimension(SZDIB_(G),SZDJB_(G)) :: Au, Av ! The retarding lateral stress contributions [R L3 Z T-2 ~> kg m s-2]
  real(wp), dimension(SZDIB_(G),SZDJB_(G)) :: u_last, v_last ! Previous velocities [L T-1 ~> m s-1]
  real(wp), dimension(SZDIB_(G),SZDJB_(G)) :: H_node ! Ice shelf thickness at corners [Z ~> m].
  real(wp), dimension(SZDI_(G),SZDJ_(G)) :: float_cond ! If GL_regularize=true, indicates cells containing
                                                ! the grounding line (float_cond=1) or not (float_cond=0)
  real(wp), dimension(SZDIB_(G),SZDJB_(G)) :: Normvec  ! Velocities used for convergence [L2 T-2 ~> m2 s-2]
  ...
  real(wp)    :: err_max, err_tempu, err_tempv, err_init ! Errors in [R L3 Z T-2 ~> kg m s-2] or [L T-1 ~> m s-1]
  real(wp)    :: max_vel  ! The maximum velocity magnitude [L T-1 ~> m s-1]
  real(wp)    :: tempu, tempv   ! Temporary variables with velocity magnitudes [L T-1 ~> m s-1]
  real(wp)    :: Norm, PrevNorm ! Velocities used to assess convergence [L T-1 ~> m s-1]
  real(wp)    :: rhoi_rhow ! The density of ice divided by a typical water density [nondim]
  ...
  if (CS%GL_regularize) then
    do j=G%jsd,G%jed ; do i=G%isd,G%ied
      if (CS%ground_frac(i,j)/=1.0_wp) CS%basal_traction(i,j) = 0.0_wp
    enddo ; enddo
  else
    do j=G%jsd,G%jed ; do i=G%isd,G%ied
  ...
  do iter=1,50
    ...
    elseif (CS%nonlin_solve_err_mode == 2) then
      ...
      err_max=0._wp ;  max_vel = 0 ; tempu = 0 ; tempv = 0 ; err_tempu = 0
      do J=G%jscB,G%jecB ; do I=G%iscB,G%iecB
        if (CS%umask(I,J) == 1) then
    ...
    elseif (CS%nonlin_solve_err_mode == 3) then
      PrevNorm = Norm ; Norm = 0.0_wp ; Normvec=0.0_wp
      do J=Jscq_sv,Jecq ; do I=Iscq_sv,Iecq
        if (CS%umask(I,J) == 1) Normvec(I,J) = Normvec(I,J) + u_shlf(I,J)**2
        if (CS%vmask(I,J) == 1) Normvec(I,J) = Normvec(I,J) + v_shlf(I,J)**2
      enddo; enddo
      Norm = sqrt( reproducing_sum( Normvec, Is_sum, Ie_sum, Js_sum, Je_sum, unscale=US%L_T_to_m_s**2 ) )
      err_max = 2._wp*abs(Norm-PrevNorm) ; err_init = Norm+PrevNorm
    endif
...
subroutine ice_shelf_solve_inner(CS, ISS, G, US, u_shlf, v_shlf, taudx, taudy, H_node, float_cond, &
  ...
  integer,                intent(out)   :: iters !< The number of iterations used in the solver.
  type(time_type),        intent(in)    :: Time !< The current model time
  real(wp), dimension(8,4,SZDI_(G),SZDJ_(G)), &
...
! assumed - u, v, taud, visc, basal_traction are valid on the halo
  ...
  real(wp), dimension(SZDIB_(G),SZDJB_(G),3) :: sum_vec_3d ! Array used for various residuals
                                                       ! sum_vec_3d(:,:,1:2) [m s-1] [m kg s-2]
                                                       ! sum_vec_3d(:,:,3)   [m2 kg2 s-4]
  real(wp)    :: beta_k      ! Ratio of residuals used to update search direction [nondim]
  real(wp)    :: resid0tol2  ! Convergence tolerance times the initial residual [m2 kg2 s-4]
  real(wp)    :: sv3dsum     ! An unused variable returned when taking global sum of residuals [various]
  real(wp)    :: sv3dsums(3) ! The index-wise global sums of sum_vec_3d
                         ! sv3dsums(:,:,1:2) [m s-1] [m kg s-2]
                         ! sv3dsums(:,:,3)   [m2 kg2 s-4]
  real(wp)    :: alpha_k     ! A scaling factor for iterative corrections [nondim]
  real(wp)    :: resid_scale ! A scaling factor for redimensionalizing the global residuals
                         ! [L T-1 ~> m s-1] [R L3 Z T-2 ~> m kg s-2]
  real(wp)    :: resid2_scale ! A scaling factor for redimensionalizing the global squared residuals
  ...
  do iter = 1,CS%cg_max_iterations
    ...
    sum_vec_3d(:,:,:) = 0.0_wp; sv3dsums(:)=0.0_wp
...
subroutine ice_shelf_advect_thickness_x(CS, G, LB, time_step, hmask, h0, h_after_uflux, uh_ice)
  ...
  real(wp), dimension(SZDIB_(G),SZDJ_(G)), &
  ...
  integer :: i, j
  integer :: ish, ieh, jsh, jeh
  real(wp) :: u_face     ! Zonal velocity at a face [L T-1 ~> m s-1]
  real(wp) :: h_face     ! Thickness at a face for transport [Z ~> m]
  real(wp) :: slope_lim  ! The value of the slope limiter, in the range of 0 to 2 [nondim]
...
!  isd = G%isd ; ied = G%ied ; jsd = G%jsd ; jed = G%jed
  ...
  do j=jsh,jeh ; do I=ish-1,ieh
    if (CS%u_face_mask(I,j) == 4._wp) then ! The flux itself is a specified boundary condition.
    ...
    elseif ((hmask(i,j) == 1 .or. hmask(i,j) == 3) .or. (hmask(i+1,j) == 1 .or. hmask(i+1,j) == 3)) then
      u_face = 0.5_wp * (CS%u_shelf(I,J-1) + CS%u_shelf(I,J))
      h_face = 0.0_wp ! This will apply when the source cell is iceless or not fully ice covered.
      ...
      if (u_face > 0) then
        ...
        elseif (hmask(i,j) == 1) then ! There can be eastward flow through this face.
          if ((hmask(i-1,j) == 1 .or. hmask(i-1,j) == 3) .and. &
            ...
            h_face = h0(i,j) - slope_lim * (0.5_wp * (h0(i,j)-h0(i+1,j)))
          else
            h_face = h0(i,j)
          endif
        endif
        ...
        elseif (hmask(i+1,j) == 1) then
          if ((hmask(i,j) == 1 .or. hmask(i,j) == 3) .and. &
            ...
            h_face = h0(i+1,j) - slope_lim * (0.5_wp * (h0(i+2,j)-h0(i+1,j)))
          else
            h_face = h0(i+1,j)
          endif
        endif
  ...
  do j=jsh,jeh ; do i=ish,ieh
    ...
    if ((hmask(i,j) == 0) .and. ((uh_ice(I-1,j) > 0.0_wp) .or. (uh_ice(I,j) < 0.0_wp))) hmask(i,j) = 2
  enddo ; enddo
...
subroutine ice_shelf_advect_thickness_y(CS, G, LB, time_step, hmask, h0, h_after_vflux, vh_ice)
  ...
  real(wp), dimension(SZDI_(G),SZDJB_(G)), &
  ...
  integer :: i, j
  integer :: ish, ieh, jsh, jeh
  real(wp) :: v_face     ! Pseudo-meridional velocity at a face [L T-1 ~> m s-1]
  real(wp) :: h_face     ! Thickness at a face for transport [Z ~> m]
  real(wp) :: slope_lim  ! The value of the slope limiter, in the range of 0 to 2 [nondim]
  ...
  do J=jsh-1,jeh ; do i=ish,ieh
    if (CS%v_face_mask(i,J) == 4._wp) then ! The flux itself is a specified boundary condition.
    ...
    elseif ((hmask(i,j) == 1 .or. hmask(i,j) == 3) .or. (hmask(i,j+1) == 1 .or. hmask(i,j+1) == 3)) then
      v_face = 0.5_wp * (CS%v_shelf(I-1,J) + CS%v_shelf(I,J))
      h_face = 0.0_wp ! This will apply when the source cell is iceless or not fully ice covered.
      ...
      if (v_face > 0) then
        ...
        elseif (hmask(i,j) == 1) then ! There can be northward flow through this face.
          if ((hmask(i,j-1) == 1 .or. hmask(i,j-1) == 3) .and. &
            ...
            ! This is a 2nd-order centered scheme with a slope limiter.  We could try PPM here.
            h_face = h0(i,j) - slope_lim * (0.5_wp * (h0(i,j)-h0(i,j+1)))
          else
            h_face = h0(i,j)
          endif
        endif
        ...
        elseif (hmask(i,j+1) == 1) then
          if ((hmask(i,j) == 1 .or. hmask(i,j) == 3) .and. &
            ...
            h_face = h0(i,j+1) - slope_lim * (0.5_wp * (h0(i,j+2)-h0(i,j+1)))
          else
            h_face = h0(i,j+1)
          endif
        endif
  ...
  do j=jsh,jeh ; do i=ish,ieh
    ...
    ! Update the masks of cells that have gone from no ice to partial ice.
    if ((hmask(i,j) == 0) .and. ((vh_ice(i,J-1) > 0.0_wp) .or. (vh_ice(i,J) < 0.0_wp))) hmask(i,j) = 2
  enddo ; enddo
...
subroutine shelf_advance_front(CS, ISS, G, hmask, uh_ice, vh_ice)
  ...
  real(wp) :: h_reference ! A reference thicknesss based on neighboring cells [Z ~> m]
  real(wp) :: h_reference_ew !contribution to reference thickness from east + west cells [Z ~> m]
  real(wp) :: h_reference_ns !contribution to reference thickness from north + south cells [Z ~> m]
  real(wp) :: tot_flux    ! The total ice mass flux [Z L2 ~> m3]
  real(wp) :: tot_flux_ew ! The contribution to total ice mass flux from east + west cells [Z L2 ~> m3]
  real(wp) :: tot_flux_ns ! The contribution to total ice mass flux from north + south cells [Z L2 ~> m3]
  real(wp) :: partial_vol ! The volume covered by ice shelf [Z L2 ~> m3]
  real(wp) :: dxdyh       ! Cell area [L2 ~> m2]
  character(len=160) :: mesg  ! The text of an error message
  integer, dimension(4) :: mapi, mapj, new_partial
  real(wp), dimension(SZDI_(G),SZDJ_(G),4) :: flux_enter  ! The ice volume flux into the
                                              ! cell through the 4 cell boundaries [Z L2 ~> m3].
  real(wp), dimension(SZDI_(G),SZDJ_(G),4) :: flux_enter_replace ! An updated ice volume flux into the
                                              ! cell through the 4 cell boundaries [Z L2 ~> m3].
  ...
  do j=jsc-1,jec+1 ; do i=isc-1,iec+1
    if ((hmask(i,j) == 0) .or. (hmask(i,j) == 2)) then
      flux_enter(i,j,1) = max(uh_ice(I-1,j), 0.0_wp)
      flux_enter(i,j,2) = max(-uh_ice(I,j), 0.0_wp)
      flux_enter(i,j,3) = max(vh_ice(i,J-1), 0.0_wp)
      flux_enter(i,j,4) = max(-vh_ice(i,J), 0.0_wp)
    endif
  enddo ; enddo
      ...
      if (CS%reentrant_y .OR. (((j+j_off) <= G%domain%njglobal) .AND. &
        ...
        do i=isc-1,iec+1
          ...
          if (CS%reentrant_x .OR. (((i+i_off) <= G%domain%niglobal) .AND. &
            ...
            n_flux = 0
            h_reference_ew = 0.0_wp
            h_reference_ns = 0.0_wp
            tot_flux_ew = 0.0_wp
            tot_flux_ns = 0.0_wp
              ...
              elseif ((partial_vol / G%areaT(i,j)) < h_reference) then
               ...
               !  ISS%mass_shelf(i,j) = partial_vol * CS%density_ice
                ...
                if (n_flux == 0) then ! there is nowhere to put the extra ice!
                  ...
                  do k=1,2
                    if (new_partial(k) == 1) &
                      flux_enter_replace(i+2*k-3,j,3-k) = partial_vol / real(n_flux, wp)
                    if (new_partial(k+2) == 1) &
                      flux_enter_replace(i,j+2*k-3,5-k) = partial_vol / real(n_flux, wp)
                  enddo
                endif
...
subroutine ice_shelf_min_thickness_calve(G, h_shelf, area_shelf_h, hmask, thickness_calve, halo)
  type(ocean_grid_type), intent(in)    :: G  !< The grid structure used by the ice shelf.
  real(wp), dimension(SZDI_(G),SZDJ_(G)), intent(inout) :: h_shelf !< The ice shelf thickness [Z ~> m].
  real(wp), dimension(SZDI_(G),SZDJ_(G)), intent(inout) :: area_shelf_h !< The area per cell covered by
                                             !! the ice shelf [L2 ~> m2].
  real(wp), dimension(SZDI_(G),SZDJ_(G)), intent(inout) :: hmask !< A mask indicating which tracer points are
                                             !! partly or fully covered by an ice-shelf
  real(wp),                  intent(in)    :: thickness_calve !< The thickness at which to trigger calving [Z ~> m].
...
!        (CS%ground_frac(i,j) == 0.0)) then
    if ((h_shelf(i,j) < thickness_calve) .and. (area_shelf_h(i,j) > 0._wp)) then
      h_shelf(i,j) = 0.0_wp
...
subroutine calve_to_mask(G, h_shelf, area_shelf_h, hmask, calve_mask)
  ...
  real(wp), dimension(SZDI_(G),SZDJ_(G)), intent(inout) :: hmask !< A mask indicating which tracer points are
                                                             !! partly or fully covered by an ice-shelf
  real(wp), dimension(SZDI_(G),SZDJ_(G)), intent(in)    :: calve_mask !< A mask that indicates where the ice
                                                             !! shelf can exist, and where it will calve.

  integer                        :: i,j

  do j=G%jsc,G%jec ; do i=G%isc,G%iec
    if ((calve_mask(i,j) == 0.0_wp) .and. (hmask(i,j) /= 0.0_wp)) then
      h_shelf(i,j) = 0.0_wp
      area_shelf_h(i,j) = 0.0_wp
      hmask(i,j) = 0.0_wp
    endif
...
!    (it is assumed that base_ice = bed + OD)
  ...
  real(wp), dimension(SIZE(OD,1),SIZE(OD,2))  :: S     ! surface elevation [Z ~> m].
  real(wp), dimension(SZDI_(G),SZDJ_(G)) :: sx_e, sy_e !element contributions to driving stress
  real(wp)    :: rho, rhow, rhoi_rhow ! Ice and ocean densities [R ~> kg m-3]
  real(wp)    :: sx, sy    ! Ice shelf top slopes [Z L-1 ~> nondim]
  real(wp)    :: neumann_val ! [R Z L2 T-2 ~> kg s-2]
  real(wp)    :: dxh, dyh,Dx,Dy  ! Local grid spacing [L ~> m]
  real(wp)    :: grav      ! The gravitational acceleration [L2 Z-1 T-2 ~> m s-2]
  real(wp)    :: scale     ! Scaling factor used to ensure surface slope magnitude does not exceed CS%max_surface_slope
  ...
  do j=jsc-1,jec+1
    do i=isc-1,iec+1
      ...
      if (ISS%hmask(i,j) == 1 .or. ISS%hmask(i,j) == 3) then
        ...
        sx_e(i,j) = (-.25_wp * G%areaT(i,j)) * ((rho * grav) * (max(ISS%h_shelf(i,j),CS%min_h_shelf) * sx))
        sy_e(i,j) = (-.25_wp * G%areaT(i,j)) * ((rho * grav) * (max(ISS%h_shelf(i,j),CS%min_h_shelf) * sy))

        CS%sx_shelf(i,j) = sx ; CS%sy_shelf(i,j) = sy

        !Stress (Neumann) boundary conditions
        if (CS%ground_frac(i,j) == 1) then
          neumann_val = ((.5_wp * grav) * (rho * max(ISS%h_shelf(i,j),CS%min_h_shelf)**2 - rhow * CS%bed_elev(i,j)**2))
        else
          neumann_val = (.5_wp * grav) * ((1-rho/rhow) * (rho * max(ISS%h_shelf(i,j),CS%min_h_shelf)**2))
        endif
        if ((CS%u_face_mask_bdry(I-1,j) == 2) .OR. &
          ...
          taudx(I-1,J-1) = taudx(I-1,J-1) - .5_wp * dyh * neumann_val
          taudx(I-1,J) = taudx(I-1,J) - .5_wp * dyh * neumann_val
        endif
        ...
        if ((CS%u_face_mask_bdry(I,j) == 2) .OR. &
          ...
          taudx(I,J-1) = taudx(I,J-1) + .5_wp * dyh * neumann_val
          taudx(I,J) = taudx(I,J) + .5_wp * dyh * neumann_val
        endif
        ...
        if ((CS%v_face_mask_bdry(i,J-1) == 2) .OR. &
          ...
          taudy(I-1,J-1) = taudy(I-1,J-1) - .5_wp * dxh * neumann_val
          taudy(I,J-1) = taudy(I,J-1) - .5_wp * dxh * neumann_val
        endif
        ...
        if ((CS%v_face_mask_bdry(i,J) == 2) .OR. &
          ...
          taudy(I-1,J) = taudy(I-1,J) + .5_wp * dxh * neumann_val
          taudy(I,J) = taudy(I,J) + .5_wp * dxh * neumann_val
        endif
      endif
    enddo
  enddo
...
subroutine CG_action(CS, uret, vret, u_shlf, v_shlf, Phi, Phisub, umask, vmask, hmask, H_node, &
  ...
  real(wp), dimension(SZDI_(G),SZDJ_(G)), &
                         intent(in)    :: hmask !< A mask indicating which tracer points are
                                             !! partly or fully covered by an ice-shelf
  real(wp), dimension(SZDI_(G),SZDJ_(G),CS%visc_qps), &
  ...
  real(wp),                  intent(in)    :: dens_ratio !< The density of ice divided by the density
...
! Phi_k is equal to 1 at vertex k, and 0 at vertex l /= k, and bilinear
  ...
  real(wp) :: ux, uy, vx, vy ! Components of velocity shears or divergence [T-1 ~> s-1]
  real(wp) :: uq, vq  ! Interpolated velocities [L T-1 ~> m s-1]
  integer :: iq, jq, iphi, jphi, i, j, ilq, jlq, Itgt, Jtgt, qp, qpv
  logical :: visc_qp4
  real(wp), dimension(2) :: xquad  ! Nondimensional quadrature ratios [nondim]
  real(wp), dimension(2,2) :: Ucell, Vcell, Usub, Vsub  ! Velocities at the nodal points around the cell [L T-1 ~> m s-1]
  real(wp), dimension(2,2) :: Hcell   ! Ice shelf thickness at notal (corner) points [Z ~> m]
  real(wp), dimension(2,2,4) :: uret_qp, vret_qp                ! Temporary arrays in [R Z L3 T-2 ~> kg m s-2]
  real(wp), dimension(SZDIB_(G),SZDJB_(G),4) :: uret_b, vret_b  ! Temporary arrays in [R Z L3 T-2 ~> kg m s-2]

  xquad(1) = .5_wp * (1-sqrt(1._wp/3)) ; xquad(2) = .5_wp * (1+sqrt(1._wp/3))
  ...
  uret(:,:) = 0.0_wp; vret(:,:)=0.0_wp
  uret_b(:,:,:)=0.0_wp ; vret_b(:,:,:)=0.0_wp

  do j=js,je ; do i=is,ie ; if (hmask(i,j) == 1 .or. hmask(i,j)==3) then

    uret_qp(:,:,:)=0.0_wp; vret_qp(:,:,:)=0.0_wp
      ...
      uret_b(I-1,J-1,4) = 0.25_wp*((uret_qp(1,1,1)+uret_qp(1,1,4))+(uret_qp(1,1,2)+uret_qp(1,1,3)))
      vret_b(I-1,J-1,4) = 0.25_wp*((vret_qp(1,1,1)+vret_qp(1,1,4))+(vret_qp(1,1,2)+vret_qp(1,1,3)))

      !element contribution to NW node (node 3, which sees the current element as element 2)
      uret_b(I-1,J  ,2) = 0.25_wp*((uret_qp(1,2,1)+uret_qp(1,2,4))+(uret_qp(1,2,2)+uret_qp(1,2,3)))
      vret_b(I-1,J  ,2) = 0.25_wp*((vret_qp(1,2,1)+vret_qp(1,2,4))+(vret_qp(1,2,2)+vret_qp(1,2,3)))

      !element contribution to SE node (node 2, which sees the current element as element 3)
      uret_b(I  ,J-1,3) = 0.25_wp*((uret_qp(2,1,1)+uret_qp(2,1,4))+(uret_qp(2,1,2)+uret_qp(2,1,3)))
      vret_b(I  ,J-1,3) = 0.25_wp*((vret_qp(2,1,1)+vret_qp(2,1,4))+(vret_qp(2,1,2)+vret_qp(2,1,3)))

      !element contribution to NE node (node 4, which sees the current element as element 1)
      uret_b(I  ,J  ,1) = 0.25_wp*((uret_qp(2,2,1)+uret_qp(2,2,4))+(uret_qp(2,2,2)+uret_qp(2,2,3)))
      vret_b(I  ,J  ,1) = 0.25_wp*((vret_qp(2,2,1)+vret_qp(2,2,4))+(vret_qp(2,2,2)+vret_qp(2,2,3)))
...
subroutine CG_action_subgrid_basal(Phisub, H, U, V, bathyT, dens_ratio, Ucontr, Vcontr)
  real(wp), dimension(:,:,:,:,:,:), &
                        intent(in)    :: Phisub !< Quadrature structure weights at subgridscale
                                            !! locations for finite element calculations [nondim]
  real(wp), dimension(2,2), intent(in)    :: H  !< The ice shelf thickness at nodal (corner) points [Z ~> m].
  real(wp), dimension(2,2), intent(in)    :: U  !< The zonal ice shelf velocity at vertices [L T-1 ~> m s-1]
  real(wp), dimension(2,2), intent(in)    :: V  !< The meridional ice shelf velocity at vertices [L T-1 ~> m s-1]
  real(wp),                 intent(in)    :: bathyT !< The depth of ocean bathymetry at tracer points
                                            !! relative to sea-level [Z ~> m].
  real(wp),                 intent(in)    :: dens_ratio !< The density of ice divided by the density
                                            !! of seawater [nondim]
  real(wp), dimension(2,2), intent(out)   :: Ucontr !< The areal average of u-velocities where the ice shelf
                                            !! is grounded, or 0 where it is floating [L T-1 ~> m s-1].
  real(wp), dimension(2,2), intent(out)   :: Vcontr !< The areal average of v-velocities where the ice shelf
                                            !! is grounded, or 0 where it is floating [L T-1 ~> m s-1].

  real(wp), dimension(SIZE(Phisub,3),SIZE(Phisub,3),2,2) :: Ucontr_sub, Vcontr_sub ! The contributions to Ucontr and Vcontr
                                                                               !! at each sub-cell
  real(wp), dimension(2,2,SIZE(Phisub,3),SIZE(Phisub,3)) :: uloc_arr !The local sub-cell u-velocity [L T-1 ~> m s-1]
  real(wp), dimension(2,2,SIZE(Phisub,3),SIZE(Phisub,3)) :: vloc_arr !The local sub-cell v-velocity [L T-1 ~> m s-1]
  real(wp), dimension(2,2) :: Ucontr_q, Vcontr_q !Contributions to a node from each quadrature point in a sub-grid cell
  real(wp)    :: subarea ! The fractional sub-cell area [nondim]
  real(wp)    :: hloc    ! The local sub-cell ice thickness [Z ~> m]
  integer :: nsub, i, j, qx, qy, m, n

  nsub = size(Phisub,3)
  subarea = 1.0_wp / (nsub**2)

  uloc_arr(:,:,:,:) = 0.0_wp; vloc_arr(:,:,:,:)=0.0_wp
  ...
  do n=1,2 ; do m=1,2 ; do j=1,nsub ; do i=1,nsub
    ...
    Ucontr_sub(i,j,m,n) = (subarea * 0.25_wp) * ((Ucontr_q(1,1) + Ucontr_q(2,2)) + (Ucontr_q(1,2)+Ucontr_q(2,1)))
    Vcontr_sub(i,j,m,n) = (subarea * 0.25_wp) * ((Vcontr_q(1,1) + Vcontr_q(2,2)) + (Vcontr_q(1,2)+Vcontr_q(2,1)))
  enddo; enddo ; enddo ; enddo
...
subroutine sum_square_matrix(sum_out, mat_in, n)
  ...
  real(wp), dimension(n,n), intent(in) :: mat_in !< The n x n matrix whose elements will be summed
  real(wp), intent(out) :: sum_out !< The sum of the elements of matrix mat_in
  integer :: s0,e0,s1,e1
...
subroutine matrix_diagonal(CS, G, US, float_cond, H_node, ice_visc, basal_trac, hmask, dens_ratio, &
  ...
  real(wp), dimension(8,4,SZDI_(G),SZDJ_(G)), &
                          intent(in)    :: Phi !< The gradients of bilinear basis elements at Gaussian
                                             !! quadrature points surrounding the cell vertices [L-1 ~> m-1]
  real(wp), dimension(:,:,:,:,:,:), intent(in) :: Phisub !< Quadrature structure weights at subgridscale
                                            !! locations for finite element calculations [nondim]
  real(wp), dimension(SZDIB_(G),SZDJB_(G)), &
...
! returns the diagonal entries of the matrix for a Jacobi preconditioning
  ...
  real(wp) :: ux, uy, vx, vy ! Interpolated weight gradients [L-1 ~> m-1]
  real(wp) :: uq, vq
  real(wp), dimension(2)   :: xquad
  real(wp), dimension(2,2) :: Hcell, sub_ground
  real(wp), dimension(2,2,4) :: u_diag_qp, v_diag_qp
  real(wp), dimension(SZDIB_(G),SZDJB_(G),4) :: u_diag_b, v_diag_b
  logical :: visc_qp4
  ...
  do j=jsc-1,jec+1 ; do i=isc-1,iec+1 ; if (hmask(i,j) == 1 .or. hmask(i,j)==3) then
    ...
    u_diag_qp(:,:,:)=0.0_wp; v_diag_qp(:,:,:)=0.0_wp
    ...
    !element contribution to SW node (node 1, which sees the current element as element 4)
    u_diag_b(I-1,J-1,4) = 0.25_wp*((u_diag_qp(1,1,1)+u_diag_qp(1,1,4))+(u_diag_qp(1,1,2)+u_diag_qp(1,1,3)))
    v_diag_b(I-1,J-1,4) = 0.25_wp*((v_diag_qp(1,1,1)+v_diag_qp(1,1,4))+(v_diag_qp(1,1,2)+v_diag_qp(1,1,3)))

    !element contribution to NW node (node 3, which sees the current element as element 2)
    u_diag_b(I-1,J  ,2) = 0.25_wp*((u_diag_qp(1,2,1)+u_diag_qp(1,2,4))+(u_diag_qp(1,2,2)+u_diag_qp(1,2,3)))
    v_diag_b(I-1,J  ,2) = 0.25_wp*((v_diag_qp(1,2,1)+v_diag_qp(1,2,4))+(v_diag_qp(1,2,2)+v_diag_qp(1,2,3)))

    !element contribution to SE node (node 2, which sees the current element as element 3)
    u_diag_b(I  ,J-1,3) = 0.25_wp*((u_diag_qp(2,1,1)+u_diag_qp(2,1,4))+(u_diag_qp(2,1,2)+u_diag_qp(2,1,3)))
    v_diag_b(I  ,J-1,3) = 0.25_wp*((v_diag_qp(2,1,1)+v_diag_qp(2,1,4))+(v_diag_qp(2,1,2)+v_diag_qp(2,1,3)))

    !element contribution to NE node (node 4, which sees the current element as element 1)
    u_diag_b(I  ,J  ,1) = 0.25_wp*((u_diag_qp(2,2,1)+u_diag_qp(2,2,4))+(u_diag_qp(2,2,2)+u_diag_qp(2,2,3)))
    v_diag_b(I  ,J  ,1) = 0.25_wp*((v_diag_qp(2,2,1)+v_diag_qp(2,2,4))+(v_diag_qp(2,2,2)+v_diag_qp(2,2,3)))
...
subroutine CG_diagonal_subgrid_basal (Phisub, H_node, bathyT, dens_ratio, f_grnd)
  real(wp), dimension(:,:,:,:,:,:), &
                        intent(in) :: Phisub !< Quadrature structure weights at subgridscale
                                             !! locations for finite element calculations [nondim]
  real(wp), dimension(2,2), intent(in) :: H_node !< The ice shelf thickness at nodal (corner)
                                             !! points [Z ~> m].
  real(wp),              intent(in)    :: bathyT !< The depth of ocean bathymetry at tracer points [Z ~> m].
  real(wp),              intent(in)    :: dens_ratio !< The density of ice divided by the density
                                                 !! of seawater [nondim]
  real(wp), dimension(2,2), intent(out) :: f_grnd !< The weighted fraction of the sub-cell where the ice shelf
                                              !! is grounded [nondim]

  real(wp), dimension(SIZE(Phisub,3),SIZE(Phisub,3),2,2) :: f_grnd_sub ! The contributions to nodal f_grnd
                                                                   !! from each sub-cell
  integer, dimension(2,2,SIZE(Phisub,3),SIZE(Phisub,3)) :: grnd_stat !0 at floating quad points, 1 at grounded
  real(wp), dimension(2,2) :: f_grnd_q  !Contributions to a node from each quadrature point in a sub-grid cell
  real(wp)    :: subarea ! The fractional sub-cell area [nondim]
  real(wp)    :: hloc    ! The local sub-region thickness [Z ~> m]
  integer :: nsub, i, j, qx, qy, m, n
  ...
  do n=1,2 ; do m=1,2 ; do j=1,nsub ; do i=1,nsub
    ...
    enddo ; enddo
    !calculate sub-cell contribution to each node by summing up quadrature point contributions from the sub-cell
    f_grnd_sub(i,j,m,n) = (subarea * 0.25_wp) * ((f_grnd_q(1,1) + f_grnd_q(2,2)) + (f_grnd_q(1,2)+f_grnd_q(2,1)))
  enddo ; enddo ; enddo ; enddo
...
subroutine IS_dynamics_post_data_2(CS, ISS, G)
  ...
  type(ocean_grid_type),  intent(in)    :: G  !< The grid structure used by the ice shelf.
  real(wp), dimension(SZDIB_(G),SZDJB_(G)) :: H_node ! Ice shelf thickness at corners [Z ~> m].
  real(wp), dimension(SZDIB_(G),SZDJB_(G)) :: Hu  ! Ice shelf u_flux at corners [Z L T-1 ~> m2 s-1].
  real(wp), dimension(SZDIB_(G),SZDJB_(G)) :: Hv  ! Ice shelf v_flux at corners [Z L T-1 ~> m2 s-1].
  real(wp), dimension(SZDI_(G),SZDJ_(G)) :: Hux  ! Ice shelf d(u_flux)/dx at cell centers [Z T-1 ~> m s-1].
  real(wp), dimension(SZDI_(G),SZDJ_(G)) :: Hvy  ! Ice shelf d(v_flux)/dy at cell centers [Z T-1 ~> m s-1].
  real(wp), dimension(SZDI_(G),SZDJ_(G)) :: flux_div ! horizontal flux divergence div(uH) [Z T-1 ~> m s-1].
  real(wp), dimension(SZDI_(G),SZDJ_(G),3) :: strain_rate ! strain-rate components xx,yy, and xy [T-1 ~> s-1]
  real(wp), dimension(SZDI_(G),SZDJ_(G),2) :: p_strain_rate ! horizontal principal strain-rates [T-1 ~> s-1]
  real(wp), dimension(SZDI_(G),SZDJ_(G),3) :: dev_stress ! deviatoric stress components xx,yy, and xy [R L Z T-2 ~> Pa]
  real(wp), dimension(SZDI_(G),SZDJ_(G),2) :: p_dev_stress ! horizontal principal deviatoric stress [R L Z T-2 ~> Pa]
  real(wp), dimension(SZDI_(G),SZDJ_(G))  :: ice_visc ! area-averaged ice viscosity [R L2 T-1 ~> Pa s]
  real(wp) :: p1,p2 ! Used to calculate strain-rate principal components [T-1 ~> s-1]
  integer :: i, j
  ...
  if (CS%id_duHdx > 0 .or. CS%id_dvHdy > 0 .or. CS%id_fluxdiv > 0) then
    ...
    Hu(:,:) = 0.0_wp; Hv(:,:) = 0.0_wp; Hux(:,:) = 0.0_wp ; Hvy(:,:) = 0.0_wp ; flux_div(:,:) = 0.0_wp
    do J=G%jscB,G%jecB ; do I=G%iscB,G%iecB
  ...
  if (CS%id_devstress_xx > 0  .or. CS%id_devstress_yy > 0  .or. CS%id_devstress_xy > 0  .or. &
    ...
    do j=G%jsc,G%jec ; do i=G%isc,G%iec
      ...
      if ((ISS%hmask(i,j) == 1) .or. (ISS%hmask(i,j) == 3)) then
        ...
        strain_rate(i,j,3) = 0.5_wp * ((((CS%u_shelf(I-1,J-1) * CS%PhiC(2,i,j)) + (CS%u_shelf(I,J  ) * CS%PhiC(8,i,j))) + &
    ...
    if (CS%id_pstrainrate_1 > 0 .or. CS%id_pstrainrate_2 > 0 .or. &
      ...
      p_strain_rate(:,:,:) = 0.0_wp
      do j=G%jsc,G%jec ; do i=G%isc,G%iec
        p1 = 0.5_wp*( strain_rate(i,j,1) + strain_rate(i,j,2))
        p2 = sqrt( (( 0.5_wp * (strain_rate(i,j,1) - strain_rate(i,j,2)) )**2) + (strain_rate(i,j,3)**2) )
...
!  real, dimension(SZDIB_(G),SZDJB_(G)) ::  eII, ux, uy, vx, vy
  ...
  real(wp) :: Visc_coef, n_g
  real(wp) :: ux, uy, vx, vy
  real(wp) :: eps_min   ! Velocity shears [T-1 ~> s-1]
  ...
  do j=jsc,jec ; do i=isc,iec
    ...
    if ((ISS%hmask(i,j) == 1) .OR. (ISS%hmask(i,j) == 3)) then
      ...
      if (trim(CS%ice_viscosity_compute) == "CONSTANT") then
        CS%ice_visc(i,j,1) = 1e15_wp * (US%kg_m3_to_R*US%m_to_L*US%m_s_to_L_T) * &
      ...
      elseif (model_qp1) then
        ...
        Visc_coef = (CS%AGlen_visc(i,j))**(-1._wp/n_g)
        ...
        CS%ice_visc(i,j,1) = (G%areaT(i,j) * max(ISS%h_shelf(i,j),CS%min_h_shelf)) * &
            max(0.5_wp * Visc_coef * &
            (US%s_to_T**2 * (((ux**2) + (vy**2)) + ((ux*vy) + 0.25_wp*((uy+vx)**2)) + eps_min**2))**((1._wp-n_g)/(2._wp*n_g)) * &
...
! this may be subject to change later... to make it "hybrid"
  ...
  integer :: giec, gjec, gisc, gjsc, isc, jsc, iec, jec, is, js
  real(wp) :: umid, vmid ! Velocities [L T-1 ~> m s-1]
  real(wp) :: eps_min ! A minimal strain rate used in the Glens flow law expression [T-1 ~> s-1]
  real(wp) :: unorm ! The magnitude of the velocity in mks units for use with fractional powers [m s-1]
  real(wp) :: alpha ! Coulomb coefficient [nondim]
  real(wp) :: Hf !"floatation thickness" for Coulomb friction [Z ~> m]
  real(wp) :: fN ! Effective pressure (ice pressure - ocean pressure) for Coulomb friction [R Z L T-2 ~> Pa]
  real(wp) :: fB !for Coulomb Friction [(T L-1)^CS%CF_PostPeak ~> (s m-1)^CS%CF_PostPeak]
  ...
  if (CS%CoulombFriction) then
    if (CS%CF_PostPeak/=1.0_wp) THEN
      alpha = (CS%CF_PostPeak-1.0_wp)**(CS%CF_PostPeak-1.0_wp) / CS%CF_PostPeak**CS%CF_PostPeak ![nondim]
    else
      alpha = 1.0_wp
    endif
  endif
  ...
  do j=jsd+1,jed
    do i=isd+1,ied
      if ((ISS%hmask(i,j) == 1) .OR. (ISS%hmask(i,j) == 3)) then
        umid = ((u_shlf(I,J) + u_shlf(I-1,J-1)) + (u_shlf(I,J-1) + u_shlf(I-1,J))) * 0.25_wp
        vmid = ((v_shlf(I,J) + v_shlf(I-1,J-1)) + (v_shlf(I,J-1) + v_shlf(I-1,J))) * 0.25_wp
        ...
        if (CS%CoulombFriction) then
          !Effective pressure
          Hf = max((CS%density_ocean_avg/CS%density_ice) * CS%bed_elev(i,j), 0.0_wp)
          ...
          CS%basal_traction(i,j) = ((G%areaT(i,j) * CS%C_basal_friction(i,j)) * &
              (unorm**(CS%n_basal_fric-1.0_wp) / (1.0_wp + fB * unorm**CS%CF_PostPeak)**(CS%n_basal_fric))) * &
...
subroutine update_OD_ffrac(CS, G, US, ocean_mass, find_avg)
  ...
  real(wp)    :: I_rho_ocean ! A typical specific volume of the ocean [R-1 ~> m3 kg-1]
  real(wp)    :: I_counter

  I_rho_ocean = 1.0_wp / CS%density_ocean_avg
  ...
  do j=jsc,jec ; do i=isc,iec
    ...
    if (ocean_mass(i,j)*I_rho_ocean > CS%thresh_float_col_depth) then
      CS%ground_frac_rt(i,j) = CS%ground_frac_rt(i,j) + 1.0_wp
    endif
  enddo ; enddo
  CS%OD_rt_counter = CS%OD_rt_counter + 1

  if (find_avg) then
    I_counter = 1.0_wp / real(CS%OD_rt_counter, wp)
    do j=jsc,jec ; do i=isc,iec
      CS%ground_frac(i,j) = 1.0_wp - (CS%ground_frac_rt(i,j) * I_counter)
      CS%OD_av(i,j) = CS%OD_rt(i,j) * I_counter

      CS%OD_rt(i,j) = 0.0_wp ; CS%ground_frac_rt(i,j) = 0.0_wp; CS%OD_rt_counter = 0
    enddo ; enddo
...
subroutine bilinear_shape_functions (X, Y, Phi, area)
  real(wp), dimension(4),   intent(in)    :: X   !< The x-positions of the vertices of the quadrilateral [L ~> m].
  real(wp), dimension(4),   intent(in)    :: Y   !< The y-positions of the vertices of the quadrilateral [L ~> m].
  real(wp), dimension(8,4), intent(inout) :: Phi !< The gradients of bilinear basis elements at Gaussian
                                             !! quadrature points surrounding the cell vertices [L-1 ~> m-1].
  real(wp),                 intent(out)   :: area !< The quadrilateral cell area [L2 ~> m2].
...
! ... will all cells have the same shape and dimension?
  ...
  real(wp), dimension(4) :: xquad, yquad ! [nondim]
  real(wp) :: a,b,c,d  ! Various lengths [L ~> m]
  real(wp) :: xexp, yexp ! [nondim]
  integer :: node, qpoint, xnode, ynode

  xquad(1:3:2) = .5_wp * (1-sqrt(1._wp/3)) ; yquad(1:2) = .5_wp * (1-sqrt(1._wp/3))
  xquad(2:4:2) = .5_wp * (1+sqrt(1._wp/3)) ; yquad(3:4) = .5_wp * (1+sqrt(1._wp/3))
  ...
  do qpoint=1,4
    ...
    do node=1,4
      ...
      xnode = 2-mod(node,2) ; ynode = ceiling(REAL(node, wp)/2)
...
! This should be a one-off; once per nonlinear solve? once per lifetime?
  ...
  real(wp), dimension(4) :: xquad, yquad ! [nondim]
  real(wp) :: a, d       ! Interpolated grid spacings [L ~> m]
  real(wp) :: xexp, yexp ! [nondim]
  integer :: node, qpoint, xnode, ynode
...
subroutine bilinear_shape_fn_grid_1qp(G, i, j, Phi)
  ...
  integer,               intent(in)    :: i   !< The i-index in the grid to work on.
  integer,               intent(in)    :: j   !< The j-index in the grid to work on.
  real(wp), dimension(8),    intent(inout) :: Phi !< The gradients of bilinear basis elements at Gaussian
                                              !! quadrature points surrounding the cell vertices [L-1 ~> m-1].
...
! Phi_i is equal to 1 at vertex i, and 0 at vertex k /= i, and bilinear
  ...
  real(wp) :: a, d       ! Interpolated grid spacings [L ~> m]
  real(wp) :: xexp=0.5_wp, yexp=0.5_wp ! [nondim]
  integer :: node, qpoint, xnode, ynode

    ! d(x)/d(x*)
    if (J>1) then
      a = 0.5_wp * (G%dxCv(i,J-1) + G%dxCv(i,J))
    else
      a = G%dxCv(i,J)
    endif
    ...
    ! d(y)/d(y*)
    if (I>1) then
      d = 0.5_wp * (G%dyCu(I-1,j) + G%dyCu(I,j))
    else
      d = G%dyCu(I,j)
    endif
...
subroutine bilinear_shape_functions_subgrid(Phisub, nsub)
  ...
  real(wp), dimension(2,2,nsub,nsub,2,2), &
  ...
  integer :: i, j, qx, qy
  real(wp),dimension(2)    :: xquad
  real(wp)                 :: x0, y0, x, y, fracx

  xquad(1) = .5_wp * (1-sqrt(1._wp/3)) ; xquad(2) = .5_wp * (1+sqrt(1._wp/3))
  fracx = 1.0_wp/real(nsub, wp)
  ...
  do j=1,nsub ; do i=1,nsub
    ...
    do qy=1,2 ; do qx=1,2
      x = x0 + fracx*xquad(qx)
      y = y0 + fracx*xquad(qy)
      Phisub(qx,qy,i,j,1,1) = (1.0_wp-x) * (1.0_wp-y)
      Phisub(qx,qy,i,j,1,2) = (1.0_wp-x) * y
      Phisub(qx,qy,i,j,2,1) = x * (1.0_wp-y)
      ...
      if ((hmask(i,j) == 1) .OR. (hmask(i,j) == 3)) then
        ...
        do k=0,1
          ...
          select case (int(CS%u_face_mask_bdry(I-1+k,j)))
            case (5)
              umask(I-1+k,J-1:J) = 3._wp
              u_face_mask(I-1+k,j) = 5._wp
            case (3)
              umask(I-1+k,J-1:J) = 3._wp
              vmask(I-1+k,J-1:J) = 3._wp
              u_face_mask(I-1+k,j) = 3._wp
            case (6)
              vmask(I-1+k,J-1:J) = 3._wp
              u_face_mask(I-1+k,j) = 6._wp
            case (2)
              u_face_mask(I-1+k,j) = 2._wp
            case (4)
              umask(I-1+k,J-1:J) = 0._wp
              u_face_mask(I-1+k,j) = 4._wp
            case (0)
              umask(I-1+k,J-1:J) = 0._wp
              u_face_mask(I-1+k,j) = 0._wp
            case (1)  ! stress free x-boundary
              umask(I-1+k,J-1:J) = 0._wp
            case default
              umask(I-1+k,J-1) = max(1._wp , umask(I-1+k,J-1))
              umask(I-1+k,J)   = max(1._wp , umask(I-1+k,J))
          end select
        enddo
        ...
        do k=0,1
          ...
          select case (int(CS%v_face_mask_bdry(i,J-1+k)))
            case (5)
              vmask(I-1:I,J-1+k) = 3._wp
              v_face_mask(i,J-1+k) = 5._wp
            case (3)
              vmask(I-1:I,J-1+k) = 3._wp
              umask(I-1:I,J-1+k) = 3._wp
              v_face_mask(i,J-1+k) = 3._wp
            case (6)
              umask(I-1:I,J-1+k) = 3._wp
              v_face_mask(i,J-1+k) = 6._wp
            case (2)
              v_face_mask(i,J-1+k) = 2._wp
            case (4)
              vmask(I-1:I,J-1+k) = 0._wp
              v_face_mask(i,J-1+k) = 4._wp
            case (0)
              vmask(I-1:I,J-1+k) = 0._wp
              v_face_mask(i,J-1+k) = 0._wp
            case (1) ! stress free y-boundary
              vmask(I-1:I,J-1+k) = 0._wp
            case default
              vmask(I-1,J-1+k) = max(1._wp , vmask(I-1,J-1+k))
              vmask(I,J-1+k)   = max(1._wp , vmask(I,J-1+k))
          end select
        enddo
...
subroutine interpolate_H_to_B(G, h_shelf, hmask, H_node, min_h_shelf)
  ...
  real(wp), dimension(SZDIB_(G),SZDJB_(G)), &
                         intent(inout) :: H_node !< The ice shelf thickness at nodal (corner)
                                             !! points [Z ~> m].
  real(wp), intent(in) :: min_h_shelf !< The minimum ice thickness used during ice dynamics [Z ~> m].
  ...
  do j=jsc-1,jec
    do i=isc-1,iec
      ...
      do l=1,2; jc=j-1+l; do k=1,2; ic=i-1+k
        if (hmask(ic,jc) == 1.0_wp .or. hmask(ic,jc) == 3.0_wp) then
...
!    into partial cells
  ...
  real(wp), dimension(SZDI_(G),SZDJ_(G))   :: th_after_uflux, th_after_vflux, TH ! Integrated temperatures [C Z ~> degC m]
  integer                           :: isd, ied, jsd, jed, i, j, isc, iec, jsc, jec
  real(wp) :: Tsurf ! Surface air temperature [C ~> degC].  This is hard coded but should be an input argument.
  real(wp) :: adot  ! A surface heat exchange coefficient [R Z T-1 ~> kg m-2 s-1].


  ! For now adot and Tsurf are defined here adot=surf acc 0.1m/yr, Tsurf=-20oC, vary them later
  adot = (0.1_wp/(365.0_wp*86400.0_wp))*US%m_to_Z*US%T_to_s * CS%density_ice
  Tsurf = -20.0_wp*US%degC_to_C

  isd = G%isd ; ied = G%ied ; jsd = G%jsd ; jed = G%jed
  isc = G%isc ; iec = G%iec ; jsc = G%jsc ; jec = G%jec

  th_after_uflux(:,:) = 0.0_wp
  th_after_vflux(:,:) = 0.0_wp
...
!   if ((ISS%hmask(i,j) == 1) .or. (ISS%hmask(i,j) == 2)) then
    if (ISS%h_shelf(i,j) > 0.0_wp) then
...
subroutine ice_shelf_advect_temp_x(CS, G, time_step, hmask, h0, h_after_uflux)
  ...
  real(wp), dimension(-2:2) :: stencil ! A copy of the neighboring thicknesses times temperatures [C Z ~> degC m]
  real(wp) :: u_face     ! Zonal velocity at a face, positive if out [L T-1 ~> m s-1]
  real(wp) :: flux_diff  ! The difference in fluxes [C Z ~> degC m]
  real(wp) :: phi        ! A limiting ratio [nondim]
  ...
  do j=jsd+1,jed-1
    if (((j+j_off) <= G%domain%njglobal+G%domain%njhalo) .AND. &
      ...
      stencil(:) = 0.0_wp ! This is probably unnecessary, as the code is written
      ...
      do i=is,ie
        ...
        if (((i+i_off) <= G%domain%niglobal+G%domain%nihalo) .AND. &
          ...
          if (hmask(i,j) == 1) then
            ...
            if (CS%u_face_mask(I-1,j) == 4._wp) then
              ...
              u_face = 0.5_wp * (CS%u_shelf(I-1,J-1) + CS%u_shelf(I-1,J))
            ...
            if (CS%u_face_mask(I,j) == 4._wp) then
...
subroutine ice_shelf_advect_temp_y(CS, G, time_step, hmask, h_after_uflux, h_after_vflux)
  ...
  real(wp), dimension(-2:2) :: stencil ! A copy of the neighboring thicknesses times temperatures [C Z ~> degC m]
  real(wp) :: v_face     ! Pseudo-meridional velocity at a cell face, positive if out [L T-1 ~> m s-1]
  real(wp) :: flux_diff  ! The difference in fluxes [C Z ~> degC m]
  real(wp) :: phi
      ...
      do j=js,je
        ...
        if (((j+j_off) <= G%domain%njglobal+G%domain%njhalo) .AND. &
          ...
          if (hmask(i,j) == 1) then
            ...
            if (CS%v_face_mask(i,J-1) == 4._wp) then
              ...
              v_face = 0.5_wp * (CS%v_shelf(I-1,J-1) + CS%v_shelf(I,J-1))
            ...
            if (CS%v_face_mask(i,J) == 4._wp) then
