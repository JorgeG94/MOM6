...
function tracer_Z_init(tr, h, filename, tr_name, G, GV, US, missing_val, land_val, scale)
  ...
  real(wp),        optional, intent(in)    :: missing_val !< The missing value for the tracer [CU ~> conc]
  real(wp),        optional, intent(in)    :: land_val  !< A value to use to fill in land points [CU ~> conc]
  real(wp),        optional, intent(in)    :: scale     !< A factor by which to scale the output tracers from the
  ...
  real(wp)    :: e(SZK_(GV)+1)  ! The z-star interface heights [Z ~> m].
  real(wp)    :: landval    ! The tracer value to use in land points [CU ~> conc]
  real(wp)    :: sl_tr      ! The normalized slope of the tracer
                        ! within the cell, in tracer units [CU ~> conc]
  real(wp)    :: htot(SZI_(G)) ! The vertical sum of h [H ~> m or kg m-2].
  real(wp)    :: dilate     ! The amount by which the thicknesses are dilated to
                        ! create a z-star coordinate [Z H-1 ~> nondim or m3 kg-1]
                        ! or other units reflecting those of h
  real(wp)    :: missing    ! The missing value for the tracer [CU ~> conc]
  real(wp)    :: scale_fac  ! A factor by which to scale the output tracers from the units in the
...
# include "version_variable.h"
  ...
  scale_fac = 1.0_wp ; if (present(scale)) then ; scale_fac = scale ; endif

  landval = 0.0_wp ; if (present(land_val)) landval = land_val
  ...
  allocate(tr_in(G%isd:G%ied,G%jsd:G%jed,nz_in), source=0.0_wp)
  allocate(tr_1d(nz_in), source=0.0_wp)
  ...
  if (present(missing_val)) then
    do j=js,je ; do i=is,ie
      if (G%mask2dT(i,j) == 0.0_wp) then
        tr_in(i,j,1) = landval
      elseif (abs(tr_in(i,j,1) - missing_val) <= 1e-6_wp*abs(missing_val)) then
    ...
    do k=2,nz_in ; do j=js,je ; do i=is,ie
      if (abs(tr_in(i,j,k) - missing_val) <= 1e-6_wp*abs(missing_val)) &
  ...
  if (has_edges) then
    do j=js,je
      ...
      do i=is,ie ; if (G%mask2dT(i,j)*htot(i) > 0.0_wp) then
          ...
          elseif (e(K) < z_edges(nz_in+1)) then
            ...
            if (k_bot > k_top) then
              ...
              tr(i,j,k) = tr(i,j,k) + wt(kz) * &
                  (tr_1d(kz) + 0.5_wp*sl_tr*(z2(kz) + z1(kz)))
              ! For the piecewise parabolic form add the following...
              !     + C1_3*cur_tr*(z2(kz)**2 + z2(kz)*z1(kz) + z1(kz)**2))
            endif
            k_bot_prev = k_bot
      ...
      do i=is,ie ; if (G%mask2dT(i,j)*htot(i) > 0.0_wp) then
        ...
        do k=1,nz
          ...
          elseif (z_edges(nz_in) > e(K)) then
            ...
            kz = k_top
            if (k_top < nz_in) then
              tr(i,j,k) = wt(kz)*0.5_wp*((tr_1d(kz) + tr_1d(kz+1)) + &
                                      (tr_1d(kz+1) - tr_1d(kz))*(z2(kz)+z1(kz)))
            else
              tr(i,j,k) = wt(kz)*tr_1d(nz_in)
            endif
            do kz=k_top+1,k_bot-1
              tr(i,j,k) = tr(i,j,k) + wt(kz)*0.5_wp*(tr_1d(kz) + tr_1d(kz+1))
            enddo
            if (k_bot > k_top) then
              kz = k_bot
              tr(i,j,k) = tr(i,j,k) + wt(kz)*0.5_wp*((tr_1d(kz) + tr_1d(kz+1)) + &
...
subroutine tracer_z_init_array(tr_in, z_edges, nk_data, e, land_fill, G, nlay, nlevs, &
  ...
  real(wp), dimension(SZI_(G),SZJ_(G),nk_data), &
                              intent(in)  :: tr_in !< The z-space array of tracer concentrations
                                                   !! that is read in [A]
  real(wp), dimension(nk_data+1), intent(in)  :: z_edges !< The depths of the cell edges in the input z* data
                                                   !! [Z ~> m] or [m]
  integer,                    intent(in)  :: nlay  !< The number of vertical layers in the target grid
  real(wp), dimension(SZI_(G),SZJ_(G),nlay+1), &
                              intent(in)  :: e     !< The depths of the target layer interfaces [Z ~> m] or [m]
  real(wp),                       intent(in)  :: land_fill !< fill in data over land [B]
  integer, dimension(SZI_(G),SZJ_(G)), &
                              intent(in)  :: nlevs !< The number of input levels with valid data
  real(wp),                       intent(in)  :: eps_z !< A negligibly thin layer thickness [Z ~> m].
  real(wp), dimension(SZI_(G),SZJ_(G),nlay), &
                              intent(out) :: tr    !< tracers in model space [B]
  real(wp),             optional, intent(in)  :: scale !< A factor by which to scale the output tracers from the
                                                   !! input tracers [B A-1 ~> 1]

  ! Local variables
  real(wp) :: tr_1d(nk_data) ! A copy of the input tracer concentrations in a column [B]
  real(wp) :: e_1d(nlay+1)   ! A 1-d column of interface heights, in the same units as e [Z ~> m] or [m]
  real(wp) :: sl_tr          ! The tracer concentration slope times the layer thickness, in tracer units [B]
  real(wp) :: wt(nk_data)    ! The fractional weight for each layer in the range between z1 and z2 [nondim]
  real(wp) :: z1(nk_data)    ! The fractional depth of the top limit of the part of a z-cell that contributes to
                         ! a layer, relative to the cell center and normalized by the cell thickness [nondim].
  real(wp) :: z2(nk_data)    ! The fractional depth of the bottom limit of the part of a z-cell that contributes to
                         ! a layer, relative to the cell center and normalized by the cell thickness [nondim].
                         ! Note that -1/2 <= z1 <= z2 <= 1/2.
  real(wp) :: scale_fac      ! A factor by which to scale the output tracers from the input tracers [B A-1 ~> 1]
  ...
  do j=js,je
    i_loop: do i=is,ie
      if (nlevs(i,j) == 0 .or. G%mask2dT(i,j) == 0._wp) then
        tr(i,j,:) = land_fill
        cycle i_loop
      endif
      ...
      do k=1,nlay
        ...
        elseif (e_1d(k) < z_edges(nlevs(i,j)+1)) then
          ...
          if (k_bot > k_top) then
            ...
            ! This is the piecewise linear form.
            tr(i,j,k) = tr(i,j,k) + wt(kz) * (tr_1d(kz) + 0.5_wp*sl_tr*(z2(kz) + z1(kz)))
            ! For the piecewise parabolic form add the following...
            !     + C1_3*cur_tr*(z2(kz)**2 + z2(kz)*z1(kz) + z1(kz)**2))
          endif
          k_bot_prev = k_bot
...
subroutine read_Z_edges(filename, tr_name, z_edges, nz_out, has_edges, &
  ...
  real(wp),             intent(inout) :: missing !< The missing value, if one has been found [CU ~> conc]
  real(wp),             intent(in)    :: scale   !< A scaling factor for z_edges into new units [Z m-1 ~> 1]
  real(wp),             intent(in)    :: missing_scale  !< A scaling factor to use to convert the
  ...
  allocate(z_edges(nz_edge), source=0.0_wp)
  ...
  if (scale /= 1.0_wp) then ; do k=1,nz_edge ; z_edges(k) = scale*z_edges(k) ; enddo ; endif
...
subroutine find_overlap(e, Z_top, Z_bot, k_max, k_start, k_top, k_bot, wt, z1, z2)
  real(wp), dimension(:), intent(in)  :: e      !< Column interface heights, [Z ~> m] or other units.
  real(wp),               intent(in)  :: Z_top  !< Top of range being mapped to, in the units of e [Z ~> m].
  real(wp),               intent(in)  :: Z_bot  !< Bottom of range being mapped to, in the units of e [Z ~> m].
  ...
  real(wp), dimension(:), intent(out) :: wt     !< Relative weights of each layer from k_top to k_bot [nondim].
  real(wp), dimension(:), intent(out) :: z1     !< Depth of the top limits of the part of
  ...
  real(wp), dimension(:), intent(out)   :: z2     !< Depths of the bottom limit of the part of
       !! a layer that contributes to a depth level, relative to the cell center and normalized
       !! by the cell thickness [nondim].  Note that -1/2 <= z1 < z2 <= 1/2.

  ! Local variables
  real(wp)    :: Ih   ! The inverse of the vertical distance across a layer, in the inverse of the units of e [Z-1 ~> m-1]
  real(wp)    :: e_c  ! The height of the layer center, in the units of e [Z ~> m]
  real(wp)    :: tot_wt  ! The sum of the thicknesses contributing to a layer [Z ~> m]
  real(wp)    :: I_totwt ! The Adcroft reciprocal of tot_wt [Z-1 ~> m-1]
  integer :: k

  wt(:) = 0.0_wp ; z1(:) = 0.0_wp ; z2(:) = 0.0_wp ; k_bot = k_max
  ...
  if (e(K+1) <= Z_bot) then
    wt(k) = 1.0_wp ; k_bot = k
    Ih = 0.0_wp ; if (e(K) /= e(K+1)) Ih = 1.0_wp / (e(K)-e(K+1))
    e_c = 0.5_wp*(e(K)+e(K+1))
    ...
    if (e(K) /= e(K+1)) then
      z1(k) = (0.5_wp*(e(K)+e(K+1)) - MIN(e(K), Z_top)) / (e(K)-e(K+1))
    else ; z1(k) = -0.5_wp ; endif
    z2(k) = 0.5_wp
    k_bot = k_max
    do k=k_top+1,k_max
      if (e(K+1) <= Z_bot) then
        k_bot = k
        wt(k) = e(K) - Z_bot ; z1(k) = -0.5_wp
        if (e(K) /= e(K+1)) then
          z2(k) = (0.5_wp*(e(K)+e(K+1)) - Z_bot) / (e(K)-e(K+1))
        else ; z2(k) = 0.5_wp ; endif
      else
        wt(k) = e(K) - e(K+1) ; z1(k) = -0.5_wp ; z2(k) = 0.5_wp
      endif
      tot_wt = tot_wt + wt(k) ! wt(k) is always > 0.
      if (k>=k_bot) exit
    enddo

    I_totwt = 0.0_wp ; if (tot_wt > 0.0_wp) I_totwt = 1.0_wp / tot_wt
...
function find_limited_slope(val, e, k) result(slope)
  real(wp), dimension(:), intent(in) :: val !< A column of the values that are being interpolated, in arbitrary units [A]
  real(wp), dimension(:), intent(in) :: e   !< A column's interface heights [Z ~> m] or other units.
  integer,            intent(in) :: k   !< The layer whose slope is being determined.
  real(wp) :: slope !< The normalized slope in the intracell distribution of val [A]
  ! Local variables
  real(wp) :: amn, cmn ! Limited differences and curvatures in the values [A]
  real(wp) :: d1, d2   ! Layer thicknesses, in the units of e [Z ~> m]

  if ((val(k)-val(k-1)) * (val(k)-val(k+1)) >= 0.0_wp) then
    slope = 0.0_wp ! ; curvature = 0.0
  else
    d1 = 0.5_wp*(e(K-1)-e(K+1)) ; d2 = 0.5_wp*(e(K)-e(K+2))
    if (d1*d2 > 0.0_wp) then
      ...
      amn = min(abs(slope), 2.0_wp*(max(val(k-1), val(k), val(k+1)) - val(k)))
      cmn = 2.0_wp*(val(k) - min(val(k-1), val(k), val(k+1)))
      slope = sign(1.0_wp, slope) * min(amn, cmn)
...
subroutine determine_temperature(temp, salt, R_tgt, EOS, p_ref, niter, k_start, G, GV, US, PF, &
  ...
  real(wp), dimension(SZK_(GV)),     intent(in)    :: R_tgt !< desired potential density [R ~> kg m-3].
  type(EOS_type),                intent(in)    :: EOS   !< seawater equation of state control structure
  real(wp),                          intent(in)    :: p_ref !< reference pressure [R L2 T-2 ~> Pa].
  ...
  real(wp), dimension(SZI_(G)) :: press ! Reference pressures [R L2 T-2 ~> Pa]
  real(wp) :: dT_dS_gauge   ! The relative penalizing of temperature to salinity changes when
                        ! minimizing property changes while correcting density [C S-1 ~> degC ppt-1].
  real(wp) :: I_denom       ! The inverse of the magnitude squared of the density gradient in
                        ! T-S space when stretched with dT_dS_gauge [S2 R-2 ~> ppt2 m6 kg-2]
  real(wp) :: T_min, T_max  ! The minimum and maximum temperatures [C ~> degC]
  real(wp) :: S_min, S_max  ! Minimum and maximum salinities [S ~> ppt]
  real(wp) :: tol_T     ! The tolerance for temperature matches [C ~> degC]
  real(wp) :: tol_S     ! The tolerance for salinity matches [S ~> ppt]
  real(wp) :: tol_rho   ! The tolerance for density matches [R ~> kg m-3]
  real(wp) :: max_t_adj ! The largest permitted temperature changes with each iteration
                    ! when old_fit is true [C ~> degC]
  real(wp) :: max_s_adj ! The largest permitted salinity changes with each iteration
...
# include "version_variable.h"
  ...
  if (fit_together) then
    ...
    call get_param(PF, mdl, "DETERMINE_TEMP_DT_DS_WEIGHT", dT_dS_gauge, &
                 ...
                 units="degC ppt-1", default=10.0_wp, scale=US%degC_to_C*US%S_to_ppt)
  else
    call get_param(PF, mdl, "DETERMINE_TEMP_T_ADJ_RANGE", max_t_adj, &
                 ...
                 units="degC", default=1.0_wp, scale=US%degC_to_C, do_not_log=just_read)
  ...
  do j=js,je
    dS(:,:) = 0._wp ! Needs to be zero everywhere since there is a maxval(abs(dS)) later...
    T(:,:) = temp(:,j,:)
    S(:,:) = salt(:,j,:)
    dT(:,:) = 0.0_wp
    domore(:) = .true.
...
!         if (abs(rho(i,k)-R_tgt(k))>tol_rho .and. abs(T(i,k)-land_fill) < epsln) then
          if (abs(rho(i,k)-R_tgt(k))>tol_rho) then
            ...
            if (.not.fit_together) then
              ...
              I_denom = 1.0_wp / (drho_dS(i,k)**2 + dT_dS_gauge**2*drho_dT(i,k)**2)
