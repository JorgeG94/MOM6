...
subroutine tracer_vertdiff(h_old, ea, eb, dt, tr, G, GV, &
  ...
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV)), intent(in)    :: h_old  !< layer thickness before entrainment
                                                                     !! [H ~> m or kg m-2]
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV)), intent(in)    :: ea     !< amount of fluid entrained from the layer
                                                                     !! above [H ~> m or kg m-2]
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV)), intent(in)    :: eb     !< amount of fluid entrained from the layer
                                                                     !! below [H ~> m or kg m-2]
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV)), intent(inout) :: tr     !< tracer concentration in concentration units [CU]
  real(wp),                                      intent(in)    :: dt     !< amount of time covered by this call [T ~> s]
  real(wp), dimension(SZI_(G),SZJ_(G)), optional,intent(in)    :: sfc_flux !< surface flux of the tracer in units of
                                                                     !! [CU R Z T-1 ~> CU kg m-2 s-1] or
                                                                     !! [CU H ~> CU m or CU kg m-2] if
                                                                     !! convert_flux_in is .false.
  real(wp), dimension(SZI_(G),SZJ_(G)), optional,intent(in)    :: btm_flux !< The (negative upward) bottom flux of the
                                                                     !! tracer in [CU R Z T-1 ~> CU kg m-2 s-1] or
                                                                     !! [CU H ~> CU m or CU kg m-2] if
                                                                     !! convert_flux_in is .false.
  real(wp), dimension(SZI_(G),SZJ_(G)), optional,intent(inout) :: btm_reservoir !< amount of tracer in a bottom reservoir
                                                                     !! [CU R Z ~> CU kg m-2]
  real(wp),                             optional,intent(in)    :: sink_rate !< rate at which the tracer sinks
                                                                     !! [Z T-1 ~> m s-1]
  logical,                          optional,intent(in)    :: convert_flux_in !< True if the specified sfc_flux needs
                                                                     !! to be integrated in time

  ! local variables
  real(wp) :: sink_dist !< The distance the tracer sinks in a time step [H ~> m or kg m-2].
  ...
  real(wp) :: c1(SZI_(G),SZK_(GV))    !< c1 is used by the tridiagonal solver [nondim].
  real(wp) :: h_minus_dsink(SZI_(G),SZK_(GV)) !< The layer thickness minus the
                    !! difference in sinking rates across the layer [H ~> m or kg m-2].
                    !! By construction, 0 <= h_minus_dsink < h_work.
  real(wp) :: sink(SZI_(G),SZK_(GV)+1) !< The tracer's sinking distances at the
  ...
  real(wp) :: h_tr      !< h_tr is h at tracer points with a h_neglect added to
  ...
  !$OMP do
  do j=js,je ; do i=is,ie ; sfc_src(i,j) = 0.0_wp ; btm_src(i,j) = 0.0_wp ; enddo ; enddo
  ...
  if (present(sink_rate)) then
    ...
    do j=js,je
      ...
      if (present(btm_reservoir)) then
        ...
        do i=is,ie ; sink(i,nz+1) = 0.0_wp ; enddo
      ...
      do i=is,ie
        sink(i,1) = 0.0_wp ; h_minus_dsink(i,1) = (h_old(i,j,1) + sink(i,2))
      enddo
      ...
      do k=2,nz-1 ; do i=is,ie ; if (G%mask2dT(i,j) > 0.0_wp) then
...
subroutine tracer_vertdiff_Eulerian(h_old, ent, dt, tr, G, GV, &
  ...
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV)), intent(in)    :: h_old  !< layer thickness before entrainment
                                                                     !! [H ~> m or kg m-2]
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV)+1), intent(in)  :: ent    !< Amount of fluid mixed across interfaces
                                                                     !! [H ~> m or kg m-2]
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV)), intent(inout) :: tr     !< tracer concentration in concentration units [CU]
  real(wp),                                      intent(in)    :: dt     !< amount of time covered by this call [T ~> s]
  real(wp), dimension(SZI_(G),SZJ_(G)), optional,intent(in)    :: sfc_flux !< surface flux of the tracer in units of
  ...
  if (present(sink_rate)) then
    ...
    do j=js,je
      ...
      do i=is,ie ; if (G%mask2dT(i,j) > 0.0_wp) then
        ...
        b1(i) = 1.0_wp / (b_denom_1 + ent(i,j,2))
        d1(i) = b_denom_1 * b1(i)
        h_tr = h_old(i,j,1) + h_neglect
        tr(i,j,1) = (b1(i)*h_tr)*tr(i,j,1) + b1(i)*sfc_src(i,j)
      endif ; enddo
      ...
      do i=is,ie ; if (G%mask2dT(i,j) > 0.0_wp) then
        ...
        b_denom_1 = h_minus_dsink(i,nz) + d1(i) * (ent(i,j,nz) + sink(i,nz)) + &
                    h_neglect
        b1(i) = 1.0_wp / (b_denom_1 + ent(i,j,nz+1))
        h_tr = h_old(i,j,nz) + h_neglect
        tr(i,j,nz) = b1(i) * ((h_tr * tr(i,j,nz) + btm_src(i,j)) + &
...
subroutine applyTracerBoundaryFluxesInOut(G, GV, Tr, dt, fluxes, h, evap_CFL_limit, minimum_forcing_depth, &
  ...
  type(verticalGrid_type),                    intent(in   ) :: GV !< ocean vertical grid structure
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV)),  intent(inout) :: Tr !< Tracer concentration on T-cell [conc]
  ...
  real(wp),                                       intent(in   ) :: evap_CFL_limit !< Limit on the fraction of the
                                                                  !! water that can be fluxed out of the top
                                                                  !! layer in a timestep [nondim]
  real(wp),                                       intent(in   ) :: minimum_forcing_depth !< The smallest depth over
                                                                  !! which fluxes can be applied [H ~> m or kg m-2]
  real(wp), dimension(SZI_(G),SZJ_(G)), optional, intent(in   ) :: in_flux_optional !< The total time-integrated
                                                                  !! amount of tracer that enters with freshwater
                                                                  !! [conc H ~> conc m or conc kg m-2]
  real(wp), dimension(SZI_(G),SZJ_(G)), optional, intent(in) :: out_flux_optional !< The total time-integrated
  ...
  real(wp) :: IforcingDepthScale ! The inverse of the scale over which to apply forcing [H-1 ~> m-1 or m2 kg-1]
  real(wp) :: dThickness         ! The change in a layer's thickness [H ~> m or kg m-2]
  real(wp) :: dTracer            ! The change in the integrated tracer content of a layer [conc H ~> conc m or conc kg m-2]
  real(wp) :: fractionOfForcing  ! The fraction of the forcing to apply to a layer [nondim]
  real(wp) :: hOld               ! The layer thickness before surface forcing is applied [H ~> m or kg m-2]
  real(wp) :: Ithickness         ! The inverse of the new layer thickness [H-1 ~> m-1 or m2 kg-1]

  real(wp) :: h2d(SZI_(G),SZK_(GV))     ! A 2-d work copy of layer thicknesses [H ~> m or kg m-2]
  real(wp) :: Tr2d(SZI_(G),SZK_(GV))    ! A 2-d work copy of tracer concentrations [conc]
  real(wp) :: in_flux(SZI_(G),SZJ_(G))  ! The total time-integrated amount of tracer that
                                    ! enters with freshwater [conc H ~> conc m or conc kg m-2]
  real(wp) :: out_flux(SZI_(G),SZJ_(G)) ! The total time-integrated amount of tracer that
                                    ! leaves with freshwater [conc H ~> conc m or conc kg m-2]
  real(wp) :: netMassIn(SZI_(G))   ! The remaining mass entering ocean surface [H ~> m or kg m-2]
  real(wp) :: netMassOut(SZI_(G))  ! The remaining mass leaving ocean surface [H ~> m or kg m-2]
  real(wp) :: in_flux_1d(SZI_(G))  ! The remaining amount of tracer that enters with
                               ! the freshwater [conc H ~> conc m or conc kg m-2]
  real(wp) :: out_flux_1d(SZI_(G)) ! The remaining amount of tracer that leaves with
                               ! the freshwater [conc H ~> conc m or conc kg m-2]
  real(wp) :: hGrounding(maxGroundings) ! The remaining fresh water flux that was not able to be
  ...
  in_flux(:,:) = 0.0_wp ; out_flux(:,:) = 0.0_wp
...
!$OMP                                  dThickness,dTracer,hOld,Ithickness)
  ...
  do j=js,je
    ...
    do i=is,ie
      if (G%mask2dT(i,j)>0._wp) then
        ...
        do k=1,1
          ...
          if (h2d(i,k) > 0.0_wp) then
            ...
            if (dThickness /= 0._wp .or. dTracer /= 0._wp) tr2d(i,k) = (hOld*tr2d(i,k)+ dTracer)*Ithickness
          endif
      ...
      if (abs(in_flux_1d(i))+abs(out_flux_1d(i)) /= 0.0_wp) then
!$OMP critical
