...
subroutine full_convection(G, GV, US, h, tv, T_adj, S_adj, p_surf, Kddt_smooth, halo)
  ...
  real(wp),                    intent(in)    :: Kddt_smooth  !< A smoothing vertical diffusivity
...
! logical :: use_EOS    ! If true, density is calculated from T & S using an equation of state.
  real(wp), dimension(SZI_(G),SZK0_(G)) :: &
  ...
  real(wp) :: mix_len  ! The length-scale of mixing, when it is active [H ~> m or kg m-2]
  real(wp) :: h_b, h_a ! The thicknesses of the layers above and below an interface [H ~> m or kg m-2]
  real(wp) :: b_b, b_a ! Inverse pivots used by the tridiagonal solver [H-1 ~> m-1 or m2 kg-1].
  ...
  mix_len = (1.0e20_wp * nz) * (G%max_depth * US%Z_to_m * GV%m_to_H)

  do j=js,je
    mix(:,:) = 0.0_wp ; d_b(:,:) = 1.0_wp
    ! These would be Te_b(:,:) = tv%T(:,j,:), etc., but the values are not used
    Te_b(:,:) = 0.0_wp ; Se_b(:,:) = 0.0_wp
    ...
    do i=is,ie
      ...
      Te_a(i,0) = 0.0_wp ; Se_a(i,0) = 0.0_wp
    enddo
    ...
    do itt=1,nz ! At least 2 interfaces will change with each full pass, or the
      ...
      do K=2,nz ; do i=is,ie ; if (do_i(i)) then
        ...
        b_a = 1.0_wp / ((h_a + d_a(i,K-1)*mix(i,K-1)) + mix(i,K))
        if (mix(i,K) <= 0.0_wp) then
          c_a(i,K) = 0.0_wp ; d_a(i,K) = 1.0_wp
        else
          d_a(i,K) = b_a * (h_a + d_a(i,K-1)*mix(i,K-1)) ! = 1.0-c_a(i,K)
          c_a(i,K) = 1.0_wp ; if (d_a(i,K) > epsilon(b_a)) c_a(i,K) = b_a * mix(i,K)
        endif
      ...
      do K=nz,2,-1 ; do i=is,ie ; if (do_i(i)) then
        ...
        b_b = 1.0_wp / ((h_b + d_b(i,K+1)*mix(i,K+1)) + mix(i,K))
        if (mix(i,K) <= 0.0_wp) then
          c_b(i,K) = 0.0_wp ; d_b(i,K) = 1.0_wp
        else
          d_b(i,K) = b_b * (h_b + d_b(i,K+1)*mix(i,K+1)) ! = 1.0-c_b(i,K)
          c_b(i,K) = 1.0_wp ; if (d_b(i,K) > epsilon(b_b)) c_b(i,K) = b_b * mix(i,K)
        endif
    ...
    do i=is,ie ; do_i(i) = ((G%mask2dT(i,j) > 0.0_wp) .and. last_down(i)) ; enddo
    do i=is,ie ; if (do_i(i)) then
      h_a = h(i,j,nz) + h_neglect
      b_a = 1.0_wp / (h_a + d_a(i,nz)*mix(i,nz))
...
function is_unstable(dRho_dT, dRho_dS, h_a, h_b, mix_A, mix_B, T_a, T_b, S_a, S_b, &
  ...
  real(wp), intent(in) :: dRho_dT !< The derivative of in situ density with temperature [R C-1 ~> kg m-3 degC-1]
  real(wp), intent(in) :: dRho_dS !< The derivative of in situ density with salinity [R S-1 ~> kg m-3 ppt-1]
  real(wp), intent(in) :: h_a     !< The thickness of the layer above [H ~> m or kg m-2]
  real(wp), intent(in) :: h_b     !< The thickness of the layer below [H ~> m or kg m-2]
  real(wp), intent(in) :: mix_A   !< The time integrated mixing rate of the interface above [H ~> m or kg m-2]
  real(wp), intent(in) :: mix_B   !< The time integrated mixing rate of the interface below [H ~> m or kg m-2]
  real(wp), intent(in) :: T_a     !< The initial temperature of the layer above [C ~> degC]
  real(wp), intent(in) :: T_b     !< The initial temperature of the layer below [C ~> degC]
  real(wp), intent(in) :: S_a     !< The initial salinity of the layer below [S ~> ppt]
  real(wp), intent(in) :: S_b     !< The initial salinity of the layer below [S ~> ppt]
  real(wp), intent(in) :: Te_aa   !< The estimated temperature two layers above rescaled by d_A [C ~> degC]
  real(wp), intent(in) :: Te_bb   !< The estimated temperature two layers below rescaled by d_B [C ~> degC]
  real(wp), intent(in) :: Se_aa   !< The estimated salinity two layers above rescaled by d_A [S ~> ppt]
  real(wp), intent(in) :: Se_bb   !< The estimated salinity two layers below rescaled by d_B [S ~> ppt]
  real(wp), intent(in) :: d_A     !< The rescaling dependency across the interface above [nondim]
  real(wp), intent(in) :: d_B     !< The rescaling dependency across the interface below [nondim]
  ...
  is_unstable = (dRho_dT * ((h_a * h_b * (T_b - T_a) + &
                 ...
                 dRho_dS * ((h_a * h_b * (S_b - S_a) + &
                            ...
                            (h_a*mix_B * (Se_bb - d_B*S_a) + &
                             h_b*mix_A * (d_A*S_b - Se_aa)) ) < 0.0_wp)
...
subroutine smoothed_dRdT_dRdS(h, dz, tv, Kddt, dR_dT, dR_dS, G, GV, US, j, p_surf, halo)
  ...
  type(thermo_var_ptrs),   intent(in)  :: tv   !< A structure pointing to various
                                               !! thermodynamic variables
  real(wp),                    intent(in)  :: Kddt !< A diffusivity times a time increment [H Z ~> m2 or kg m-1].
  real(wp), dimension(SZI_(G),SZK_(GV)+1), &
  ...
  ! Local variables
  real(wp) :: mix(SZI_(G),SZK_(GV)+1)  ! The diffusive mixing length (kappa*dt)/dz
                                   ! between layers within in a timestep [H ~> m or kg m-2].
  real(wp) :: b1(SZI_(G))              ! A tridiagonal solver variable [H-1 ~> m-1 or m2 kg-1]
  real(wp) :: d1(SZI_(G))              ! A tridiagonal solver variable [nondim]
  real(wp) :: c1(SZI_(G),SZK_(GV))     ! A tridiagonal solver variable [nondim]
  real(wp) :: T_f(SZI_(G),SZK_(GV))    ! Filtered temperatures [C ~> degC]
  real(wp) :: S_f(SZI_(G),SZK_(GV))    ! Filtered salinities [S ~> ppt]
  real(wp) :: pres(SZI_(G))            ! Interface pressures [R L2 T-2 ~> Pa].
  real(wp) :: T_EOS(SZI_(G))           ! Filtered and vertically averaged temperatures [C ~> degC]
  real(wp) :: S_EOS(SZI_(G))           ! Filtered and vertically averaged salinities [S ~> ppt]
  real(wp) :: kap_dt_x2                ! The product of 2*kappa*dt [H Z ~> m2 or kg m-1].
  real(wp) :: dz_neglect, h0           ! A negligible vertical distances [Z ~> m]
  real(wp) :: h_neglect                ! A negligible thickness to allow for zero thicknesses
                                   ! [H ~> m or kg m-2].
  real(wp) :: h_tr                     ! The thickness at tracer points, plus h_neglect [H ~> m or kg m-2].
  ...
  if (Kddt <= 0.0_wp) then
    ...
    h0 = 1.0e-16_wp*sqrt(GV%H_to_m*US%m_to_Z*Kddt) + dz_neglect
    do i=is,ie
      mix(i,2) = kap_dt_x2 / ((dz(i,1)+dz(i,2)) + h0)

      h_tr = h(i,j,1) + h_neglect
      b1(i) = 1.0_wp / (h_tr + mix(i,2))
      d1(i) = b1(i) * h(i,j,1)
    ...
    do k=2,nz-1 ; do i=is,ie
      ...
      b1(i) = 1.0_wp / ((h_tr + d1(i)*mix(i,K)) + mix(i,K+1))
    ...
    do i=is,ie
      ...
      b1(i) = 1.0_wp / (h_tr + d1(i)*mix(i,nz))
  ...
  do K=2,nz
    do i=is,ie
      T_EOS(i) = 0.5_wp*(T_f(i,k-1) + T_f(i,k))
      S_EOS(i) = 0.5_wp*(S_f(i,k-1) + S_f(i,k))
    enddo
