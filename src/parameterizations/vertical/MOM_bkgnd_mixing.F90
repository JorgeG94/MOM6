...
type, public :: bkgnd_mixing_cs ; private
  ...
  ! Parameters
  real(wp)    :: Bryan_Lewis_c1         !< The vertical diffusivity values for  Bryan-Lewis profile
                                    !! at |z|=D [Z2 T-1 ~> m2 s-1]
  real(wp)    :: Bryan_Lewis_c2         !< The amplitude of variation in diffusivity for the
                                    !! Bryan-Lewis diffusivity profile [Z2 T-1 ~> m2 s-1]
  real(wp)    :: Bryan_Lewis_c3         !< The inverse length scale for transition region in the
                                    !! Bryan-Lewis diffusivity profile [Z-1 ~> m-1]
  real(wp)    :: Bryan_Lewis_c4         !< The depth where diffusivity is Bryan_Lewis_bl1 in the
                                    !! Bryan-Lewis profile [Z ~> m]
  real(wp)    :: bckgrnd_vdc1           !< Background diffusivity (Ledwell) when
                                    !! horiz_varying_background=.true. [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
  real(wp)    :: bckgrnd_vdc_eq         !< Equatorial diffusivity (Gregg) when
                                    !! horiz_varying_background=.true. [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
  real(wp)    :: bckgrnd_vdc_psim       !< Max. PSI induced diffusivity (MacKinnon) when
                                    !! horiz_varying_background=.true. [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
  real(wp)    :: bckgrnd_vdc_Banda      !< Banda Sea diffusivity (Gordon) when
                                    !! horiz_varying_background=.true. [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
  ...
  real(wp)    :: N0_2Omega              !< ratio of the typical Buoyancy frequency to
                                    !! twice the Earth's rotation period, used with the
                                    !! Henyey scaling from the mixing [nondim]
  real(wp)    :: Henyey_max_lat         !< A latitude poleward of which the Henyey profile
                                    !! is returned to the minimum diffusivity [degrees_N]
  real(wp)    :: prandtl_bkgnd          !< Turbulent Prandtl number used to convert
                                    !! vertical background diffusivity into viscosity [nondim]
  real(wp)    :: Kd_tanh_lat_scale      !< A nondimensional scaling for the range of
                                    !! diffusivities with Kd_tanh_lat_fn [nondim]. Valid values
                                    !! are in the range of -2 to 2; 0.4 reproduces CM2M.
  real(wp)    :: Kd_tot_ml              !< The mixed layer diapycnal diffusivity [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
                                    !! when no other physically based mixed layer turbulence
                                    !! parameterization is being used.
  real(wp)    :: Hmix                   !< mixed layer thickness [H ~> m or kg m-2] when no physically based
...
subroutine bkgnd_mixing_init(Time, G, GV, US, param_file, diag, CS, physical_OBL_scheme)
  ...
  ! Local variables
  real(wp) :: Kv                    ! The interior vertical viscosity [H Z T-1 ~> m2 s-1 or Pa s] - read to set Prandtl
  ...
  real(wp) :: prandtl_bkgnd_comp    ! Kv/CS%Kd [nondim]. Gets compared with user-specified prandtl_bkgnd.
...
# include "version_variable.h"
  ...
  if (CS%physical_OBL_scheme) then
    ...
    call get_param(param_file, mdl, "KDML", CS%Kd_tot_ml, &
                   units="m2 s-1", default=-1._wp, scale=GV%m2_s_to_HZ_T, do_not_log=.true.)
    if (CS%Kd_tot_ml>0._wp) call MOM_error(FATAL, &
    ...
    call get_param(param_file, mdl, "KD_ML_TOT", CS%Kd_tot_ml, &
                 units="m2 s-1", default=-1.0_wp, scale=GV%m2_s_to_HZ_T, do_not_log=.true.)
    if (CS%Kd_tot_ml>0._wp) call MOM_error(FATAL, &
    ...
    if (abs(CS%Kd_tot_ml - CS%Kd) <= 1.0e-15_wp*abs(CS%Kd)) then
      call get_param(param_file, mdl, "KDML", CS%Kd_tot_ml, &
                 ...
                 units="m2 s-1", default=Kd_z*US%Z2_T_to_m2_s, scale=GV%m2_s_to_HZ_T, do_not_log=.true.)
      if (abs(CS%Kd_tot_ml - CS%Kd) > 1.0e-15_wp*abs(CS%Kd)) &
...
!  call openParameterBlock(param_file,'MOM_BACKGROUND_MIXING')
  ...
  if (CS%horiz_varying_background) then
    ...
    call get_param(param_file, mdl, "BCKGRND_VDC1", CS%bckgrnd_vdc1, &
                   ...
                   units="m2 s-1", default=0.16e-04_wp, scale=GV%m2_s_to_HZ_T)
    ...
    call get_param(param_file, mdl, "BCKGRND_VDC_EQ", CS%bckgrnd_vdc_eq, &
                   ...
                   units="m2 s-1", default=0.01e-04_wp, scale=GV%m2_s_to_HZ_T)
    ...
    call get_param(param_file, mdl, "BCKGRND_VDC_PSIM", CS%bckgrnd_vdc_psim, &
                   ...
                   units="m2 s-1", default=0.13e-4_wp, scale=GV%m2_s_to_HZ_T)
    ...
    call get_param(param_file, mdl, "BCKGRND_VDC_BAN", CS%bckgrnd_vdc_Banda, &
                   ...
                   units="m2 s-1", default=1.0e-4_wp, scale=GV%m2_s_to_HZ_T)
  endif
  ...
  if (CS%Bryan_Lewis_diffusivity .or. CS%horiz_varying_background) then
    ...
    if (CS%Kd /= 0.0_wp) prandtl_bkgnd_comp = Kv / CS%Kd

    if ( abs(CS%prandtl_bkgnd - prandtl_bkgnd_comp)>1.e-14_wp) then
  ...
  if (CS%Kd>0.0_wp .and. (trim(CS%bkgnd_scheme_str)=="BRYAN_LEWIS_DIFFUSIVITY" .or.&
  ...
  if (CS%Henyey_IGW_background) then
    call get_param(param_file, mdl, "HENYEY_N0_2OMEGA", CS%N0_2Omega, &
                  ...
                  "scaling from the mixing.", units="nondim", default=20.0_wp)
  ...
  if (CS%Kd_tanh_lat_fn) &
    call get_param(param_file, mdl, "KD_TANH_LAT_SCALE", CS%Kd_tanh_lat_scale, &
                 ...
                 "-2 to 2; 0.4 reproduces CM2M.", units="nondim", default=0.0_wp)
...
subroutine calculate_bkgnd_mixing(h, tv, N2_lay, Kd_lay, Kd_int, Kv_bkgnd, j, G, GV, US, CS)
  ...
  real(wp), dimension(SZI_(G),SZK_(GV)),         intent(in)    :: N2_lay !< squared buoyancy frequency associated
                                                                  !! with layers [T-2 ~> s-2]
  real(wp), dimension(SZI_(G),SZK_(GV)),         intent(out)   :: Kd_lay !< The background diapycnal diffusivity of each
                                                                  !! layer [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
  real(wp), dimension(SZI_(G),SZK_(GV)+1),       intent(out)   :: Kd_int !< The background diapycnal diffusivity of each
                                                                  !! interface [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
  real(wp), dimension(SZI_(G),SZK_(GV)+1),       intent(out)   :: Kv_bkgnd !< The background vertical viscosity at
  ...
  ! local variables
  real(wp), dimension(SZK_(GV)+1) :: depth_int  !< Distance from surface of the interfaces [m]
  real(wp), dimension(SZK_(GV)+1) :: Kd_col     !< Diffusivities at the interfaces [m2 s-1]
  real(wp), dimension(SZK_(GV)+1) :: Kv_col     !< Viscosities at the interfaces [m2 s-1]
  real(wp), dimension(SZI_(G))    :: Kd_sfc     !< Surface value of the diffusivity [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
  real(wp), dimension(SZI_(G))    :: depth      !< Distance from surface of an interface [H ~> m or kg m-2]
  real(wp), dimension(SZI_(G),SZK_(GV)) :: dz   !< Height change across layers [Z ~> m]
  real(wp) :: depth_c    !< depth of the center of a layer [H ~> m or kg m-2]
  real(wp) :: I_Hmix     !< inverse of fixed mixed layer thickness [H-1 ~> m-1 or m2 kg-1]
  real(wp) :: I_x30      !< 2/acos(2) = 1/(sin(30 deg) * acosh(1/sin(30 deg))) [nondim]
  real(wp) :: deg_to_rad !< factor converting degrees to radians [radians degree-1], pi/180.
  real(wp) :: abs_sinlat !< absolute value of sine of latitude [nondim]
  real(wp) :: min_sinlat ! The minimum value of the sine of latitude [nondim]
  real(wp) :: bckgrnd_vdc_psin !< PSI diffusivity in northern hemisphere [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
  real(wp) :: bckgrnd_vdc_psis !< PSI diffusivity in southern hemisphere [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
  integer :: i, k, is, ie, js, je, nz

  is  = G%isc ; ie  = G%iec ; js  = G%jsc ; je  = G%jec ; nz = GV%ke

  ! set some parameters
  deg_to_rad = atan(1.0_wp)/45.0_wp ! = PI/180
  min_sinlat = 1.e-10_wp
  ...
  if (CS%Bryan_Lewis_diffusivity) then
    ...
    do i=is,ie
      ...
      Kd_col(:) = 0.0_wp ; Kv_col(:) = 0.0_wp  ! Is this line necessary?
      ...
      enddo
      do k=1,nz
        Kd_lay(i,k) = Kd_lay(i,k) + 0.5_wp * GV%m2_s_to_HZ_T * (Kd_col(K) + Kd_col(K+1))
      enddo
    enddo ! i loop
  ...
  elseif (CS%horiz_varying_background) then
    ...
    do i=is,ie
      bckgrnd_vdc_psis = CS%bckgrnd_vdc_psim * exp(-(0.4_wp*(G%geoLatT(i,j)+28.9_wp))**2)
      bckgrnd_vdc_psin = CS%bckgrnd_vdc_psim * exp(-(0.4_wp*(G%geoLatT(i,j)-28.9_wp))**2)
      ...
      elseif (G%geoLatT(i,j) <= 10.0_wp) then
        Kd_int(i,1) = Kd_int(i,1) + CS%bckgrnd_vdc1 * (G%geoLatT(i,j)/10.0_wp)**2
      else
        Kd_int(i,1) = Kd_int(i,1) + CS%bckgrnd_vdc1
      endif

      ! North Banda Sea
      if ( (G%geoLatT(i,j) < -1.0_wp)  .and. (G%geoLatT(i,j) > -4.0_wp) .and. &
           ( mod(G%geoLonT(i,j)+360.0_wp,360.0_wp) > 103.0_wp) .and. &
           ( mod(G%geoLonT(i,j)+360.0_wp,360.0_wp) < 134.0_wp) ) then
        Kd_int(i,1) = CS%bckgrnd_vdc_Banda
      endif

      ! Middle Banda Sea
      if ( (G%geoLatT(i,j) <= -4.0_wp) .and. (G%geoLatT(i,j) > -7.0_wp) .and. &
           ( mod(G%geoLonT(i,j)+360.0_wp,360.0_wp) > 106.0_wp) .and. &
           ( mod(G%geoLonT(i,j)+360.0_wp,360.0_wp) < 140.0_wp) ) then
        Kd_int(i,1) = CS%bckgrnd_vdc_Banda
      endif

      ! South Banda Sea
      if ( (G%geoLatT(i,j) <= -7.0_wp) .and. (G%geoLatT(i,j) > -8.3_wp) .and. &
           ( mod(G%geoLonT(i,j)+360.0_wp,360.0_wp) > 111.0_wp) .and. &
           ( mod(G%geoLonT(i,j)+360.0_wp,360.0_wp) < 142.0_wp) ) then
        Kd_int(i,1) = CS%bckgrnd_vdc_Banda
      endif
    ...
    if (CS%Henyey_IGW_background) then
      I_x30 = 2.0_wp / invcosh(CS%N0_2Omega*2.0_wp) ! This is evaluated at 30 deg.
      do i=is,ie
    ...
    elseif (CS%Kd_tanh_lat_fn) then
      do i=is,ie
        ...
        Kd_sfc(i) = max(CS%Kd_min, CS%Kd * (1.0_wp + &
            CS%Kd_tanh_lat_scale * 0.5_wp*tanh((abs(G%geoLatT(i,j)) - 35.0_wp)/5.0_wp) ))
      enddo
    ...
    if ((.not.CS%physical_OBL_scheme) .and. (CS%Kd /= CS%Kd_tot_ml)) then
      ...
      I_Hmix = 1.0_wp / (CS%Hmix + GV%H_subroundoff)
      do i=is,ie ; depth(i) = 0.0_wp ; enddo
      do k=1,nz ; do i=is,ie
        depth_c = depth(i) + 0.5_wp*h(i,j,k)
        if (depth_c <= CS%Hmix) then ; Kd_lay(i,k) = CS%Kd_tot_ml
        elseif (depth_c >= 2.0_wp*CS%Hmix) then ; Kd_lay(i,k) = Kd_sfc(i)
        else
          Kd_lay(i,k) = ((Kd_sfc(i) - CS%Kd_tot_ml) * I_Hmix) * depth_c + (2.0_wp*CS%Kd_tot_ml - Kd_sfc(i))
        endif
    ...
    do i=is,ie
      Kd_int(i,1) = 0.0_wp; Kv_bkgnd(i,1) = 0.0_wp
      Kd_int(i,nz+1) = 0.0_wp; Kv_bkgnd(i,nz+1) = 0.0_wp
    enddo
    do K=2,nz ; do i=is,ie
      Kd_int(i,K) = 0.5_wp*(Kd_lay(i,k-1) + Kd_lay(i,k))
