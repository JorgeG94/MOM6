...
type, public :: interface_filter_CS ; private
  ...
  real(wp)    :: max_smoothing_CFL   !< Maximum value of the smoothing CFL for interface height filtering [nondim]
  real(wp)    :: filter_rate         !< The rate at which grid-scale anomalies are damped away [T-1 ~> s-1]
...
subroutine interface_filter(h, uhtr, vhtr, tv, dt, G, GV, US, CDp, CS)
  ...
  real(wp) :: de_smooth(SZI_(G),SZJ_(G),SZK_(GV)+1) ! Change in the heights of interfaces after one pass
                                         ! of Laplacian smoothing [Z ~> m], positive downward to avoid
                                         ! having to change other signs in the call to interface_filter.
  real(wp) :: uhD(SZIB_(G),SZJ_(G),SZK_(GV)) ! Smoothing u*h fluxes within a timestep [L2 H ~> m3 or kg]
  real(wp) :: vhD(SZI_(G),SZJB_(G),SZK_(GV)) ! Smoothing v*h fluxes within a timestep [L2 H ~> m3 or kg]
  ...
  real(wp) :: diag_sfn_y(SZI_(G),SZJB_(G),SZK_(GV)+1)       ! Diagnostic of the y-face streamfunction
                                                        ! [H L2 T-1 ~> m3 s-1 or kg s-1]
  real(wp) :: filter_strength ! The amount of filtering within a each iteration [nondim]
  ...
  if ((.not.CS%interface_filter) .or. (CS%filter_rate <= 0.0_wp) .or. (CS%filter_order < 2)) return
  ...
  elseif (filter_itts == 2) then
    ...
    filter_strength = min((CS%filter_rate*dt)**(1.0_wp/filter_itts), CS%max_smoothing_CFL)
  endif
  hs = filter_itts-1
  if (CS%isotropic_filter) then
    !$OMP parallel do default(shared)
    do j=js-hs,je+hs ; do I=is-(hs+1),ie+hs
      Lsm2_u(I,j) = (0.25_wp*filter_strength) / ((G%IdxCu(I,j)**2) + (G%IdyCu(I,j)**2))
    enddo ; enddo
    !$OMP parallel do default(shared)
    do J=js-(hs+1),je+hs ; do i=is-hs,ie+hs
      Lsm2_v(i,J) = (0.25_wp*filter_strength) / ((G%IdxCv(i,J)**2) + (G%IdyCv(i,J)**2))
    enddo ; enddo
  else
    !$OMP parallel do default(shared)
    do j=js-hs,je+hs ; do I=is-(hs+1),ie+hs
      Lsm2_u(I,j) = (0.125_wp*filter_strength) *  (min(G%areaT(i,j), G%areaT(i+1,j)) * G%IdyCu(I,j))**2
    enddo ; enddo
    !$OMP parallel do default(shared)
    do J=js-(hs+1),je+hs ; do i=is-hs,ie+hs
      Lsm2_v(i,J) = (0.125_wp*filter_strength) *  (min(G%areaT(i,j), G%areaT(i,j+1)) * G%IdxCv(i,J))**2
    enddo ; enddo
  endif
  ...
  do itt=2,filter_itts
    ...
    !$OMP parallel do default(shared)
    do j=js-hs,je+hs
      do i=is-hs,ie+hs ; de_smooth(i,j,nz+1) = 0.0_wp ; enddo
  ...
  if (query_averaging_enabled(CS%diag)) then
    if (CS%id_sfn_x > 0) then
      diag_sfn_x(:,:,1) = 0.0_wp ; diag_sfn_x(:,:,nz+1) = 0.0_wp
    ...
    if (CS%id_sfn_y > 0) then
      diag_sfn_y(:,:,1) = 0.0_wp ; diag_sfn_y(:,:,nz+1) = 0.0_wp
...
subroutine filter_interface(h, e, Lsm2_u, Lsm2_v, uhD, vhD, tv, G, GV, US, halo_size)
  ...
  real(wp), dimension(SZIB_(G),SZJ_(G)),           intent(in)  :: Lsm2_u !< Interface smoothing lengths squared
                                                                    !! at u points [L2 ~> m2]
  real(wp), dimension(SZI_(G),SZJB_(G)),           intent(in)  :: Lsm2_v !< Interface smoothing lengths squared
                                                                    !! at v points [L2 ~> m2]
  ...
  real(wp) :: uhtot(SZIB_(G),SZJ_(G))  ! The vertical sum of uhD [H L2 ~> m3 or kg].
  real(wp) :: vhtot(SZI_(G),SZJB_(G))  ! The vertical sum of vhD [H L2 ~> m3 or kg].
  ...
  real(wp) :: Sfn           ! The overturning streamfunction [H L2 ~> m3 or kg].
  ...
  h_neglect = GV%H_subroundoff ; hn_2 = 0.5_wp*h_neglect
  ...
  do j=js-1,je+1
    do i=is-1,ie+1
      ...
      h_avail(i,j,1) = max(0.25_wp*G%areaT(i,j)*(h(i,j,1)-GV%Angstrom_H),0.0_wp)
      h_avail_rsum(i,j,2) = h_avail(i,j,1)
    enddo
    do k=2,nz ; do i=is-1,ie+1
      h_avail(i,j,k) = max(0.25_wp*G%areaT(i,j)*(h(i,j,k)-GV%Angstrom_H),0.0_wp)
  ...
  do j=js,je
    do I=is-1,ie ; uhtot(I,j) = 0.0_wp ; enddo
    do K=nz,2,-1
      do I=is-1,ie
  ...
  !$OMP parallel do default(shared) private(Slope,Sfn_est,Sfn)
  do J=js-1,je
    do i=is,ie ; vhtot(i,J) = 0.0_wp ; enddo
    do K=nz,2,-1
      do i=is,ie
...
# include "version_variable.h"
  real(wp) :: interface_filter_time   ! The grid-scale interface height filtering timescale [T ~> s]
  ...
  call get_param(param_file, mdl, "INTERFACE_FILTER_TIME", interface_filter_time, &
                 ...
                 default=0.0_wp, units="s", scale=US%s_to_T)
  CS%filter_rate = 0.0_wp
  if (interface_filter_time > 0.0_wp) CS%filter_rate = 1.0_wp / interface_filter_time
  CS%interface_filter  = (interface_filter_time > 0.0_wp)
  call get_param(param_file, mdl, "INTERFACE_FILTER_MAX_CFL", CS%max_smoothing_CFL, &
                 ...
                 "marginally unstable value.", units="nondimensional", default=0.8_wp)
  if (CS%max_smoothing_CFL < 0.0_wp) CS%max_smoothing_CFL = 0.0_wp
