...
subroutine horizontal_viscosity(u, v, h, uh, vh, diffu, diffv, MEKE, VarMix, G, GV, US, &
  ...
  do kstart=1,nz,nkblock
    ...
    !$omp target teams distribute parallel do collapse(3) private(k)
    do kk = 1, kmax
    do J = js_vort, je_vort
    do I = is_vort, ie_vort
      k = kstart + kk - 1
    ...
    if (CS%use_Leithy) then
      ...
      !$omp target teams distribute parallel do collapse(3) private(k, dudx_smooth, dvdy_smooth)
      do kk = 1, kmax
      do j = Jsq, Jeq+1
      do i = Isq, Ieq+1
        k = kstart + kk - 1
    ...
    if (CS%bound_Ah .or. CS%bound_Kh) then
      !$omp target teams distribute parallel do collapse(3) private(k, h_min)
      do kk = 1, kmax
      do j = js_Kh, je_Kh
      do i = is_Kh, ie_Kh
        k = kstart + kk - 1
    ...
    if (CS%Laplacian) then
      ...
      if ((CS%Leith_Kh) .or. (CS%Leith_Ah) .or. (CS%use_Leithy)) then
        if (CS%use_QG_Leith_visc) then
          !$omp target teams distribute parallel do collapse(3) private(grad_vort, grad_vort_qg)
          do kk = 1, kmax
          do j = js_Kh, je_Kh
          do i = is_Kh, ie_Kh
      ...
      if (CS%id_grid_Re_Kh>0) then
        !$omp target teams distribute parallel do collapse(3) private(k, KE, grid_Kh)
        do kk = 1, kmax
        do j = js, je
        do i = is, ie
          k = kstart + kk - 1
    ...
    if (CS%anisotropic) then
      !$omp target teams distribute parallel do collapse(3) private(local_strain)
      do kk = 1, kmax
      do j = Jsq, Jeq+1
      do i = Isq, Ieq+1
    ...
    if (CS%biharmonic) then
      ...
      if ((CS%Smagorinsky_Ah) .or. (CS%Leith_Ah) .or. (CS%use_Leithy)) then
        if (CS%Smagorinsky_Ah) then
          if (CS%bound_Coriolis) then
            !$omp target teams distribute parallel do collapse(3) private(AhSm)
            do kk = 1, kmax
            do j = js_Kh, je_Kh
            do i = is_Kh, ie_Kh
        ...
        if (CS%Leith_Ah) then
          !$omp target teams distribute parallel do collapse(3) private(Del2vort_h, AhLth)
          do kk = 1, kmax
          do j = js_Kh, je_Kh
          do i = is_Kh, ie_Kh
      ...
      if (CS%EY24_EBT_BS) then
          !$omp target teams distribute parallel do collapse(3) private(k, tmp)
          do kk = 1, kmax
          do j = js_Kh, je_Kh
          do i = is_Kh, ie_Kh
            k = kstart + kk - 1
      ...
      if (CS%id_grid_Re_Ah > 0) then
        !$omp target teams distribute parallel do collapse(3) private(k, KE, grid_Ah)
        do kk = 1, kmax
        do j = js, je
        do i = is, ie
          k = kstart + kk - 1
      ...
      !$omp target teams distribute parallel do collapse(3) private(k, d_del2u, d_del2v, d_str)
      do kk = 1, kmax
      do j = Jsq, Jeq+1
      do i = Isq, Ieq+1
        k = kstart + kk - 1
    ...
    if ((CS%Smagorinsky_Kh) .or. (CS%Smagorinsky_Ah)) then
      !$omp target teams distribute parallel do collapse(3) private(sh_xy_sq, sh_xx_sq)
      do kk = 1, kmax
      do J = js-1, Jeq
      do I = is-1, Ieq
    ...
    !$omp target teams distribute parallel do collapse(3) private(h2uq, h2vq)
    do kk = 1, kmax
    do J = js-1, Jeq
    do I = is-1, Ieq
    ...
    if (CS%no_slip) then
      !$omp target teams distribute parallel do collapse(3) private(hu, hv)
      do kk = 1, kmax
      do J = js-1, Jeq
      do I = is-1, Ieq
    ...
    if (CS%biharmonic) then
      ...
      if (CS%Smagorinsky_Ah .or. CS%Leith_Ah) then
        ...
        if (CS%Leith_Ah) then
          !$omp target teams distribute parallel do collapse(3) private(AhLth)
          do kk = 1, kmax
          do J = js-1, Jeq
          do I = is-1, Ieq
      ...
      if (CS%Re_Ah > 0.0) then
        !$omp target teams distribute parallel do collapse(3) private(k, KE)
        do kk = 1, kmax
        do J = js-1, Jeq
        do I = is-1, Ieq
          k = kstart + kk - 1
    ...
    if (CS%use_GME) then
      ...
      !$omp target teams distribute parallel do collapse(3) private(k, GME_coeff)
      do kk = 1, kmax
      do j = Jsq-1, Jeq+2
      do i = Isq-1, Ieq+2
        k = kstart + kk - 1
  ...
  if (CS%id_FrictWorkIntz > 0) then
    do j=js,je
      do i=is,ie
      FrictWorkIntz(i,j,1) = FrictWork(i,j,1)
      enddo
  ...
  if (CS%id_FrictWorkIntz_bh > 0) then
    do j=js,je
      do i=is,ie
      FrictWorkIntz_bh(i,j,1) = FrictWork_bh(i,j,1)
      enddo
      do k=2,nz ; do i=is,ie
...
subroutine hor_visc_Leith_grad(G, GV, US, CS, VarMix, nkblock, kstart, kmax, &
  ...
  ! Vorticity gradient
  !$omp target teams distribute parallel do collapse(3) private(DY_dxBu)
  do kk = 1, kmax
  do J = js-2, je_Kh
  do i = is_Kh-1, ie_Kh+1
  ...
  !$omp target teams distribute parallel do collapse(3) private(DX_dyBu)
  do kk = 1, kmax
  do j = js_Kh-1, je_Kh+1
  do I = is-2, ie_Kh
  ...
  !$omp target teams distribute parallel do collapse(3) private(DY_dxBu, DX_dyBu)
  do kk = 1, kmax
  do J = js_Kh-1, je_Kh
  do I = is_Kh-1, ie_Kh
    DY_dxBu = G%dyBu(I,J) * G%IdxBu(I,J)
...
subroutine hor_visc_Leithy_Ah(G, GV, CS, nkblock, kstart, kmax, is_Kh, ie_Kh, &
  ...
  ! Get Ah
  !$omp target teams distribute parallel do collapse(3) private(Del2vort_h, AhLthy)
  do kk = 1, kmax
  do j = js_Kh, je_Kh
  do i = is_Kh, ie_Kh
    Del2vort_h = 0.25 * ((Del2vort_q(I,J,kk) + Del2vort_q(I-1,J-1,kk)) + &
