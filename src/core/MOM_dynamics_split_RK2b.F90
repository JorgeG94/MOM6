...
type, public :: MOM_dyn_split_RK2b_CS ; private
  ...
  real(wp) ALLOCABLE_, dimension(NIMEMB_PTR_,NJMEM_)        :: du_av_inst !< The barotropic zonal velocity increment
                                                                  !! between filtered and instantaneous velocities
                                                                  !! [L T-1 ~> m s-1]
  real(wp) ALLOCABLE_, dimension(NIMEM_,NJMEMB_PTR_)        :: dv_av_inst !< The barotropic meridional velocity increment
                                                                  !! between filtered and instantaneous velocities
                                                                  !! [L T-1 ~> m s-1]
...
subroutine step_MOM_dyn_split_RK2b(u_av, v_av, h, tv, visc, Time_local, dt, forces, &
  ...
  real(wp), dimension(SZIB_(G),SZJ_(G),SZK_(GV)), target :: u_inst ! Instantaneous zonal velocity [L T-1 ~> m s-1].
  real(wp), dimension(SZI_(G),SZJB_(G),SZK_(GV)), target :: v_inst ! Instantaneous meridional velocity [L T-1 ~> m s-1].
  real(wp), dimension(SZI_(G),SZJ_(G),SZK_(GV))  :: h_av ! The average of the layer thicknesses at the beginning
  ...
  hp(:,:,:) = h(:,:,:)
  up(:,:,:) = 0.0_wp ; vp(:,:,:) = 0.0_wp ; u_inst(:,:,:) = 0.0_wp ; v_inst(:,:,:) = 0.0_wp
...
! u_bc_accel = CAu + PFu + diffu(u[n-1])
  ...
  if (CS%id_ueffA > 0) then
    ueffA(:,:,:) = 0
    do k=1,nz ; do j=js,je ; do I=Isq,Ieq
      if (abs(u_av(I,j,k)) > 0._wp) ueffA(I,j,k) = uh(I,j,k) / u_av(I,j,k)
    enddo ; enddo ; enddo
  ...
  if (CS%id_veffA > 0) then
    veffA(:,:,:) = 0
    do k=1,nz ; do J=Jsq,Jeq ; do i=is,ie
      if (abs(v_av(i,J,k)) > 0._wp) veffA(i,J,k) = vh(i,J,k) / v_av(i,J,k)
    enddo ; enddo ; enddo
...
subroutine register_restarts_dyn_split_RK2b(HI, GV, US, param_file, CS, restart_CS, uh, vh)
  ...
  ALLOC_(CS%du_av_inst(IsdB:IedB,jsd:jed)) ; CS%du_av_inst(:,:) = 0.0_wp
  ALLOC_(CS%dv_av_inst(isd:ied,JsdB:JedB)) ; CS%dv_av_inst(:,:) = 0.0_wp
